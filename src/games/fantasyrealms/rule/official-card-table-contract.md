...
2. 当前逐卡 canonical `name` 与 `text` 仍保留英文原文；这是当前官方主真相源口径，不因 runtime 中文展示而被覆盖。
3. 当前 runtime 中文展示名已通过 `所有卡牌.png` 的逐卡视觉映射接入 `displayNameZh`；该字段用于 UI / `aria-label` / ActionLog 展示，不反向覆盖 canonical 英文 `name`。
4. 当前 53 张基础卡都已有结构化 `textZh`；该字段是**基于官方英文 `text` 派生、并结合现有中文卡图做运行时对照**的仓库内中文效果文案，用于 zh-CN fallback 文本展示与后续结构化消费。它是当前 runtime 中文真相源，但不冒充外部“官方中文主真相源”。
5. 当前 53 张基础卡都已有 atlas face 映射，并已有测试锁定 `zh-CN` 下不会落回 fallback 英文卡面；因此 runtime 卡面既可通过中文 atlas 视觉呈现，也可在 fallback 路径走结构化 `textZh`。
6. `规则.txt` 只负责玩法口径对照，不得反向拿其中少量示例卡名去覆盖完整逐卡数据表。
7. `src/games/fantasyrealms/data/cards.ts` 是当前实现落点，后续若调整任何 `id`、花色映射、分值、中文展示名或效果文本，必须先回写本合同。
8. `foundation.ts` 与 `domain/index.ts` 当前共同消费这份官方卡表，后续不得再回退到另一套“示例牌库”。

## 冲突 / 未完成项

| 项目 | 当前状态 | 说明 |
| --- | --- | --- |
| 运行时逐卡中文展示名 | 已完成 | `displayNameZh` 已接入 53 张基础卡，并已用于 UI / `aria-label` / ActionLog；视觉映射来源为 `所有卡牌.png`。 |
| 逐卡中文效果文案 | 已完成（repo 派生 runtime 真相源） | `textZh` 已接入 53 张基础卡，并已用于 zh-CN fallback 文本展示；当前来源是基于官方英文 `text` 的仓库内派生翻译，并结合中文卡图做运行时对照。 |
| 外部官方中文主真相源 | 未建立（非基础版 runtime blocker） | 当前仓库没有单独导入一份外部官方结构化中文卡表；若后续需要把中文字段升级成“外部官方主真相源”，仍需补独立来源合同。 |
| 完整官方计分语义 | 已完成（基础版 runtime 范围内） | 当前双人/多人基础版流程、公开弃牌链路、终局结算与胜者裁定已有 current live + online evidence；扩展版仍未接入。 |
