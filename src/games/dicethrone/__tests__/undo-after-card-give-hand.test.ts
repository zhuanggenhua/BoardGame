 ...
 * 2. 掷骰 → 确认骰子（CONFIRM_ROLL 只记日志，不创建撤销快照）
 * 3. 选定攻击（SELECT_ABILITY 创建快照 S2）
 * 4. 响应窗口打开，玩家 1 打出"抬一手"（响应窗口中 PLAY_CARD 不创建撤销快照）
 * 5. 玩家 1 选择对手骰子 → 重掷 → 交互完成 → 响应窗口关闭
 * 6. 玩家请求撤回 → 应能撤回到 S2（选定攻击前）
 * 7. 再次撤回 → 应能撤回到 S1
 *
 * 验证：撤回后快照栈正确递减，不会"卡住"
 *
 * 白名单命令（创建快照）：ADVANCE_PHASE, PLAY_CARD, PLAY_UPGRADE_CARD, SELL_CARD, SELECT_ABILITY
 */
...
describe('撤销系统：抬一手打出后的多级撤回', () => {
    it('完整流程：打出抬一手 → 完成交互 → 连续撤回应成功', () => {
        // 骰子值：5 个拳，选定攻击后，对手用“抬一手”重掷 1 颗
        const random = createQueuedRandom([1, 1, 1, 1, 1, /* 重掷 */ 2]);
        const runner = createRunner(random, true);

        // 阶段 1：推进到 offensiveRoll → 掷骰 → 确认 → 选定攻击 → 打出抬一手
        // 快照创建点：ADVANCE_PHASE(S1) + SELECT_ABILITY(S2) = 2
        ...
        // ADVANCE_PHASE(S1) + SELECT_ABILITY(S2) = 2
    ...
    it('maxSnapshots=5 限制：快照不超过上限', () => {
        const random = createQueuedRandom([1, 1, 1, 1, 1, 2, 3, 4, 5, 6]);
        const runner = createRunner(random, true);
        ...
        const result1 = runner.run({
            ...
            commands: [
                ...
                cmd('ROLL_DICE', '0'),
                cmd('CONFIRM_ROLL', '0'),       // 只记日志，不创建撤销快照
                cmd('SELECT_ABILITY', '0', { abilityId: 'fist-technique-5' }), // S2
