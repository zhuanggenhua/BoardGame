...
describe('撤销系统：抬一手打出后的多级撤回', () => {
    it('完整流程：打出抬一手 → 完成交互 → 连续撤回应成功', () => {
        ...
        // 快照创建点：ADVANCE_PHASE(S1) + CONFIRM_ROLL(S2) + SELECT_ABILITY(S3) = 3
        ...
        // ADVANCE_PHASE(S1) + CONFIRM_ROLL(S2) + SELECT_ABILITY(S3) = 3
    ...
    it('maxSnapshots=5 限制：快照不超过上限', () => {
        ...
        const result1 = runner.run({
            ...
            commands: [
                ...
                cmd('SELECT_ABILITY', '0', { abilityId: 'fist-technique-5' }), // S3
