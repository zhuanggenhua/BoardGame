...
describe('DiceThrone 战术家 / 咒缚海盗机制', () => {
    ...
    it('火药桶维持投骰结果可被弹一手改成 3 并避免爆炸', () => {
        ...
        const rollEvents = diceThroneFlowHooks.onPhaseEnter?.({
            state,
            from: 'discard',
            to: 'upkeep',
            command: command('ADVANCE_PHASE', '0'),
            random: createQueuedRandom([2]),
            exitEvents: [],
        } as Parameters<NonNullable<typeof diceThroneFlowHooks.onPhaseEnter>>[0]) as DiceThroneEvent[];
        const afterRoll = applyEvents(state.core, rollEvents);

        const play = executePipeline(
            { domain: DiceThroneDomain, systems: testSystems },
            { ...state, core: afterRoll },
            command('PLAY_CARD', '1', { cardId: 'card-flick' }),
            fixedRandom,
            ['0', '1'],
        );
        ...
        const modified = executePipeline(
            { domain: DiceThroneDomain, systems: testSystems },
            play.state,
            command('MODIFY_DIE', '1', { dieId: 0, newValue: 3 }),
            fixedRandom,
            ['0', '1'],
        );
    ...
    it('human 面做好标记会结算 CP、奖励骰伤害、抽牌和诅咒金币选择', () => {
        ...
        expect(result.state.core.players['1'].resources[RESOURCE_IDS.HP]).toBe(hpBefore - 2);
        expect(result.state.core.players['0'].hand.length).toBe(handBefore + 1);

        const prompt = getSimpleChoicePrompt(result.state, 'make-your-mark');
        const accept = prompt.options.find(option => (
            option.value as { statusId?: string; value?: number }
        ).statusId === STATUS_IDS.CURSED_COIN);
        expect(accept).toBeDefined();
