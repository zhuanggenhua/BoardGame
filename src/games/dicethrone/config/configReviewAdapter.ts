...
export const DICETHRONE_CONFIG_REVIEW_FIELD_DEFINITIONS: readonly DiceThroneConfigReviewFieldDefinition[] = [
  ...
  { key: 'sfxKey', valueKind: 'string', applicability: 'all', requiredForAudit: true, meaning: '音效绑定', evidence: [RULE_EVIDENCE.cards, RULE_EVIDENCE.abilities, RULE_EVIDENCE.tokens], fieldPath: (row) => fieldPath(row, 'sfxKey'), getValue: (row) => row.sfxKey },
];
