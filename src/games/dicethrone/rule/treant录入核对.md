...
| `野蛮生长 II` | 大顺子；造成 `8` 伤害并投掷 `5` 骰；增加 `1×树枝` 伤害；若投出树叶，获得生命源泉；养成 `1×螺旋` 树灵 | `乱花迷眼`：`2 树枝 + 2 螺旋`；施加刺藤；造成 `4` 不可防御伤害 | `WILD_GROWTH_2` 当前已按主路线 + `乱花迷眼` 双分支 `variants` 落地；对象级 direct closeout L3 已补齐 | `wild-growth` 升级版应保留主路线与 `乱花迷眼` 两个 `variants` | **结构与实现已对齐；对象级 L3 已达标，关键 displayOnly 收口 L4 已锁，剩批次级治理与旧文档统一收口** |
