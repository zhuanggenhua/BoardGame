...
- 截至 2026-06-04，`细心呵护 II / 培育`、`自然之触 II / 自然之怜`、`复仇枝蔓 II / 苦痛根系`、`野蛮生长 II / 乱花迷眼` 都已完成同卡双分支落地，并补到对象级 direct closeout `L3`。
...
| `复仇枝蔓 II` 主分支 / `乱花迷眼` | 标准 token + 伤害直结算链 | `复仇枝蔓 II` 主分支 | 是 | 都走标准 `grantToken(opponent, thorn, 1) + damage(*)` 管线，差异只剩伤害值与 `unblockable` 静态标记 | `乱花迷眼` 可复用该 family 的 `L4` |
...
| `野性怒吼 / 野性怒吼 II` | 大顺子 + `displayOnly` 5 骰后继续攻击 / 养成链 | — | 否 | 基础版与升级版同属 `大顺子` 触发；区别是基础版 `6` 伤害、升级版 `8` 伤害；奖励骰后续复用野蛮生长 II 的树枝加伤、树叶生命源泉、树灵养成链 | 已按 2026-07-03 反馈补基础版与升级版联查；仍需后续补完整玩家板槽位合同截图证据 |
...
| `野蛮生长 II` | `2 树枝 + 3 树叶`；造成 `4` 伤害并投掷 `5` 骰；增加 `1×树枝` 伤害；若投出树叶，获得生命源泉；养成 `1×螺旋` 树灵 | `乱花迷眼`：`2 树枝 + 2 螺旋`；施加刺藤；造成 `4` 不可防御伤害 | `WILD_GROWTH_2` 当前已按主路线 + `乱花迷眼` 双分支 `variants` 落地；对象级 direct closeout L3 已补齐 | `wild-growth` 升级版应保留主路线与 `乱花迷眼` 两个 `variants` | **结构与实现已对齐；对象级 L3 已达标，关键 displayOnly 收口 L4 已锁，剩批次级治理与旧文档统一收口** |
| `野性怒吼 II` | `大顺子`；造成 `8` 伤害并投掷 `5` 骰；增加 `1×树枝` 伤害；若投出树叶，获得生命源泉；养成 `1×螺旋` 树灵 | 当前裁图未见明确下挂第二分支 | 2026-07-03 已补 `WILD_ROAR_2`，触发条件为 `largeStraight`；基础版 `野性怒吼` 同为 `largeStraight`，造成 `6` 伤害；升级卡替换目标应为 `wild-roar` | `wild-roar` 基础/升级版应同属大顺子 family；`wild-growth` 不应承接 12345 大顺子反馈 | **2026-07-03 反馈修复已对齐；回归测试覆盖 12345 可选野性怒吼 II，且野蛮生长 II 不被 12345 误触发** |
...
- `野性怒吼 / 野性怒吼 II`
  - 应是同一基础技能 `wild-roar` 的大顺子基础/升级版本；升级版图面为 `大顺子` 造成 `8` 伤害并投掷 `5` 骰。
