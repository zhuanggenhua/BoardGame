...
function buildQidahenFeignedRetreatInteraction(
    ...
    const interaction = createSimpleChoice(
        `qidahen-feigned-retreat-${selection.targetRuntimeRegionId}`,
        state.core.factions[selection.factionId]?.playerId ?? state.core.currentPlayer,
        `诈败诱敌 · ${selection.targetRegionName}`,
        [{
            id: 'skip',
            label: '不使用',
...
function buildQidahenRaidAndAmbushInteraction(
    ...
    const options = selection.phase === 'select-troop-kind'
    ...
    const interaction = createSimpleChoice(
        ...
        state.core.factions[selection.factionId]?.playerId ?? state.core.currentPlayer,
        `偷袭与伏击 · ${selection.targetRegionName}`,
        options,
        {
