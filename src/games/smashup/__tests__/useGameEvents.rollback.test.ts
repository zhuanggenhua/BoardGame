...
describe('useGameEvents rollback consumer', () => {
  ...
  it('进入 immediate extra minion prompt family 后不再重复生成 after_interaction feedback', async () => {
    const limitEntry = createEntry(1, {
      type: SU_EVENTS.LIMIT_MODIFIED,
      payload: {
        playerId: '0',
        limitType: 'minion',
    ...
    const { result } = renderHook(
      () => useGameEvents({
        G: createState([limitEntry], '0', 'smashup_immediate_extra_minion'),
        myPlayerId: '0',
        fxBus,
        baseRefs: { current: new Map<number, HTMLElement>() },
      }),
    );
  ...
  it('进入 immediate extra action prompt family 后不再重复生成 after_interaction feedback', async () => {
    const limitEntry = createEntry(1, {
      type: SU_EVENTS.LIMIT_MODIFIED,
      payload: {
        playerId: '0',
    ...
    const { result } = renderHook(
      () => useGameEvents({
        G: createState([limitEntry], '0', 'smashup_immediate_extra_action'),
        myPlayerId: '0',
        fxBus,
        baseRefs: { current: new Map<number, HTMLElement>() },
      }),
