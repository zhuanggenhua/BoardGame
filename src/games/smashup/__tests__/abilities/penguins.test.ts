...
describe('企鹅派系能力', () => {
    ...
    it('跳上船预约的牌库顶伙伴不会被同一计分窗口的冰滑道抽走', () => {
        ...
        expect(prompted?.state.sys.interaction?.current).toBeDefined();
        const leapingOption = prompted?.state.sys.interaction?.current?.data.options.find((option: any) =>
            option.value?.kind === 'trigger'
            && option.value.triggerId.includes('penguins_leaping_aboard'),
        );
        expect(leapingOption).toBeDefined();
        if (!prompted || !leapingOption) throw new Error('企鹅 afterScoring 队列未提供跳上船选择');
        ...
        const chosen = runCommand(
            prompted.state,
            {
                ...
                playerId: '0',
                payload: { optionId: leapingOption.id },
            } as any,
            defaultTestRandom,
        );
