...
function assertZhongguoCardPreview(def: CardDef, expectedIndex: number, atlasId = SMASHUP_ATLAS_IDS.CARDS13): void {
...
describe('SmashUp zhongguo 四派系 intake 静态合同', () => {
    ...
    it.each(ZHONGGUO_FACTIONS)('$factionId 卡牌数量、拷贝数与卡图索引正确', (fixture) => {
        ...
        for (const [defId, index] of Object.entries(fixture.expectedCardIndexes)) {
            ...
            assertZhongguoCardPreview(def as CardDef, index, fixture.expectedCardAtlasId);
        }
    });
    ...
    it('四个 zhongguo 派系完成审计后不再标记为实施中', () => {
