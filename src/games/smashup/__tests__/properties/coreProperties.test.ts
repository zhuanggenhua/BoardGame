...
describe('Property 18: Me First 窗口协议', () => {
    ...
    test('计分前响应窗口普通行动卡若需要基地，必须选择达标基地', () => {
        const responseActionCard = makeCard('s-1', 'ancient_egyptians_plague_of_locusts', 'action');
        const state: SmashUpCore = {
            players: {
                '0': makePlayer('0', [SMASHUP_FACTION_IDS.ANCIENT_EGYPTIANS, SMASHUP_FACTION_IDS.PIRATES], { hand: [responseActionCard] }),
                '1': makePlayer('1', [SMASHUP_FACTION_IDS.ROBOTS, SMASHUP_FACTION_IDS.ALIENS]),
            },
            turnOrder: ['0', '1'],
            currentPlayerIndex: 0,
        ...
        expect(missingTarget.valid).toBe(false);
        expect(missingTarget.error).toBe('该行动卡需要选择一个达标基地');

        const invalidTarget = validate(matchState, {
            type: SU_COMMANDS.PLAY_ACTION,
            playerId: '0',
            payload: { cardUid: 's-1', targetBaseIndex: 0 },
        } as any);
        expect(invalidTarget.valid).toBe(false);
        expect(invalidTarget.error).toBe('只能选择达到临界点的基地');

        const validTarget = validate(matchState, {
            type: SU_COMMANDS.PLAY_ACTION,
            playerId: '0',
            payload: { cardUid: 's-1', targetBaseIndex: 1 },
        } as any);
