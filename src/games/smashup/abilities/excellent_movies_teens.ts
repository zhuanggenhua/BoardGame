...
function transferOngoingToBase(
    card: { uid: string; defId: string; ownerId: PlayerId; metadata?: Record<string, unknown>; talentUsed?: boolean },
    targetBaseIndex: number,
    now: number,
    sourcePlayerId?: PlayerId,
): SmashUpEvent {
    return buildOngoingAttachedEvent(card, targetBaseIndex, 'base', now, undefined, sourcePlayerId);
}
...
function buildCardBoxedEvent(playerId: PlayerId, card: CardInstance, from: 'hand' | 'deck' | 'discard', reason: string, now: number): SmashUpEvent {
    return {
...
function firstOtherBaseWithMinion(core: SmashUpCore, minionUid: string, fromBaseIndex: number): number | undefined {
    return core.bases.findIndex((_, index) => index !== fromBaseIndex && index >= 0);
}
...
function firstOwnPrintedPower3Minion(ctx: AbilityContext, baseIndex = ctx.baseIndex) {
    return firstMinionAtBase(ctx.state, baseIndex, minion =>
        minion.controller === ctx.playerId && isPrintedPower(minion.defId, 3)
    );
}
...
function buildActionSearchToHandEvents(ctx: AbilityContext, reason: string): SmashUpEvent[] {
    const player = ctx.state.players[ctx.playerId];
    if (!player) return [];
    const deckCard = player.deck.find(card => card.type === 'action');
    const discardCard = player.discard.find(card => card.type === 'action');
    const located = deckCard
        ? { card: deckCard, zone: 'deck' as const }
        : discardCard
            ? { card: discardCard, zone: 'discard' as const }
            : undefined;
    if (!located) return [];
    const recovered = buildRecoverOrTransferToHandEvent(ctx.playerId, located.card, located.zone, reason, ctx.now);
    return [
        ...(located.zone === 'deck'
            ? [inspectDeck(ctx.playerId, ctx.playerId, player.deck.length, reason, ctx.now)]
            : []),
        ...(recovered ? [recovered] : []),
    ];
}
...
function destroyTargetMinionAtBase(
    state: SmashUpCore,
    playerId: PlayerId,
    baseIndex: number,
    reason: string,
    ...
    if (!base) return [];
    const target = targetMinionUid
        ? base.minions.find(minion => minion.uid === targetMinionUid)
        : base.minions[0];
    if (!target) return [];
...
function registerWraithrustlers(): void {
    ...
    registerSimpleAbility('wraithrustlers_ancient_sumerian_god', 'onPlay', ctx => ({
        events: [],
    }));
