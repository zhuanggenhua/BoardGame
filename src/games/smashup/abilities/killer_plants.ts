...
function killerPlantWeedEater(ctx: AbilityContext): AbilityResult {
    ...
    const evt: SmashUpEvent = {
        ...
        payload: {
            ...
            baseIndex: ctx.baseIndex, baseDefId: ctx.state.bases[ctx.baseIndex].defId, baseDefId: ctx.state.bases[ctx.baseIndex].defId, baseDefId: ctx.state.bases[ctx.baseIndex].defId,
            amount: -2,
        ...
        for (const m of base.minions) {
            ...
            if (eligible.length === 1) {
                ...
                const playedEvt: MinionPlayedEvent = {
                    ...
                    payload: { playerId: m.controller, cardUid: card.uid, defId: card.defId, baseIndex: sproutBaseIndex, baseDefId: ctx.state.bases[sproutBaseIndex].defId, baseDefId: ctx.state.bases[sproutBaseIndex].defId, baseDefId: ctx.state.bases[sproutBaseIndex].defId, power },
                    timestamp: ctx.now,
                };
...
function killerPlantVenusManTrap(ctx: AbilityContext): AbilityResult {
    ...
    if (eligible.length === 1) {
        ...
        const playedEvt: MinionPlayedEvent = {
            type: SU_EVENTS.MINION_PLAYED,
            payload: { playerId: ctx.playerId, cardUid: card.uid, defId: card.defId, baseIndex: ctx.baseIndex, baseDefId: ctx.state.bases[ctx.baseIndex].defId, baseDefId: ctx.state.bases[ctx.baseIndex].defId, baseDefId: ctx.state.bases[ctx.baseIndex].defId, power },
            timestamp: ctx.now,
        };
        return {
            events: [
...
function killerPlantOvergrowthTrigger(ctx: TriggerContext): SmashUpEvent[] {
    ...
    for (let i = 0; i < ctx.state.bases.length; i++) {
        ...
        events.push({
            ...
            payload: { baseIndex: i, baseDefId: ctx.state.bases[i].defId, baseDefId: ctx.state.bases[i].defId, baseDefId: ctx.state.bases[i].defId, delta, reason: 'killer_plant_overgrowth' },
            timestamp: ctx.now,
