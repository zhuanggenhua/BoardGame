...
import { TITAN_CARDS } from './titans';
...
export function getFactionTitan(factionId: string): TitanCardDef | undefined {
    for (const def of _cardRegistry.values()) {
        if (def.type === 'titan' && def.faction === factionId) return def;
    }
    return undefined;
}
...
export function getFactionCards(factionId: FactionId): CardDef[] {
    return Array.from(_cardRegistry.values()).filter(c => c.faction === factionId && c.type !== 'titan');
}
