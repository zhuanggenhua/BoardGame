...
import {
    ...
    type FieldSourceTargetSource,
    type FieldSourceToMinionTargetValue,
} from './fieldInteractionOptions';
export {
    FIELD_SOURCE_TARGET_PROMPT_TARGET_TYPE,
...
export type {
    FieldObjectType,
    FieldSourceActionValue,
    FieldSourceTargetSource,
    FieldSourceTargetTarget,
    FieldSourceTargetValue,
    FieldSourceToBaseTargetValue,
    FieldSourceToMinionTargetValue,
} from './fieldInteractionOptions';
import type {
    SmashUpCore,
    MinionOnBase,
...
function collectMinionPlayedFollowupTriggers(
    ...
    const queuedBase = collectBaseAbilityTriggers({
        ...
        triggerMinionFromDeck: params.playedEvt.payload.fromDeck === true,
        frameId,
        sourceEventId,
        now,
    });
...
/**
 * 任意场上来源对象 -> 随从目标的共享包装。
 * 用于“先点来源本体，再点目标随从”的交互；不要再让普通 minion targetType 反向代表来源。
 */
export function buildFieldSourceToMinionTargetOptions<TExtra extends Record<string, unknown> = Record<string, never>>(
    source: FieldSourceTargetSource,
    targets: { uid: string; defId: string; baseIndex: number; label: string }[],
    context: {
        state: SmashUpCore;
        sourcePlayerId: PlayerId;
        sourceDefId?: string;
    ...
    extra?: TExtra,
): EnginePromptOption<FieldSourceToMinionTargetValue<TExtra>>[] {
    const sourceKind = context.sourceKind
        ?? (source.type === 'action' || source.type === 'ongoing' ? 'action' : 'nonAction');
    const effectSourceKind = inferSemanticSourceKind(sourceKind, context.sourceDefId ?? source.defId);
    const respectActionProtection = context.respectActionProtection ?? sourceKind === 'action';
    const filteredTargets = (context.semanticRole ?? 'target') !== 'target'
        ? targets
        : targets.filter(target => {
            const minion = context.state.bases[target.baseIndex]?.minions.find(candidate => candidate.uid === target.uid);
            if (!minion) return false;
            return isMinionTargetAllowed(context.state, minion, target.baseIndex, {
                sourcePlayerId: context.sourcePlayerId,
                sourceKind: effectSourceKind,
                effectType: context.effectType ?? 'destroy',
                respectActionProtection,
                mode: 'preview',
            });
        });
    ...
    return buildFieldSourceTargetOptions<TExtra>(
        ...
        filteredTargets.map(target => {
            ...
            return {
                ...
                label: target.label,
                ...(minion ? {
                    aiHint: buildMinionTargetAiHint({
                        state: context.state,
                        minion,
                        sourcePlayerId: context.sourcePlayerId,
        ...
        extra,
    ) as EnginePromptOption<FieldSourceToMinionTargetValue<TExtra>>[];
}
