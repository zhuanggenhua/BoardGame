...
function getMandatoryFrameTriggers(state: MatchState<SmashUpCore>, frameId: string): TriggerInstance[] {
    return getSessionFrameTriggers(state, frameId).filter(trigger => trigger.resolutionClass === 'mandatory');
}
...
function getOptionalFrameTriggers(
    ...
    return getSessionFrameTriggers(state, frameId).filter(
        trigger => trigger.resolutionClass === 'optional' && trigger.ownerPlayerId === playerId,
    );
}
...
function applyReactionPostProcessing(
    ...
    const postProcessState: MatchState<SmashUpCore> = {
        ...state,
        core: reducedCore,
        sys: {
            ...state.sys,
    ...
    const postProcessed = reactionPostProcessor
        ...
        : {
            events: rawEvents,
            matchState: postProcessState,
        };

    return {
        state: postProcessed.matchState ?? {
            ...state,
            core: reducedCore,
        },
...
function createSessionFromPendingFrame(
    ...
    const phase: SmashUpReactionPhase = frameTriggers.some(trigger => trigger.resolutionClass === 'mandatory')
        ? 'mandatory'
        : 'optional';
