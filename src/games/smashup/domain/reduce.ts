...
export function reduce(state: SmashUpCore, event: SmashUpEvent): SmashUpCore {
    switch (event.type) {
        ...
        case SU_EVENTS.ACTION_PLAYED: {
            const { playerId, cardUid, defId: payloadDefId, isExtraAction, fromBuried, fromDiscard, fromStored, ownerId, discardPlaySourceId, consumesNormalLimit } = event.payload as any;
            const player = state.players[playerId];
            ...
            const defId = card?.defId ?? buriedLookup?.buried.defId ?? payloadDefId;
