...
import type { SmashUpCore, BaseInPlay, MinionOnBase, TitanState } from '../domain/types';
...
export const BaseZone: React.FC<{
    ...
    phase?: string;
}> = ({ base, baseIndex, core, turnOrder, isDeployMode, isMinionSelectMode, selectableMinionUids, multiSelectedMinionUids, isSelectable, isDimmed, selectableOngoingUids, isMyTurn, myPlayerId, dispatch, onClick, onMinionSelect, onOngoingSelect, onViewMinion, onViewAction, onViewBase, onViewTitan, tokenRef, isTutorialTargetAllowed, phase }) => {
    ...
    return (
        ...
        >
            ...
            {titansOnBase.length > 0 && (
                <div
                    className="mt-[0.6vw] flex items-center justify-center gap-[0.6vw] z-20"
                >
                    {titansOnBase.map((titan, index) => (
                        <TitanCard
                            key={titan.uid}
                            titan={titan}
                            core={core}
                            ...
                            dispatch={dispatch}
                            onView={() => onViewTitan(titan.defId)}
...
const TitanCard: React.FC<{
    ...
    const resolvedName = resolveCardName(def, t) || titan.defId;
    const resolvedText = resolveCardText(def, t);
    const title = resolvedText ? `${resolvedName}\n${resolvedText}` : resolvedName;
    const conf = PLAYER_CONFIG[parseInt(titan.controllerId, 10) % PLAYER_CONFIG.length];
    const tutorialAllowed = isTutorialTargetAllowed ? isTutorialTargetAllowed(titan.uid) : true;
    const titanActivationKey = `titan-${titan.uid}`;
    const isTitanActivationArmed = isActivationArmed(titanActivationKey);

    const canUseTalent = Boolean(
        def?.type === 'titan'
        && def.activatableAbilityKinds?.includes('talent')
        && def.abilityTags?.includes('talent')
        && resolveTalent(titan.defId)
        && isMyTurn
        && phase === 'playCards'
        && titan.controllerId === myPlayerId
        && tutorialAllowed
        && !titan.talentUsed,
    );
    const canActivateOngoing = Boolean(
        def?.type === 'titan'
        && def.activatableAbilityKinds?.includes('ongoing')
        && def.abilityTags?.includes('ongoingActivation')
    ...
    const canActivateSpecial = Boolean(
        def?.type === 'titan'
        && def.activatableAbilityKinds?.includes('special')
        && def.abilityTags?.includes('special')
        && resolveSpecial(titan.defId)
        && isMyTurn
        && titan.controllerId === myPlayerId
        && tutorialAllowed
        && !isSpecialLimitBlocked(core, titan.defId, baseIndex)
        && (phase !== 'scoreBases' || getScoringEligibleBaseIndices(core).includes(baseIndex)),
    );
    const canActivate = canUseTalent || canActivateOngoing || canActivateSpecial;
    ...
    const activate = () => {
        ...
        if (canActivateOngoing) {
            dispatch(SU_COMMANDS.ACTIVATE_TITAN_ONGOING, { titanUid: titan.uid, baseIndex });
            return;
        }
        if (canActivateSpecial) {
            dispatch(SU_COMMANDS.ACTIVATE_SPECIAL, { titanUid: titan.uid, baseIndex });
        }
    };

    const rotation = (index % 2 === 0 ? -1 : 1) * (index + 1);
    ...
    return (
        <motion.div
            ...
            onClick={(event) => {
                event.stopPropagation();
                if (isCoarsePointer && canActivate) {
            ...
            className={`group relative aspect-[0.714] bg-white p-[0.2vw] rounded-[0.2vw] border-[0.15vw] shadow-md transition-all ${
                canActivate
                    ? isTitanActivationArmed
                        ? 'cursor-pointer border-amber-300 ring-4 ring-amber-300 shadow-[0_0_18px_rgba(251,191,36,0.75)]'
                        : 'cursor-pointer border-amber-400 ring-2 ring-amber-400 shadow-[0_0_15px_rgba(251,191,36,0.6)]'
            ...
            animate={canActivate
                ? { scale: 1, opacity: 1, y: 0, rotate: [rotation - 1, rotation + 1, rotation - 1], transition: { rotate: { repeat: Infinity, duration: 1.6, ease: 'easeInOut' } } }
                : { scale: 1, opacity: 1, y: 0, rotate: rotation }}
            transition={{ type: 'spring', stiffness: 320, damping: 24, delay: index * 0.05 }}
        >
            <div className="w-full h-full bg-slate-100 relative overflow-hidden rounded-[0.1vw]">
                <CardPreview
                ...
                />
                {!def?.previewRef && (
                    <div className="absolute inset-0 flex flex-col items-center justify-center bg-gradient-to-br from-slate-100 to-slate-300 text-center px-[0.4vw]">
                        <div className="text-[0.7vw] font-black text-slate-900 leading-tight">{resolvedName}</div>
                        <div className="mt-[0.25vw] text-[0.45vw] font-semibold text-slate-700">Titan</div>
                    </div>
                )}
                {canActivate && (
                    <motion.div
            ...
            <button
                ...
                className="absolute top-[0.15vw] right-[0.15vw] w-[1.4vw] h-[1.4vw] flex items-center justify-center bg-black/60 hover:bg-amber-500/80 text-white rounded-full opacity-0 hover:opacity-100 group-hover:opacity-100 transition-[opacity,background-color] duration-200 shadow-lg border border-white/20 z-40 cursor-zoom-in"
            >
            ...
            {titan.powerCounters > 0 && (
                <div className="absolute -top-[0.35vw] -left-[0.35vw] min-w-[1.2vw] h-[1.2vw] rounded-full flex items-center justify-center text-[0.55vw] font-black text-amber-900 bg-gradient-to-br from-amber-300 to-amber-500 shadow-md border-[0.1vw] border-white px-[0.1vw] z-30">
                    +{titan.powerCounters}
                </div>
            )}

            {def?.type === 'titan' && def.abilityTags?.includes('talent') && titan.talentUsed && (
