...
import { PromptOverlay, resolveI18nKeys, resolveI18nParams, resolvePromptText } from './ui/PromptOverlay';
...
import {
    ...
    shouldPreferMinionReplacementSelection,
    type MinionUiPlayIntent,
...
import {
    ...
    isFieldSourceActionValue,
    isFieldSourceTargetValue,
...
const SmashUpBoard: FC<Props> = ({ G, dispatch, playerID: rawPlayerID, reset, matchData, isMultiplayer }) => {
    ...
    const handActionChoiceStyle = {
        ...floatingHintStyle,
        bottom: `${layout.floatingActionBottom + (isMobileViewport ? 76 : 96)}px`,
    };
    ...
    const resolvePromptOptionLabel = useCallback((opt: { label: string; labelKey?: string; labelParams?: Record<string, string | number> }) => {
        if (typeof opt.labelKey === 'string') {
            return t(opt.labelKey, {
                ...(resolveI18nParams(opt.labelParams, t) ?? {}),
    ...
    const [pendingHandActionSpecialMode, setPendingHandActionSpecialMode] = useState<'hand-special' | 'minion-replacement' | null>(null);
    ...
    // - overlay: 选项不全在当前手牌本体上，继续走 PromptOverlay
    ...
    const fieldSourceTargetPrompt = useMemo(() => {
        return buildFieldSourceTargetPromptModel({
            isCurrentPromptForPlayer,
            targetType: currentPromptData?.targetType,
            options: currentPrompt?.options,
        });
    }, [currentPrompt?.options, currentPromptData?.targetType, isCurrentPromptForPlayer]);
    const fieldSourceActionPrompt = useMemo(() => {
        return buildFieldSourceActionPromptModel({
            isCurrentPromptForPlayer,
            targetType: currentPromptData?.targetType,
            options: currentPrompt?.options,
        });
    ...
    }, [currentPrompt, fieldSourceActionPrompt, resolvePromptOptionLabel]);
    const fieldSourceTargetOptionIdsByBaseIndex = fieldSourceTargetSelection.targetOptionIdsByBaseIndex;
    const fieldSourceTargetOptionIdsByMinionUid = fieldSourceTargetSelection.targetOptionIdsByMinionUid;
    const fieldSourceTargetSelectableMinionUids = fieldSourceTargetSelection.selectableMinionUids;
    const fieldSourceTargetSelectableOngoingUids = fieldSourceTargetSelection.selectableOngoingUids;
    const fieldSourceTargetSelectableTitanUids = fieldSourceTargetSelection.selectableTitanUids;
    const fieldSourceActionOptionIdsBySourceUid = fieldSourceActionPrompt?.sourceOptionIdsByUid;
    const fieldSourceActionSelectableMinionUids = fieldSourceActionPrompt?.sourceMinionUids;
    const fieldSourceActionSelectableOngoingUids = fieldSourceActionPrompt?.sourceOngoingUids;
    const fieldSourceActionSelectableTitanUids = fieldSourceActionPrompt?.sourceTitanUids;
    ...
    const reactionChoicePlayableCardUids = useMemo<Set<string>>(() => {
        const result = new Set<string>();
        if (!isReactionChoicePrompt || !isCurrentPromptForPlayer || !currentPrompt) return result;
        for (const opt of currentPrompt.options) {
            if (opt.disabled) continue;
            if (isSmashUpReactionHandPlayValue(opt.value)) {
                result.add(opt.value.cardUid);
            }
        }
        return result;
    }, [currentPrompt, isCurrentPromptForPlayer, isReactionChoicePrompt]);
    const reactionChoiceTargetStateByCardUid = useMemo<Map<string, {
        baseIndices: Set<number>;
        minionUids: Set<string>;
        hasUntargetedAction: boolean;
    }>>(() => {
        ...
        for (const opt of currentPrompt.options) {
            if (opt.disabled || !isSmashUpReactionHandPlayValue(opt.value)) continue;
            const entry = result.get(opt.value.cardUid) ?? {
                baseIndices: new Set<number>(),
                minionUids: new Set<string>(),
                hasUntargetedAction: false,
            };
            const baseIndex = getSmashUpReactionChoiceBaseIndex(opt.value);
            if (typeof baseIndex === 'number') {
                entry.baseIndices.add(baseIndex);
            }
            const targetMinionUid = getSmashUpReactionChoiceTargetMinionUid(opt.value);
            if (targetMinionUid) {
                entry.minionUids.add(targetMinionUid);
            }
            if (opt.value.kind === 'play_action' && typeof baseIndex !== 'number' && !targetMinionUid) {
                entry.hasUntargetedAction = true;
            }
            result.set(opt.value.cardUid, entry);
        }

        return result;
    }, [currentPrompt, isCurrentPromptForPlayer, isReactionChoicePrompt]);
    const selectedReactionChoiceTargetState = useMemo(() => {
        if (!selectedCardUid) return undefined;
        return reactionChoiceTargetStateByCardUid.get(selectedCardUid);
    }, [reactionChoiceTargetStateByCardUid, selectedCardUid]);
    const reactionChoiceDisabledCardUids = useMemo<Set<string> | undefined>(() => {
        if (!isReactionChoicePrompt || !isCurrentPromptForPlayer || !myPlayer) return undefined;
        const disabled = new Set<string>();
        for (const card of myPlayer.hand) {
            if (!reactionChoicePlayableCardUids.has(card.uid)) {
                disabled.add(card.uid);
            }
        }
        return disabled.size > 0 ? disabled : undefined;
    }, [isCurrentPromptForPlayer, isReactionChoicePrompt, myPlayer, reactionChoicePlayableCardUids]);
    const findReactionPlayOptionId = useCallback((params: {
        ...
        const option = currentPrompt.options.find(opt => {
            ...
            return matchesSmashUpReactionHandPlayTarget(opt.value, params);
        });
        return option?.id;
    }, [currentPrompt]);
    ...
    const handPromptDisabledUids = handPromptCardState.disabledCardUids;
    ...
    const isMultiDirectHandSelect = isDirectHandSelectPrompt
        && !!currentPrompt?.multi
        && multiMinionConstraints.max !== 1;
    ...
    const multiSelectedMinionUids = useMemo<Set<string>>(() => {
        ...
        for (const optId of multiSelectedOptionIds) {
            ...
            const val = opt?.value as { minionUid?: string; cardUid?: string; targetType?: string } | undefined;
            const minionUid = val?.minionUid ?? (val?.targetType === 'minion' ? val.cardUid : undefined);
            if (minionUid) uids.add(minionUid);
        }
        return uids;
    ...
    const multiSelectedHandCardUids = useMemo<Set<string>>(() => {
        if (!isMultiDirectHandSelect || !currentPrompt) return new Set();
        const uids = new Set<string>();
        for (const optId of multiSelectedOptionIds) {
            const opt = currentPrompt.options.find(o => o.id === optId);
            const val = opt?.value as { cardUid?: string } | undefined;
    ...
    const boardSelectableMinionUids = useMemo<Set<string>>(() => {
        ...
        const uids = new Set<string>();
        for (const opt of currentPrompt.options) {
            const val = opt.value as { minionUid?: string; cardUid?: string; targetType?: string; skip?: boolean } | undefined;
            if (val?.skip === true) continue;
            const minionUid = val?.minionUid ?? (val?.targetType === 'minion' ? val.cardUid : undefined);
            if (minionUid) uids.add(minionUid);
        }
    ...
    const interactionSelectTitle = useMemo(() => {
        ...
        else if ((isMinionSelectPrompt || fieldSourceTargetPrompt || fieldSourceActionPrompt) && currentPrompt) raw = currentPrompt.title;
        ...
        return raw
            ...
            : '';
    }, [isBaseSelectPrompt, isBuriedSelectPrompt, isMinionSelectPrompt, fieldSourceTargetPrompt, fieldSourceActionPrompt, isMonsterSelectPrompt, isDirectHandSelectPrompt, isOngoingSelectPrompt, isBoardSelectPrompt, isTitanReactionPrompt, currentPrompt, t]);
    ...
    const interactionSelectSubtitle = useMemo(() => {
        ...
        else if ((isMinionSelectPrompt || fieldSourceTargetPrompt || fieldSourceActionPrompt) && currentPrompt) raw = currentPrompt.subtitle ?? '';
        ...
        return raw
            ...
            : '';
    }, [isBaseSelectPrompt, isBuriedSelectPrompt, isMinionSelectPrompt, fieldSourceTargetPrompt, fieldSourceActionPrompt, isMonsterSelectPrompt, isDirectHandSelectPrompt, isOngoingSelectPrompt, isBoardSelectPrompt, currentPrompt, t]);
    ...
    const activePromptSurface = useMemo<'none' | 'hand' | 'board' | 'overlay'>(() => {
        ...
        if (isBaseSelectPrompt || isBuriedSelectPrompt || isMinionSelectPrompt || isMonsterSelectPrompt || fieldSourceTargetPrompt || fieldSourceActionPrompt || isOngoingSelectPrompt || isBoardSelectPrompt || isDiscardMinionPrompt || isDiscardCardPrompt || isTitanReactionPrompt || titanPromptBaseSelection) {
            return 'board';
        }
        return 'overlay';
    }, [
        ...
        fieldSourceTargetPrompt,
        fieldSourceActionPrompt,
    ...
    const resolvePlayableCardMode = useCallback((card: CardInstance): 'minion' | 'minion-replacement' | 'action' | 'ongoing' | 'ongoing-minion' | 'action-minion' | 'hand-special' | null => {
    ...
    const getDeployableBaseStateForCard = useCallback((card: CardInstance, cardMode: 'minion' | 'minion-replacement' | 'action' | 'ongoing' | 'ongoing-minion' | 'action-minion' | 'hand-special' | null): { deployableBaseIndices: Set<number>; deployBlockReason: string | null } => {
        const indices = new Set<number>();
        ...
        if (isReactionChoicePrompt && isCurrentPromptForPlayer) {
            const reactionTargetState = reactionChoiceTargetStateByCardUid.get(card.uid);
            if (!reactionTargetState) {
                return { deployableBaseIndices: indices, deployBlockReason: t('ui.card_not_in_options') };
            }
            return {
                deployableBaseIndices: new Set(reactionTargetState.baseIndices),
        ...
        for (let i = 0; i < core.bases.length; i++) {
            if (cardMode === 'minion' || cardMode === 'minion-replacement') {
                const playPlan = resolveMinionUiPlayPlan(matchState, playerID, card, i, {
                    intent: cardMode === 'minion-replacement' ? 'replacement' : 'auto',
                });
        ...
        return {
            deployableBaseIndices: indices,
            deployBlockReason: cardMode === 'minion' || cardMode === 'minion-replacement' ? firstValidationError : null,
        };
    }, [core, getBaseLimitedQuotaMeta, isAfterScoringResponse, isCurrentPromptForPlayer, isMeFirstResponse, isReactionChoicePrompt, matchState, playerID, reactionChoiceTargetStateByCardUid, reactionWindow?.sourceBaseIndex, reactionWindow?.windowType, t]);
    ...
    const ongoingMinionTargetUids = useMemo<Set<string>>(() => {
        ...
        if (isReactionChoicePrompt && isCurrentPromptForPlayer && selectedCardUid) {
            return new Set(reactionChoiceTargetStateByCardUid.get(selectedCardUid)?.minionUids ?? []);
        }
        const card = myPlayer?.hand.find(entry => entry.uid === selectedCardUid);
        return collectMinionTargetUids(deployableBaseIndices, card);
    }, [collectMinionTargetUids, deployableBaseIndices, isCurrentPromptForPlayer, isReactionChoicePrompt, myPlayer?.hand, playerID, reactionChoiceTargetStateByCardUid, selectedCardMode, selectedCardUid]);
    ...
    // --- Handlers ---
    const handlePlayMinion = useCallback((cardUid: string, baseIndex: number, intent: MinionUiPlayIntent = 'auto') => {
        if (!isTutorialCommandAllowed(SU_COMMANDS.PLAY_MINION)) {
            playDeniedSound();
            return;
        }
        const card = myPlayer?.hand.find(entry => entry.uid === cardUid);
        const playPlan = card ? resolveMinionUiPlayPlan(G, rootPid, card, baseIndex, { intent }) : null;
        ...
        if (!playAsAction && isReactionChoicePrompt && isCurrentPromptForPlayer) {
            playDeniedSound();
        ...
        dispatch(SU_COMMANDS.PLAY_MINION, {
            cardUid,
            baseIndex,
            ...(playAsAction ? { playAsAction: true } : {}),
            ...(playPlan?.replacementHandCardUid ? { replacementHandCardUid: playPlan.replacementHandCardUid } : {}),
        });
        setSelectedCardUid(null);
        setSelectedCardMode(null);
    }, [G, dispatch, isCurrentPromptForPlayer, isReactionChoicePrompt, isTutorialCommandAllowed, myPlayer?.hand, respondReactionPlayOption, rootPid, t]);
    ...
    const handlePlayOngoingAction = useCallback((cardUid: string, baseIndex: number) => {
        ...
        setSelectedCardUid(null);
        setSelectedCardMode(null);
    }, [dispatch, isCurrentPromptForPlayer, isReactionChoicePrompt, isTutorialCommandAllowed, myPlayer, respondReactionPlayOption, t]);
    ...
    const handlePlayActionWithoutTarget = useCallback((card: CardInstance) => {
        ...
        setSelectedCardMode(null);
        return true;
    }, [dispatch, isCurrentPromptForPlayer, isReactionChoicePrompt, isTutorialCommandAllowed, isTutorialTargetAllowed, myPlayer, reactionWindow, respondReactionPlayOption, t, validateImmediateActionPlay, toastCommandFeedback]);
    ...
    const handleBaseClick = useCallback((index: number) => {
        ...
        if (fieldSourceTargetPrompt) {
            ...
            const optionId = fieldSourceTargetOptionIdsByBaseIndex.get(index);
            if (!optionId) {
                toast(t('ui.invalid_base_target'));
                return;
            }
        ...
        if (selectedCardUid) {
            ...
            } else if (selectedCardMode === 'minion-replacement') {
                handlePlayMinion(selectedCardUid, index, 'replacement');
            } else {
    ...
    const handleCardClick = useCallback((card: CardInstance) => {
        ...
        if (isDirectHandSelectPrompt && currentPrompt) {
            const option = currentPrompt.options.find(
                opt => !opt.disabled && (opt.value as { cardUid?: string })?.cardUid === card.uid
            );
            if (option) {
        ...
        if (isReactionChoicePrompt && isCurrentPromptForPlayer) {
            const targetState = reactionChoiceTargetStateByCardUid.get(card.uid);
            if (!targetState || !reactionChoicePlayableCardUids.has(card.uid)) {
                playDeniedSound();
                return;
            }
            ...
            if (targetState.minionUids.size > 0) {
                const cardDef = getCardDef(card.defId) as ActionCardDef | FusionCardDef | undefined;
                const isOngoing = cardDef?.subtype === 'ongoing';
                setSelectedCardUid(card.uid);
                setSelectedCardMode(isOngoing ? 'ongoing-minion' : 'action-minion');
                setPendingFusionChoiceUid(null);
                return;
            }

            if (targetState.baseIndices.size > 0) {
                setSelectedCardUid(card.uid);
            ...
            if (targetState.hasUntargetedAction && respondReactionPlayOption({ kind: 'play_action', cardUid: card.uid })) {
                setSelectedCardUid(null);
                setSelectedCardMode(null);
                setPendingFusionChoiceUid(null);
                return;
        ...
        if (isInResponseWindow) {
            if (isReactionChoicePrompt && isCurrentPromptForPlayer && !reactionChoicePlayableCardUids.has(card.uid)) {
                playDeniedSound();
                return;
            }
        ...
        if (card.type === 'minion') {
            const regularMinionBaseIndices = getRegularMinionPlayableBaseIndices(matchState, rootPid, card.uid);
            if (shouldOfferHandSpecialActionChoice({
                matchState,
                playerId: rootPid,
                card,
                normalPlayableBaseIndices: regularMinionBaseIndices,
            })) {
                setPendingHandActionChoiceUid(card.uid);
                setPendingHandActionSpecialMode('hand-special');
                setPendingFusionChoiceUid(null);
                setSelectedCardUid(card.uid);
                setSelectedCardMode(null);
                return;
            ...
            if (shouldOfferMinionReplacementActionChoice({
                matchState,
                playerId: rootPid,
                card,
                regularPlayableBaseIndices: regularMinionBaseIndices,
            })) {
                setPendingHandActionChoiceUid(card.uid);
                setPendingHandActionSpecialMode('minion-replacement');
                setPendingFusionChoiceUid(null);
                setSelectedCardUid(card.uid);
                setSelectedCardMode(null);
                return;
            }
            if (shouldPreferMinionReplacementSelection({
                matchState,
                playerId: rootPid,
                card,
                regularPlayableBaseIndices: regularMinionBaseIndices,
            })) {
                if (selectedCardUid === card.uid && selectedCardMode === 'minion-replacement') {
                    setSelectedCardUid(null);
                    setSelectedCardMode(null);
                } else {
                    setSelectedCardUid(card.uid);
                    setSelectedCardMode('minion-replacement');
                }
                setPendingHandActionChoiceUid(null);
                setPendingHandActionSpecialMode(null);
                return;
    ...
    const confirmHandActionChoice = useCallback((mode: 'minion' | 'hand-special') => {
        ...
        const specialMode = pendingHandActionSpecialMode ?? 'hand-special';
        setPendingHandActionChoiceUid(null);
        setPendingHandActionSpecialMode(null);
        ...
        if (mode === 'minion') {
            const normalPlayableBaseIndices = getRegularMinionPlayableBaseIndices(G, rootPid, card.uid);
            ...
            specialMode === 'hand-special'
                ? getHandSpecialPlayableBaseIndices(G, rootPid, card.uid).size === 0
                : getMinionReplacementPlayableBaseIndices(G, rootPid, card).size === 0
        ) {
            playDeniedSound();
            toastCommandFeedback(t('ui.no_valid_targets'));
            setSelectedCardUid(null);
        ...
        setSelectedCardUid(card.uid);
        setSelectedCardMode(mode === 'minion' ? 'minion' : specialMode);
    }, [G, myPlayer, pendingHandActionChoiceUid, pendingHandActionSpecialMode, rootPid, t, toastCommandFeedback]);
    ...
    const handleMinionSelect = useCallback((minionUid: string, baseIndex: number) => {
        if (fieldSourceActionPrompt) {
            const optionId = fieldSourceActionOptionIdsBySourceUid?.get(minionUid);
            if (optionId) {
                setSelectedCardUid(null);
                setSelectedCardMode(null);
                setSelectedFieldPromptSourceUid(null);
        ...
        if (fieldSourceTargetPrompt) {
            const selectedSourceEntry = selectedFieldPromptSourceUid
                ? fieldSourceTargetPrompt.sourceTargetOptions.get(selectedFieldPromptSourceUid)
                : undefined;
            const targetOptionId = selectedSourceEntry?.targetOptionIdsByMinionUid.get(minionUid);
            if (targetOptionId) {
                setSelectedCardUid(null);
                setSelectedCardMode(null);
                setSelectedFieldPromptSourceUid(null);
                respondCurrentPrompt({ optionId: targetOptionId });
                return;
            }
            if (fieldSourceTargetPrompt.sourceMinionUids.has(minionUid)) {
                setSelectedCardUid(null);
                setSelectedCardMode(null);
                setSelectedFieldPromptSourceUid((current) => current === minionUid ? null : minionUid);
                return;
            }
            playDeniedSound();
            return;
        ...
        if ((isMinionSelectPrompt || isBoardSelectPrompt) && currentPrompt) {
            ...
            const option = currentPrompt.options.find((opt) => {
                if (opt.disabled) return false;
                const value = opt.value as { minionUid?: string; cardUid?: string; targetType?: string } | undefined;
                const optionMinionUid = value?.minionUid ?? (value?.targetType === 'minion' ? value.cardUid : undefined);
                return optionMinionUid === minionUid;
            });
            if (!option) return;
    ...
    const handleOngoingSelect = useCallback((ongoingUid: string) => {
        if (fieldSourceActionPrompt) {
            const optionId = fieldSourceActionOptionIdsBySourceUid?.get(ongoingUid);
            if (optionId) {
                setSelectedCardUid(null);
                setSelectedCardMode(null);
                setSelectedFieldPromptSourceUid(null);
        ...
        if (fieldSourceTargetPrompt) {
            if (fieldSourceTargetPrompt.sourceOngoingUids.has(ongoingUid)) {
                setSelectedCardUid(null);
                setSelectedCardMode(null);
    ...
    const handleFieldSourceTitanSelect = useCallback((titanUid: string) => {
        if (fieldSourceActionPrompt) {
            const optionId = fieldSourceActionOptionIdsBySourceUid?.get(titanUid);
            if (optionId) {
                setSelectedCardUid(null);
                setSelectedCardMode(null);
                setSelectedFieldPromptSourceUid(null);
        ...
        if (fieldSourceTargetPrompt.sourceTitanUids.has(titanUid)) {
            setSelectedCardUid(null);
            setSelectedCardMode(null);
            setSelectedFieldPromptSourceUid((current) => current === titanUid ? null : titanUid);
            return;
        }
        playDeniedSound();
    ...
    const handleCardDragPlay = useCallback((card: CardInstance, dropTarget: HandAreaDropTarget) => {
        ...
        const commandType = cardMode === 'minion' || cardMode === 'minion-replacement' ? SU_COMMANDS.PLAY_MINION : SU_COMMANDS.PLAY_ACTION;
        ...
        if (card.type === 'minion' && cardMode === 'minion') {
            const regularMinionBaseIndices = getRegularMinionPlayableBaseIndices(G, rootPid, card.uid);
            if (shouldOfferHandSpecialActionChoice({
                matchState: G,
                playerId: rootPid,
                card,
            ...
            if (shouldPreferMinionReplacementSelection({
                ...
                const replacementBaseIndices = getMinionReplacementPlayableBaseIndices(G, rootPid, card);
                if (dropTarget.kind !== 'base' || !replacementBaseIndices.has(dropTarget.baseIndex)) {
                    playDeniedSound();
                    return;
                }
                handlePlayMinion(card.uid, dropTarget.baseIndex, 'replacement');
                setPendingHandActionChoiceUid(null);
                setPendingHandActionSpecialMode(null);
                return;
            }
        ...
        handlePlayMinion(card.uid, dropTarget.baseIndex, cardMode === 'minion-replacement' ? 'replacement' : 'auto');
    ...
    return (
        <UndoProvider value={{ G: (matchState ?? G) as MatchState<SmashUpCore>, dispatch, playerID, isGameOver: !!isGameOver, isLocalMode: !isMultiplayer }}>
            ...
            <div className="relative w-full h-full bg-[#3e2723] overflow-hidden font-sans select-none">
                ...
                {/* --- 交互选择提示横幅（基地/随从/手牌选择） --- */}
                <AnimatePresence>
                    {(isBaseSelectPrompt || isBuriedSelectPrompt || isMinionSelectPrompt || !!fieldSourceTargetPrompt || !!fieldSourceActionPrompt || isMonsterSelectPrompt || isDirectHandSelectPrompt || isOngoingSelectPrompt || isBoardSelectPrompt || isTitanReactionPrompt) && (
                        <motion.div
                ...
                {/* --- 场上来源-目标交互浮动操作栏（跳过/不发动按钮） --- */}
                <AnimatePresence>
                    {((fieldSourceTargetPrompt && fieldSourceTargetExtraOptions.length > 0) || (fieldSourceActionPrompt && fieldSourceActionExtraOptions.length > 0)) && (
                        <motion.div
                        ...
                        >
                            <div className="flex gap-3 pointer-events-auto">
                                {(fieldSourceTargetPrompt ? fieldSourceTargetExtraOptions : fieldSourceActionExtraOptions).map(opt => (
                                    <SmashUpGameButton
                                        key={opt.id}
                                        variant="secondary"
                                        size="md"
                ...
                {/* --- 手牌选择浮动操作栏（多选确认 + 跳过按钮） --- */}
                <AnimatePresence>
                    {isDirectHandSelectPrompt && (isMultiDirectHandSelect || handSelectExtraOptions.length > 0) && (
                        <motion.div
                            initial={{ y: 40, opacity: 0 }}
                            animate={{ y: 0, opacity: 1 }}
                            exit={{ y: 40, opacity: 0 }}
                            ...
                            <div className="flex gap-3 items-center pointer-events-auto">
                                {isMultiDirectHandSelect && (
                                    <>
                                        ...
                                        {multiSelectedOptionIds.size > 0 && (
                                            <SmashUpGameButton
                                                variant="secondary"
                                                size="md"
                                                onClick={() => setMultiSelectedOptionIds(new Set())}
                                            >
                ...
                <AnimatePresence>
                    {pendingHandActionChoiceCard && (
                        <motion.div
                            ...
                            exit={{ y: 40, opacity: 0 }}
                            className={floatingHintClassName}
                            style={handActionChoiceStyle}
                        >
                            <div data-testid="su-hand-action-choice-bar" className="flex flex-wrap justify-center gap-3 pointer-events-auto">
                                <SmashUpGameButton
                                    variant="primary"
                                    size="md"
                ...
                <AnimatePresence>
                    {isReactionChoicePrompt && !isTitanReactionPrompt && (reactionChoicePlayableCardUids.size > 0 || reactionChoiceExtraOptions.length > 0) && (
                        ...
                        >
                            <div className="flex gap-3 items-center pointer-events-auto">
                                {reactionChoicePlayableCardUids.size > 0 && (
                                    <div
                                        data-testid="su-reaction-hand-status"
                                        className="rounded border border-emerald-300/70 bg-slate-900/90 px-4 py-2 text-sm font-black text-emerald-100 shadow-lg"
                                    >
                                        {selectedReactionChoiceTargetState
                                            && (selectedReactionChoiceTargetState.baseIndices.size > 0 || selectedReactionChoiceTargetState.minionUids.size > 0)
                                            ? t('ui.reaction_select_target_after_hand')
                                            : t('ui.reaction_select_hand_or_pass')}
                                    </div>
                                )}
                                {reactionChoiceExtraOptions.map(opt => (
                                    <SmashUpGameButton
                                        key={opt.id}
                                        variant="secondary"
                                        size="md"
                        ...
                        >
                            {core.bases.map((base, idx) => (
                                <BaseZone
                                    ...
                                    isMinionSelectMode={!(isOngoingSelectPrompt && !isBoardSelectPrompt && !fieldSourceTargetPrompt && !fieldSourceActionPrompt) && (
                                    ...
                                    selectedMinionUids={isFieldSourceTargetReady && selectedFieldPromptSourceUid && selectedFieldSourceTargetEntry?.sourceType === 'minion' ? new Set([selectedFieldPromptSourceUid]) : undefined}
