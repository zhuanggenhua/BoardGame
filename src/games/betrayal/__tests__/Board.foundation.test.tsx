...
import Board, { resolveBetrayalRerollTargetCircleSize } from '../Board';
import {
    BETRAYAL_COMMANDS,
    BetrayalDomain,
    EXPLORER_CATALOG,
...
describe('Betrayal dice reroll hit targets', () => {
    it('物理骰重掷命中圈按可见骰体收窄，避免相邻骰子透明命中框互挡', () => {
        const size = resolveBetrayalRerollTargetCircleSize({
            id: 2,
            x: 500,
            y: 300,
            width: 78,
...
describe('Betrayal Board foundation', () => {
    ...
    it('局内首剧本入口按英雄身份打开英雄剧本书并隐藏叛徒秘密页', () => {
        ...
        const heroOpeningWasShown = advanceScenarioReaderPastOpeningIfPresent({
            label: '英雄开场',
            bodyText: '敢阻我者必死',
        });
        const expectedHeroBookProgress = heroOpeningWasShown ? '2/2' : '1/1';

        expect(screen.getByTestId('betrayal-scenario-book')).toBeInTheDocument();
        expect(screen.getByTestId('betrayal-scenario-reader-page-label-desktop-left')).toHaveTextContent('01');
        expect(screen.getByTestId('betrayal-scenario-reader-header-progress')).toHaveTextContent(expectedHeroBookProgress);
        expect(screen.getByTestId('betrayal-scenario-reader-footer-progress')).toHaveTextContent(expectedHeroBookProgress);
    ...
    it('局内首剧本入口按叛徒身份打开叛徒剧本书并隐藏英雄秘密页', () => {
        ...
        const traitorOpeningWasShown = advanceScenarioReaderPastOpeningIfPresent({
            label: '叛徒开场',
            bodyText: '牺牲朋友也在所不惜',
        });
        const expectedTraitorBookProgress = traitorOpeningWasShown ? '2/2' : '1/1';

        expect(screen.getByTestId('betrayal-scenario-book')).toBeInTheDocument();
        expect(screen.getByTestId('betrayal-scenario-reader-header-progress')).toHaveTextContent(expectedTraitorBookProgress);
        expect(screen.getByTestId('betrayal-scenario-reader-footer-progress')).toHaveTextContent(expectedTraitorBookProgress);
        expect(screen.getByTestId('betrayal-scenario-objective-page')).not.toHaveTextContent('木乃伊横行');
    ...
    it('普通回合主动点击交易后点同房目标会进入可见交易流程', () => {
        ...
        fireEvent.click(screen.getByTestId('betrayal-room-occupant-hallway-1'));
        expect(screen.getByTestId('betrayal-trade-flow-item-step')).toHaveTextContent(/选择自己或对方持有物|选择/);
        ...
        expect(screen.getByTestId('betrayal-inventory-rope-selected-outline')).toBeInTheDocument();
        fireEvent.click(screen.getByTestId('betrayal-trade-return-card-map'));
        expect(screen.getByTestId('betrayal-trade-flow-item-step')).toHaveTextContent(/你给出.*兔脚.*对方给出.*地图/);
        expect(screen.getByTestId('betrayal-action-trade')).toHaveTextContent('提出交易');
    ...
    it('普通交易在教程步切换造成运行态刷新后会保留已选己方持有物', () => {
        ...
        expect(screen.getByTestId('betrayal-action-cue')).toHaveTextContent('点提出交易');
        expect(screen.getByTestId('betrayal-trade-return-selector')).toBeInTheDocument();
    });
