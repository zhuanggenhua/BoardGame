...
export default function ActivityTierChart({ tiers, totalUsers, loading }: Props) {
    ...
    return (
        <div className="bg-white p-6 rounded-2xl border border-zinc-100 shadow-xl shadow-zinc-200/50 flex flex-col h-[400px]">
            <div className="mb-2">
                <h3 className="text-lg font-bold text-zinc-900">{adminT('activity_tier.title')}</h3>
                <p className="text-xs text-zinc-500 mt-1">{adminT('activity_tier.description')}</p>
            </div>
            ...
            {loading ? (
                ...
                <div className="flex-1 w-full min-h-0 relative">
                        ...
                        <PieChart>
                            ...
                            <Tooltip
                                ...
                                formatter={(value?: number, name?: string) => {
                                    ...
                                    return [adminT('activity_tier.tooltip_value', { count: v, percent: pct }), `${name ?? ''} · ${tier?.description ?? ''}`];
                                }}
                            />
                            <Legend
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none pb-8">
                        ...
                        <div className="text-[10px] text-zinc-400 uppercase tracking-wider">{adminT('activity_tier.total_users')}</div>
                    </div>
                </div>
            )}
        </div>
