...
## 2026-07-12 剧本入口与书本式阅读补验收

### 命令

- `npx eslint src/games/betrayal/Board.tsx e2e/betrayal/basic-flow.e2e.ts`
  - 结果：通过；仍有 4 个既有 React Compiler memoization 警告，位置在运行时持有物相关逻辑，和本轮剧本阅读 UI 无关。
- `npm run test:e2e:file -- e2e/betrayal/basic-flow.e2e.ts "移动端横屏角色选择包含默认角色、翻页和剧本弹窗"`
  - 结果：`1 passed`
- `npm run test:e2e:file -- e2e/betrayal/basic-flow.e2e.ts "从角色选择确认到恶兆前运行时"`
  - 结果：`1 passed`
...
- 实际看到：弹窗里当前剧本为“赤红杰克归来”，有“阅读完整剧本”和“使用此剧本”两个清楚入口。
- 验收结论：剧本选择步骤已经有独立入口，房间外可看到当前剧本。

### PC 书本式阅读首页

- 路径：`D:\gongzuo\webgame\BoardGame\evidence\betrayal-basic-flow\02b-山屋惊魂-基本流程-书本式剧本阅读首页.jpg`
- 实际看到：点击“阅读完整剧本”后进入独立前台阅读层，内容呈现为摊开的旧书双页，而不是长滚动信息面板。
- 实际看到：左页是剧本封面和档案信息，右页是第一章“山屋异象”；底部有上一页/下一页和 `1/5` 页码。
- 验收结论：阅读载体已改为沉浸式书本阅读，首页可读且入口链路真实。

### PC 书本式阅读末页

- 路径：`D:\gongzuo\webgame\BoardGame\evidence\betrayal-basic-flow\02c-山屋惊魂-基本流程-书本式剧本阅读末页.jpg`
- 实际看到：通过“下一页”真实翻到 `5/5`，末页显示“胜负判定”，右页为空白纸面，符合书本末页语义。
- 实际看到：末页“下一页”禁用，“上一页”仍可用，证明不是滚动到底部，而是翻页状态收口。
- 验收结论：完整剧本内容可以用翻页读到末页，长文本不再挤在单个滚动面板里。
...
- 实际看到：当前剧本和阅读入口在横屏下可见，“使用此剧本”仍能直接收口。
- 验收结论：移动横屏有剧本选择步骤入口，且未牺牲原角色选择布局。

### 移动端横屏书本式阅读首页

- 路径：`D:\gongzuo\webgame\BoardGame\evidence\betrayal-basic-flow\09b-山屋惊魂-移动端横屏-书本式剧本阅读首页.jpg`
- 实际看到：阅读层在移动横屏下仍是书本双页，标题、封面、章节正文和翻页按钮都可见。
- 实际看到：阅读层通过 HUD 前台层承载，右下全局“离开”悬浮入口不再拦截“下一页”按钮。
- 验收结论：移动端点击翻页可用，阅读 UI 不再被全局悬浮按钮挡住。

### 移动端横屏书本式阅读末页

- 路径：`D:\gongzuo\webgame\BoardGame\evidence\betrayal-basic-flow\09c-山屋惊魂-移动端横屏-书本式剧本阅读末页.jpg`
- 实际看到：移动横屏通过连续点击“下一页”到达 `5/5`，末页显示“胜负判定”。
