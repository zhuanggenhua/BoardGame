...
- `npm run test:e2e:ci:file -- e2e/auth-account-login.e2e.ts "AuthModal login should remain usable on desktop"`
- `npm run typecheck`
...
- 截图路径：`D:\gongzuo\webgame\BoardGame\test-results\evidence-screenshots\auth-modal-desktop-login-filled.png`
...
- 我还尝试用裸 Playwright 跑 `e2e/account-settings.e2e.ts` 和 `e2e/lobby.e2e.ts` 的相关用例，但失败点落在现有测试入口本身没有稳定打开对应弹窗，不是“眼睛按钮不存在”或“显隐切换失败”。
