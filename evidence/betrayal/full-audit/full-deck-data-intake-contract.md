...
| 胸针精神伤害真实入口回归 | `node scripts/infra/run-e2e-single.mjs ci e2e/betrayal/non-p0-representative.e2e.ts "胸针精神伤害真实链路"`：1 passed | 覆盖指环攻击造成非强制精神伤害后，受伤玩家可从真实页面开启胸针改为通用伤害，并结算回牌桌 |
...
| I12 | 胸针 | 未锁英文名 | 物品 | 原始 item atlas frame 11；`temp/betrayal-asset-source-diagnostics-2026-07-28/item-11-r1c3-full.jpg` | TTS 图包已读：无论何时你受到物理或精神伤害时，你可以替换为承受通用伤害 | item frame 11 已裁完整单卡；sha256 `d2f8f52deaec13442770c78e08e3119dd8d6693753bc2812f5c7b5d976914f82`；atlas alias 已接 | `in-runtime / min-domain-verified / min-ui-representative / mummy-forced-damage-e2e-boundary / non-forced-physical-damage-e2e-representative / non-forced-mental-damage-e2e-representative` | 已补待分配物理/精神伤害替换为通用伤害、未声明时仍按原伤害类型限制分配、日志记录；当前树已补 Board 组件伤害分配代表链，补木乃伊强制伤害下不适用的真实入口 / 截图边界，补灰尘普通攻击非强制物理伤害下可用的真实入口 / 截图代表链，并补指环攻击非强制精神伤害下可用的真实入口 / 截图代表链，但不等同全部来源真实 Playwright / 截图闭环 | `locked` | S0 字段、最小领域效果、Board 组件代表链、木乃伊强制伤害不适用边界、普通攻击物理伤害可用入口和指环攻击精神伤害可用入口已接；后续补更多伤害来源、减伤叠加、死亡保护和作祟伤害组合回归 |
...
| 胸针 | 物理或精神伤害可声明改成通用伤害；未声明仍按原类型限制分配；Board 组件伤害分配页可开启胸针并提交 `useBrooch: true`；木乃伊强制速度/力量伤害真实入口下不显示胸针开关、不开放通用伤害入口且不记为已用；灰尘普通攻击真实入口下可把非强制物理伤害改为通用伤害；指环攻击真实入口下可把非强制精神伤害改为通用伤害 | `min-domain-verified / min-ui-representative / partial-combo / mummy-forced-damage-e2e-boundary / non-forced-physical-damage-e2e-representative / non-forced-mental-damage-e2e-representative` | 更多伤害来源、减伤叠加、死亡保护和作祟伤害组合回归 |
...
| 胸针 | 伤害类型改写、通用伤害 | `min-domain-verified / min-ui-representative / mummy-forced-damage-e2e-boundary / non-forced-physical-damage-e2e-representative / non-forced-mental-damage-e2e-representative / partial-combo` | 已补 Board 组件伤害分配代表链；木乃伊强制速度/力量伤害下不适用的真实入口 / 截图链已补；灰尘普通攻击非强制物理伤害下可用的真实入口 / 截图链已补；指环攻击非强制精神伤害下可用的真实入口 / 截图链已补；继续标出更多伤害来源、减伤叠加、死亡保护和作祟伤害消费者 | 更多来源与组合回归 |
...
| 物理伤害事件 | 地狱蝙蝠、断手、摇曳灯光、着火的人、炸药失败探索者分支，另有灰尘普通攻击代表入口 | 6.12 / 6.13 已登记物理伤害、胸针、盔甲、奇异护符、头骨、兔脚等消费者；灰尘普通攻击已证明一条非强制物理伤害可由胸针改写为通用伤害 | 物理伤害是否先被盔甲减免、更多物理伤害来源是否可被胸针改写、奇异护符是否只在实际承受物理伤害后触发、死亡保护窗口顺序 | `contract-indexed / downstream-open`；后续授权后做组合矩阵 |
...
| 物品伤害改写 / 减免 | 胸针、盔甲、头戴耳机、奇异护符、皮夹克、幸运硬币、牙齿项链、头骨、兔脚 | 6.13 / 6.14 已按消费桶登记；胸针已有 Board 组件代表链、木乃伊强制伤害不适用真实入口和灰尘普通攻击非强制物理伤害可用真实入口 | 同一伤害来源遇到改写、减免、属性恢复、死亡保护、重掷时的顺序；成功保护后是否回滚死亡、是否保留或掩埋遗物 | `contract-indexed / downstream-open` |
...
| NB-06 物品伤害减免 / 伤害改写 / 死亡保护 | WO-02 | 头戴耳机、奇异护符、胸针、皮夹克、幸运硬币、牙齿项链 | 记录减免、改写、实际承受、濒死恢复和死亡保护顺序；胸针普通攻击物理伤害真实入口已补证，但不外推到精神伤害或全部来源 | 不改伤害 / 减免 / 死亡保护规则；不新增组合领域测试 |
