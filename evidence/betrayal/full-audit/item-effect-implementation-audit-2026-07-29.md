...
| 2026-07-31 静态复核 | 临时 `npx tsx -` 矩阵脚本直接导入 `BETRAYAL_DISCOVERY_POOLS.possessions` 与 `POSSESSION_USE_EFFECTS`，对 22 张物品 + 9 张预兆的 31 张可持有牌做消费者组映射 |
...
| 恐怖玩偶 | 最近属性检定全骰重掷 | 已有最近属性检定重掷入口；固定骰、攻击、作祟检定不放行；当前树 Board 组件代表链已证明选中恐怖玩偶后，最近属性检定骰盘会为全部骰子生成可重掷目标；真实入口 E2E 已证明从牌桌持有区点恐怖玩偶后开放全部骰子，点击任一骰子会重掷全部属性检定骰，回滚原分支并按新骰重算最终属性 | `L3 real-entry representative / partial-combo` | 作祟特殊行动属性检定通用回滚快照和更多重掷消费者组合未闭合。 |
...
| 幸运硬币 | 空白骰重掷、空白精神伤害 | 已覆盖事件/房间属性检定空白骰重掷、空白后精神伤害、倒塌房间组合代表链；当前树 Board 组件代表链已证明选中幸运硬币后，最近属性检定骰盘只高亮空白骰，非空白骰不会生成可重掷目标；真实入口 E2E 已证明从牌桌持有区点幸运硬币后只开放空白骰目标，点击任一空白骰会重掷所有空白骰，且重投后每个空白进入精神伤害分配 | `L3 real-entry representative / partial-combo` | 作祟特殊行动、死亡保护和更多伤害分配组合未闭合。 |
| 皮夹克 | 防御攻击额外 1 骰 | 防御攻击最小运行链存在；当前树 Board 组件代表链已证明真实攻击入口结算后，攻击投骰复盘显示进攻总点、防御总点和防御额外 1 骰；真实入口 E2E 已证明攻击持有皮夹克的叛徒时复盘显示防御额外 1 骰，随后进入真实物理伤害分配并收口回牌桌 | `L3 real-entry representative / partial-combo` | 怪物攻击、作祟攻击和更多攻击来源组合未闭合。 |
| 牙齿项链 | 回合结束濒死属性 +1 | 回合结束触发、濒死属性筛选、选择后提升、跳过和非法属性拒绝存在；当前树 Board 组件链已证明选择面板显示濒死属性、确认前不可提交、跳过无需先选属性，真实入口 Playwright / 截图链已证明点击牌桌“结束回合”后弹出牙齿项链选择，且只开放濒死力量，确认后力量轨 +1、pending 清空并交给下一位 | `L3 real-entry representative / partial-combo` | 作祟回合、房间回合末、怪物回合和死亡保护组合未闭合。 |
| 手电筒 | 事件属性检定额外骰 | 事件属性检定消费者已有代表链 | `covered-by-existing-contract / consumer-review-on-change` | 新增事件属性检定消费者时逐项确认是否加骰。 |
| 头戴耳机 | 精神伤害 -1 | 精神伤害减免已有消费者链；Board 组件代表链覆盖伤害分配页显示原始精神伤害、头戴耳机减免和实际分配数；真实入口 Playwright / 截图链已补：指环精神攻击造成精神伤害后，受伤玩家页面显示原始伤害、头戴耳机减免来源和实际待分配点数，并可按减免后伤害分配收口。 | `L3 real-entry representative / partial-combo` | 更多精神伤害来源、死亡保护和作祟精神伤害组合仍未闭合。 |
| 奇异护符 | 承受物理伤害后获得 1 神志 | 已覆盖实际承受物理伤害后获得神志，并排除通用伤害和直接属性降低误触发；当前树 Board 摘要会明示“实际承受物理伤害后 +1 神志”，物理伤害分配日志会显示奇异护符触发；真实入口 Playwright / 截图链已补：无武器物理攻击造成实际物理伤害后，受伤玩家分配伤害并触发奇异护符获得 1 点神志，日志 / 反馈可见。 | `L3 real-entry representative / partial-combo` | 减伤、死亡保护、作祟物理伤害和更多物理伤害来源组合未闭合。 |
...
| 枪 | 视线速度攻击，失败不反伤 | 已覆盖速度攻击、视线目标、失败不反伤和用后交易限制；当前组件断言证明枪选中后才画武器视线线；真实入口 E2E 已证明从攻击武器入口选择枪后显示视线连线，视线叛徒目标高亮，结算记录为速度攻击、物理伤害且失败不反伤，并在伤害分配后收口回牌桌 | `L3 real-entry representative / partial-combo` | 更多视线边界、怪物目标、作祟攻击和多武器组合未闭合。 |
...
| 电锯 | 攻击额外 1 骰 | 已覆盖攻击额外骰和用后交易限制；真实入口 E2E 已证明从攻击武器入口选择电锯后，同房叛徒目标高亮，攻击骰盘为力量 4 + 额外 1 骰共 5 骰，造成物理伤害并在伤害分配后收口回牌桌 | `L3 real-entry representative / partial-combo` | 怪物目标、作祟攻击和多武器互斥组合未闭合。 |
| 炸药 | 代替攻击，选当前/相邻房间，群体速度检定，埋葬 | 已覆盖代替常规攻击、目标板块、每名探索者/怪物分别速度检定、失败物理伤害和怪物受伤后端；当前树已补真实页面代表链：动作 rail 进入炸药房间目标态，当前 / 相邻已发现房间本体高亮，点击房间板块后消耗炸药、埋葬并让失败探索者进入 4 点物理伤害分配 | `L3 real-entry representative / partial-combo` | 非法原因展示、更多怪物 / 作祟组合、特殊免疫和墙体 / 门位相邻边界未闭合。 |
...
| 恐怖玩偶真实入口 / 截图链 | `npx eslint e2e/betrayal/scary-doll-reroll.e2e.ts` 0 errors；`npm run test:e2e:file -- e2e/betrayal/scary-doll-reroll.e2e.ts` 1 passed。截图位于 `evidence/山屋惊魂-恐怖玩偶重掷完整链路/`，证明真实页面中恐怖玩偶可从持有区选中，三颗 0 点属性检定骰均可重掷，点击任一骰子后结果更新为 2/2/2，总点数 6，知识 +1，旧速度惩罚被回滚，恐怖玩偶标记为本回合已用并清空选骰层。 |
| 幸运硬币真实入口 / 截图链 | `npx eslint e2e/betrayal/lucky-coin-reroll.e2e.ts` 0 errors；`npm run test:e2e:file -- e2e/betrayal/lucky-coin-reroll.e2e.ts` 1 passed。截图位于 `evidence/山屋惊魂-幸运硬币重掷完整链路/`，证明真实页面中幸运硬币可从持有区选中，只开放第 1 / 第 3 颗空白骰目标，非空白第 2 颗骰不可选；点击任一空白骰会重掷所有空白骰，重投后仍有 1 个空白时进入 1 点精神伤害分配，选择神志并确认后回到牌桌、选骰层清空。 |
...
| 本轮新增验证 | 炸药目标板块当前树补证：`node --max-old-space-size=8192 ./node_modules/eslint/bin/eslint.js src/games/betrayal/Board.tsx src/games/betrayal/__tests__/Board.foundation.test.tsx` 0 errors；`Board.foundation.test.tsx -t "炸药\|十字弓\|枪\|砍刀"` 4 passed / 131 skipped；`firstScenarioRuntime.test.ts -t "炸药"` 8 passed / 686 skipped。该补证证明主动作可自动进入炸药房间目标态、点击房间板块走 `dynamite-room` 结算。 |
| 本轮新增验证 | 炸药真实入口 / 截图链当前树补证：`npm run test:e2e:file -- e2e/betrayal/non-p0-representative.e2e.ts "炸药房间攻击真实链路"` 1 passed；`node scripts/infra/vitest-cli-safe.mjs run src/games/betrayal/__tests__/firstScenarioRuntime.test.ts --config vitest.config.core.ts --configLoader native --pool forks --no-file-parallelism --maxWorkers 1 --testTimeout 300000 --hookTimeout 300000 -t "炸药"` 8 passed / 690 skipped；`node scripts/infra/vitest-cli-safe.mjs run src/games/betrayal/__tests__/Board.foundation.test.tsx --config vitest.config.core.ts --configLoader native --pool forks --no-file-parallelism --maxWorkers 1 --testTimeout 300000 --hookTimeout 300000 -t "炸药"` 1 passed / 176 skipped，退出码 0，尾部仍有既有 `ECONNRESET` 噪声；`npx eslint e2e/betrayal/non-p0-representative.e2e.ts` 0 errors。该补证证明真实页面能从动作 rail 进入炸药目标态，当前 / 相邻房间本体高亮，点击相邻房间后消耗炸药、埋葬并进入 4 点物理伤害分配；仍不证明非法原因、特殊免疫、全部怪物 / 作祟或墙体 / 门位边界组合完成。 |
| 本轮新增验证 | 牙齿项链结束回合真实入口 / 截图链当前树补证：`npx eslint e2e/betrayal/tooth-necklace-end-turn.e2e.ts e2e/betrayal/betrayalTestHelpers.ts` 0 errors；`node scripts/infra/vitest-cli-safe.mjs run src/games/betrayal/__tests__/firstScenarioRuntime.test.ts --config vitest.config.core.ts --configLoader native --pool forks --no-file-parallelism --maxWorkers 1 --testTimeout 300000 --hookTimeout 300000 -t "牙齿项链"` 8 passed / 690 skipped；`node scripts/infra/vitest-cli-safe.mjs run src/games/betrayal/__tests__/Board.foundation.test.tsx --config vitest.config.core.ts --configLoader native --pool forks --no-file-parallelism --maxWorkers 1 --testTimeout 300000 --hookTimeout 300000 -t "牙齿项链"` 1 passed / 176 skipped，退出码 0，尾部仍有既有 `ECONNRESET` 噪声；`npm run test:e2e:file -- e2e/betrayal/tooth-necklace-end-turn.e2e.ts` 1 passed。截图位于 `evidence/山屋惊魂-牙齿项链结束回合完整链路/`，证明真实页面点击结束回合后进入牙齿项链濒死属性选择，只开放力量，确认后力量从 1 提到 2 并交给下一位；仍不证明作祟回合、房间回合末、怪物回合或死亡保护组合完成。 |
| 2026-07-31 P0 复核 | `npx vitest run src/games/betrayal/__tests__/firstScenarioRuntime.test.ts -t "当前 22 张物品牌\|恐怖玩偶\|幸运硬币\|牙齿项链\|胸针\|奇异护符\|神秘秒表\|天使之羽\|炸药\|骨制钥匙\|急救包\|镜子\|奇怪的药品\|地图\|武器\|十字弓\|枪\|砍刀\|电锯\|皮夹克\|头戴耳机"`：164 passed / 534 skipped；`npx vitest run src/games/betrayal/__tests__/Board.foundation.test.tsx -t "恐怖玩偶\|幸运硬币\|牙齿项链\|胸针\|奇异护符\|炸药\|骨制钥匙\|急救包\|书本\|十字弓\|枪\|砍刀\|武器\|持有物卡片会暴露"`：19 passed / 158 skipped，退出码 0，尾部仍有既有 `ECONNRESET` 噪声。结论：当前未发现“规则允许但玩家没有入口 / 命令不消费 / 状态不收口”的 P0 物品入口阻塞；残余继续按真实 Playwright / 截图和组合验证分层处理。 |
| 2026-07-31 可持有牌消费者组静态复核 | 临时 `npx tsx -` 矩阵脚本对 31 张可持有牌得到 `unmapped=[]`、`missingRuntime=[]`、`missingTests=[]`；消费者组为主动使用 8 张、武器 6 张、伤害减免 / 改写 5 张、重掷 3 张、交易 1 张、探索声明 / 事件检定辅助 5 张、回合结束 / 特殊行动 2 张、房间攻击 1 张、死亡保护 1 张。脚本当时显示皮夹克、枪、电锯未被 E2E 文件按中文名或 id 直接命中，归类为 P1 真实入口 / 截图补证；不是 P0 运行时消费缺失。本轮后续行已补三张牌的单卡命名真实入口。 |
| 2026-07-31 皮夹克 / 枪 / 电锯真实入口补证 | `npm run test:e2e:file -- e2e/betrayal/non-p0-representative.e2e.ts "枪视线攻击真实链路"` 1 passed，截图 `evidence/山屋惊魂-枪视线攻击完整链路/`；`npm run test:e2e:file -- e2e/betrayal/non-p0-representative.e2e.ts "电锯攻击真实链路"` 1 passed，截图 `evidence/山屋惊魂-电锯攻击完整链路/`；`npm run test:e2e:file -- e2e/betrayal/non-p0-representative.e2e.ts "皮夹克防御真实链路"` 1 passed，截图 `evidence/山屋惊魂-皮夹克防御额外骰完整链路/`。三条证明单卡命名真实入口代表链已补，残余降级为怪物 / 作祟攻击、多武器互斥、视线边界和更多伤害组合。 |
| 2026-07-31 头戴耳机 / 奇异护符真实入口补证 | `npx vitest run src/games/betrayal/__tests__/firstScenarioRuntime.test.ts -t "奇异护符\|头戴耳机"` 20 passed / 678 skipped；`npx vitest run src/games/betrayal/__tests__/Board.foundation.test.tsx -t "头戴耳机\|奇异护符\|持有物卡片会暴露"` 2 passed / 175 skipped，退出码 0，尾部仍有既有 `ECONNRESET` 噪声；`npm run test:e2e:file -- e2e/betrayal/non-p0-representative.e2e.ts "头戴耳机精神减伤真实链路"` 1 passed，截图 `evidence/山屋惊魂-头戴耳机精神减伤完整链路/`；`npm run test:e2e:file -- e2e/betrayal/non-p0-representative.e2e.ts "奇异护符物理伤害真实链路"` 1 passed，截图 `evidence/山屋惊魂-奇异护符物理伤害触发完整链路/`。两条证明单卡命名真实入口代表链已补，残余降级为更多伤害来源、减伤叠加、死亡保护和作祟伤害组合。 |
...
| 最近属性检定重掷类 | 重掷后原属性检定分支被回滚并按新骰重算；幸运硬币空白结果进入精神伤害分配；当前树 Board 组件代表链已证明恐怖玩偶全骰可选、幸运硬币只允许空白骰；恐怖玩偶真实入口 E2E 已证明从持有区进入重掷、三骰全开、重掷后回写原事件分支与最终属性；幸运硬币真实入口 E2E 已证明从持有区进入重掷、只开放空白骰、重掷所有空白骰并在仍有空白时进入精神伤害分配。 | 未发现旧测试失效；恐怖玩偶和幸运硬币过去缺真实入口证据的结论已失效；恐怖玩偶作祟特殊行动属性检定仍缺通用回滚快照，幸运硬币仍缺作祟特殊行动、死亡保护和更多伤害分配组合。 |
...
| 伤害改写/减免类 | 胸针把待分配物理/精神伤害改成通用伤害；当前树 Board 组件代表链已证明玩家页面可以切换为通用伤害并提交 `useBrooch: true`；木乃伊攻击真实入口已证明强制速度/力量伤害下胸针不适用，持有胸针也不会开放通用伤害入口；灰尘普通攻击真实入口已证明非强制物理伤害可开启胸针并改为通用伤害；指环攻击真实入口已证明非强制精神伤害可开启胸针并改为通用伤害；盔甲、头戴耳机按类型减免，当前树 Board 组件和头戴耳机真实入口代表链已证明减伤后显示原始伤害、减免来源和实际分配数；奇异护符只在实际物理伤害后加神志，且当前树已补持有区摘要、伤害日志和真实入口代表链。 | 旧“胸针伤害改写 UI 仍缺”结论已失效；旧“木乃伊攻击下胸针完全未补证”口径已收窄为“木乃伊强制伤害不适用已证明”；旧“胸针非强制伤害真实入口全缺”口径已收窄为“普通攻击物理伤害入口和指环攻击精神伤害入口已证明”；旧“头戴耳机减伤提示 UI 仍缺 / 真实入口截图链仍缺”已失效；旧“奇异护符 UI/日志提示仍缺 / 真实入口截图链仍缺”已失效；更多伤害来源、减伤叠加、死亡保护和作祟伤害组合仍缺。 |
| 回合结束/额外行动类 | 牙齿项链能拦截回合结束并在选择/跳过后继续交接；当前树 Board 组件代表链已证明牙齿项链结束回合面板可直接跳过，也可选择濒死属性后确认；本轮真实入口 E2E 已证明牌桌“结束回合”会弹出牙齿项链选择面板，只开放濒死属性，确认后提升属性并交接给下一位；神秘秒表能在作祟后安排额外行动，本轮 E2E 已证明页面使用后结束回合仍回到当前玩家。 | 牙齿项链旧测试暴露“跳过前错误要求先选属性”的 UI 门控缺口，已补组件回归和真实入口代表链；作祟、怪物回合、房间回合末和死亡保护组合仍缺。 |
| 地图/移动类 | 地图类移动到已发现房间，骨制钥匙 / 移动限制类按合同走对应位置消费者；本轮 E2E 已证明地图 / 笔记本 / 日记 / 手稿从真实房间板块选择已发现目标，当前树 E2E 已证明骨制钥匙穿墙目标、移动结算和移动模式收口代表链；炸药已补真实页面目标板块选择、消耗、埋葬和伤害分配代表链。 | 未发现旧测试失效；墙体、门位、跨楼层、作祟地图规则、特殊移动限制、炸药非法原因展示和边界组合仍需重审。 |
| 炸药 | 使用后埋葬，选定目标房间内每个目标分别速度检定；失败探索者进入物理伤害分配，失败怪物走怪物受伤后端；当前树真实入口 E2E 已证明动作 rail 进入目标板块选择，房间板块本体高亮，点击相邻房间后消耗炸药并让失败探索者进入 4 点物理伤害分配。 | 旧结论“UI 目标选择仍缺 / 真实 Playwright 与截图链仍缺”已失效；特殊免疫、非法原因展示和更多怪物 / 作祟组合仍缺。 |
...
| --- | --- | --- | --- |
| 主动使用牌同族 | 搜索范围：`src/games/betrayal`、`e2e/betrayal`、`evidence/betrayal/full-audit`；根因关键词：`POSSESSION_USE_EFFECTS`、`selectedInventoryReplacementRollTotal`、`replacementRollTotal`、`pendingExtraTurnAfterCurrentTurn`、`UI 数字选择`、`额外行动 UI`；命中 `possessionEffects.ts`、`game.ts`、`Board.tsx`、灰尘主动持有牌 E2E、牙齿项链结束回合 E2E 和三份审计账本。 | 急救包、奇怪的药品、镜子、地图及三张 alias、神秘秒表、天使之羽、书本、面具已经进入同一条主动牌页面代表链；牙齿项链结束回合选择已补 Board 组件和真实入口代表链；旧“八张主动牌 / 天使之羽无数字选择 / 神秘秒表无额外行动 UI / 牙齿项链结束回合 UI 未承接”的结论降级为历史失效口径。 | 这仍不是全部物品主动能力完成；牙齿项链作祟 / 房间回合末 / 死亡保护组合，以及重掷 / 伤害改写等其它物品组合仍需单独审。 |
...
| 地图/移动同族 | 搜索范围同上；根因关键词：`canUseSkeletonKeyForMove`、`useSkeletonKey`、`MOVE_TO_ROOM`、`placeExplorer`、`skeletonKeyMoveTargetRoomIds`、`dynamite-room`；命中 `game.ts`、`Board.tsx`、`Board.foundation.test.tsx`、`firstScenarioRuntime.test.ts`、`first-scenario-use-possession.e2e.ts`、`non-p0-representative.e2e.ts` 和审计总账。 | 骨制钥匙已从“UI 待补”修正为真实页面穿墙移动代表链已补；地图 / alias 主动放置代表链仍有效；炸药目标板块已补真实页面代表链。 | 墙体 / 门位 / 同层 / 相邻 / 作祟地图空间组合、炸药非法原因展示、地图非法原因和特殊移动限制仍需扩审。 |
| 炸药目标板块同族 | 搜索范围同上；根因关键词：`attack-room`、`dynamiteAttackWeaponCard`、`isDynamiteRoomTargetingMode`、`selectedAttackWeaponCardId`、`dynamite-room`；命中 `Board.tsx`、`Board.foundation.test.tsx`、`firstScenarioRuntime.test.ts`、`non-p0-representative.e2e.ts` 和审计总账。 | 炸药已从“只有领域、UI 目标选择未补”修正为“领域 + Board 组件 + 真实 Playwright / 截图代表链”：动作 rail 可进入炸药房间目标态，当前 / 相邻已发现房间本体高亮，点击相邻房间后进入 `dynamite-room` 结算并产生伤害分配。 | 仍不能外推成非法原因展示、特殊免疫、墙体 / 门位相邻边界或更多怪物 / 作祟组合完成。 |
...
- 攻击类物品已有当前树 UI 代表链：徒手、砍刀、指环、匕首、十字弓相邻、枪视线攻击、电锯额外 1 骰攻击和武器禁用原因定向复跑通过；仍需补多武器互斥全排列、怪物目标、作祟攻击、视线边界和交易限制组合。
- 重掷/替换类物品中恐怖玩偶已补真实入口代表链和原事件分支回滚证据；幸运硬币已补真实入口空白骰重掷和精神伤害分配代表链；仍需补作祟特殊行动属性检定通用回滚快照、其它替代 / 重掷对象和新增骰子消费者准入。
- 伤害类物品中胸针 Board 组件伤害改写代表链已补，木乃伊强制伤害下胸针不适用的真实入口 / 截图链已补，灰尘普通攻击非强制物理伤害下胸针可用的真实入口 / 截图链已补，指环攻击非强制精神伤害下胸针可用的真实入口 / 截图链已补；头戴耳机精神减伤和奇异护符物理伤害触发真实入口 / 截图代表链已补；仍需补更多减伤叠加、死亡保护、作祟伤害和盔甲/奇异护符/头骨/兔脚组合。
- 回合结束类物品中牙齿项链结束回合 Board 组件链和真实 Playwright / 截图代表链已补；仍需补房间回合末、怪物 / 作祟回合和死亡保护组合。
- 地图/移动类物品已有地图及三张 alias 的真实页面目标选择代表链，骨制钥匙穿墙移动真实页面代表链已补，炸药房间目标真实页面代表链已补；仍需补墙体 / 门位 / 楼层、作祟地图限制、炸药非法原因展示和其它移动消费者。
...
| 2026-07-31 静态消费者组复核 | 31 张可持有牌均能映射到明确消费者组，且当前运行时和单测 / E2E 文本均有命中；主动使用牌不要求在 Board 里逐张硬编码，`POSSESSION_USE_EFFECTS` 是主动效果真相入口，Board 走通用持有物使用承接。皮夹克、枪、电锯当时仍缺按单卡命名真实入口；本轮已用三条定向 E2E 补齐首条真实入口代表链，不再作为 P1 单卡命名入口缺口。 |
| 本轮补检 | 攻击武器桶已消费当前树 E2E 复跑结果：旧十字弓视线口径降级为历史；当前枪 / 幻影摄影师承担视线代表链，十字弓承担同板块 / 相邻代表链。 |
| 本轮补检 | 皮夹克、枪、电锯已消费当前树三条真实入口 E2E：皮夹克证明被攻击时防御额外 1 骰可见并进入伤害分配，枪证明真实视线连线和速度攻击，电锯证明 5 骰力量攻击。当前残余降级为怪物 / 作祟攻击、多武器互斥和边界组合，不能继续写成单卡真实入口未补。 |
...
| 旧结论失效 | 旧结论“炸药目标板块 UI 仍缺 / 真实 Playwright 与截图链仍缺”已不准确；当前树已补领域、Board 组件和真实页面动作 rail 到房间板块点击、消耗、埋葬、伤害分配代表链。当前残余降级为非法原因、特殊免疫、空间边界和怪物 / 作祟组合，不能继续写成功能未实现。 |
| 旧结论失效 | 旧结论“牙齿项链结束回合 UI 仍缺 / 真实 Playwright 与截图链仍缺”已不准确；当前树已补 Board 组件代表链和真实页面结束回合入口代表链，并用红测证明旧页面曾错误要求先选濒死属性才能跳过。当前残余降级为作祟回合、房间回合末、怪物回合和死亡保护组合，不能继续写成功能未实现。 |
| 旧结论失效 | 恐怖玩偶过去缺真实入口证据的结论已不准确；当前树已补真实页面持有区选中、三骰全开、点击任一骰重掷全部属性检定骰、回滚原事件分支和属性收口代表链。当前残余降级为作祟特殊行动属性检定通用回滚快照和更多重掷消费者组合，不能继续按“当前缺入口”处理。 |
...
| 旧结论失效 | 旧结论“奇异护符 UI/日志提示仍缺 / 真实入口截图链仍缺”已不准确；当前树已补 Board 持有区摘要、物理伤害分配日志和真实页面无武器物理伤害触发链，证明玩家能看到常规物理伤害触发规则和结算反馈。当前残余降级为减伤、死亡保护、作祟物理伤害和更多物理伤害来源组合，不能继续写成玩家可见规则为空或真实入口缺失。 |
| 旧结论失效 | 旧结论“头戴耳机真实 Playwright / 截图链仍缺”已不准确；当前树已补指环精神攻击真实入口链，证明精神伤害分配页显示原始伤害、头戴耳机减免来源和实际分配点数。当前残余降级为更多精神伤害来源、死亡保护和作祟精神伤害组合，不能继续写成真实入口缺失。 |
