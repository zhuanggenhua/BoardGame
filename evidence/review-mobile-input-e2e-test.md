...
## 范围
- 评价入口链路：`e2e/review.e2e.ts`
- 详情弹窗入口稳定性：`e2e/review.e2e.ts`
- 相关实现：`src/components/review/GameReviewSection.tsx`、`src/components/review/ReviewForm.tsx`

## 本轮验证
- `npx eslint e2e/review.e2e.ts`
- `node scripts/infra/cleanup_test_connections.js --e2e --shared`
- `PW_SERVER_RUNTIME=tsx node scripts/infra/run-e2e-single.mjs ci --file e2e/review.e2e.ts --case "移动端评价输入聚焦后仍应保持可见"`
  - 结果：`1 passed`
...
### 1. 移动端评价弹窗输入区
- 截图：`D:\gongzuo\webgame\BoardGame\test-results\evidence-screenshots\review-form-mobile-input-visible.png`
- 我实际看到的现象：
