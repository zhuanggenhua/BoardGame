...
- 修复后真实在线 E2E 通过：空回合 `returnedElapsedMs=2545`；连续两次可见召唤的服务端事件间隔 `1011ms`。
...
| 无解交互和 circuit 诊断拆分 | 服务端重构后，无解交互反馈、emergency skip 转 cancel、circuit breaker state snapshot、命令失败反馈 payload 不能改格式 | `src/engine/transport/onlineAiUnsatisfiableInteraction.ts`、`onlineAiCircuitFeedbackDiagnostics.ts`、`commandFailureFeedbackPayload.ts` | `server.ts` 仍负责真实状态写入、circuit 记账、冷却去重和上报；新增模块只承载纯判断与 JSON 构造 | `onlineAiUnsatisfiableInteraction.test.ts`；`onlineAiCircuitFeedbackDiagnostics.test.ts`；`commandFailureFeedbackPayload.test.ts`；`server.test.ts` targeted 回归 | 通过 |
| 真实空回合 | 空阶段推进不应逐段吃 1 秒，但交还真人不应瞬间 | Summoner Wars 在线 E2E | `returnedElapsedMs=2545`，满足 `>=900ms` 且 `<3000ms` | Playwright 真实在线 E2E | 通过 |
| 真实连续召唤 | 两次可见召唤之间应保留约 1 秒 | Summoner Wars 在线 E2E | 服务端事件间隔 `1011ms`；页面轮询采样间隔 `928ms`，不作为 1 秒节奏断言权威 | Playwright 真实在线 E2E | 通过 |
...
- `node scripts/infra/vitest-cli-safe.mjs run src/engine/transport/__tests__/commandFailureFeedbackPayload.test.ts src/engine/transport/__tests__/onlineAiUnsatisfiableInteraction.test.ts src/engine/transport/__tests__/onlineAiCircuitFeedbackDiagnostics.test.ts src/engine/transport/__tests__/onlineAiWatchdogFeedbackDiagnostics.test.ts src/engine/transport/__tests__/onlineAiWatchdogSequenceFingerprinting.test.ts src/engine/transport/__tests__/onlineAiRepeatedRecoveryUnblockExecutor.test.ts src/engine/transport/__tests__/onlineAiLegalActionRecoveryExecutor.test.ts src/engine/transport/__tests__/commandFailureReason.test.ts src/engine/transport/__tests__/onlineAiLegalActionCommand.test.ts src/engine/transport/__tests__/onlineAiRecoveryDispatch.test.ts src/engine/transport/__tests__/onlineAiRecoveryOwnership.test.ts src/engine/transport/__tests__/onlineAiExecutor.test.ts --configLoader native`
  - 结果：通过；12 files passed，37 passed。
...
- `node scripts/infra/vitest-cli-safe.mjs run src/engine/transport/__tests__/server.test.ts -t "command failure|circuit breaker|buildOnlineAiRecoveryFingerprint|blockerFingerprint|unsatisfiable-interaction-auto-skipped|在线 AI watchdog 选出的动作若被权威领域状态拒绝|同一卡点重复恢复三次|同一合法动作连续命令失败达到上限" --configLoader native`
...
- `node scripts/infra/run-e2e-single.mjs isolated e2e/summonerwars/summonerwars-ai-delay-diagnostic.e2e.ts`
  - 结果：通过；2 passed。
  - 空回合：match `P5OSrGiVDTW`，`returnedElapsedMs=2545`。
  - 连续召唤：match `bMhe7c2ecMX`，`firstToSecondSummonEventGapMs=1011`，`firstToSecondSummonGapMs=928`（页面轮询采样值，不作为 1 秒节奏断言权威）。
