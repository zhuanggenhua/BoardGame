...
| L0/L1/L2/L3/L4 证据层级 | passed | 14/14 对象有 L0/L1；12 个能力均有 L2；真实入口 L3 覆盖选派系、进局、枯萎法师“鲜血灌注”、鲜血萨满、狂热菌菇、共生自愈成功与空选、释放菌袍成功与空选 | 满足当前发布口径：玩家决策入口均有 L3，自动/被动链路以 L2 + 系统判等收口 |
...
| L4 共享链判等矩阵 | passed | 枯萎法师、鲜血萨满、狂热菌菇、共生自愈、释放菌袍均有 direct E2E；自动/被动对象按阶段、死亡、回合系统测试收口 | 当前发布口径已闭环 |
| 旧 evidence/旧结论对账回写 | passed | 本文替换旧降级口径，改为 `full_audit_e2e_verified`，并回写新增交互证据 | 已回写当前文档；未发现其他莫古旧 evidence 需要继续降级 |
| 真实入口 E2E 与截图核验 | passed | 8 条定向 E2E、18 张截图、1 张最终审计合图；已核验入口、选中、进局、鲜血灌注、共生自愈、释放菌袍、鲜血萨满、狂热菌菇的选择与结算截图 | 新增玩家交互与事件牌路径已覆盖 |
| 收口声明 | passed | 本文按对象、入口、共享链、截图和测试结果回写 | 莫古对象级审计按当前发布口径收口 |
...
| 菌化野兽 | C3 | 攻击阶段结束优先消耗 1 充能，否则自伤 | `mogu_parasite` | 寄生测试 |
...
| 菌化野兽 | 卡图裁片 | `mogu_infection` / `mogu_parasite` | 击杀后、攻击阶段结束 | 死亡后处理/阶段处理 | 需弃牌堆目标；寄生消耗 1 充能优先 | 替换敌方/自我维护 | 无充能时自伤 | 阶段推进 | L2 | passed |
