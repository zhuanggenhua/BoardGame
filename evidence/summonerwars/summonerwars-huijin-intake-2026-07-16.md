...
本节只针对 2026-07-28 玩家反馈新增回写，不改写本文在 2026-07-16 当时完成的资源、录入和其它灰烬对象证据。

| 对象 | 旧结论 | 失效原因 | 当前替代证据 |
| --- | --- | --- | --- |
| 灰烬野兽「野火」 | 本文多处写为 `passed` / L2 passed / 阶段自动链完成 | 玩家反馈“移动阶段被动打到 0 血时单位不消失”推翻旧口径；旧证据没有单独证明真实 `ADVANCE_PHASE` 的移动阶段开始事件会先进入死亡后处理再写回棋盘 | `evidence/summonerwars/summonerwars-new-factions-implementation-reaudit-2026-07-28.md`；`pipeline.ts` preCommand 领域后处理；`postProcessDeathChecks`；`flow.test.ts -t "野火"` |
| 灼烧 | 本文多处写为 `passed` / 复用事件牌目标选择 UI | 玩家反馈“打出来选择不了目标”，且灼烧应可选择自己士兵/英雄；旧证据没有证明手牌打出会进入系统目标选择，也没有证明目标集合为任意阵营士兵/英雄 | `evidence/summonerwars/summonerwars-new-factions-implementation-reaudit-2026-07-28.md`；`getHuijinScorchTargets`；`systems.ts` / `validate.ts` / `useEventCardModes.ts` 同源；`interaction-chain-comprehensive.test.ts -t "huijin_scorch|灼烧"` |

当前裁定：本文后续未逐行改写的 `passed` 只代表 2026-07-16 历史快照；涉及野火、灼烧和旧完成类口径的引用，均已被 2026-07-28 新 evidence 替代。灰烬当前对外口径不得再直接引用本文作为完整完成证据。
...
| 机制实现 | 威势、召集护卫、怒焰召唤、点燃、护主、火焰喷吐、还击、庇护、缠门、冲撞、烈火降生、野火、快速射击、炫目光芒、灼烧、神族复仇、凤凰之魂 | partial_invalidated_2026-07-28：野火、灼烧旧 `passed` 已失效 |
...
结论等级：历史收口口径已被 2026-07-28 玩家反馈部分推翻；本文作为 2026-07-16 历史快照保留，灰烬当前代码验证口径以 2026-07-28 新派系补审 evidence 为准。

修订理由：2026-07-28 玩家反馈证明旧审计对两个关键点漏证：灰烬野兽「野火」没有证明真实移动阶段开始伤害会触发死亡离场；「灼烧」没有证明手牌打出会进入目标选择，也没有证明友方士兵/英雄可选。本文其它资源链、录入合同和非相关对象证据仍按历史记录保留；但不得再把本文整体解释为灰烬当前全量收口证据。
...
| 阶段/生命周期收口 | partial_invalidated_2026-07-28 | 野火移动阶段开始致死离场旧证据不足，已由 2026-07-28 新 evidence 与 `flow.test.ts -t "野火"` 替代 |
| 残余范围声明 | partial_invalidated_2026-07-28 | 旧“当前代码验证范围内无灰烬新增派系残余”已被野火、灼烧玩家反馈推翻；当前残余口径见 2026-07-28 新 evidence |
| 旧 evidence / 旧结论对账回写 | partial_invalidated_2026-07-28 | 旧“没有需要原地降级的灰烬旧收口文档”不再成立；本节已原地回写野火、灼烧失效 |
...
| 灰烬野兽・野火 | C1 移动阶段开始触发；C2 每个相邻敌方受 1 伤；C3 友方/非相邻不受伤；C4 与凤凰之魂组合时可增幅 | `flowHooks.ts` 阶段触发；`execute.ts`/`execute/helpers.ts` 后处理；`abilities-huijin.test.ts` | 阶段开始自动伤害 family；与凤凰之魂共用技能伤害后处理 | D1/D2/D3/D6/D8/D12/D18/D22 | L1/L2；L3 N/A，阶段自动链 | 历史 `passed` 已失效：旧证据未覆盖真实 `ADVANCE_PHASE` 致死离场；当前替代证据见 2026-07-28 新 evidence |
...
| 灼烧 | C1 召唤阶段打出；C2 选召唤师 2 格以内士兵或英雄；C3 造成 2 伤；C4 卡进入弃牌；C5 战斗日志来源可解析 | `eventCards.ts` 事件执行；`actionLog.ts` 来源解析；`abilities-huijin.test.ts` | 事件目标伤害 family；只新增范围和来源 | D1/D2/D3/D5/D7/D8/D12/D18/D22 | L1/L2；L3 复用事件牌目标选择 UI | 历史 `passed` 已失效：旧证据未覆盖打出后目标选择入口和任意阵营士兵/英雄目标集合；当前替代证据见 2026-07-28 新 evidence |
...
| 灰烬野兽 | 野火 | `PHASE_START_ABILITIES.move` 触发；移动阶段开始时每个相邻敌方受1伤 | 历史 L2 passed 已失效；缺真实 `ADVANCE_PHASE` 致死离场证明，见 2026-07-28 新 evidence |
...
| 灼烧 | 目标伤害 | 事件牌打出后，对召唤师2格内士兵/英雄造成2伤；战斗日志来源可解析 | 历史 L2 passed 已失效；缺手牌入口目标选择和友方单位可选证明，见 2026-07-28 新 evidence |
...
| 灰烬野兽・野火 | 移动阶段开始时对每个相邻敌方 1 伤 | `huijin_wildfire`、`PHASE_START_ABILITIES.move` | 移动阶段开始自动触发 | `flowHooks.ts` 阶段触发，伤害后处理进入 `execute/helpers.ts` | 只作用相邻敌方；友方和非相邻单位不应受伤 | 每个相邻敌方受 1 伤，可被凤凰之魂增幅 | 无相邻敌方时不应误伤 | 自动链无交互残留，阶段继续推进 | L1/L2/L4；L3 N/A | 历史 `passed` 已失效：缺真实阶段开始致死离场证据 |
...
| 灼烧 | 召唤阶段指定召唤师 2 格内士兵或英雄，造成 2 伤 | `huijin-scorch` event | 事件牌目标选择入口 | `eventCards.ts` 目标伤害，`actionLog.ts` 来源解析 | 目标必须在召唤师 2 格内且为士兵或英雄；事件打出后进弃牌 | 目标受 2 伤 | 超距/非单位目标不应生效 | 事件结算完成后无交互残留 | L1/L2；L3 复用事件目标选择 UI | 历史 `passed` 已失效：缺 UI 入口同源与友方目标覆盖 |
...
| 阶段/移动自动 family | 野火图文合同已锁定 | `PHASE_START_ABILITIES.move` 注册 | 领域测试覆盖相邻敌方伤害与凤凰之魂组合 | 无玩家入口，L3 N/A | 阶段自动触发后流程继续，伤害后处理去重 | 历史 `passed` 已失效：缺真实 `ADVANCE_PHASE` 致死离场证明 |
...
| 事件牌 family | 炫目光芒、灼烧、神族复仇、凤凰之魂图文合同已锁定 | 事件定义、playPhase、isActive、日志来源已接入 | 灼烧、持续攻击替换、神族复仇、凤凰之魂均有领域行为证据 | 事件打出/目标选择 UI 为成熟共享链 | 持续事件来源归属、非攻击技能增幅和重复增幅负例已覆盖 | 灼烧历史 `passed` 已失效：缺手牌目标选择入口和任意阵营目标集证明；其它对象保留历史证据 |
...
| D22 伤害计算管线配置 | 火焰喷吐、还击、庇护、野火、快速射击、事件伤害 | 历史记录中攻击伤害、技能伤害、持续事件伤害有最终状态断言；但野火致死离场与灼烧目标入口旧证据已于 2026-07-28 失效，当前替代证据见新派系补审文档 |
...
| 野火 / 凤凰之魂 | 非攻击技能伤害后处理链 | 技能伤害后处理 family | 是 | 野火先产生技能伤害，凤凰之魂后处理按来源集合增幅；同一 finalState 验证伤害叠加 | 差异为自动触发时机和增幅来源 | 野火历史 `passed` 已失效：旧判等漏掉真实阶段开始伤害后的死亡离场；凤凰之魂旧证据保留 |
| 灼烧 | 事件牌目标伤害链 | 成熟事件牌目标选择/伤害链 | 是 | 召唤阶段打出、目标过滤、伤害和弃牌均沿用事件牌执行框架 | 差异为 2 格范围和 2 点伤害 | 历史 `passed` 已失效：旧判等漏掉 UI 入口同源和友方目标覆盖 |
...
| 对账范围 | 检索命中 | 当前裁定 | 修订动作 |
| --- | --- | --- | --- |
| `evidence/summonerwars` | 2026-07-28 玩家反馈命中本文野火、灼烧旧 `passed` 行 | 本文自身就是需要原地降级的灰烬旧收口文档 | 已新增“2026-07-28 旧结论失效回写”，当前灰烬野火/灼烧证据以新派系补审文档为准 |
...
| 边界 | 当前状态 | 说明 |
| --- | --- | --- |
| 当前代码验证范围 | partial_invalidated_2026-07-28 | 野火和灼烧旧结论已被玩家反馈推翻；当前替代证据见 2026-07-28 新派系补审文档 |
| 生产部署状态 | 不属于当前代码验证范围 | 本文只证明当前仓库代码和当前资源主源证据；生产部署、正式发布、线上版本覆盖由发布流程另行证明 |
| 服务器素材主源 | passed | 三个灰烬压缩资源和 Android 差异索引均已有 HEAD 200 或上传记录 |
| 旧证据风险 | partial_invalidated_2026-07-28 | 本文自身需要降级：野火、灼烧旧 `passed` 不能再作为当前收口证据 |
...
| 命令 | 结果 | 处理 |
| --- | --- | --- |
| `npm run audit:evidence:selfcheck -- evidence/summonerwars/summonerwars-huijin-intake-2026-07-16.md` | `[audit-evidence-completeness] OK` | 历史自检结果仅代表 2026-07-16 当时；2026-07-28 玩家反馈已推翻野火、灼烧旧收口口径，本文不能再凭该历史结果保留当前收口表述 |

## 当前结论

录入阶段已锁定素材、图集顺序、卡名、基础数值、起始单位和规则子句。静态运行时接入、资源压缩、manifest、服务器上传与远端回查按 2026-07-16 历史记录保留。2026-07-28 玩家反馈推翻了本文对灰烬野兽「野火」和「灼烧」的旧完成口径：野火旧证据没有证明真实移动阶段开始伤害会触发死亡离场；灼烧旧证据没有证明打出后进入目标选择和友方士兵/英雄可选。当前灰烬野火、灼烧和新派系全集补审口径以 `evidence/summonerwars/summonerwars-new-factions-implementation-reaudit-2026-07-28.md` 为准；生产部署和正式发布仍由发布流程另行证明。
