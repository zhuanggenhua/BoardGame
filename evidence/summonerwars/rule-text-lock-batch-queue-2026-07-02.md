...
| P2 | 36 | B3 12 个 + B4 5 个 + B5 18 个 `locked-规则原文已锁`；B5 1 个 `disputed-对象归属待裁定` | locked 对象进入实现对照；`ferocity` 先裁定对象归属 |
| P3 | 13 | 12 个 `locked-规则原文已锁`；1 个 `disputed-对象归属待裁定` | locked 对象进入实现对照；`entangle` 先裁定对象归属 |
...
| B7-P3-移动穿越与相邻离开 | P3 | `climb`、`entangle`、`evasion`、`flying`、`rebound`、`slow`、`swift`、`trample` | 已锁 7 个官方英文原文和原子子句，见 `evidence/summonerwars/b7-p3-movement-and-adjacency-rule-text-lock-matrix-2026-07-02.md`；`entangle` 转 disputed。 |
...
- `ferocity` 仍保持 `disputed-对象归属待裁定`：官方 Relentless 锁到史米革，部落投石手归属未锁；不得写规则断言测试或改机制代码。
...
| `entangle` | disputed | 官方缓存不能证明城塞骑士承载 Engage/Entangle | 本地城塞骑士挂载与官方城塞骑士邻近条目冲突；先裁定对象归属 | 不写断言测试，不改机制代码 |
...
- 68 个风险对象已全部完成入口合同分流：4 个 `locked-L4已补`，62 个 `locked-规则原文已锁`，2 个 `disputed-对象归属待裁定`，0 个 `待建合同-入口已补`。
- 两个 disputed 仍不能写规则断言测试或改机制代码：`ferocity`、`entangle`。
