...
- 能力判断：`useCoarsePointer()` 的兼容语义扩展为“粗指针或无悬浮能力”，查询条件为 `(pointer: coarse), (hover: none)`，覆盖部分报告细指针但不支持悬浮的平板环境。
...
- 模拟能力：`pointer: coarse = false`、`hover: none = true`，用于覆盖“细指针报告但没有悬浮能力”的 iPad / 平板环境。
