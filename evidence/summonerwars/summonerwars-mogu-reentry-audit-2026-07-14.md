...
- 当前结论等级：2026-07-16 的“当前代码验证口径已收口”已被 2026-07-26 补审降级为历史阶段性结论；本文档只覆盖莫古子范围。当前 `FACTION_CATALOG` 实施中派系全集为莫古、灰烬、永恒议会，三派系 D8 阶段退出真实推进链、D55 AI legal-actions / 自动跳过消费者一致性补审汇总以 `evidence/summonerwars/summonerwars-phase-ai-supplemental-audit-2026-07-26.md` 为准；未代表已部署到生产。
- 当前状态：已完成 slot 0-10 完整单卡主裁图核录入和 slot 11-15 空槽合同；已修正静态配置字段、聚焦领域测试和莫古真实入口 E2E；线上反馈点名链路均有等价最小复现与真实入口验证；2026-07-15 追加效果审计发现并修复畸形巨怪“最终形态”缺少指定 5+ 充能菌化野兽门禁的问题；2026-07-15 再补枯萎法师“鲜血灌注”每回合一次按钮使用后可用态修复；2026-07-15 继续补释放菌袍“误点召唤师只召 1 个”和菌化野兽“寄生没有二选一 UI”的执行层/UI 链路修复；2026-07-16 当前树 isolated 聚合复跑莫古整文件 E2E 13/13 通过，枯萎法师按钮残留专项 E2E 1/1 通过；线上莫古相关反馈 2 条已回写为 `resolved`；2026-07-26 追加发现旧口径没有覆盖 `ADVANCE_PHASE` 阶段退出消费者和 AI legal-actions / 空牌库自动跳过消费者，新增三派系实施中批次替代证据见 `evidence/summonerwars/summonerwars-phase-ai-supplemental-audit-2026-07-26.md`。
...
| 未覆盖范围 | 生产环境部署、AI 策略评分优劣重调 | 本文保留 2026-07-16 莫古当前代码、测试、审计证据与线上反馈状态的历史收口记录；生产部署仍需发布流程另行证明；实施中三派系 D8/D55 补审以 2026-07-26 专项补审记录为准；腐坏手动指定目标 UI 已由 2026-07-27 专项补修覆盖 |

## 2026-07-26 追加失效点与替代证据

| 项 | 追加结论 |
| --- | --- |
| 失效点 | 2026-07-16 的 L4/真实入口证据没有单独证明两层消费者：① `ADVANCE_PHASE` / `onPhaseExit` 真实阶段退出是否消费所有 `onPhaseEnd` 技能；② AI legal-actions 在牌库为空时是否仍暴露召唤阶段事件、复活、特殊召唤等合法动作。 |
| 影响对象 | 本文档内的莫古子范围对象全集：库鞭克、托恩、畸形巨怪、玛硕达、枯萎法师、鲜血萨满、菌化野兽、菌袍疫病体、命令、共生自愈、狂热菌菇、释放菌袍、起始城门、传送门；整批实施中范围还包括灰烬与永恒议会，跨派系同类自动跳过样本为雷塔勒斯「复活死灵」和伊路特-巴尔「火祀召唤」。 |
| 替代证据 | `evidence/summonerwars/summonerwars-phase-ai-supplemental-audit-2026-07-26.md` 记录莫古、灰烬、永恒议会三派系实施中全集的 D8/D55 补维度、结构测试、行为测试和验证命令。 |
| 当前边界 | 该替代证据覆盖实施中三派系 D8/D55 补审；本文只作为莫古子范围旧结论回写，不覆盖生产部署、AI 策略评分优劣重调；腐坏手动指定目标 UI 已由 2026-07-27 专项补修覆盖。 |

## 2026-07-26 漏审归因与同类扩审

| 项 | 记录 |
| --- | --- |
| 漏审归因 | 旧测试失效点不是规则合同本身，而是证据停在中间态和代表链：阶段能力只证明 resolver/真实 E2E 某条链路，未单独证明 `ADVANCE_PHASE` / `onPhaseExit` 真实阶段退出消费者；复活死灵和特殊召唤只证明人工/命令链，未证明 AI legal-actions 与自动跳过消费者。 |
| 同类扩审记录 | 搜索范围覆盖实施中三派系对象全集、`PHASE_START_ABILITIES`、`PHASE_END_ABILITIES`、全部阶段触发定义、`buildSummonerWarsAiLegalActions`、`buildActivatedAbilityActions`、`buildSummonActions`、`buildEventCardActions`、复活死灵、火祀召唤、最终形态。 |
| 命中项 | 莫古阶段退出类命中玛硕达「腐坏」、菌化野兽「寄生」、菌袍疫病体「爆裂」；莫古 AI 召唤阶段命中畸形巨怪「最终形态」与「狂热菌菇」；灰烬命中「召集护卫」「野火」「凤凰之魂」；永恒议会命中「洞察」「探寻」「心念侵袭」与主管卡图「坚毅」；跨派系 AI 召唤类命中雷塔勒斯「复活死灵」、伊路特-巴尔「火祀召唤」。 |
| 残余扩审范围 | 当前只补硬合同：有合法动作时 AI 不只剩结束阶段，阶段结束技能必须被真实阶段退出消费；AI 策略评分优劣、生产部署状态不在本次补审范围内。腐坏手动指定目标 UI 已在 2026-07-27 补为正式 simple-choice 交互。 |
...
| 命中 D 维度 | passed_with_2026_07_26_under_construction_batch_supplement | D1/D3/D5/D8/D12/D14/D18/D23/D52 已登记到 `D 维度命中记录`；2026-07-26 按实施中三派系全集追加 D55（AI legal-actions / 自动跳过消费者一致性）与 D8 阶段退出真实推进链补证，见专项补审记录 |
...
| 降级后当前状态 | 旧结论失效；本文档支撑 2026-07-16 前的莫古 S0/S1/L2/L3/L4 当前代码验证口径；2026-07-26 起，阶段退出真实推进链与 AI 自动跳过消费者必须同时引用实施中三派系专项补审记录，不能只引用本文档旧口径 |
...
- 已更新 `.spec/knowledge/standards/testing-audit-dimensions.md`：D5 明确“可选主效果与可选子动作”必须拆审；D6 明确自伤/代价伤害/消灭等副作用必须传播到死亡后处理和后续被动；D18 明确“跳过整个效果”“不做子动作但继续主效果”“已死亡对象不再被后续效果作用”要分开覆盖。
- 已更新 `.spec/knowledge/standards/testing-audit-dimensions-semantics-interaction.md`：新增 D5 子项“可选主效果与可选子动作拆分”和 D6 子项“副作用死亡后的连锁传播”。
...
| D55 共享合同多消费者一致性 | 莫古对象全集；实施中批次灰烬、永恒议会；雷塔勒斯、伊路特-巴尔作为跨派系同类样本 | AI legal-actions、空牌库自动跳过、真实 `ADVANCE_PHASE` 是否消费同一合法动作/阶段触发真相 | 2026-07-26 实施中三派系补审新增：莫古「狂热菌菇」与畸形巨怪「最终形态」、灰烬「凤凰之魂」、永恒议会「洞察」「探寻」「心念侵袭」在牌库为空时仍暴露召唤阶段合法动作；复活死灵、火祀召唤作为跨派系同类入口通过；所有 `onPhaseEnd` 技能必须登记到 `PHASE_END_ABILITIES`，所有 `onPhaseStart` 技能必须登记到 `PHASE_START_ABILITIES` 或声明等价系统消费链；腐坏/爆裂通过 `ADVANCE_PHASE` 验证最终状态 |
...
- 已回代录入规范：`.spec/skills/data-entry-workflow/SKILL.md`、`.spec/knowledge/standards/data-entry.md` 增加“缩略图/降采样图不得定稿字段”。
- 已回代审计规范：`.spec/skills/game-audit-workflow/SKILL.md`、`.spec/knowledge/standards/testing-audit-core-principles.md` 增加“系统交互存在不等于真实 UI 可点”，要求 simple-choice/prompt/横幅继续证明真实按钮或候选入口。
