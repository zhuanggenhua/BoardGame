...
本表按新增派系全面审计要求维护；只有全部项目为 `passed`，本文件才允许使用“已审计/已收口”口径。
...
| 真实入口 E2E 与截图核验 | `passed` | `summonerwars-shadow.e2e.ts` 11 个场景、36 张当前工作区整屏截图，逐张 UI 审计通过 |
...
| --- | --- | --- | --- | --- | --- | --- |
| `shadow` / 暗影精灵 | `passed` | `passed` | `passed` | `passed` | `passed` | `passed` |
...
- 真实入口 E2E 与审计：`passed`（11 个真实入口场景、36 张原始整屏截图均通过 AI UI 审计 `PASS`）。该证据覆盖全部 13 个能力和 4 张事件卡的真实选择/结算路径；截图来自当前工作区 `main` 的同一真实入口。

## 当前实现与证据矩阵（2026-08-04）
...
- `L3` 真实入口：`e2e/summonerwars/summonerwars-shadow.e2e.ts` 共 `11 passed`，覆盖派系选择/初始化、多目标事件、移动后能力、召唤后能力、主动能力与持续替换、阶段结束分支、攻击后推拉、死亡触发和穿透攻击。
- `L4` 复杂交互：13 个能力和 4 张事件卡均已有真实浏览器选择、跳过（适用时）与结算截图；不再保留暗影精灵能力级 `scoped-debt`。
...
- 真实入口 E2E：`11 passed`；覆盖派系选择/初始化和 10 个新增交互族，共 36 张当前真实入口原图。
...
- 因上述边界，本批当前完成级别是“静态接入 + 全部规则 L2 + 全部能力/事件卡真实入口 L3/L4 + 36 张逐图 UI 审计通过”；暗影精灵派系目录仍保留 `under_construction` 状态标记，本轮没有把它擅自改为正式发布标记。
