...
| 真实探求者 | 士兵，费用1，生命3，战力1，近战，星形 | 猛攻：本单位在被召唤的回合中，获得战力+2。佯攻：在本单位攻击之后，你可以将其推拉1–2个区格。 |
...
| 真实探求者 | C1 召唤当回合战力 +2；C2 攻击后可推拉 1–2 格；C3 后续回合和跳过不保留额外战力/位移 |
...
| 真实探求者 | 佯攻：攻击后可推拉本单位 1–2 格 | `systems.ts` 攻击后交互 → `executors/shadow.ts` | passed | `shadow-event-interactions.test.ts` 选择/跳过/路径限制，含 1 格候选 | `summonerwars-shadow.e2e` 真实 2 格推拉结算/跳过截图；L2 定向测试补 1 格候选 | `passed:L3/L4` |
| 暗影骑士 | 暗影召唤：召唤时选择无同技能友方卡牌、直接部署到目标邻格并造成 1 伤害 | `systems.ts` 召唤后交互 → `executors/shadow.ts` | passed | `shadow-event-interactions.test.ts` 远离原位置的目标/邻格部署/伤害/跳过 | `summonerwars-shadow.e2e` 召唤后目标/位置选择与伤害 | `passed:L3/L4` |
...
| 塔莉娅·撕裂帷幕 | 英雄卡 / `shadow_tear_the_veil` | 移动后按钮 → 友方普通士兵与传送门邻格 | `systems.ts` → `executors/shadow.ts` | 每回合一次；敌方传送门已受伤且相邻；目标只允许普通士兵 | 将友方普通士兵放到传送门邻格 | 英雄、条件不满足或跳过保持原位；同回合不重复 | 交互完成后清除等待中的交互状态 | L0-L4 | `passed` |
...
| 真实探求者·佯攻 | 士兵卡 / `shadow_feint` | 攻击后按钮 → 1–2 格内位置 | `systems.ts` → `executors/shadow.ts` | 路径合法、距离 1–2 格 | 推拉自身到所选位置 | 跳过保持原位；超范围/阻挡位置不可选 | 交互完成后清除等待中的交互状态 | L0-L4 | `passed` |
| 暗影骑士·暗影召唤 | 士兵卡 / `shadow_shadow_summon` | 召唤后按钮 → 无同技能友方卡牌与目标邻格 | `systems.ts` → `executors/shadow.ts` | 目标不能拥有同技能；目标须为友方；不要求邻接暗影骑士原位置 | 暗影骑士直接部署到目标邻格并对目标造成 1 伤 | 无合法目标或跳过不部署、不伤害 | 交互完成后清除等待中的交互状态 | L0-L4 | `passed` |
...
| 暗影精灵三处规则边界回归 | 佯攻提供 1–2 格；暗影召唤只要求目标邻格、不要求邻接原召唤位置；撕裂帷幕只接受普通士兵 | `shadow-event-interactions.test.ts` 的 1 格候选、远离原位目标、英雄负向断言 |
...
- 本轮修订：把“目录内 15 张素材”改成正式运行目录、`temp` 原始单卡输入和排除素材三层；把服务器资源状态从未重查改为已上传并远端 hash 对账；补入全面审计自检表、规则子句表、完整技能流程矩阵、框架消费合同、L4 判等矩阵和 D52 可视合同；同步修正佯攻的 1–2 格范围、暗影召唤的目标邻格语义和撕裂帷幕的普通士兵目标边界。
- 失效结论：旧正文中把 15 张 JPG 直接归为用户指定正式目录、以及在未重做远端回查时直接写服务器主源通过的说法不再有效；当前以本文件的分层资源表和远端回查明细为准。
- 失效结论：旧实现/审计曾把佯攻候选缩成只有 2 格、把暗影召唤限制为邻接原召唤位置、把撕裂帷幕目标放宽到英雄；这些结论已由当前卡面规则、实现修复和定向回归测试替代。
