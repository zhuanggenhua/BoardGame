...
> 历史说明：本文件记录的是兄弟 worktree `D:\gongzuo\webgame\BoardGame\.worktrees\fantasyrealms` 的 `2026-06-06` 候选实现；文中的“堆叠布局”已统一视为“紧凑横屏布局”。

## 目标

核对 `FantasyRealms` 在平板横屏 / 中间断点紧凑横屏布局下，首帧仍能直接看到当前回合主操作按钮。
...
`<=1180px` 的紧凑横屏布局现在都能在首帧直接执行当前回合主操作，不再要求玩家先滚到牌桌下方。
