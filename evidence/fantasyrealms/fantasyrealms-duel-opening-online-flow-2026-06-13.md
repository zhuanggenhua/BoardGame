...
- 创建房间弹窗这一步是合格的：`确认创建` 主按钮完整可见，`加入 AI` 状态也可见，前图能直接看出下一步。
- 进入对局后的首屏现在不再只是“空手牌 + 空桌面”。左上牌库除了真实牌堆本体，还新增了可见 cue `点此摸 2 张`，底部短提示也明确写了 `点左上牌库，先摸 2 张`，前图已能直接指出下一步。
- 摸牌后，`2` 张真实手牌成为本步候选对象，顶部短标签切到 `弃牌`，不再是看不出该做什么的空态。
- 选中一张手牌后，卡面出现 `已选` 状态，右侧出现 `确认弃置`，前图能直接看出下一步确认动作落点。
- 确认弃置后，页面切到 `AI 2 号位`，同时保留 `R2` 和 `1/12`，这说明当前是合法等待 AI 推进，而不是流程挂起。
...
- 肉眼结论：
  - 左上牌库本体清晰可见，旁边直接贴着 `点此摸 2 张`。
  - 底部短提示写明 `点左上牌库，先摸 2 张`，因此这张前图不再像“空桌面卡死”。
