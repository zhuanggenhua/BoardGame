...
> 历史阶段说明：
> 本文记录的是 2026-06-06 当时桌面宽屏候选下的一张近终盘代表态。
> 文中出现的“不是堆叠态”只是为了和当时另一套横屏断点候选做区分，不代表今天仍存在独立 `stacked` 正式家族。
> 今天应将其理解为：**历史桌面宽屏代表态证据**，不是当前正式布局命名依据。

- 时间：2026-06-06
- 目标页面：`/play/fantasyrealms/local?players=6`
- 目标语义：PC 桌面进行中对局页，不是终局复盘，不是当时的横屏断点候选
- 验收视口：`1920x1080`
...
- 公开弃牌保持两排交错节奏：近终盘 9 张明牌不是死板矩阵，而是上排 5 张、下排 4 张交错咬合，9 张卡均为 `190x264`，并保留桌面式纵向搭接。
