...
- 这组截图证明的是“公开弃牌已经存在的中盘代表态”，不是开局。
- 开局空弃牌时，不应提前出现 `拿公开牌` 入口；只能先摸牌。
- 中盘公开弃牌存在时，才允许同时出现 `摸牌` 与 `拿公开牌` 两个直接入口。
- 进入拿公开牌选牌态后，不应再保留一个同名大按钮与目标并列。
...
- `evidence/fantasyrealms/direct-click-2026-06-18/01-桌面摸牌与拿公开牌-横幅提示.png`
  - 证明这是“公开弃牌已存在的中盘桌面态”，因此同时存在“摸牌”和“拿公开牌”两个直接入口。
