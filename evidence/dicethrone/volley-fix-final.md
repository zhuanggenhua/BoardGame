...
> 2026-06-06 当前有效口径：本文只覆盖 `Volley` 特写/伤害修正 UI 这一组历史专项修复，不是当前 DiceThrone 所有 `Volley` 行为、所有多骰奖励骰展示链路都已彻底收口的证明，也不是新英雄补审出口。阅读时必须把它理解成历史局部修复记录。
...
**当前阅读说明**：本文只能证明 `Volley` 特写/伤害修正 UI 这组专项问题曾被修复，不能外推为当前所有 `Volley` 行为、所有多骰奖励骰显示或 DiceThrone 当前整体审计都已收口。
