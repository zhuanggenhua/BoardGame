# DiceThrone 攻击修正徽章残留专项修复证据

> 2026-06-06 当前有效口径：本文只保留“攻击修正徽章 / bonusDamage 在 main2 阶段未清除”这一条 UI + 状态清理链路的专项修复证据，不代表 DiceThrone 全体战斗链、任一单英雄，或四位新英雄整批当前已经审计完成。它现在只能证明当时 `pendingBonusDamage` 清理与相关 UI 残留问题被专项修过，不能外推成 DiceThrone 当前整体收口。
...
通过在 main2 阶段转换时清除 `pendingBonusDamage`，确保伤害加成显示在攻击结算后立即消失。单元测试验证了本文覆盖的专项修复，包括边界情况和结构共享优化。攻击修正徽章的清除依赖 EventStream 中的 `ATTACK_RESOLVED` 事件，这条结论只说明该专项链路在当轮被排查，不外推 DiceThrone 当前总体完成态。

## 当前阅读说明

- 本文只覆盖攻击修正徽章 / bonusDamage 残留这一条专项，不覆盖更广范围战斗链、对象级 `L3/L4` 或新英雄整批完成态。
- 即使本文中的修复和测试在当轮成立，也不能把它当成当前 DiceThrone 或新英雄整批“全面收口”的证明。
