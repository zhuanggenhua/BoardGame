...
> 2026-06-06 当前有效口径：本文只保留“攻击已发起后禁止重新选择技能”这一条验证逻辑的专项修复证据，不代表 DiceThrone 全体规则链、任一单英雄，或四位新英雄整批当前已经审计完成。它现在只能证明当时 `commandValidation.ts` 里这条规则门禁被找回并补回；若要判断新英雄补审现状或更广范围共享合同状态，应回到新英雄总汇总与对应单英雄主审计文档。
...
- 本文是单条验证逻辑恢复证据，不是 DiceThrone 当前总审计出口。
