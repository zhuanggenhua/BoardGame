...
> 2026-06-06 当前有效口径：本文只记录“护盾已减伤但动画仍显示原始伤害”这一条历史 UI/展示专项修复，不是当前 DiceThrone 伤害结算链、也不是新英雄补审的完成证明。阅读时只能把它理解为单问题修复证据；当前是否仍有其他伤害展示或规则残余，需以现行代码、测试和主审计文档为准。
...
**当前阅读说明**：本文只能证明“护盾减伤后的动画跳字错误”这条专项问题曾被修复，不能外推为当前所有伤害动画、所有护盾消费链或 DiceThrone 整体审计都已收口。
