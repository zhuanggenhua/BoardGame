...
> 2026-06-06 当前有效口径：本文只保留月精灵 `Volley / Exploding Arrow I/II/III` 多骰显示链的专项修复证据，不代表月精灵整英雄、DiceThrone 全体英雄，或四位新英雄整批当前已经审计完成。它现在只能证明月精灵当时这几条多骰 `BONUS_DIE_ROLLED` / 特写展示链被补过，不能外推成 DiceThrone 当前更大范围机制或审计口径已经收口。
...
本专项在当轮已修复。月精灵的 4 个多骰技能现在都会正确显示 5 个骰子的投掷结果，而不是只显示 1 个。修复方案参考了狂战士技能的正确实现模式，并进行了代码优化（在循环内直接统计，避免二次遍历）。

## 当前阅读说明

- 本文只覆盖月精灵多骰显示这一条专项，不覆盖月精灵整英雄审计完成态、DiceThrone 全量多骰机制，或新英雄整批完成态。
- 若要判断新英雄批次当前残余，应回到对应单英雄主审计文档与四位新英雄总汇总，而不是本文这类专项修复证据。
