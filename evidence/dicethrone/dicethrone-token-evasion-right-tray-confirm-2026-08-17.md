...
- 当前修复：闪避 Token 掷出临时骰后，共享响应提示退场，右侧骰盘显示单个“确认”按钮；点击该按钮发出同一个 Token 响应收口命令，确认后清理闪避骰并回到主阶段。
...
| `e2e/dicethrone/dicethrone-token-response-window.e2e.ts` | 闪避用例从“共享响应框确认”改为“右侧骰盘确认”，并断言右侧确认按钮可见、可用、共享响应提示退场 | 测试不再固化错误修法 |
...
- `npx vitest run src/games/dicethrone/ui/__tests__/DiceTray.test.tsx src/games/dicethrone/ui/__tests__/diceStagePolicy.test.ts --reporter dot`
  - 结果：`2 files / 22 tests passed`。
...
- `node scripts/infra/run-e2e-single.mjs isolated e2e/dicethrone/dicethrone-die-modification.e2e.ts "闪避骰进入当前骰区后，战术优势可重掷并重新计算免伤"`
  - 首跑结果：测试状态已收口、伤害已结算，但旧断言还要求确认后按钮必须隐藏；该断言不属于本轮目标，已删除。
  - 复跑结果：`1 passed`。
