...
> 2026-06-06 当前有效口径：本文只代表 2026-03-09 那轮 POD 审计收尾时“仍需人工确认”的历史清单，不是当前 DiceThrone 审计的未完成总表，也不是新英雄补审 blocker 列表。若要判断当前仍未收口的事项，应以最新对象级/批次级主审计文档为准，不能把这里的历史确认项直接当成当前残余。
...
**当前阅读说明**：本文只能说明当时对 `expectedDamage` 简化的风险判断与建议，不构成当前版本仍然存在同一 blocker 的证明；是否仍需处理，必须回到现行代码、现行测试和最新主审计文档重新判断。
