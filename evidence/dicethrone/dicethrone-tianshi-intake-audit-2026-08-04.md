...
本文件不把“24 条成功路径通过”扩写成所有规则分支穷尽。历史真实入口已覆盖九个技能和 15 张专属卡各自至少一条直接成功路径，并覆盖奖励骰、防御重投、复合升级壳、目标选择和神圣净化空选跳过；未覆盖的其它否定分支和组合分支仍单独列为残余范围。2026-08-14 新增中英文分支名后，i18n 已按本轮验证记录重新确认：全仓检查被 Betrayal 既有 raw JSX 可见文案阻塞，炽天使 JSON 窄验证通过。
...
- 命令/结果：tianshi-behavior.test.ts 51/51 通过；tianshi-rule-matrix.test.ts 18/18 通过；tianshi-intake.test.ts 历史 3/3 通过，合计 72/72。
- 证明内容：飞行进攻/防御分支；眩光五种骰面和三种攻击结果；神圣降临三玩家逐层伤害；起飞与神圣降临直接伤害通过共享伤害结算触发神圣祝福致死保护；神圣净化自己/对手分支和状态移除；神圣祝福致死保护；天使斗篷免费重投一次；圣击圣洁吊坠/双翼否定分支；神圣惩戒 II 2 点不可防御伤害；神圣裁决三段选择；6 张复合升级牌打出时只替换技能、不自动执行下半区；福音临世保留目标选择；圣刃 II / 智天使无净化。
