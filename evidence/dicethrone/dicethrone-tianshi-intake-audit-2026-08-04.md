...
- 当前结论：本地 intake、逐技能/逐卡历史成功路径、代表性领域矩阵和服务器媒体发布已通过；2026-08-14 接续修正“复合升级牌下半区在打出升级牌时自动结算”的规则建模错误。当前领域回归锁定 6 张复合升级牌只执行升级替换，下半区进入升级后技能 variants；未覆盖组合分支仍保留为残余范围。

本文件不把“24 条成功路径通过”扩写成所有规则分支穷尽。历史真实入口已覆盖九个技能和 15 张专属卡各自至少一条直接成功路径，并覆盖攻击奖励骰、当时记录的防御重投、复合升级壳、目标选择和神圣净化空选跳过；未覆盖的其它否定分支和组合分支仍单独列为残余范围。2026-08-14 新增中英文分支名后，i18n 已按本轮验证记录重新确认：全仓检查被 Betrayal 既有 raw JSX 可见文案阻塞，炽天使 JSON 窄验证通过。

### 1.1 2026-08-29 当前纠正

本历史 evidence 中“天使斗篷奖励骰”“防御掷骰奖励骰”等表述及其对应旧测试语义已失效：天使斗篷使用防御阶段的普通骰，只有升级版才允许免费重投 1 次普通防御骰。当前修复与回归证据见 `evidence/dicethrone/dicethrone-tianshi-angelic-cloak-attack-continuation-fix-2026-08-29.md`；该证据新增了僧侣 8 点攻击 → 普通防御骰为 1 → 天使斗篷反击 → 主攻击收口的完整领域链，炽天使生命由 50 降到 42。下文保留历史结果，但不得再把旧的奖励骰模型当作当前覆盖证据。
...
- 炽天使规则矩阵当前为 18 条领域测试，覆盖技能失败否定、圣刃三档升级数值、凯旋归来 II 四面奖励骰与 8 点基础伤害、圣击圣洁吊坠/双翼否定分支、天使战术奖励骰分支、神圣惩戒 II 2 点不可防御伤害、至高圣洁成功分支、神圣裁决目标范围、圣刃 III 四同眩光命中/未命中，以及复合升级牌只保留 replaceAbility、下半区进入升级后技能 variants 的结构矩阵。
...
| 神圣裁决三段选择与目标范围真实入口复核 | 历史真实入口已复核 | 领域测试和同一官方 runner 历史证据已证明眩光、2 个飞行、净化三段选择及最终状态 | 不再阻塞神圣裁决历史真实入口口径 | passed | 预算释放后曾重跑同一官方 runner；两张中间选择态和一张最终收口原始截图逐张审计为 PASS |
| 服务器上传和公开 URL HEAD | 发布门禁 | 10 个炽天使运行时媒体已上传，公开 URL 全部 HEAD 200 | 不再阻塞本地或线上媒体收口 | passed | 发布批次 20260804164030910；后续只需在变更资源时复查 |
| npm run i18n:check | 共享门禁 | 炽天使新增 key 已补齐且 game-dicethrone JSON 窄验证通过；全仓仍有 Betrayal 既有 raw JSX 可见文案未收口 | 不阻塞当前炽天使规则实现；阻塞“全仓 i18n 通过”口径 | blocked_by_betrayal | 先收口或隔离 Betrayal 可见文案，再复跑全仓 i18n |
...
| 真实入口 E2E 与截图核验 | historical_passed_with_scoped_debt | 同一官方 runner 历史分组为技能/升级组 15/15、卡牌/复合组 9/9，合计 24/24；39 张截图作为历史运行产物，三张关键修复图已有视觉 PASS。2026-08-14 本轮未重跑真实入口 E2E，不能把历史复合升级成功路径解释为下半区应在升级时结算 |
...
| 残余范围声明 | passed_with_i18n_blocked | 明确列出未覆盖规则组合；全仓 i18n 当前被 Betrayal 既有 raw JSX 可见文案阻塞，远端媒体发布已单独记录为通过 |
| 旧 evidence / 旧结论对账回写 | passed | 三份炽天使 rule 文档已回写当前 51/51 行为测试、18/18 矩阵测试、历史 24/24 分组真实入口、2026-08-14 复合升级自动结算修正和 E2E 辅助竞态结论 |
...
| 天使斗篷（angelic-cloak） | 历史记录曾写作防御掷骰奖励骰；当前纠正为防御阶段普通骰，升级版免费重投一次 | 历史证据已降级；当前以 2026-08-29 修复 evidence 为准 | 旧测试只证明错误的奖励骰模型；当前新增证据证明普通防御骰、主攻击最终扣血和临时状态清理 |
...
| 19 | 无上之力 II / 福音临世 | upgrade-tianshi-supreme-power-2-gospel-arrival | 2026-08-14 修正：升级牌只消耗 CP 并替换无上之力；福音临世为升级后技能分支，领域回归锁定打出升级牌时不立刻发神圣降临、飞行、净化、眩光或目标选择 |
| 20 | 神圣惩戒 II / 神圣指令 | upgrade-tianshi-divine-punishment-2-divine-command | 2026-08-14 修正：升级牌只消耗 CP 并替换神圣惩戒；神圣指令为升级后技能分支，领域回归锁定打出升级牌时不立刻治疗、伤害或目标选择 |
| 21 | 神圣净化 II | upgrade-tianshi-divine-purification-2 | L3 passed：消耗 2 CP、升级并回到无交互状态 |
| 22 | 天使长之志 II / 神之庇佑 | upgrade-tianshi-archangel-resolve-2-divine-protection | 2026-08-14 修正：升级牌只消耗 CP 并替换天使长之志；神之庇佑为升级后技能分支，领域回归锁定打出升级牌时不立刻开目标选择或发 Token |
...
| 26 | 圣洁光辉 II / 起飞 | upgrade-tianshi-holy-radiance-2-takeoff | 2026-08-14 修正：升级牌只消耗 CP 并替换圣洁光辉；起飞为升级后技能分支，领域回归锁定打出升级牌时不立刻开目标选择、发飞行或造成伤害 |
| 27 | 圣刃 III / 智天使 II | upgrade-tianshi-holy-blade-3-cherub-2 | 2026-08-14 修正：升级牌只消耗 CP 并替换圣刃；智天使 II 为升级后技能分支，领域回归锁定打出升级牌时不立刻发 Token |
...
| 31 | 圣刃 II / 智天使 | upgrade-tianshi-holy-blade-2-cherub | 2026-08-14 修正：升级牌只消耗 CP 并替换圣刃；智天使为升级后技能分支，领域回归锁定打出升级牌时不立刻发 Token，且技能分支不发净化 |
...
- 命令/结果：tianshi-behavior.test.ts 当前 56/56 通过；tianshi-rule-matrix.test.ts 18/18 通过；tianshi-intake.test.ts 历史 3/3 通过；当前修复 evidence 另记录完整攻击链回归。
- 证明内容：飞行进攻/防御分支；眩光五种骰面和三种攻击结果；神圣降临三玩家逐层伤害；起飞与神圣降临直接伤害通过共享伤害结算触发神圣祝福致死保护；神圣净化自己/对手分支和状态移除；神圣祝福致死保护；天使斗篷普通防御骰四类骰面、升级版免费重投一次，以及僧侣 8 点攻击后的主攻击最终扣血；圣击圣洁吊坠/双翼否定分支；神圣惩戒 II 2 点不可防御伤害；神圣裁决三段选择；6 张复合升级牌打出时只替换技能、不自动执行下半区；福音临世保留目标选择；圣刃 II / 智天使无净化。
...
- 历史修正后结果：原始单次长链曾在约 244 秒和 304 秒处超时；拆分同一官方 runner 后，技能/升级组 15/15、卡牌/复合组 9/9，合计 24/24 通过。该历史结果证明真实手牌能完成升级壳和当时记录的成功路径，不证明下半区应该在打出升级牌时结算。2026-08-14 本轮未重跑真实入口 E2E，当前以领域回归证明复合升级自动结算 bug 已修。
...
- npm run i18n:check：本轮复跑未通过；失败点在 Betrayal 既有 raw JSX 可见文案（src/games/betrayal/Board.tsx 与 src/pages/BetrayalConfigReview.tsx），不是炽天使新增分支文案。炽天使中英文 game-dicethrone JSON 窄验证通过。
...
- 没有把本地资源存在当作服务器已上传。
- 全仓 npm run i18n:check 本轮复跑未通过，阻塞点在 Betrayal 既有 raw JSX 可见文案；该阻塞不被反向解释为炽天使规则修复失败，也不能被写成全仓 i18n 通过。

## 8. 共享根因与残余范围

### 8.1 当前残余

1. 技能和卡牌的主要成功路径已有历史真实入口 24/24 基线证据；2026-08-14 本轮未重跑真实入口 E2E，复合升级自动结算问题由领域回归覆盖。所有未覆盖的规则失败/否则/组合分支仍按 scoped-debt 保留，不能从成功路径外推。
2. 神圣裁决已按官方卡槽补齐为眩光、2 个飞行、净化三段玩家选择；领域矩阵、真实入口链和三张新增原始截图的视觉审计均已通过。两张中间选择态曾短暂受全局预算占用影响，预算释放后已按同一入口补拍并收口。
3. 全仓 i18n 命令本轮未通过；失败点在 Betrayal 既有 raw JSX 可见文案，炽天使中英文 JSON 窄验证通过。这是共享门禁残余，不是炽天使规则修复本身。

### 8.2 后续唯一入口

- 继续前先读本文件和三份炽天使 rule 文档。
- 若继续扩审，只进入“未覆盖规则子句的失败/否则/组合路径”这一条已登记入口；新增文案时至少保留炽天使 JSON 窄验证，若要写全仓 i18n 通过，必须先处理当前 Betrayal 阻塞并复跑。
- 不得把历史 24 条基线成功路径或本轮领域回归外推为全部规则组合已证明。
...
- 当前替代结论：本地静态接入、51/51 行为测试、18/18 矩阵测试、直接伤害共享结算与神圣祝福保护、历史 24/24 基线真实入口均已登记；2026-08-14 复合升级自动结算修正已通过领域回归，尚未重跑真实入口 E2E；未覆盖组合仍按边界保留，新增中英文分支名后已复跑 i18n 检查，但全仓失败点在 Betrayal 既有 raw JSX 可见文案，炽天使 JSON 窄验证通过。
...
- 原始单次长链曾超时；修正 E2E 辅助等待后，同一真实入口历史拆分为 15/15 + 9/9，合计 24/24 通过。原始截图基线逐张图面审计 PASS，综合 93/100；2026-08-14 本轮未重跑该真实入口 E2E，当前不能把历史截图当作复合升级下半区分支的本轮验收。
- 飞行、眩光、神圣净化、神圣降临、神圣祝福、天使斗篷奖励骰和神圣裁决已有代表性领域验证。
- 当前结论是“本地 intake 与代表性行为已验证，仍有残余范围”。
- 本轮全仓 i18n 未通过；失败点在 Betrayal 既有 raw JSX 可见文案，炽天使 JSON 窄验证通过。
...
- 炽天使完整可玩已完成。
- 声称全仓 npm run i18n:check 已经通过。当前只能说：本轮全仓 i18n 被 Betrayal 既有 raw JSX 可见文案阻塞，炽天使 JSON 窄验证通过。
