# 王权骰铸对手视角下一阶段按钮专项修复证据

> 2026-06-06 当前有效口径：本文只保留“对手视角下一阶段按钮应继续显示为禁用态”这一条 UI 共享链路的专项修复证据，不代表 DiceThrone 全体 UI、任一单英雄，或四位新英雄整批当前已经审计完成。它现在只能证明当时 `showAdvancePhaseButton` 这条共享显示逻辑被回修，不能外推成 DiceThrone 当前整体或 POD 历史问题已经全部收口。
...
- 本文只覆盖“对手视角下一阶段按钮”这一条 UI shared seam，不覆盖更广范围 DiceThrone UI 或新英雄整批完成态。
- 文中关于 POD 历史范围的讨论只代表当时排查语境，不能当作当前 DiceThrone 总审计出口。
...
根据 `evidence/_shared/pod-commit-scope-audit.md` 和 `evidence/_shared/POD-RECOVERY-FINAL-STATUS.md`，`6ea1f9f` 提交（feat: add Smash Up POD faction support）包含了大量非 POD 相关的修改：
