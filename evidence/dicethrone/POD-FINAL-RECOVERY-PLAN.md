...
> 状态提示（2026-06-05）：已归档的历史最终计划稿。文中“需要恢复/等待执行”只代表 2026-03-04 当时，不能再解释为当前待办；当前只作历史参考。
>
> 2026-06-06 当前有效口径：本文是 2026-03-04 那一轮 POD 恢复工作的历史计划稿，不代表当前 DiceThrone 整体、任一单英雄，或四位新英雄整批已经审计完成。它现在只能作为当时恢复顺序与思路的历史参考，不能外推成当前 DiceThrone 总审计出口。
...
- 本文是历史恢复计划，不是 DiceThrone 当前总审计出口。
- 文中“需要恢复/等待执行/执行顺序”都只代表 2026-03-04 当时，不能替代当前新英雄默认全面审计留档所需的对象级矩阵、批次级 `L4` 判等与现行 evidence 入口。
