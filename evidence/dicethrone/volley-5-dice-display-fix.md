...
> 2026-06-06 当前有效口径：本文只覆盖 “Volley 说明投 5 骰但前台只显示 1 颗” 这一条历史专项修复，不是当前 DiceThrone 所有多骰显示、所有 `BONUS_DIE_ROLLED` 消费链路都已彻底收口的证明，也不是新英雄补审出口。阅读时只能把它理解成单条多骰显示修复记录。
...
**当前阅读说明**：本文只能证明 `Volley` 只显示 1 颗骰子的专项问题曾被修复，不能外推为当前所有多骰显示、所有 `BONUS_DIE_ROLLED` 消费链路或 DiceThrone 当前整体审计都已收口。
