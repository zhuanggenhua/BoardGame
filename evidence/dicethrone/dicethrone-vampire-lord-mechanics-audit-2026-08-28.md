...
判定说明：静态数据、资源链和已上传素材仍保留有效；旧 evidence 中“完成态 / 当前范围已收口”的旧结论曾被嗜血之爪漏审推翻，2026-09-11 又确认专属行动牌旧合同仍传播“获得类统一处理 / 攻击修正统一 +1 / 血石抽牌”等错误，并确认 `slot-29 嗜血之爪 III` 图面为 2CP、旧 4CP 合同失效，`slot-30 嗜血之爪 II` 图面为 1CP、旧 2CP 合同失效。2026-09-16 进一步确认旧四档窗口证据只覆盖预置伤害后状态，没有覆盖“不可防御自然攻击造成伤害后是否暂停到四档窗口”的触发链；当前已按本地卡图补齐鲜血之力四档、催眠 token 入口、嗜血之爪奖励、专属行动牌逐卡结算、嗜血之爪 II / III 成本和不可防御自然流程触发；角色仍为 `in_progress`，移除实施中标记必须经过真人明确批准。
...
| 最终权威结果 | `passed` | 领域测试断言对手 HP、攻击者 `blood_power`、卡牌弃牌堆、手牌 / 牌库、流血层数、CP 和防御骰不污染攻击骰快照；E2E 断言基础魅惑之力不可防御自然攻击后对手 HP 50->46、吸血鬼领主 HP 40->44、鲜血之力 4->0、攻击收口到主要阶段二；魔血附身奖励骰确认前显示为攻击方吸血鬼领主骰。 |
| 交互真实入口 | `passed` | 四档鲜血之力、催眠、魔血附身奖励骰归属、嗜血之爪、不死防御和实施中玩家入口已分别通过真实入口；2026-09-16 追加证明不可防御自然攻击不会直接跳过四档窗口；审计前隐藏态与中间实施态历史门禁也有独立证据。 |
...
- 鲜血之力四档真实 E2E 逐档确认按钮入口、使用后的扣除事件、对应效果和本回合禁用；第二档无可移除状态时保持可发现但禁用，不产生状态选择残留。2026-09-16 追加自然链验证：基础魅惑之力不可防御攻击造成 4 点伤害后，先暂停到“是否发动鲜血之力”窗口，发动后治疗 4 点并收口。上述证据覆盖触发队列、阶段推进、确认边界、正式事件和无残留收口。
- 催眠真实 E2E 重新确认：对手通过正式确认当前骰区后打开 `afterRollConfirmed` 响应窗口；合法时机下左侧催眠 token 本体有静态可用高亮、可点击且中心点命中 token 命中区；旧右侧“催眠重掷”按钮不存在。点击 token 本体后消耗催眠并投临时骰，5/6 后对手骰本体变为可选，确认后对手骰被正式重掷，响应窗口与选择状态清空。2026-09-16 当前 PASS 清单：`evidence/dicethrone/vampire-lord-hypnosis-response-pass-2026-09-16.json`。
...
- 命令：`node scripts/infra/vitest-cli-safe.mjs run src/games/dicethrone/__tests__/vampire-lord-mechanics.test.ts --configLoader native --pool forks --no-file-parallelism --maxWorkers 1 -t "基础魅惑之力造成不可防御伤害后仍暂停，让鲜血之力 4 档吸血"`。
- 结果：2026-09-16 首跑修复后通过 1 条；证明基础魅惑之力自然选择后，点击结算攻击先造成 4 点不可防御伤害、保留攻击状态、打开四档机会、发动后消耗 4 个鲜血之力并治疗 4 点，最后进入主要阶段二。
- 命令：`node scripts/infra/vitest-cli-safe.mjs run src/games/dicethrone/__tests__/vampire-lord-mechanics.test.ts --configLoader native --pool forks --no-file-parallelism --maxWorkers 1 -t "基础魅惑之力造成不可防御伤害后仍暂停，让鲜血之力 4 档吸血|对手确认防御骰后，吸血鬼持有催眠应打开 afterRollConfirmed 响应窗口|催眠只有在 afterRollConfirmed 响应窗口、持有催眠且当前骰区存在对手骰时可用|催眠投出 5/6 后选择一颗对手骰并通过正式重掷命令改变该骰"`。
- 结果：2026-09-16 通过 4 条；覆盖本轮点名的催眠响应窗口、催眠可用条件、催眠临时骰改对手骰收口，以及不可防御自然攻击后的鲜血之力四档暂停。
- 命令：`node scripts/infra/run-e2e-command.mjs isolated e2e/dicethrone/vampire-lord-real-entry.e2e.ts --grep "基础魅惑之力不可防御攻击后应暂停并允许鲜血之力 4 档吸血"`。
- 结果：2026-09-16 通过 1 条；证明真实页面入口中，对手 HP 50->46 后出现“是否发动鲜血之力”窗口，点击发动后吸血鬼领主 HP 40->44、鲜血之力 4->0、攻击收口到主要阶段二；PASS 清单为 `evidence/dicethrone/vampire-lord-blood-power-unblockable-natural-flow-pass-2026-09-16.json`。
- 命令：`node scripts/infra/run-e2e-command.mjs isolated e2e/dicethrone/vampire-lord-real-entry.e2e.ts --grep "催眠应在对手确认骰后打开响应窗口，并通过点击 token 本体改对手骰|鲜血之力 4 档应通过攻击后弹窗按已造成伤害治疗并收口|鲜血之力 4 档攻击后弹窗允许不发动并继续收口"`。
- 结果：2026-09-16 通过 3 条；证明催眠当前仍会在对手确认防御骰后打开响应阶段，旧四档弹窗使用 / 跳过分支没有被本轮自然流程修复带坏。
...
## 2026-09-16 不可防御自然流程重审补充

- 用户原始症状：`四个鲜血之力吸血` 在不可防御伤害后没有触发；预期是“造成伤害后触发”，也就是攻击实际扣血后必须先给玩家一次是否发动第 4 档吸血的窗口，不能直接收口到主要阶段二。
- 首跑红测：`node scripts/infra/run-e2e-command.mjs isolated e2e/dicethrone/vampire-lord-real-entry.e2e.ts --grep "基础魅惑之力不可防御攻击后应暂停并允许鲜血之力 4 档吸血"` 在修复前失败；失败现场显示对手 HP 已从 50 到 46、攻击者 CP 2->3，但页面仍停在“结算攻击”，没有 `dicethrone-passive-opportunity-modal` 对应的玩家可见“是否发动鲜血之力”窗口。
- 根因类型：测试层级错配 + 代表态冒充自然流程。旧 9/14 四档用例从预置的伤害后状态起步，证明“窗口出现后可以发动 / 跳过 / 治疗 / 收口”，没有证明“基础魅惑之力这种不可防御自然攻击造成伤害后，系统会把攻击暂停到这个窗口”。
- 实现修复：伤害后机会判断现在不只依赖同一批事件里已经有“攻击收口”事件；只要正式扣血后攻击状态已经进入“伤害后等待”、本次攻击已造成伤害、玩家有可用第 4 档，就在攻击收口前给一次窗口，并在同一次攻击状态更新中同步“伤害已落地”。
- 验证结论：2026-09-16 低层目标测试和真实入口 E2E 均通过；真实入口断言对手 HP 50->46 后出现“是否发动鲜血之力”，点击发动后吸血鬼领主 HP 40->44、鲜血之力 4->0、攻击收口到主要阶段二。对应 PASS 清单：`evidence/dicethrone/vampire-lord-blood-power-unblockable-natural-flow-pass-2026-09-16.json`。
- 终极路径补证：2026-09-18 真实入口 E2E 通过 1 条；从五个血滴触发血色杀戮、完成抽牌堆搜牌、造成 10 点伤害并把鲜血之力从 2 提到 4，随后真实出现“是否发动鲜血之力”窗口；点击发动后吸血鬼领主生命 40->50、鲜血之力 4->0、进入主要阶段二。对应 PASS 清单：`evidence/dicethrone/vampire-lord-bloody-slaughter-blood-power-pass-20260918.json`，四张整页原始截图路径见清单。
- 旧结论处理：`evidence/dicethrone/vampire-lord-passive-opportunity-ui-pass-2026-09-14-v2.json` 仍可证明“预置伤害后窗口的消费链”，但不能再单独支撑“不可防御自然流程已审计 / 已收口”。之后审计同类“造成伤害后可选能力”时，必须同时覆盖自然攻击入口和窗口消费入口。
...
- 失效原因：嗜血之爪相同数字获得鲜血之力是独立最终权威状态；专属行动牌的投骰分支、按骰值向上取整、按利爪 / 流血层数加伤、花费选择和血石不抽牌也都是独立规则语义，旧代码、旧测试和旧 evidence 均未完整覆盖。
