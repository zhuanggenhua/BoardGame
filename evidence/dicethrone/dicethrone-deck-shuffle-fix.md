# DiceThrone 起手牌洗牌专项修复证据

> 2026-06-06 当前有效口径：本文只保留“起手牌顺序未消费 `initialDeckCardIds` 导致跨局恒定”这一条牌库初始化链路的专项修复证据，不代表 DiceThrone 全体初始化流程、任一单英雄，或四位新英雄整批当前已经审计完成。它现在只能证明当时牌库初始化 shared seam 被修正并验证过，不能外推成 DiceThrone 当前整体收口。
...
- 本文只覆盖起手牌洗牌这一条初始化 shared seam，不覆盖更广范围 DiceThrone 初始化或新英雄整批完成态。
- 即使本文中的修复、测试和用户确认在当轮成立，也不能把它当成当前 DiceThrone 或新英雄整批“全面收口”的证明。
