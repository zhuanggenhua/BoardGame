...
> 2026-06-06 当前有效口径：本文只覆盖“攻击修正徽章在进入 `main2` 阶段时未清除”这一条历史专项修复，不是当前 DiceThrone 所有攻击修正徽章问题都已彻底解决的证明，也不是新英雄补审出口。阅读时必须把它理解成该系列问题中的一条局部修复记录。
...
2. **乐观回滚处理修复**（`evidence/_shared/active-modifier-badge-fix-complete.md`）- 修复乐观引擎回滚后状态不同步的问题
3. **ATTACK_RESOLVED 清除修复**（`evidence/_shared/active-modifier-badge-fix-complete.md`）- 修复攻击结束时不清除徽章的问题
...
**当前阅读说明**：本文只能证明 `main2` 阶段清除这条专项问题曾被处理，不能外推为当前所有阶段切换、所有徽章清理时机或 DiceThrone 当前整体审计都已收口。
