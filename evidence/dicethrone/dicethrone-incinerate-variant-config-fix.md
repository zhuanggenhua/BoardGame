...
> 2026-06-06 当前有效口径：本文是 2026-03-04 那轮排查“焚灭无法选择”时的历史诊断/配置分析文档，不是当前 `Incinerate` 已完全收口的证明，也不是当前 DiceThrone 总审计或新英雄补审出口。文末仍保留“等待用户反馈/测试建议”的历史语境，阅读时必须把它理解成阶段性排查记录。
...
- `evidence/dicethrone/dicethrone-incinerate-selection-debug.md` - 调试日志文档
...
**当前阅读说明**：本文只能作为“当时如何继续排查 `Incinerate` 无法选择”的历史诊断记录，不能外推为当前 `Incinerate` 机制状态，也不能替代后续更明确的修复证据。
