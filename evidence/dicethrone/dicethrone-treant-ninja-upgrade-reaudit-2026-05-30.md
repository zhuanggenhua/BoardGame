...
- `毒刃 II` 也已从“疑似录入/实现双错”降级成“对象级在线 L3 已补齐”：
  - 图片真相源 `temp/ninja-upgrade-crops/poisonblade2_precise.png` 证明卡面语义就是“小顺子 -> 投 1 骰 -> 忍刀 1 毒 / 手里剑或面具 2 毒 -> 造成 5 伤害”。
  - 当前实现 `POISON_BLADE_2` 与该语义一致；本轮补的缺口是对象级证据，而不是新增实现。
  - 2026-06-04 在线复跑已确认：树精 `rooted` 防御链介入后，`[1,2,3]` 三个树枝会防止 `3` 点伤害，因此 `毒刃 II` 面具分支真实收口应为 `对手 HP 30->28 / delayed_poison=2 / pendingAttack=false`；旧 `30->24` 断言属于错误证据，不是实现 bug。
