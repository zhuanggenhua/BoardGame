...
| 双面状态 family | implemented / audit-only | `诅咒金币 / 火药桶 / 凋零 / 休战` 的 writer、consumer 与清理链都已落地；当前真实剩余不是规则没实施，而是 family 级合法复用登记与最终 completion verdict 仍未封版 |
| 双面奖励骰 family | implemented / audit-only | overlay 与各对象核心 downstream consumer 都已落地；当前真实剩余不是 overlay 没开，而是奖励骰对象的 downstream consumer seam 差异仍需按子 family 留档 |
...
| `npx vitest run src/games/dicethrone/__tests__/zhanshujia-cursed-pirate-mechanics.test.ts --configLoader native` | 1 file / 71 tests passed（2026-06-06） |
