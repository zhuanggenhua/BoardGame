...
> 2026-08-26 玩家板槽位覆盖说明：本文中前段真实入口截图文字仍保留了 2026-05/06 当时的旧槽位描述，例如把 `开拓战场` 写在 `lightning`、把 `战略转移` 写在 `calm`。这些槽位描述已被 2026-07-04 全量回图审计覆盖；当前战术家玩家板合同以 `src/games/dicethrone/rule/战术家录入核对.md`、`evidence/dicethrone/升级牌槽位全量回图审计-2026-07-04.md`、`abilitySlotMapping.ts` 和 `AbilityOverlays.test.tsx` 为准：`lightning -> strategic-shift`，`calm -> expand-battlefield`。

结论等级：旧结论失效（仅限本文中前段关于战术家 `开拓战场 / 战略转移` 的玩家板槽位描述）。

- 旧结论：本文中前段曾把 `开拓战场` 的真实入口写成 `lightning`，把 `战略转移` 的真实入口写成 `calm`。
- 失效原因：2026-07-04 回到正式玩家板图面与升级牌图面后，确认这两处物理槽位反向；旧截图文字只能证明当时运行时怎样解析，不能继续证明玩家板图面真相。
- 当前替代入口：当前槽位合同改由 `src/games/dicethrone/rule/战术家录入核对.md`、`evidence/dicethrone/升级牌槽位全量回图审计-2026-07-04.md`、`abilitySlotMapping.ts` 和 `AbilityOverlays.test.tsx` 承接。
- 降级后当前状态：本文仍保留战术家 / 咒缚海盗对象级历史审计轨迹；其中旧槽位描述已降级为历史错录证据，不再作为当前玩家板槽位合同。
- 同类扩审记录：2026-08-26 同轮横向搜索了 `pyro-blast / fiery-combo / wild-roar / nature-touch / strategic-shift / expand-battlefield` 等历史错位关键词，命中火法、树精、战术家三组旧槽位污染；当前运行时代码与 `AbilityOverlays.test.tsx` 均已锁到最新合同。
- 漏审归因：旧 evidence 把运行时截图解析当成玩家板图面真相，且旧测试断言过窄，没有把升级牌 `replaceAbility(targetAbilityId)`、升级牌中文标题、玩家板中文槽位标题三者强制对账。
