...
> 2026-06-06 当前有效口径：本文只证明 DiceThrone `BONUS_DIE_ROLLED` 特效描述这一条**专项范围**在当时已做过完整核对与修复，不代表 DiceThrone 全游戏、Treant / Ninja、或“四位新英雄总补审”已经完成。若要判断当前新英雄批次残余、对象级 `L3/L4` 或整批发布口径，应以 `evidence/dicethrone/dicethrone-new-factions-full-cycle-audit-2026-05-15.md`、`evidence/dicethrone/dicethrone-new-factions-reaudit-wiki-diff-2026-05-17.md` 及对应单英雄 / 升级重审文档为准。

## 审计结果

已完成对所有 `BONUS_DIE_ROLLED` 事件的特效描述专项审计，共检查 **16 个**投骰子事件。
...
所有投骰子的特效描述专项审计已完成。主要修复了：
1. ✅ 看箭：添加动态效果描述
2. ✅ 爆裂箭 I/II/III：添加完整计算公式

其他所有投骰子事件的特效描述都已正确实现，无需修改。

这份结论只覆盖“投骰子特效描述”这一专项，不外推技能本体实现、对象级 `L3/L4`、单英雄审计完成态或新英雄整批审计完成态。
...
- `evidence/dicethrone/watch-out-display-fix.md`（看箭修复详情）
