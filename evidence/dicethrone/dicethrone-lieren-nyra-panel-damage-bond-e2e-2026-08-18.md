...
验收状态：PASS。上一版“左侧玩家面板内部左上 / 左上正式响应面板”的 PASS 口径已废弃，不能作为本轮收口证据。
...
- 妮拉状态徽章改挂在 `player-board-surface` 内，锚点语义为“玩家板图片左上角空白带”；左侧 HUD、生命 / CP 行和状态图标区不再承载妮拉。
...
- `02-牌桌-妮拉在玩家板图片左上空白.jpg`：妮拉徽章在中间女猎手玩家板图片内部左上空白带；左侧 Buff / 状态图标、生命 / CP 条、牌堆和回合顺序保持原位置，没有被妮拉占位或挤压。
...
- `npm run spec:lint`
- `node scripts/verify/open-verified-image.mjs --pass-manifest evidence/dicethrone/nyra-player-board-badge-center-modal-pass-manifest-2026-08-18.json --viewer pureref --paths <00-04 标记图>`：一次性打开 5 张标记图；执行前 PureRef 进程为 `11892,52536`，执行后为 `11892,44916,52536`，本次新增进程 `44916`。
- 同一命令不加 `--force-reopen` 立即重跑：脚本拒绝重复打开，提示同一 PASS 清单和同一组媒体已打开过，未再次启动查看器。

## 图面裁决

verdict: PASS

- 妮拉位置：PASS，状态小徽章位于中间女猎手玩家板图片内部左上空白带；不是左侧 HUD，也没有压住 Buff / 状态图标或生命 / CP。
...
- 开图门禁：PASS 清单改用 `nyra-player-board-badge-center-modal-pass-manifest-2026-08-18.json`；旧 `nyra-damage-bond-pass-manifest-2026-08-18.json` 保持 `REVISE`，不得再用于开图。
- PureRef 打开：PASS，一次命令把序号索引和四张标记图传入 PureRef；后续同图组重复打开已被脚本拒绝。
