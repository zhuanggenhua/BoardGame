...
async function configureProbeContext(context: BrowserContext, storageKey: string) {
  await context.addInitScript(() => {
    ...
    (window as Window & { __SW_ATTACK_DEBUG__?: boolean }).__SW_ATTACK_DEBUG__ = true;
    try {
      window.localStorage.setItem('sw_attack_debug', '1');
    } catch {
      // localStorage can be unavailable in early init contexts.
    }
  });
...
async function captureAttackProbeDebug(page: Page, label: string) {
  const snapshot = await page.evaluate(() => {
    const w = window as Window & {
      __SW_ATTACK_DEBUG_LOGS__?: unknown;
      __BG_GET_STATE__?: () => unknown;
    };
    const state = typeof w.__BG_GET_STATE__ === 'function' ? w.__BG_GET_STATE__() as any : null;
    const eventStream = state?.G?.sys?.eventStream ?? state?.sys?.eventStream ?? null;
    ...
    return {
      ...
      hasDiceOverlay: Boolean(document.querySelector('[data-testid="sw-dice-result-overlay"]')),
      attackDebugLogs: w.__SW_ATTACK_DEBUG_LOGS__ ?? [],
      eventStream,
      visibleText,
      activeElementTestId: (document.activeElement as HTMLElement | null)?.getAttribute?.('data-testid') ?? null,
    };
  }).catch((error) => ({ evaluateError: error instanceof Error ? error.message : String(error) }));
  const outPath = path.join(evidenceDir, `debug-${label}.json`);
    ...
    try {
      ...
      windows.push(await runMeasuredWindow(hostPage, cdp, 'ranged-kill-full-fx', async () => {
        ...
        try {
          ...
          const debugPath = await captureAttackProbeDebug(hostPage, 'ranged-kill-overlay-missing');
          throw new Error(`远程击杀窗口没有出现骰子结果弹层；已写入诊断: ${debugPath}\n${error instanceof Error ? error.message : String(error)}`);
        }
