...
- “给用户看图”统一走全局 `D:\codex-home\skills\show-image-to-user\SKILL.md`；AI 看图验收按全局 `D:\codex-home\AGENTS.md` 的区分口径执行。本流程只额外要求：Dice Throne 卡牌录入需要 AI 验收时，必须核对正式 `ability-cards` 主真相源，写出图面可见文字、分支结构、数值和当前实现差异后，才允许把合同标记为 `passed/locked`。
...
- **同一批新英雄的继续补审视为持续授权**：如果用户已经围绕同一批 Dice Throne 新英雄要求“继续补审 / 继续把其他能力都审掉 / 修出漏项后继续扩审”，且没有明确缩范围，就默认继续按同批对象矩阵推进，不再重复追问“这张卡 / 这个 token / 这条兄弟链路还要不要审”。
...
- **底图/选图接入不等于双面实现完成**：即使 `human-player-board` / `player-board` 已进入运行时、`ASSETS.PLAYER_BOARD` 可按面取图、选角后能看到新底图，也只能证明资源链和首批图面合同接上；若尚未证明自动翻面链、逐槽对象合同、每张面逐对象 L3/L4 与双面重审计，就不得写“规则都实施了”或“该英雄已完成”。
