...
| 地图不能套两层，外面不应再有一圈 | 16:9 牌桌壳、4:3 地图视窗和圆角 / 渐变暗带叠加后，玩家会把桌面留白误认成第二张棋盘，地图边界和桌面边界职责混淆 | `game-ui-design` 地图素材保真与坐标父容器；`board-coordinate-contract.md` 完整 4x3 坐标合同；本轮真实入口截图复核 | 地图只保留一个正式 4:3 舞台和同一坐标父容器；外部仅允许无装饰的中性桌面底色；不得再叠加无职责圆角、阴影、左右暗带或上下渐变圈；验收同时看地图与舞台矩形、图片比例和四角边界 | covered-by-current-runtime-audit |
