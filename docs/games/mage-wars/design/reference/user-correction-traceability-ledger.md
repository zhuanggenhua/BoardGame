...
| 地图不能套两层，外面不应再有一圈 | 16:9 牌桌壳、4:3 地图视窗和圆角 / 渐变暗带叠加后，玩家会把桌面留白误认成第二张棋盘；只证明 transform 变化仍可能只是框内平移 | `ui-change-gates.md` 地图 / 棋盘视窗可自由查看、开放场景不套比例框；`board-coordinate-contract.md` 完整 4x3 坐标合同；本轮真实入口截图复核 | 地图视窗铺满真实牌桌地图层，正式 4x3 原图作为同一坐标父容器里的大场景；HUD 不得留在 16:9 安全区 / 安全壳内，己方法师状态、法术书牌列、计划确认区和已计划法术按真实视口九宫格锚点对齐；不得再叠加无职责圆角、阴影、左右暗带、上下渐变圈或外层黑带；验收同时看视窗贴边、场景两轴不小于视窗且至少一轴可移动、拖拽后覆盖视窗、对象热区贴竞技场，以及底部三块同一基线 | covered-by-current-runtime-audit |
