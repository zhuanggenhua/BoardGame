...
| 骰子、token 不能省略 | PASS。攻击骰、效果骰、燃烧 token、守卫 / 行动 token 同屏可见；伤害状态用受伤对象本体覆盖层 + 数字徽章承载。 |
| 开放式直选 / 场地本体高亮 | PASS。来源单位使用青蓝描边并抬起；对象 / 玩家目标只高亮本体，不再给所在格追加整格绿框；显性区域目标、移动格和召唤格才使用整格薄描边。交互语义落在竞技场区域 / 场上对象本体上。 |
