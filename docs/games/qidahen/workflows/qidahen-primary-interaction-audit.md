...
- 阻塞不再是“完全没有普通手牌图集或确认行”：2026-07-04 已确认 atlas05 是普通手牌图集，并把 49 张 `passed` 普通手牌接入正式发牌/预览流。但这只是局部接入，仍不是完整普通手牌全集真相源：
  ...
  - `src/games/qidahen/domain/commands.ts`
    - 已把规则摘要中明确写有“不能在攻城、守城时使用”、明确野战限定或明确守城专用的战术牌接入待结算战斗门禁；同时把“敌人增援时”“取消对手宣告的附兵劫掠”这类触发型战术，以及明确“野战步兵阶段使用”或“提前在炮兵阶段”的阶段型战术，从当前攻方普通 `PLAY_TACTIC_CARD` 窗口保守拦截。
...
- 因此当前正式局已修正“可证明非行动牌仍是 unknown 壳”的问题，并且 atlas05 已提供普通手牌真相源；49 张已确认普通手牌已经进入正式发牌/摸牌、预览流与运行态规则摘要字段。正式运行态已经具备一层真实手牌对象入口：
  - atlas05 已确认事件牌可从手牌本体进入 `执行事件` 记录层，并记录“打出哪张事件牌”和规则摘要；规则摘要写有“使用后移出游戏”的事件牌，打出牌本身不进入弃牌堆；持续事件牌会保留“不进入弃牌堆”的持续事件文案；其中 `七大恨` 已有一张真实事件效果，会在当前选中的后金本土建立 2 个 3 级后金步兵，并同步地区兵力与棋子集合；`成立汉八旗 / 成立满八旗 / 成立蒙八旗` 已有真实持续事件效果，会在后金控制的对应族属区域各建立 1 个 2 级后金步兵，并让对应族属区域之后视为后金本土；`蒙古大旱 / 东北大旱` 已能按指定族属区域写入旱灾事件标记，同步地图可见旱灾标记，并在年中土地税赋与新年补给/兵力耗损里把旱灾区域的有效人口视为 0；`翻山越岭` 已能生成一次突袭待结算，并把本次进攻里的长城、山脉边界临时按平原边界计算
  - atlas05 已确认军备牌可从手牌本体进入 `升级军备`，并记录“打出哪张军备牌”和目标军备升级结果；其中 `步兵铁甲` / `骑兵铁甲` 已有结构化战斗窄口，升级后会让对应步兵/骑兵防御等级进入战斗掷骰与损伤结算；`火炮技术` 已有炮兵战斗窄口，升级后会让每个炮兵额外掷 1 颗骰；`骑兵火器` 已有一层真实军备效果，升级后会让野战骑兵阶段己方每个骑兵部队额外掷 1 颗骰；`骏马育种` 已能让骑兵/调骑移动档位移动力 +1；`西式棱堡` 已有一层真实军备效果，升级后会让城战守方每个部队投 2 颗骰；`连环火铳` 已有一层真实军备效果，升级后会让野战步兵阶段己方每个步兵部队额外掷 1 颗骰；`红衣大炮` 的来源牌专属野战防守/攻城进攻加成仍未实现
  - atlas05 已确认银两牌可作为支付牌进入弃牌执行链
- 在这层手牌规则真相补齐前：
  - `执行事件` 只能判定为“正式打出记录层 + 使用后移出游戏/持续事件弃牌处理 + 七大恨建兵 + 八旗持续本土语义 + 旱灾标记状态/可见 token + 旱灾在土地税赋/补给耗损里的有效人口语义 + 翻山越岭一次性进攻边界语义已接入”，不能判定为事件效果全集完成
  - `升级军备` 已去掉抽象势力行动按钮，并补到已确认军备牌的局部真实入口；`步兵铁甲` / `骑兵铁甲` 已能让对应步兵/骑兵防御等级进入结构化战斗结算；`火炮技术` 已补到每炮兵 +1 骰的局部真实效果；`骑兵火器` 已补到野战骑兵阶段每骑兵 +1 骰的局部真实效果；`骏马育种` 已补到骑兵/调骑移动档位移动力 +1 的局部真实效果；`西式棱堡` 已补到城战守方每部队 2 骰的局部真实效果；`连环火铳` 已补到野战步兵阶段每步兵 +1 骰的局部真实效果；`红衣大炮` 仍未补来源牌专属加成
  - `战术牌` 只补到“规则摘要明确禁止攻城/守城、明确野战限定、明确守城专用、明确依赖增援/附兵劫掠触发，或明确要求具体兵种阶段/炮兵阶段的牌会被当前攻方战术窗口门禁拦截；打出后的战报与行动记录会保留具体规则摘要；`鸟真超哈` 会让本次野战攻方步兵掷骰等级 +1；`骑兵冲锋` 会让本次野战攻方每个骑兵部队额外掷 2 颗骰；`战车阵` 会让本次战斗攻方步兵防御等级 +1；`机里耐步兵` 会在进攻明军的野战中让攻方每个步兵部队额外掷 1 颗骰；`巴雅喇` 会让本次进攻守方所有部队防御等级 -1”这一层，仍不是完整战术时机全集或逐张战术效果
  - 任何“正式局已经完成事件效果全集、战术时机全集或银两资源链”的口径都不成立
...
| 升级军备 | 先决定打出哪张军备牌，再弃 1 张手牌 | atlas05 已确认军备牌会由手牌本体直入预览；无已确认军备牌时不再提供右侧抽象入口 | 再弃 1 张手牌、再落到军备升级结果 | 已确认军备手牌本体 | 顶部行动横幅、右侧步骤卡 | **局部闭环**：12 张已确认普通军备牌已接上真实承接物并记录升级结果；`步兵铁甲` / `骑兵铁甲` 已接入对应兵种防御等级结算，`火炮技术` 已接入每炮兵 +1 骰，`骑兵火器` 已接入野战骑兵阶段每骑兵 +1 骰，`骏马育种` 已接入骑兵/调骑移动档位移动力 +1，`西式棱堡` 已接入城战守方每部队 2 骰，`连环火铳` 已接入野战步兵阶段每步兵 +1 骰，但不代表事件、战术、银两或军备效果全集完成；`红衣大炮` 来源牌专属加成仍未实现 |
...
- atlas05 的 49 张普通事件、军备、战术和银两牌已具备规则级真相源；其中事件牌能稳定承接“打出哪张事件牌”的记录合同，军备牌能稳定承接“打出哪张军备牌”的升级合同；`步兵铁甲` / `骑兵铁甲` 已推进到对应兵种防御等级结算的局部真实军备效果，`火炮技术` 已推进到每炮兵 +1 骰的局部真实军备效果，`骑兵火器` 已推进到野战骑兵阶段每骑兵 +1 骰的局部真实军备效果，`骏马育种` 已推进到骑兵/调骑移动档位移动力 +1 的局部真实军备效果，`西式棱堡` 已推进到城战守方每部队 2 骰的局部真实军备效果，`连环火铳` 已推进到野战步兵阶段每步兵 +1 骰的局部真实军备效果；`红衣大炮` 来源牌专属加成仍未实现
...
5. atlas05 已确认通过项覆盖事件、战术、军备、银两四类普通手牌，并已完成 `cardKind / cardDefId / armamentId` 局部运行时接入和定向测试；12 张普通军备牌已能从手牌本体直入“升级军备”，并在执行摘要里写明打出的军备牌名和目标军备升级结果；`步兵铁甲` / `骑兵铁甲`、`火炮技术`、`骑兵火器`、`骏马育种`、`西式棱堡` 与 `连环火铳` 已各自补到一层真实战斗或移动窄口效果。事件效果全集、战术时机与银两资源链仍未闭环，因此“真实手牌入口全集”“正式可直接打出所有事件牌”的完成口径仍不成立。
...
| 手牌行动：执行事件 | 规则书允许时应先选要打出的事件牌 | atlas05 已确认事件手牌会直接暴露单牌入口 | 已确认事件手牌本体 | 当前先进入正式打出记录层；“使用后移出游戏”的事件牌不进入弃牌堆；`七大恨`、八旗持续事件、旱灾标记与 `翻山越岭` 一次性进攻边界已有真实状态结算；完整事件效果自身仍待逐张实现 | **记录层已接入但未闭环**：已确认事件牌能记录具体牌名和规则摘要，并已接入使用后移出游戏/持续事件弃牌处理、`七大恨` 建兵、八旗本土语义、旱灾标记状态/地图 token、旱灾在年中土地税赋与新年补给/兵力耗损里的有效人口语义，以及 `翻山越岭` 对本次进攻长城/山脉边界的平原化处理；但没有把每张事件牌的实际效果、额外成本、目标选择、其他人口读取入口等规则全集实现完 |
...
- `封贡敕书` 在后金/蒙古普通支付里已按“视作 2 张银两”计入支付值和执行记录；大明支付同牌仍只算 1 张普通手牌
- 已确认事件牌已能从手牌本体进入 `执行事件` 打出记录层，行动记录会保留具体牌名和规则摘要；`七大恨`、八旗持续事件、旱灾标记和 `翻山越岭` 已进入真实状态结算层；旱灾已接入年中土地税赋与新年补给/兵力耗损的有效人口语义；尚未建立事件效果全集执行链

却没有：

- 逐张事件牌的完整事件效果、额外成本、持续事件区/移出游戏等结算链；旱灾除年中土地税赋与新年补给/兵力耗损外的其他人口读取入口仍待逐项核对
- 银两在所有资源弃牌、特殊事件替代支付与跨势力弃牌场景中的完整资源链闭环；`封贡敕书` 的事件效果、跨势力弃牌和进入对手弃牌堆也仍未实现

同时当前手牌对象仍有未闭环部分：

- atlas 中可审计的非行动牌已经能记录 `cardKind / cardDefId`
- atlas05 的 49 张普通手牌已能记录稳定卡名、卡类、规则摘要与军备目标
- 12 张普通军备牌已能承担“打出军备牌并升级对应军备”的运行时动作；`步兵铁甲` / `骑兵铁甲` 已进一步接入对应兵种防御等级结算；`火炮技术` 已进一步接入每炮兵 +1 骰的局部真实效果；`骑兵火器` 已进一步接入野战骑兵阶段每骑兵 +1 骰的局部真实效果；`骏马育种` 已进一步接入骑兵/调骑移动档位移动力 +1 的局部真实效果；`西式棱堡` 已进一步接入城战守方每部队 2 骰的局部真实效果；`连环火铳` 已进一步接入野战步兵阶段每步兵 +1 骰的局部真实效果
- 2 张银两牌已能作为支付牌被消费并在执行记录里保留资源牌身份
- `封贡敕书` 已补普通支付价值：后金/蒙古支付时按 2 点计入，其他势力仍按 1 点
- 当前攻方战术窗口已按现有战斗上下文拦截野战限定、攻守城禁用、守城专用、增援/附兵劫掠触发型和明确阶段型战术牌；可打出的战术牌会在战报与行动记录里保留具体规则摘要，且 `鸟真超哈` 已接入本次野战攻方步兵掷骰等级 +1，`骑兵冲锋` 已接入本次野战攻方每个骑兵部队额外掷 2 颗骰，`战车阵` 已接入本次战斗攻方步兵防御等级 +1，`机里耐步兵` 已接入进攻明军野战时攻方每个步兵部队额外掷 1 颗骰，`巴雅喇` 已接入本次进攻守方所有部队防御等级 -1
...
1. 规则书同层一级入口里的 `执行事件 / 升级军备 / 势力行动`，当前正式局只稳定暴露了：
   - 势力行动按钮列
   - atlas05 已确认事件牌直入 `执行事件` 记录层，执行记录会保留具体牌名和规则摘要；“使用后移出游戏”的事件牌不会进入弃牌堆；持续事件牌会按持续事件保留不进弃牌堆；`七大恨` 已能真实建立后金本土 2 个 3 级步兵；八旗持续事件已能建立对应族属的 2 级后金步兵并扩展后金本土判定；完整事件效果仍未实现
   - 已识别军备牌直入预览与执行；未识别军备牌不再退回抽象 `upgrade-armament`
   - 已识别银两牌作为支付牌消费时会在执行记录里保留资源牌身份
   - `封贡敕书` 在后金/蒙古普通支付中会按 2 点计入支付值，但仍没有实现事件效果和跨势力弃牌链
2. 正式局仍缺：
   - 除 `七大恨` 建兵与八旗持续本土语义外，逐张事件牌的完整事件效果、额外成本、目标选择等结算链
...
3. 这不是按钮文案问题，而是正式手牌对象只补到了 atlas05 已确认普通手牌的局部身份：
   ...
   - atlas05 的已确认事件牌已能在 `执行事件` 记录层保留具体牌名与规则摘要；“使用后移出游戏”的事件牌已经不再增加弃牌堆；持续事件牌会按持续事件不进弃牌堆；`七大恨` 已能建立后金本土 2 个 3 级步兵；八旗持续事件已能建立对应族属的 2 级后金步兵并扩展后金本土判定；这不是完整事件效果闭环
   - atlas05 的 12 张普通军备牌已能在升级军备摘要里保留具体牌名与升级目标；`步兵铁甲` / `骑兵铁甲` 已能让对应步兵/骑兵防御等级进入结构化战斗结算；`火炮技术` 已能让每个炮兵额外掷 1 颗骰；`骑兵火器` 已能在野战骑兵阶段给己方每个骑兵部队额外掷 1 颗骰；`骏马育种` 已能让骑兵/调骑移动档位移动力 +1；`西式棱堡` 已能在城战中给守方每个部队投 2 颗骰；`连环火铳` 已能在野战步兵阶段给己方每个步兵部队额外掷 1 颗骰；`红衣大炮` 来源牌专属加成仍未实现
   - atlas05 的 2 张银两牌已能作为支付牌被消费，并在行动记录里保留“银两资源牌”身份
   - `封贡敕书` 已在后金/蒙古普通行动支付中按“视作 2 张银两”计入支付值；这不是完整事件效果或银两资源链闭环
   - 规则摘要明确“不能在攻城、守城时使用”、明确野战限定、明确守城专用、明确依赖增援/附兵劫掠触发，或明确要求“野战步兵阶段使用/提前在炮兵阶段”的战术牌已能在当前攻方战术窗口被门禁拦截；守城专用牌不会再被攻方野战或城战窗口误放行，触发型和阶段型战术也不会再被普通待结算战斗窗口泛用；可打出的战术牌会在战报与行动记录里保留具体规则摘要；`鸟真超哈` 已能在本次野战中给攻方步兵掷骰等级 +1；`骑兵冲锋` 已能在本次野战中给攻方每个骑兵部队额外掷 2 颗骰；`战车阵` 已能在本次战斗中给攻方步兵防御等级 +1；`机里耐步兵` 已能在进攻明军的野战中给攻方每个步兵部队额外掷 1 颗骰；`巴雅喇` 已能在本次进攻中给守方所有部队防御等级 -1
