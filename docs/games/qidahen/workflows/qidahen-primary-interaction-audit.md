...
- 因此当前正式局已修正“可证明非行动牌仍是 unknown 壳”的问题，并且 atlas05 已提供普通手牌真相源；49 张已确认普通手牌已经进入正式发牌/摸牌、预览流与运行态规则摘要字段。正式运行态已经具备一层真实手牌对象入口：
  ...
  - atlas05 已确认军备牌可从手牌本体进入 `升级军备`，并记录“打出哪张军备牌”和目标军备升级结果；其中 `骑兵火器` 已有一层真实军备效果，升级后会让野战骑兵阶段己方每个骑兵部队额外掷 1 颗骰；`西式棱堡` 已有一层真实军备效果，升级后会让城战守方每个部队投 2 颗骰；`连环火铳` 已有一层真实军备效果，升级后会让野战步兵阶段己方每个步兵部队额外掷 1 颗骰
...
- 在这层手牌规则真相补齐前：
  ...
  - `升级军备` 已去掉抽象势力行动按钮，并补到已确认军备牌的局部真实入口；`骑兵火器` 已补到野战骑兵阶段每骑兵 +1 骰的局部真实效果；`西式棱堡` 已补到城战守方每部队 2 骰的局部真实效果；`连环火铳` 已补到野战步兵阶段每步兵 +1 骰的局部真实效果
...
| 升级军备 | atlas05 已确认军备牌由手牌本体进入正式动作预览；无已确认军备牌时不再提供右侧抽象 `upgrade-armament` 按钮 | 已确认军备手牌本体 | 局部真实可用 | 当前已避免“同一动作同时出现手牌直点入口和右侧重复主入口”；12 张普通军备牌已能记录具体牌名和目标军备升级结果，`骑兵火器` 已能在野战骑兵阶段让己方每个骑兵部队额外掷 1 颗骰，`西式棱堡` 已能在城战中让守方每个部队投 2 颗骰，`连环火铳` 已能在野战步兵阶段让己方每个步兵部队额外掷 1 颗骰，但不代表事件、战术、银两或军备效果全集完成 |
...
| 升级军备 | 先决定打出哪张军备牌，再弃 1 张手牌 | atlas05 已确认军备牌会由手牌本体直入预览；无已确认军备牌时不再提供右侧抽象入口 | 再弃 1 张手牌、再落到军备升级结果 | 已确认军备手牌本体 | 顶部行动横幅、右侧步骤卡 | **局部闭环**：12 张已确认普通军备牌已接上真实承接物并记录升级结果；`骑兵火器` 已接入野战骑兵阶段每骑兵 +1 骰，`西式棱堡` 已接入城战守方每部队 2 骰，`连环火铳` 已接入野战步兵阶段每步兵 +1 骰，但不代表事件、战术、银两或军备效果全集完成 |
...
- atlas05 的 49 张普通事件、军备、战术和银两牌已具备规则级真相源；其中事件牌能稳定承接“打出哪张事件牌”的记录合同，军备牌能稳定承接“打出哪张军备牌”的升级合同；`骑兵火器` 已推进到野战骑兵阶段每骑兵 +1 骰的局部真实军备效果，`西式棱堡` 已推进到城战守方每部队 2 骰的局部真实军备效果，`连环火铳` 已推进到野战步兵阶段每步兵 +1 骰的局部真实军备效果
...
5. atlas05 已确认通过项覆盖事件、战术、军备、银两四类普通手牌，并已完成 `cardKind / cardDefId / armamentId` 局部运行时接入和定向测试；12 张普通军备牌已能从手牌本体直入“升级军备”，并在执行摘要里写明打出的军备牌名和目标军备升级结果；`骑兵火器`、`西式棱堡` 与 `连环火铳` 已各自补到一层真实战斗掷骰效果。事件效果全集、战术时机与银两资源链仍未闭环，因此“真实手牌入口全集”“正式可直接打出所有事件牌”的完成口径仍不成立。
...
- 12 张普通军备牌已能承担“打出军备牌并升级对应军备”的运行时动作；`骑兵火器` 已进一步接入野战骑兵阶段每骑兵 +1 骰的局部真实效果；`西式棱堡` 已进一步接入城战守方每部队 2 骰的局部真实效果；`连环火铳` 已进一步接入野战步兵阶段每步兵 +1 骰的局部真实效果
...
3. 这不是按钮文案问题，而是正式手牌对象只补到了 atlas05 已确认普通手牌的局部身份：
   ...
   - atlas05 的 12 张普通军备牌已能在升级军备摘要里保留具体牌名与升级目标；`骑兵火器` 已能在野战骑兵阶段给己方每个骑兵部队额外掷 1 颗骰；`骏马育种` 已能让骑兵/调骑移动档位移动力 +1；`西式棱堡` 已能在城战中给守方每个部队投 2 颗骰；`连环火铳` 已能在野战步兵阶段给己方每个步兵部队额外掷 1 颗骰
