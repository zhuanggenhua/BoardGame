...
- 因此当前正式局已修正“可证明非行动牌仍是 unknown 壳”的问题，并且 atlas05 已提供普通手牌真相源；49 张已确认普通手牌已经进入正式发牌/摸牌、预览流与运行态规则摘要字段。正式运行态已经具备一层真实手牌对象入口：
  - atlas05 已确认事件牌可从手牌本体进入 `执行事件` 记录层，并记录“打出哪张事件牌”和规则摘要
  - atlas05 已确认军备牌可从手牌本体进入 `升级军备`，并记录“打出哪张军备牌”和目标军备升级结果
  - atlas05 已确认银两牌可作为支付牌进入弃牌执行链
- 在这层手牌规则真相补齐前：
  - `执行事件` 只能判定为“正式打出记录层已接入”，不能判定为事件效果全集完成
  - `升级军备` 已去掉抽象势力行动按钮，并补到已确认军备牌的局部真实入口
  - `战术牌` 只补到“规则摘要明确禁止攻城/守城、明确野战限定、明确守城专用、明确依赖增援/附兵劫掠触发，或明确要求具体兵种阶段/炮兵阶段的牌会被当前攻方战术窗口门禁拦截”这一层，仍不是完整战术时机全集
...
| 执行事件 | atlas05 已确认事件牌可由手牌本体进入正式打出记录层 | 已确认事件手牌本体 | 局部真实可用 | 记录层已能稳定承接“打出哪张事件牌”并写入牌名/规则摘要；完整事件效果仍待逐张实现 |
| 升级军备 | atlas05 已确认军备牌由手牌本体进入正式动作预览；无已确认军备牌时不再提供右侧抽象 `upgrade-armament` 按钮 | 已确认军备手牌本体 | 局部真实可用 | 当前已避免“同一动作同时出现手牌直点入口和右侧重复主入口”；12 张普通军备牌已能记录具体牌名和目标军备升级结果，但不代表事件、战术、银两全集完成 |
...
| 执行事件 | 先决定打出哪张事件牌 | atlas05 已确认事件牌可由手牌本体直入打出记录层 | 当前记录牌名和规则摘要；完整牌面效果自身仍待逐张实现 | 已确认事件手牌本体 | 顶部行动横幅、右侧步骤卡 | **记录层已接入但未闭环**：规则层这一入口已能承接“打出哪张事件牌”，但逐张事件效果、额外成本、持续区/移出游戏等结算链仍缺 |
| 升级军备 | 先决定打出哪张军备牌，再弃 1 张手牌 | atlas05 已确认军备牌会由手牌本体直入预览；无已确认军备牌时不再提供右侧抽象入口 | 再弃 1 张手牌、再落到军备升级结果 | 已确认军备手牌本体 | 顶部行动横幅、右侧步骤卡 | **局部闭环**：12 张已确认普通军备牌已接上真实承接物并记录升级结果，但不代表事件、战术、银两全集完成 |
...
- atlas05 的 49 张普通事件、军备、战术和银两牌已具备规则级真相源；其中事件牌能稳定承接“打出哪张事件牌”的记录合同，军备牌能稳定承接“打出哪张军备牌”的升级合同
...
| 手牌行动：执行事件 | 规则书允许时应先选要打出的事件牌 | atlas05 已确认事件手牌会直接暴露单牌入口 | 已确认事件手牌本体 | 当前先进入正式打出记录层；完整事件效果自身仍待逐张实现 | **记录层已接入但未闭环**：已确认事件牌能记录具体牌名和规则摘要，但没有把每张事件牌的实际效果、额外成本、持续区/移除等规则全集实现完 |
...
- 已确认事件牌已能从手牌本体进入 `执行事件` 打出记录层，行动记录会保留具体牌名和规则摘要；尚未建立事件效果全集执行链

却没有：

- 逐张事件牌的完整事件效果、额外成本、持续事件区/移出游戏等结算链
...
- 当前攻方战术窗口已按现有战斗上下文拦截野战限定、攻守城禁用、守城专用、增援/附兵劫掠触发型和明确阶段型战术牌
...
1. 规则书同层一级入口里的 `执行事件 / 升级军备 / 势力行动`，当前正式局只稳定暴露了：
   - 势力行动按钮列
   - atlas05 已确认事件牌直入 `执行事件` 记录层，执行记录会保留具体牌名和规则摘要；完整事件效果仍未实现
...
3. 这不是按钮文案问题，而是正式手牌对象只补到了 atlas05 已确认普通手牌的局部身份：
   ...
   - atlas05 的已确认事件牌已能在 `执行事件` 记录层保留具体牌名与规则摘要；这不是完整事件效果闭环
   ...
   - 规则摘要明确“不能在攻城、守城时使用”、明确野战限定、明确守城专用、明确依赖增援/附兵劫掠触发，或明确要求“野战步兵阶段使用/提前在炮兵阶段”的战术牌已能在当前攻方战术窗口被门禁拦截；守城专用牌不会再被攻方野战或城战窗口误放行，触发型和阶段型战术也不会再被普通待结算战斗窗口泛用
