...
- 但当前正式 UI **没有完整暴露规则书定义的手牌行动全集**：
  ...
  - 手牌区已经能让 atlas05 已确认军备牌/事件牌直接进入正式动作预览或记录层，但事件效果全集、战术时机和银两资源链仍未闭环，不能把这层局部接入说成规则书级手牌行动全集完成
...
- 阻塞不再是“完全没有普通手牌图集或确认行”：2026-07-04 已确认 atlas05 是普通手牌图集，并把 49 张 `passed` 普通手牌接入正式发牌/预览流。但这只是局部接入，仍不是完整普通手牌全集真相源：
  ...
  - `src/games/qidahen/domain/commands.ts`
    - 已把规则摘要中明确写有“不能在攻城、守城时使用”、明确野战限定或明确守城专用的战术牌接入待结算战斗门禁；同时把“敌人增援时”“取消对手宣告的附兵劫掠”这类触发型战术，以及需要附兵模型/未参战部队增援承接链/敌方次级部队参战触发的战术，从当前攻方普通 `PLAY_TACTIC_CARD` 窗口保守拦截；`箭如雨下` 已单独开放当前攻方野战待结算战斗窄口，`乌真超哈` 特殊牌已单独开放当前攻方野战待结算窄口，`拒马` 已单独开放守方野战待结算战斗窄口。
...
- 因此当前正式局已修正“可证明非行动牌仍是 unknown 壳”的问题，并且 atlas05 已提供普通手牌真相源；49 张已确认普通手牌已经进入正式发牌/摸牌、预览流与运行态规则摘要字段。正式运行态已经具备一层真实手牌对象入口：
  - atlas05 已确认事件牌可从手牌本体进入 `执行事件` 记录层，并记录“打出哪张事件牌”和规则摘要；规则摘要写有“使用后移出游戏”的事件牌，打出牌本身不进入弃牌堆；持续事件牌会写入运行态在场承载并保留“不进入弃牌堆”的持续事件文案；其中 `七大恨` 已有一张真实事件效果，会在当前选中的后金本土建立 2 个 3 级后金步兵，并同步地区兵力与棋子集合；大明/蒙古打出 `七大恨` 时按牌面无效果并移出游戏，不建立部队也不进入弃牌堆；`成立汉八旗 / 成立满八旗 / 成立蒙八旗` 已有真实持续事件效果，会在后金控制的对应族属区域各建立 1 个 2 级后金步兵，并让对应族属区域之后视为后金本土；`蒙古大旱 / 东北大旱` 已能按指定族属区域写入旱灾事件标记，同步地图可见旱灾标记，并在年中土地税赋与新年补给/兵力耗损里把旱灾区域的有效人口视为 0；其中 `东北大旱` 已补女真人区域正向正式打出回归，确认可在建州写入旱灾标记；第二张 `蒙古大旱`（qidahen-atlas05-1637-mongol-drought-alt）也已补同源正向回归，确认可在察哈尔写入旱灾标记；`翻山越岭` 已能生成一次突袭待结算，并把本次进攻里的长城、山脉边界临时按平原边界计算
...
- 在这层手牌规则真相补齐前：
  - `执行事件` 只能判定为“正式打出记录层 + 使用后移出游戏/持续事件弃牌与在场承载处理 + 七大恨后金建兵/大明蒙古无效果移出游戏 + 八旗持续本土语义 + 旱灾标记状态/可见 token + 旱灾在土地税赋/补给耗损里的有效人口语义 + 翻山越岭一次性进攻边界语义 + 王公大会后金/大明无效果与蒙古人物打出/回收二择一窄口已接入 + 人参貂皮非后金无效果与后金目标对手给牌窄口已接入 + 封贡敕书额外弃 1 张手牌、目标对手、对手赐印招安/驱虎吞狼二选一、事件牌进入该对手弃牌堆，以及目标为大明时承接既有正式行动链窄口已接入 + 反间计 2 费人物目标选择与目标移出游戏窄口已接入 + 开门迎降敌方人物判定窄口已接入 + 需要具体地图目标、未实现人物牌堆、特定打出时机或地图目标承接链的事件会被拦截并保留手牌”，不能判定为事件效果全集完成
  ...
  - `战术牌` 只补到“规则摘要明确禁止攻城/守城、明确野战限定、明确守城专用、明确依赖增援/附兵劫掠触发，明确要求未开放兵种/炮兵阶段，或明确要求附兵模型/未参战部队增援承接链的牌会被当前攻方战术窗口门禁拦截；打出后的战报与行动记录会保留具体规则摘要；`鸟真超哈` 会让本次野战攻方步兵掷骰等级 +1；`骑兵冲锋` 会让本次野战攻方每个骑兵部队额外掷 2 颗骰；`战车阵` 会让本次战斗攻方步兵防御等级 +1；`机里耐步兵` 会在进攻明军的野战中让攻方每个步兵部队额外掷 1 颗骰，且大明作为野战防守方正式打出后会让防守明军步兵先结算并压制攻方步兵反击；`巴雅喇` 会让本次进攻守方所有部队防御等级 -1，且已开放守方正式打出窄口，让防守方步兵防御等级 +1；`步骑联合` 会在本次野战攻方步兵和骑兵同时参战时让两类部队掷骰等级 +1，并在战斗层取消对手 `连环火铳` 先结算修正；`链炮阵` 会在本次野战攻方承受损伤时让炮兵单位先承受；`策反/内应` 和普通 `策反` 会在目标战斗有敌方炮兵时让 1 个敌方炮兵临时转为攻方参战并进入攻方炮兵掷骰侧，且战报摘要保留实际打出的牌名；`箭如雨下` 已开放当前攻方野战战术窗口窄口，打出后写入攻方步兵先结算修正，并带有取消对手 `拒马` 优先修正的元数据；`拒马` 已开放守方野战待结算窄口，守方正式打出后会写入取消对手 `箭如雨下`、`连环火铳`、`骑兵火器` 先结算修正和对手骑兵按步兵阶段掷骰修正的元数据；`坚守不屈` 已开放守方城战待结算窄口，守方正式打出后会让攻城攻击方骰值先加减再除以 2 向下取整；`打草惊蛇` 已开放当前攻方合法骑兵劫掠待结算窄口，正式打出后会写入骑兵劫掠免伤修正，让劫掠骑兵不承受守方炮骑反击伤害；`机里耐步兵` 的防守明军半段已从战斗掷骰层底座推进到守方正式打出窄口，可让防守明军步兵先掷并压制攻方步兵反击；`巴雅喇` 的防守甲兵半段已从战斗掷骰层底座推进到守方正式打出窄口，可让防守方步兵按更高等级掷骰；`乌真超哈` 特殊牌的战斗掷骰层底座可在已有提前炮兵阶段攻击修正时让 1 个攻方步兵提前在炮兵阶段攻击且不重复攻击；`偷袭与伏击` 的战斗掷骰层底座可在已有指定兵种骰子等级 -1 修正时让对应兵种按降低后的等级掷骰”这一层，仍不是完整战术时机全集、次级部队版策反或逐张战术效果
  - 剩余未闭合普通手牌已有明确阻塞边界：`反间计` 已拆出并接入 2 费人物目标选择、排除努尔哈赤/林丹汗/阿巴凯、合法目标移出游戏和无目标不消耗手牌的窄口，但这不代表人物牌堆或事件效果全集完成；`开门迎降` 已拆出第一项敌方人物判定窄口，可指定敌方在场且已有判定表的人物并按年中人物额外判定同源规则结算；第二项文本/语义仍未锁定，人物牌堆与完整二择一选择链仍未闭环；`王公大会` 已关闭后金/大明无效果窄口和蒙古人物打出/回收二择一窄口；`人参貂皮` 已关闭非后金无效果与后金目标对手给牌窄口，但这不代表其它跨势力事件效果完成；`封贡敕书` 已关闭额外弃 1 张手牌、目标对手、对手赐印招安/驱虎吞狼二选一、事件牌进入该对手弃牌堆，以及目标为大明时承接赐印招安/驱虎吞狼既有正式链的窄口；目标为蒙古时已有保护性回归，确认仍停在通用未承接摘要，不误套大明正式链；蒙古目标正式行动语义、完整跨势力覆盖和事件效果全集仍未闭环；`各个击破` 已有普通执行事件入口防误判拦截，并已补具名回归确认不会被通用入口消耗或记为已结算，但真实效果仍缺攻击响应时机和多边界撤离排序；`东北大军` 已拆出当前选中女真人真实区域的结构化甲喇标记窄口，并已补回归证明可先用真实地图选择命令切换到女真人区域再打出东北大军，同时已补上轮盘行动标记在新年位置时不能使用、保留手牌且不放置甲喇标记的窄口，但仍缺甲喇专属素材/命名；`骑马步兵`、`分进合击` 和次级部队版 `策反` 已有普通攻方战术窗口防误判拦截，但真实效果仍缺附兵部队模型、敌方次级部队参战触发、目标选择、战后回归、未参战部队再移动、距离校验和增援进战斗入口；`诈败诱敌` 已补具名回归确认普通攻方战术窗口会拦截且不会消耗手牌或写入战术修正，但仍缺对手附兵劫掠宣告、附兵直接投入战斗和照常撤退链。这些不得通过现有战斗修正字段、骑兵劫掠结算层或执行事件记录层硬接。
  - 任何“正式局已经完成事件效果全集、战术时机全集或银两资源链”的口径都不成立；当前银两证据只覆盖普通支付价值与自动支付选择，封贡敕书事件只关闭额外弃 1 张手牌、目标对手、对手二选一、事件牌进入对手弃牌堆和目标大明时承接既有正式链的窄口，不覆盖蒙古目标、完整跨势力覆盖或事件效果全集
...
| 执行事件 | atlas05 已确认事件牌可由手牌本体进入正式打出记录层 | 已确认事件手牌本体 | 局部真实可用 | 记录层已能稳定承接“打出哪张事件牌”并写入牌名/规则摘要；规则摘要写有“使用后移出游戏”的事件牌不再进入弃牌堆；`七大恨` 已真实结算为后金本土建立 2 个 3 级步兵；八旗持续事件已能建立对应族属的 2 级后金步兵并扩展后金本土判定；旱灾事件已能写入区域事件标记、同步地图可见标记，并让年中土地税赋与新年补给/兵力耗损按有效人口 0 结算；`翻山越岭` 已能生成一次突袭待结算，并让该次进攻把长城/山脉边界按平原边界计算；劫掠仍保留真实人口，其他人口读取入口仍待逐项核对；完整事件效果仍待逐张实现 |
...
- 素材状态：`docs/games/qidahen/workflows/qidahen-hand-card-atlas05-manual-entry-matrix.md` 已新增 atlas05 普通手牌人工录入矩阵；`test-results/evidence-image-validation/qidahen-formal-handcard-2.4.json` 当前记录 49 张 passed 普通手牌、0 张 partial、0 张 blocked，`npm run verify:qidahen:handcards` 已通过。旧的 49 张 OCR 候选、TTS CardID 完整矩阵和运行时 faction preview 候选仍保留为排除证据；它们不能覆盖 atlas05 这一新发现，也不能再支持“本地没有任何普通手牌确认行”的结论。当前已完成 49 张确认行的正式运行时接入和定向测试，12 张普通军备牌也已补到“打出哪张军备牌 + 升级哪项军备”的摘要合同，`步兵铁甲` / `骑兵铁甲`、`火炮技术`、`红衣大炮`、`骑兵火器`、`精钢马刀`、`骏马育种`、`西式棱堡` 与 `连环火铳` 已各自补到一层真实战斗或移动窄口效果，其中 `骑兵火器` 额外补到来源牌专属骑兵先结算底座并已验证可被 `拒马` 在战斗层取消，`连环火铳` 额外补到来源牌专属步兵先结算底座，`精钢马刀` 明确不继承 `骑兵火器` 的先结算语义；事件牌里 `东北大军` 已补到当前选中女真人区域的结构化甲喇标记窄口、真实地图选择命令切换目标区域后再打出的回归窄口和新年格禁用窄口，但甲喇专属素材/命名仍未完成；后续仍需补齐事件效果全集、战术时机与银两资源链后，才能关闭 2.4 / 4.5。
...
- 已确认事件牌已能从手牌本体进入 `执行事件` 打出记录层，行动记录会保留具体牌名和规则摘要；持续事件牌会写入在场承载；`七大恨`、八旗持续事件、旱灾标记和 `翻山越岭` 已进入真实状态结算层；旱灾已接入年中土地税赋与新年补给/兵力耗损的有效人口语义；`王公大会` 已关闭后金/大明使用无效果与蒙古人物打出/回收二择一窄口；`人参貂皮` 已关闭非后金无效果与后金目标对手给牌窄口；`封贡敕书` 已关闭额外弃 1 张手牌、目标对手、对手赐印招安/驱虎吞狼二选一、事件牌进入该对手弃牌堆和目标为大明时承接既有正式行动链窄口；`反间计` 已接入 2 费人物目标选择、合法目标移出游戏和无合法目标不消耗手牌窄口；`开门迎降` 已接入第一项敌方人物判定窄口；需要具体地图目标、未实现人物牌堆、特定打出时机或地图目标选择的事件牌会被拦截并提示缺少承接链，不再被误写为当前势力弃牌堆或已移出游戏；尚未建立事件效果全集执行链

却没有：

- 逐张事件牌的完整事件效果、额外成本、持续事件转移/离场、使用后移出游戏后续归属等结算链；旱灾除年中土地税赋与新年补给/兵力耗损外的其他人口读取入口仍待逐项核对
- 银两在所有资源弃牌、特殊事件替代支付与跨势力弃牌场景中的完整资源链闭环；`封贡敕书` 已关闭额外弃 1 张手牌、目标对手、对手二选一、事件牌进入对手弃牌堆和目标大明时承接既有正式链窄口，但蒙古目标、完整跨势力覆盖和事件效果全集仍未实现
...
- 当前攻方战术窗口已按现有战斗上下文拦截野战限定、攻守城禁用、守城专用、增援/附兵劫掠触发型和未开放阶段型战术牌，并已单独开放 `箭如雨下` 的当前攻方野战窄口；守方待结算窄口已单独开放城战 `坚守不屈`、野战 `拒马`、`巴雅喇` 自身防守半段和大明防守野战 `机里耐步兵`。可打出的战术牌会在战报与行动记录里保留具体规则摘要，且 `鸟真超哈` 已接入本次野战攻方步兵掷骰等级 +1，`骑兵冲锋` 已接入本次野战攻方每个骑兵部队额外掷 2 颗骰，`战车阵` 已接入本次战斗攻方步兵防御等级 +1，`机里耐步兵` 已接入进攻明军野战时攻方每个步兵部队额外掷 1 颗骰，`巴雅喇` 已接入本次进攻守方所有部队防御等级 -1，并已开放守方正式打出窄口让防守方步兵防御等级 +1，`步骑联合` 已接入本次野战攻方步兵和骑兵同时参战时两类部队掷骰等级 +1，`链炮阵` 已接入本次野战攻方承受损伤时炮兵单位先承受，`策反/内应` 和普通 `策反` 已接入目标战斗有敌方炮兵时 1 个敌方炮兵临时转攻方参战的掷骰侧窄口，且普通 `策反` 战报摘要不再被写成 `策反/内应`；`箭如雨下` 已从结算层底座推进到当前攻方野战战术窗口窄口；`拒马` 已从结算层底座推进到守方野战待结算窄口；`坚守不屈` 已从结算层底座推进到守方城战待结算窄口；`机里耐步兵` 防守明军半段和 `巴雅喇` 防守甲兵半段已开放自身守方正式打出窄口；`打草惊蛇` 与 `偷袭与伏击` 仍只补到战斗/劫掠结算层底座或防误放行门禁；其中 `打草惊蛇` 当前会在普通待结算战术窗口被拦截，避免没有扎营触发时消耗手牌伪装成已结算；`乌真超哈` 特殊牌已开放当前攻方野战待结算窄口，但仍不代表独立炮兵阶段时机、指定目标选择、次级部队版策反或完整逐张战术时机闭环
...
6. 仍不能把七大恨判成“新游戏接近收工”，原因集中在 2.4 与 4.5：正式手牌行动全集仍缺事件效果全集、战术时机闭环与银两特殊资源场景；当前新增的 `七大恨` 建兵与八旗持续事件，只证明这些事件牌可真实结算，不能替代逐张事件效果全集。教程规则覆盖已补骑兵避战、骑兵劫掠、中立入侵、水路调度、“水路后不能再接陆路”的玩家视角候选证据、骑兵城战减值摘要证据、战术牌真实打出、攻城同章占领对照，以及轮盘主章节完成后自动续到开垦隐藏续章的 `15g` 证据，但这些教程注入态不能替代正式普通手牌全集。4.5 必须继续保持未完成。
...
3. 这不是按钮文案问题，而是正式手牌对象只补到了 atlas05 已确认普通手牌的局部身份：
   ...
   - atlas05 的已确认事件牌已能在 `执行事件` 记录层保留具体牌名与规则摘要；“使用后移出游戏”的事件牌已经不再增加弃牌堆；持续事件牌会按持续事件不进弃牌堆并写入在场承载；`七大恨` 已能建立后金本土 2 个 3 级步兵；八旗持续事件已能建立对应族属的 2 级后金步兵并扩展后金本土判定；`反间计` 已能支付 2 张手牌后进入人物目标选择，排除努尔哈赤、林丹汗和阿巴凯，选择合法人物后将其移出游戏；`开门迎降` 已能指定敌方在场且已有判定表的人物，并按年中人物额外判定同源规则结算第一项；

> 2026-07-05 归化 VP 标记窄口复核：安全裁图 `temp/qidahen-guihua-evidence/tumed_ordos_vp_wide.jpg` 显示“归化”是 VP 图标旁的独立标记，而 `city-region-20` 种子位于土默特部、`city-region-26` 种子位于鄂尔多斯部；当前不能把归化城猜成土默特部或任一运行态地区，本轮改为显式记录归化 VP 标记控制者。
`玉匣出土` 已关闭正式打出时设置归化 VP 标记控制者、同控归化 VP 标记和鄂尔多斯部时拥有者 +1 威望这两层窄口；战后占领导致拥有者失去鄂尔多斯部时，持续事件会转移给新控制者；归化 VP 标记失控触发转移仍未实现，这仍不是完整持续事件效果闭环
   ...
   - `封贡敕书` 已在后金/蒙古普通行动支付中按“视作 2 张银两”计入支付值，手动支付与直接执行自动支付均已有回归覆盖；大明普通支付同牌仍按 1 张普通手牌消费，执行记录不会显示“视作 2 张银两”；事件打出时也已要求额外弃 1 张手牌并进入对手二选一流程，且目标大明时会承接赐印招安/驱虎吞狼既有正式链。这不是蒙古目标、完整跨势力覆盖或银两资源链闭环
   - 规则摘要明确“不能在攻城、守城时使用”、明确野战限定、明确守城专用、明确依赖增援/附兵劫掠触发，或明确要求未开放兵种/炮兵阶段的战术牌已能在当前攻方战术窗口被门禁拦截；`箭如雨下` 已单独开放当前攻方野战待结算战斗窄口；守方待结算窄口已单独开放城战 `坚守不屈`、野战 `拒马`、`巴雅喇` 自身防守半段和大明防守野战 `机里耐步兵`；触发型和未开放阶段型战术不会被普通待结算战斗窗口泛用；可打出的战术牌会在战报与行动记录里保留具体规则摘要；`鸟真超哈` 已能在本次野战中给攻方步兵掷骰等级 +1；`骑兵冲锋` 已能在本次野战中给攻方每个骑兵部队额外掷 2 颗骰；`战车阵` 已能在本次战斗中给攻方步兵防御等级 +1；`机里耐步兵` 已能在进攻明军的野战中给攻方每个步兵部队额外掷 1 颗骰，并已开放大明守方正式打出窄口，让防守明军步兵先掷并压制攻方步兵反击；`巴雅喇` 已能在本次进攻中给守方所有部队防御等级 -1，并已开放守方正式打出窄口让防守方步兵防御等级 +1；`步骑联合` 已能在本次野战中给同时参战的攻方步兵和骑兵掷骰等级 +1；`策反/内应` 和普通 `策反` 已能在目标战斗有敌方炮兵时让 1 个敌方炮兵临时转为攻方参战并进入攻方炮兵掷骰侧，且普通 `策反` 会保留真实牌名摘要；`箭如雨下` 已从结算层底座推进到当前攻方野战战术窗口窄口；`拒马` 已从战斗层底座推进到守方野战待结算窄口，会写入取消对手 `箭如雨下`、`连环火铳`、`骑兵火器` 的先结算修正和对手骑兵按步兵阶段掷骰修正；`坚守不屈` 已从结算层底座推进到守方城战待结算窄口；`机里耐步兵` 防守明军半段和 `巴雅喇` 防守甲兵半段已开放自身守方正式打出窄口；`打草惊蛇` 已从劫掠结算层底座推进到当前攻方合法骑兵劫掠待结算窄口，正式打出后会让本次骑兵劫掠免受守方炮骑反击伤害；`偷袭与伏击` 已补具名回归确认普通攻方战术窗口不会在缺少敌人增援触发窗口时消耗手牌或写入战术修正；`乌真超哈` 特殊牌已开放当前攻方野战待结算窄口；当前仍未开放通用守方响应窗口、兵种阶段窗口、独立炮兵阶段目标选择、敌人增援触发窗口、全部劫掠时机、附兵劫掠、次级部队版策反或完整逐张时机
...
> 2026-07-05 剩余战术链复核：`骑马步兵` / `分进合击` / `诈败诱敌` / 次级部队版 `策反` 当前仍缺附兵身份、未参战部队再移动/距离校验、对手附兵劫掠宣告、敌方次级部队参战触发、目标选择与战后回归承接链；普通战术打出入口继续门禁拦截，不能用现有骰子/劫掠结算层冒充完整效果。

七大恨已关闭三层窄口并补控制权边界：大明/蒙古使用无效果并移出游戏；后金可在实际控制的后金本土建立 2 个 3 级步兵；后金已生效汉八旗/满八旗/蒙八旗时，每张八旗会让七大恨额外建立 1 个 3 级步兵，但八旗“视为本土”仍必须搭配后金实际控制该区域才可建兵。仍不代表附兵、增援、完整事件效果或 2.4/4.5 完成。
