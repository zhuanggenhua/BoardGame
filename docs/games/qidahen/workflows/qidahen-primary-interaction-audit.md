...
3. 这不是按钮文案问题，而是正式手牌对象只补到了 atlas05 已确认普通手牌的局部身份：
   ...
   - 规则摘要明确“不能在攻城、守城时使用”、明确野战限定、明确守城专用、明确依赖增援/附兵劫掠触发，或明确要求“野战步兵阶段使用/提前在炮兵阶段”的战术牌已能在当前攻方战术窗口被门禁拦截；守城专用牌不会再被攻方野战或城战窗口误放行，触发型和阶段型战术也不会再被普通待结算战斗窗口泛用；可打出的战术牌会在战报与行动记录里保留具体规则摘要；`鸟真超哈` 已能在本次野战中给攻方步兵掷骰等级 +1；`骑兵冲锋` 已能在本次野战中给攻方每个骑兵部队额外掷 2 颗骰
