...
- 在这层手牌规则真相补齐前：
  ...
  - `战术牌` 只补到“规则摘要明确禁止攻城/守城、明确野战限定、明确守城专用、明确依赖增援/附兵劫掠触发，或明确要求具体兵种阶段/炮兵阶段的牌会被当前攻方战术窗口门禁拦截；打出后的战报与行动记录会保留具体规则摘要；`鸟真超哈` 会让本次野战攻方步兵掷骰等级 +1；`骑兵冲锋` 会让本次野战攻方每个骑兵部队额外掷 2 颗骰；`战车阵` 会让本次战斗攻方步兵防御等级 +1；`机里耐步兵` 会在进攻明军的野战中让攻方每个步兵部队额外掷 1 颗骰”这一层，仍不是完整战术时机全集或逐张战术效果
...
- 当前攻方战术窗口已按现有战斗上下文拦截野战限定、攻守城禁用、守城专用、增援/附兵劫掠触发型和明确阶段型战术牌；可打出的战术牌会在战报与行动记录里保留具体规则摘要，且 `鸟真超哈` 已接入本次野战攻方步兵掷骰等级 +1，`骑兵冲锋` 已接入本次野战攻方每个骑兵部队额外掷 2 颗骰，`战车阵` 已接入本次战斗攻方步兵防御等级 +1，`机里耐步兵` 已接入进攻明军野战时攻方每个步兵部队额外掷 1 颗骰
...
3. 这不是按钮文案问题，而是正式手牌对象只补到了 atlas05 已确认普通手牌的局部身份：
   ...
   - 规则摘要明确“不能在攻城、守城时使用”、明确野战限定、明确守城专用、明确依赖增援/附兵劫掠触发，或明确要求“野战步兵阶段使用/提前在炮兵阶段”的战术牌已能在当前攻方战术窗口被门禁拦截；守城专用牌不会再被攻方野战或城战窗口误放行，触发型和阶段型战术也不会再被普通待结算战斗窗口泛用；可打出的战术牌会在战报与行动记录里保留具体规则摘要；`鸟真超哈` 已能在本次野战中给攻方步兵掷骰等级 +1；`骑兵冲锋` 已能在本次野战中给攻方每个骑兵部队额外掷 2 颗骰；`战车阵` 已能在本次战斗中给攻方步兵防御等级 +1；`机里耐步兵` 已能在进攻明军的野战中给攻方每个步兵部队额外掷 1 颗骰
