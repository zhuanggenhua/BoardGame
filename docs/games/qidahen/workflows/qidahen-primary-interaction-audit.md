...
- 阻塞不再是“完全没有普通手牌图集或确认行”：2026-07-04 已确认 atlas05 是普通手牌图集，并把 49 张 `passed` 普通手牌接入正式发牌/预览流。但这只是局部接入，仍不是完整普通手牌全集真相源：
  ...
  - `src/games/qidahen/domain/commands.ts`
    - 已把规则摘要中明确写有“不能在攻城、守城时使用”、明确野战限定或明确守城专用的战术牌接入待结算战斗门禁；同时把“敌人增援时”“取消对手宣告的附兵劫掠”这类触发型战术，以及需要附兵模型/未参战部队增援承接链的战术，从当前攻方普通 `PLAY_TACTIC_CARD` 窗口保守拦截；`箭如雨下` 已单独开放当前攻方野战待结算战斗窄口，`乌真超哈` 特殊牌已单独开放当前攻方野战待结算窄口，`拒马` 已单独开放守方野战待结算战斗窄口。
