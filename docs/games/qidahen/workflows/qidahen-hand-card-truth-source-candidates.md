...
- 规则 PDF：`D:\gongzuo\webgame\gameasset\七大恨 中文mod\七大恨规则.pdf`
  - 已抽取文本并沉淀复核记录：`docs/games/qidahen/workflows/qidahen-hand-card-rule-pdf-review.md`。
...
- 已把 49 张 OCR 候选整理成可提交的人工录入工作台：`docs/games/qidahen/workflows/qidahen-hand-card-manual-entry-matrix.md`。
...
- 已为 16 张 high 优先级安全标题小裁切生成放大核读索引：`docs/games/qidahen/workflows/qidahen-hand-card-high-priority-title-review.md`。
...
- 已为 7 张 medium 优先级安全标题小裁切生成可提交核读索引：`docs/games/qidahen/workflows/qidahen-hand-card-medium-priority-title-review.md`。
...
- 已为 19 张 low 优先级安全标题小裁切生成可提交核读索引：`docs/games/qidahen/workflows/qidahen-hand-card-low-priority-title-review.md`。
...
- 已为 7 张 unreadable 安全标题小裁切生成可提交核读索引：`docs/games/qidahen/workflows/qidahen-hand-card-unreadable-title-review.md`。
...
- 已对 49 张候选牌面运行本地 EasyOCR，并把可提交结论沉淀到：`docs/games/qidahen/workflows/qidahen-hand-card-easyocr-batch-review.md`。
...
- 已对 49 张候选 OCR 文本做关键词分流，并把可提交结论沉淀到：`docs/games/qidahen/workflows/qidahen-hand-card-ocr-keyword-triage.md`。
...
- 已基于 OCR 文本生成非普通手牌排除候选清单：`docs/games/qidahen/workflows/qidahen-hand-card-nonordinary-exclusion-candidates.md`。
...
- 已基于 OCR 文本生成剩余人工复核队列：`docs/games/qidahen/workflows/qidahen-hand-card-remaining-manual-review-queue.md`。
...
- 已基于 OCR 文本生成剩余 12 张候选的二次分流记录：`docs/games/qidahen/workflows/qidahen-hand-card-remaining-review-subtriage.md`。
...
- 已对最后 1 张当时待看图候选 `mongol_r07_c10` 生成小尺寸安全变体，并记录本地 OCR 结果：`docs/games/qidahen/workflows/qidahen-hand-card-final-candidate-safe-ocr.md`。
...
- 已把剩余 12 张候选整理为可提交人工复核清单：`docs/games/qidahen/workflows/qidahen-hand-card-human-review-checklist.md`。
...
- 已新增外部文本来源搜索记录：`docs/games/qidahen/workflows/qidahen-hand-card-external-source-search.md`。
...
- 已新增结构化素材来源复查：`docs/games/qidahen/workflows/qidahen-hand-card-structured-source-recheck.md`。
...
- 已新增本地素材逐牌真相源穷尽复查：`docs/games/qidahen/workflows/qidahen-hand-card-local-asset-source-exhaustion.md`。
...
- 疑似单卡素材 OCR 小批量试跑也已回写到该文档，并沉淀为长期证据 `docs/games/qidahen/workflows/qidahen-hand-card-single-card-ocr-probe.md`：33 张候选经 PIL + EasyOCR 试跑后，普通牌类关键词独占命中为 0，混合普通/非普通命中为 0，4 张命中人物或下野等非普通线索，23 张只有低信息 OCR 文本，6 张没有 OCR 文本。
- 2026-07-03 已对其中 4 张代表样本补做需求交接式安全读图验收：3 张明确为非普通手牌，1 张因纯色底块无法判断。该结果说明安全读图链路可用，但代表样本仍没有产出可直接反写的逐牌普通手牌真相源；其中 23 条低信息文本不能自动判为普通手牌，也不能自动全部排除，后续仍需继续按字段验收、人工确认或更可靠 OCR。
- 仅有的非图片来源仍是规则 PDF 与 TTS Workshop JSON，二者已经在前序复核中证明不能提供普通事件、军备、战术、银两的逐牌中文牌名、牌类、效果和军备目标。
- 因此本地素材目录没有第四条可追溯文本来源来关闭 OpenSpec `2.4`。

## 2026-07-03 完成依据决策矩阵

- 已新增完成依据决策矩阵：`docs/games/qidahen/workflows/qidahen-hand-card-truth-source-decision-matrix.md`。
...
- 已新增正式映射反写契约：`docs/games/qidahen/workflows/qidahen-hand-card-formal-mapping-contract.md`。
...
- 已新增 TTS CardID 到图集位置清单：`docs/games/qidahen/workflows/qidahen-hand-card-tts-cardid-position-map.md`。
...
- 已新增完整 CardID 人工录入工作台：`docs/games/qidahen/workflows/qidahen-hand-card-tts-cardid-full-manual-entry-matrix.md`。
...
- 已新增运行时手牌图集与 TTS 牌组交叉核验：`docs/games/qidahen/workflows/qidahen-hand-card-runtime-atlas-tts-crosswalk.md`。
...
- 已新增运行时图集候选人工录入矩阵：`docs/games/qidahen/workflows/qidahen-hand-card-runtime-atlas-manual-entry-matrix.md`。
- 矩阵共 28 条候选；后续低分辨率安全预览已全部核读并排除为人物牌、纪年/剧本类牌或人物效果相关非普通牌。
- 因此本矩阵只能作为运行时图集候选被证伪的归档证据；不得反写正式手牌规则映射，也不能关闭 `2.4`。
- 已新增运行时图集候选安全复核入口说明：`docs/games/qidahen/workflows/qidahen-hand-card-runtime-atlas-safe-review.md`。本地脚本只生成 28 条候选的小尺寸缩略图与标题裁切索引，供人工读牌/OCR 使用；这仍不是逐牌真相源。
- 已新增运行时图集候选小图 OCR 尝试记录：`docs/games/qidahen/workflows/qidahen-hand-card-runtime-atlas-ocr-attempt.md`。EasyOCR 失败是阻塞证据；后续低分辨率安全预览已经完成候选核读，仍不能作为正式真相源。
- 已新增运行时图集候选与既有 OCR 线索交叉表：`docs/games/qidahen/workflows/qidahen-hand-card-runtime-atlas-existing-ocr-crosswalk.md`。28 条候选全部命中既有 OCR/核读线索；后续安全预览已全部排除，不能作为正式真相源。
