...
>
> 2026-07-04 追加口径：atlas05 已确认事件牌已能从手牌本体进入 `play-event-card` / `执行事件` 记录层，记录具体牌名和规则效果摘要；规则摘要写有“使用后移出游戏”的事件牌，打出牌本身不进入弃牌堆；持续事件牌会保留“不进入弃牌堆”的持续事件文案。`七大恨` 已推进到一张真实事件效果，会在当前选中的后金本土建立 2 个 3 级后金步兵，并同步地区兵力与棋子集合；`成立汉八旗 / 成立满八旗 / 成立蒙八旗` 已推进到真实持续事件效果，后金打出时会在后金控制的对应族属区域各建立 1 个 2 级后金步兵，并让对应族属区域之后视为后金本土；`蒙古大旱 / 东北大旱` 已推进到旱灾标记状态层，能在合法族属区域写入正式区域事件标记并同步可见地图标记，且年中土地税赋与新年补给/兵力耗损会把旱灾区域的有效人口视为 0；`翻山越岭` 已推进到一次性进攻待结算效果，打出后会生成一次突袭待结算，并把本次进攻中的长城、山脉边界临时按平原边界计算，不永久改写地图边界；战术牌打出后的战报和行动记录现在会保留具体规则摘要，`鸟真超哈` 已能把本次野战攻方步兵掷骰等级 +1，`骑兵冲锋` 已能把本次野战攻方每个骑兵部队额外掷 2 颗骰，`战车阵` 已能把本次战斗攻方步兵防御等级 +1；`步兵铁甲` 和 `骑兵铁甲` 已有结构化战斗窄口，升级后会让对应步兵/骑兵防御等级进入战斗掷骰与损伤结算；`火炮技术` 已推进到炮兵战斗窄口，升级后会让每个炮兵额外掷 1 颗骰；`骑兵火器` 已从“能打出并升级军备”推进到一层真实军备效果，升级后会让野战骑兵阶段己方每个骑兵部队额外掷 1 颗骰；`骏马育种` 已推进到一层真实军备效果，升级后会让骑兵/调骑移动档位移动力 +1；`西式棱堡` 已推进到一层真实军备效果，升级后会让城战守方每个部队投 2 颗骰；`连环火铳` 已推进到一层真实军备效果，升级后会让野战步兵阶段己方每个步兵部队额外掷 1 颗骰。这仍只是“打出哪张事件牌 + 使用后移出游戏/持续事件弃牌处理 + 七大恨建兵 + 八旗持续本土语义 + 旱灾标记状态/可见 token + 旱灾在土地税赋/补给耗损里的有效人口语义 + 翻山越岭一次性进攻边界语义 + 战术牌打出记录保留规则摘要 + 鸟真超哈单牌战术修正 + 骑兵冲锋单牌战术修正 + 战车阵单牌战术修正 + 步兵铁甲/骑兵铁甲结构化防御等级效果 + 火炮技术炮兵额外掷骰效果 + 骑兵火器单项军备效果 + 骏马育种单项军备效果 + 西式棱堡单项军备效果 + 连环火铳单项军备效果”的局部执行合同，不代表逐张事件效果、额外成本、目标选择、全部人口读取入口、完整战术时机/其他逐张战术效果、军备效果全集或跨势力资源链已经完成；`红衣大炮` 的来源牌专属野战防守/攻城进攻加成仍未实现，劫掠仍保留真实人口，不因旱灾标记被清零。
...
- 战术牌打出后的战报与行动记录已保留具体规则摘要；`鸟真超哈` 已从纯记录推进到一张真实野战战术效果：本次待结算战斗中攻方步兵掷骰等级 +1；`骑兵冲锋` 已从纯记录推进到一张真实野战战术效果：本次待结算战斗中攻方每个骑兵部队额外掷 2 颗骰；`战车阵` 已从纯记录推进到一张真实战术效果：本次待结算战斗中攻方步兵防御等级 +1。这不是守方响应窗口、兵种阶段窗口或其他逐张战术效果闭环。
