...
| 本地素材目录穷尽复查 | 只发现图片素材、1 个 TTS JSON 和 1 个规则 PDF；没有额外文本牌表、结构化表格或压缩包；71 张图片文件名没有普通手牌牌类关键词命中；运行时 manifest 的 30 个 `cards/` 键也没有普通手牌牌类关键词命中；33 张疑似单卡素材 OCR 小批量试跑没有普通牌类关键词独占命中；4 张代表样本已按需求交接式安全读图验收 | 新扫描、小批量 OCR 和代表样本读图都没有发现未纳入前序矩阵的逐牌真相源；23 条低信息 OCR 文本仍不能替代人工确认 | 否 | `qidahen-hand-card-local-asset-source-exhaustion.md`、`qidahen-hand-card-single-card-ocr-probe.md` |
...
- 本地疑似单卡素材 OCR 小批量试跑没有产生普通牌类关键词独占命中；4 张代表样本已按需求交接式安全读图验收，结果为 3 张非普通手牌、1 张图片不足阻塞，没有得到中文牌名、牌类、效果或军备目标的可靠组合；23 条低信息 OCR 文本仍只是待人工确认线索，不能关闭 `2.4`。
