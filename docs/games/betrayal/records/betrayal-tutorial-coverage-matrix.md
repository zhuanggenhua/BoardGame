...
| 作祟触发 | 当前玩家在图书馆结束回合后，等待队友通过正常回合自然获得预兆并公开触发作祟 | 当前玩家只点击自己的结束回合；P1 / P2 的探索、确认和结束回合只能由教程 AI / 系统按正式命令推进并作为公开结果展示，不能让当前玩家切座位代操作；P1 翻出指环未触发作祟，P2 翻出狗后 2 颗骰仍未触发，P2 确认并结束后回到当前玩家，当前玩家再次结束回合，P1 翻出面具后用 3 颗骰触发作祟；触发确认属于 P1，当前玩家不代点 | P1 确认后自动打开当前玩家所属阵营的英雄剧本书；当前玩家先看到英雄开场，点击继续后看到英雄目标，关闭后回到作祟牌桌并可回看英雄目标；随后等待 P2 公开结束回合，轮回当前玩家在图书馆点击“寻找木乃伊真名”，知识检定成功后取得第 1 枚知识标记；当前玩家确认后结果层退场，并回到“结束回合”这一下一真实入口 | 不允许从“未触发作祟的预兆专题”直接跳到“作祟后目标”，也不允许停在读本回看后或仍待确认的结果层宣称后续已教完；自然触发截图必须覆盖当前玩家交回合、队友指环未触发、队友狗未触发、回到当前玩家再交回合、队友面具与 3 颗骰触发、英雄读本开场、英雄目标、回牌桌、等待队友结束、P0 找真名、第 1 枚知识标记、确认结果退场和结束回合入口；学习驱逐法术需要后续合法持书回合或代表态专题，不得作为本段同回合自然链；切座位操作队友或把默认主线第一读本做成叛徒书的旧图组只能算错误验收 / 历史诊断 |
