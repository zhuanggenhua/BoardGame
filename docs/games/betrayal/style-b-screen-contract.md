...
2. 左侧玩家区
   - 当前探索者大卡
   - 当前探索者属性轨道
   - 当前探索者持有区：要考虑超出 `2 Item + 1 Omen` 的承载，但默认先用稳定卡槽 / 稳定网格 / 轻量折叠处理；`betrayal` 当前线禁止用歪斜扇形、强旋转或明显借用其他游戏持有区语法
   - 持有物必须让玩家直接识别对象：有正式正面图或正面 atlas 时必须显示正面；缺正面时只能用弱化牌背底纹 + 大号对象名 + 类别 + `缺正面` 短状态诚实展示缺口；禁止拿 marker、属性 token、数字 token、参考卡碎片或其它素材拼成伪正面
