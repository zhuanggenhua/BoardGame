...
预兆当前数据合同为 9 张。圣符翻出作祟链、圣符探索声明链、狗远距交易链和面具移动链均已单列收口，但不能外推到其它预兆，也不能把圣符声明链外推成雕像、狗、面具等其它预兆能力已完成。
...
| 盔甲 | 受到物理伤害时减 1                                                  | 只有数据合同和领域入口，缺真实页面链                         | 受物理伤害前 -> 伤害结算 -> 盔甲减伤可见 -> 属性变化正确 -> 回牌桌                        |
...
物品当前数据合同为 11 个条目，其中 `map / notebook / journal / manuscript` 共享“放置探索者到已发现房间”效果，`flashlight / lantern` 共享事件检定加骰效果。急救包、奇怪的药品、兔脚和骨制钥匙链不能外推为所有物品使用完成。
...
| 手电筒 / 灯笼               | 事件检定额外 +2 骰                                       | 数据合同和加骰入口存在；缺真实页面链                                                               | 持有前后同一事件检定骰数对比 -> 开放式骰盘 -> 结果 -> 收口                                 |
| 头戴耳机                    | 受到精神伤害时减 1                                       | 数据合同和减伤入口存在；缺真实页面链                                                               | 精神伤害前 -> 结算 -> 减伤可见 -> 属性变化正确 -> 回牌桌                                   |
...
- 圣符触发作祟判定、圣符探索声明、haunt 阶段禁探索、驱魔失败死亡、急救包/奇怪的药品/地图代表/兔脚/骨制钥匙等物品链、同房间交易、狗远距交易、面具移动、骰盘代表链已回到规则真相和真实入口 E2E；剩余范围仍需逐项登记，不得外推成山屋全面完成。
