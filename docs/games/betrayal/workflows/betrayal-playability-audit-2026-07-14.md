...
物品当前数据合同为 11 个条目，其中 `map / notebook / journal / manuscript` 共享“放置探索者到已发现房间”效果，`flashlight / lantern` 共享事件检定加骰效果。急救包、奇怪的药品、兔脚、头戴耳机、手电筒/灯笼、魔法相机知识检定替代和骨制钥匙链不能外推为所有物品使用完成。
...
- 蜘蛛！不是“先选择线路再判定”。它是先进行神志检定；如果结果达到 `4+`，再进入本次 `4+` 效果处理：选择获得神志或速度，并选择一个相邻板块放置。这里的房间选择属于规则效果处理 / 结算输入；目标选完后的结果层只是效果处理完成后的回看与返回牌桌入口，不是才开始选择房间。
...
- 蜘蛛！关键证明图：`蜘蛛-完整链路-03-事件牌翻出已有神志检定.jpg`，用于证明它是“先神志检定，再在 `4+` 效果处理中选择奖励属性和相邻房间，最后进入效果处理结果层”。
...
- 本文档的完成边界是当前首剧本数据合同、当前 23 张事件牌、9 张预兆、11 个物品、已实现房间/骰盘触发家族和用户点名 UI 回归；官方扩展牌、其它剧本、部署、提交和线上反馈状态不在本文档边界。
