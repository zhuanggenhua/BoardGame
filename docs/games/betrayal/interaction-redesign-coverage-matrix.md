...
| 23 | 偷窃 / 尸体搜刮 | 第三版默认不能靠力量攻击偷物品；死亡留下尸体，同房间每回合可拿一个物品 / 预兆 | 若沿用旧版偷窃或删除死者，会直接错规则 | `corpseInventory`, `lootUsedThisTurn`, corpse token |
| 24 | 作祟揭示顺序 | 作祟开始先公开读双方介绍和设置，完成公开设置后再按阵营看秘密目标 | 直接打开剧本书或只给当前玩家目标会漏公共设置 | `hauntSetupQueue`, `publicIntroRead`, `secretScopes` |
