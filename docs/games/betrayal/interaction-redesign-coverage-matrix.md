...
| 8 | 物理 / 精神 / 一般伤害由玩家在指定属性之间分配 | 已补事件一般伤害、火炉房回合末物理伤害和 12 号作祟「援手」攻击奖励伤害的玩家确认分配切片：火炉房结束回合先生成待分配伤害，援手选择“造成伤害”后也生成待分配伤害；受伤玩家选择合法属性后才扣属性 / 推进；盔甲降到 0 时不生成待分配；非法属性、非受伤玩家和作祟前临界属性都会被领域校验拦截。仍缺普通攻击主链、致死攻击胜负、倒塌房间、作祟 / 怪物伤害、灰尘等统一接入 | `DamageAllocationInteraction`：伤害类型、可选属性、点数、确认分配 | `pendingDamageAllocation`, `allowedTraits`, `amount`, `allowSkull`, `nextPlayerId` | 伤害分配面板 `betrayal-damage-allocation-panel`，显示来源、点数、可选属性和属性轨后果 | 领域测试覆盖火炉房 pending、非法分配、盔甲归零、临界限制和确认后推进，并覆盖援手攻击奖励伤害的待分配、错误玩家拒绝和受伤方确认后扣减；Board 组件测试覆盖火炉房面板、未选满禁用、选择速度后确认，以及援手选择造成伤害后等待受伤方分配 | `implemented-needs-remodel` |
...
| 24 | 作祟揭示顺序 | 作祟开始先公开读双方介绍和设置，完成公开设置后再按阵营看秘密目标 | 已补作祟揭示代表读模型和牌桌短提示：一名叛徒作祟只提示“公开读英雄介绍/设置，再公开读叛徒介绍/设置”；灰尘是隐藏叛徒作祟，只提示英雄公开读法和“隐藏身份不公开”。`hauntSetupQueue` 只保留为领域/内部设置队列，不能在作祟揭示前景 UI 里显示治疗强化、秘密疾病标记、幻影摄影师、魔法相机、Essence 等明细或状态；作祟后进度条 / 主动作 / 交换或攻击提示 / 武器选择必须等揭示提示收口后再出现，且可见层、sr-only、aria/status 和 title 都遵守同一边界，不能和揭示提示同屏构成信息墙。当前仍不等于完整逐作祟 setup 命令队列、隐藏叛徒完整阵营 / 自由混战和段落级 reveal-on-use 完成 | `resolveBetrayalHauntRevealProtocol`, `resolveBetrayalHauntSetupQueue`, `publicSteps`, `secretBoundary`, `hauntSetupQueue`, `BetrayalHauntRevealCue` |
...
| 作祟揭示顺序 / 秘密边界代表链 | 作祟开场揭示层读取同源 `resolveBetrayalHauntRevealProtocol` 和 `resolveBetrayalHauntSetupQueue`：一名叛徒作祟只在牌桌前景提示双方公开读介绍/设置；灰尘按隐藏叛徒处理，不再写成无叛徒，只提示英雄公开读法、隐藏身份保持秘密、目标与秘密规则分开阅读；setup 队列保留为领域/内部队列，不作为玩家前景明细或状态墙暴露；牌桌不自动弹开剧本书，保留手动剧本书入口；揭示期隐藏灰尘进度条、主动作、交换 / 攻击提示、武器选择和对应 sr-only/status 文本，玩家点“返回牌桌”后才恢复进度 / 主动作；首剧本只写“作祟开始”也会被识别为作祟开场；本地化和 E2E 额外扫描“上屏 / off-screen / 看清后可关闭 / 阅读后关闭 / setup 队列”等内部审查话术不得进入玩家 UI | `npx vitest run src/games/betrayal/__tests__/firstScenarioRuntime.test.ts -t "作祟揭示|灰尘隐藏|魔法相机作祟揭示" --configLoader native --pool threads --no-file-parallelism --maxWorkers 1`，3 passed / 180 skipped；`npx vitest run src/games/betrayal/__tests__/Board.foundation.test.tsx -t "作祟揭示|剧本书|首剧本作祟|大宅饿了作祟|灰尘隐藏叛徒" --configLoader native --pool threads --no-file-parallelism --maxWorkers 1`，5 passed / 77 skipped；`npx eslint src/games/betrayal/Board.tsx src/games/betrayal/__tests__/Board.foundation.test.tsx e2e/betrayal/haunt-reveal-protocol.e2e.ts e2e/betrayal/event-choice-coverage.e2e.ts public/locales/zh-CN/game-betrayal.json public/locales/en/game-betrayal.json`，0 errors / 2 JSON ignored warnings；`node scripts/infra/run-e2e-single.mjs ci e2e/betrayal/haunt-reveal-protocol.e2e.ts`，2 passed；`node scripts/infra/run-e2e-single.mjs ci e2e/betrayal/event-choice-coverage.e2e.ts "一瓶微尘成功进入灰尘后可寻找解药并完成疾病交换同意"`，1 passed；证据：`evidence/betrayal-core-interactions/haunt-reveal-protocol/e2e-test.md`，截图 `01-作祟揭示-一名叛徒公开步骤.jpg` / `02-作祟揭示-一名叛徒返回牌桌.jpg` / `03-作祟揭示-隐藏叛徒公开步骤.jpg` / `04-作祟揭示-隐藏叛徒返回牌桌.jpg`；服务器相册 `http://8.148.71.102:18080/#/boardgame/betrayal-haunt-reveal-protocol` |
| 作祟 12「大宅饿了 / 援手」领域战斗 + Board 承接切片 | 官方 12 号自由混战领域链已覆盖：奇异护符 setup 和换手控制巨魔手；力量攻击另一探索者获胜后先生成“造成伤害 / 偷 1 张物品或预兆”选择，不立即扣防守者伤害；选择偷牌不造成伤害，选择造成伤害后进入受伤防守者分配，确认后才扣属性；指环这类非力量攻击获胜不能偷牌；两个巨魔手同板块时提供力量 8 合击并消耗两个巨魔手。Board 组件已承接攻击投骰收口后的伤害 / 偷牌选择、巨魔手同房合击入口、护符控制者短状态和无人持护符跳过原因；真实牌桌入口已覆盖力量攻击奖励选择、巨魔手力量 8 合击、护符换手控制权和无人持护符跳过提示。当前仍不证明完整怪物行动、12 号完整终局或 50 个作祟完成 | `npx vitest run src/games/betrayal/__tests__/firstScenarioRuntime.test.ts -t "大宅饿了|援手|巨魔手|奇异护符" --configLoader native --pool threads --no-file-parallelism --maxWorkers 1`，9 passed / 180 skipped；`npx vitest run src/games/betrayal/__tests__/firstScenarioRuntime.test.ts --configLoader native --pool threads --no-file-parallelism --maxWorkers 1`，189 passed；`npx vitest run src/games/betrayal/__tests__/Board.foundation.test.tsx -t "大宅饿了|援手|巨魔手|偷牌" --configLoader native --pool threads --no-file-parallelism --maxWorkers 1`，4 passed / 76 skipped；`npx tsc --noEmit --pretty false`，通过；`npx eslint src/games/betrayal/Board.tsx src/games/betrayal/__tests__/Board.foundation.test.tsx`，0 errors；`npx eslint src/games/betrayal/Board.tsx e2e/betrayal/betrayalTestHelpers.ts e2e/betrayal/helping-hands-combat.e2e.ts public/locales/zh-CN/game-betrayal.json public/locales/en/game-betrayal.json`，0 errors / 2 JSON ignored warnings；`node scripts/infra/run-e2e-single.mjs ci e2e/betrayal/helping-hands-combat.e2e.ts`，2 passed；证据：`evidence/betrayal-helping-hands-combat/e2e-test.md`；服务器相册 `http://8.148.71.102:18080/#/boardgame/betrayal-helping-hands-combat`；截图 `01-大宅饿了-力量攻击投骰回顾-可改骰时空白不可关闭.jpg` / `02-大宅饿了-伤害或偷牌选择.jpg` / `03-大宅饿了-偷牌后回牌桌.jpg` / `04-大宅饿了-巨魔手合击入口.jpg` / `05-大宅饿了-巨魔手合击后反馈.jpg` / `06-大宅饿了-护符换手后旧持有人无巨魔手入口.jpg` / `07-大宅饿了-护符新持有人获得巨魔手入口.jpg` / `08-大宅饿了-无人持护符巨魔手跳过.jpg`；`game-betrayal` zh-CN / en JSON 解析通过 |
...
> 注意：本表只证明已登记切片完成；卡牌结算顺序 / 掩埋语义目前是领域代表链加牌桌合并摘要，不等于完整阻塞式牌面逐步确认 UI；基础视线读模型和远程武器领域校验 / 目标高亮 / 视线线条代表链即使已补弩真实入口 E2E 和相册证据，也不等于真实远程牌面完整录入完成；魔法相机幻影摄影师视线攻击即使已补目标切换、视线线、骰盘、E2E 和相册证据，也只证明该剧本代表链，不等于完整怪物系统或所有怪物视线攻击完成；基础障碍物移动成本不等于完整怪物系统或路径预览完成，基础尸体搜刮只证明第一剧本同房间尸体拿 1 张物品 / 预兆的代表链，不等于完整死亡 UI、特殊作祟尸体用途或完整怪物尸体系统完成；作祟揭示顺序 / 秘密边界代表链只证明规则公开读法短提示、灰尘隐藏叛徒口径、setup 队列只留在领域状态不进入玩家前景和揭示收口后才恢复作祟进度的边界，不等于完整逐作祟 setup 命令队列、隐藏叛徒完整阵营 / 自由混战或段落级 reveal-on-use 完成；强制移动 owner 代表链不等于所有强制移动都已有路径预览，作祟特殊行动预算已用状态的真实入口证据只覆盖灰尘剧本“寻找解药”，不等于全部特殊行动或全部作祟完成，区域耗尽 / 区域不匹配 / 封死区域重抽 / 最小调整已有板块四条探索失败边界已补真实入口 E2E 和相册证据；其中最小调整只证明“选择一个已有板块调整并确认放置”的代表合法切片，不等于完整探索规则或所有现实桌面极端调整方案完成，交易卡状态读模型不等于交易 UI 牌面禁用提示完全完成，12 号作祟当前证明领域层官方 setup、奇异护符换手控制权、力量攻击偷牌替代伤害、巨魔手力量 8 合击、Board 组件承接，以及力量攻击奖励选择 / 巨魔手合击 / 护符换手控制权 / 无人持护符跳过提示真实入口 E2E 和截图证据，不等于完整怪物行动或 12 号完整终局，怪物系统和 50 个作祟逐条合同仍需继续推进。
