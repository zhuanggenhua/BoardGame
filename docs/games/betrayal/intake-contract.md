...
- 2026-08-25 木乃伊专项补充：旧版「木乃伊横行」规则要求「木乃伊怪物标记(大)」。3e `384x336` 怪物 / Stunned token 组仍未发现木乃伊，`evidence/betrayal/full-audit/mummy-token-source-search-2026-07-31.md` 仍证明本地图包 `Mods\Images`、TTS、`Models`、`Assetbundles` 未命中官方木乃伊 token / portrait 源图；当前地图 token 已按用户候选裁定改用 Openclipart 324162 木乃伊 SVG，路径为 `betrayal/tokens/monsters/mummy.svg`，女孩作祟 token 仍使用 Openclipart 5285 女孩 SVG，路径为 `betrayal/tokens/haunts/mummy-girl.svg`，后续替换需先走候选图筛选。旧 `large-monster-front` 只保留为历史占位记录，不再是木乃伊运行时 token；`betrayal/monsters/mummy` portrait 仍是临时占位，不能宣称 portrait 素材完成。
