...
| 5.5 三类牌堆 | 事件、物品、预兆分类洗混面朝下 | 每类牌堆独立顺序、弃置 / 掩埋底部 | `eventDeck`, `itemDeck`, `omenDeck` | 牌堆区 | 洗牌 / 抽牌 / 掩埋测试 | `design-ready` |
...
| 6.2 事件卡 | 朗读斜体文本，按指示，检定表只读匹配结果，完成后掩埋到底部 | 事件卡 resolution 分步执行，不展示无关结果表分支 | `eventCardInstance`, `pendingCardResolution`, `deckBottom` | 翻牌亮相 + 当前结果 | 事件卡结果表测试 | `design-ready` |
...
| 11.3 强制移动 | 若效果写“最多移动 N”，执行移动的人决定移动多远，即使移动别人 | 强制移动 interaction owner 是执行效果者，不一定是被移动者 | `ForcedMoveInteraction` | 目标路径选择 | 强制移动 owner 测试 | `design-ready` |
| 12 探索入口 | 在有未探索走廊的房间上，可穿过该门探索 | 入口门位来自地图对象本体选择 | `entryDoorway` | 未探索门位高亮 | 门位点击测试 | `design-ready` |
| 12 区域匹配 | 翻房间堆顶；区域匹配才可翻开放置 | 房间抽取 resolution 记录翻找和掩埋过程 | `RoomDrawResolution` | 简短翻找状态 | 区域匹配测试 | `design-ready` |
...
| 12 房间文字 / 符号 | 先结算房间文字，再按符号抽事件 / 物品 / 预兆 | 探索结算链必须有顺序队列 | `discoveryResolutionQueue` | 阻塞式翻牌 / 结算 | 顺序测试 | `design-ready` |
| 12.1 区域耗尽 | 区域房间用完时不消耗移动，继续回合 | 探索失败事件不扣移动，不结束回合 | `exploreFailedNoTile` | 短提示 + 回到移动 | 耗尽测试 | `design-ready` |
| 12.2 保持可探索 | 放置后所在区域必须仍有至少一个可通行走廊；唯一封死的板块掩埋重抽；若仍无法放置，最小调整已有板块 | 合法性校验必须看区域未来开放门位；调整是显式系统步骤，不静默改图 | `regionOpenDoorways`, `tileAdjustmentEvent` | 放置预览 + 调整提示 | 封死区域测试 | `design-ready` |
| 13 抽牌 | 带符号房间抽对应牌堆顶 | 抽牌由房间符号触发，不由 UI 手动挑牌 | `cardDrawRequest` | 牌堆翻牌 | 抽牌测试 | `design-ready` |
| 13.1 事件 | 事件完成后掩埋到底部；只读匹配结果 | 见 6.2 | `eventDeck.bottom` | 事件翻牌 | 事件掩埋测试 | `design-ready` |
...
| 14 官方补充：交易限制 | 本回合已用特殊行动的物品 / 预兆不能交易；本回合用过攻击的武器不能交易 | 交易候选动态过滤并显示原因 | `usedSpecialActionCardIds`, `attackedWithWeaponIds` | 禁用原因短提示 | 交易限制测试 | `design-ready` |
| 15 特殊行动 | 标特殊行动符号；总是可选；每个可用特殊行动每回合一次；不能重复 | action budget 按具体行动 / 来源记录，不是全局一次 | `specialActionUsageBySource` | 卡牌本体动作按钮 | 每行动一次测试 | `design-ready` |
| 15 官方补充：刚获得限制 | 不能用本回合开始时未持有的物品 / 预兆做特殊行动 | 回合开始持有物快照 | `turnStartPossessionIds` | 禁用原因短提示 | 刚获得限制测试 | `design-ready` |
| 15 官方补充：非特殊被动 | 有些物品 / 预兆效果不是特殊行动，只要持有就生效 | 效果 metadata 区分 special action / passive / reaction | `effectKind` | 被动图标 / 无按钮 | 被动生效测试 | `design-ready` |
...
| 19.2 视线 | 同方向且不换区域可视线，中间可隔房间；房间内所有角色都在视线内 | 地图坐标和区域必须支持射线 / 直线查询 | `lineOfSightGraph` | 目标高亮 / 视线线条 | 视线测试 | `design-ready` |
...
| 官方补充：障碍物 | 障碍物所在房间离开耗 2 移动力；作祟后敌对探索者 / 怪物也算障碍物 | movement cost 由房间、token、阵营动态计算 | `movementCost`, `obstacles` | 路径预览显示耗费 | 障碍物移动测试 | `design-ready` |
