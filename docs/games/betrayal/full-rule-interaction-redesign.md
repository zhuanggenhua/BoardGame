...
> 疾病交换补充（作祟 3）：灰尘回合末同房强制交换代表链已证明当前探索者结束回合时，会逐个与多名同房探索者随机交换 1 个疾病标记；多次交换按最新疾病标记状态顺序结算，避免第二次交换仍引用旧 token；发生同房强制交换后不会触发“灰尘冲动”伤害分配；编号 1 的中间持有人和最终持有人都会永久成为叛徒；新玩家回合开始后本回合交换记录清空。证据见 `evidence/betrayal-the-dust-forced-sickness-exchange/e2e-test.md`。这仍只覆盖结束回合同房强制交换代表链，不代表主动“控制冲动”同意 / 拒绝、研究 / 治愈失败交换、隐藏编号完整可见性、研究 / 治愈全路径、同时胜负政策或完整终局完成。
...
| 作祟 3 | 子账本已补齐；已有狂热病患自然怪物回合代表链，覆盖死亡叛徒自然进入速度 5 移动骰、真实房间移动、攻击同房英雄和回合交接；已有未交换疾病回合末 2 骰一般伤害分配代表链，覆盖玩家确认分配后交接和隐藏叛徒确认分配后才变狂热病患；已有回合末同房强制交换代表链，覆盖多名同房探索者逐个随机交换、避免旧 token 预览跳过后续交换、不进入冲动伤害、编号 1 中间 / 最终持有人永久感染；仍不能等同于灰尘完整作祟完成 | 后续实现继续检查主动控制冲动同意 / 拒绝、研究 / 治愈失败交换、隐藏编号可见性、研究 / 治愈全路径、同时胜负政策和完整终局边界 |
