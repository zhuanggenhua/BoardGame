...
- 问题对象：`betrayal` 全基础规则交互重设计，不是只修用户点名的剧本卡、属性轨、预兆状态条或房间朝向。
- 真相来源：
...
- `read models`：当前属性值、预兆总数、可移动格、可交易对象、预兆状态条、可用特殊行动、合法攻击目标。
...
- 风险 / 进度类规则必须有玩家化承接：预兆状态条、计数轨、作祟目标条、怪物状态、死亡 / 尸体状态；内部阈值、骰数、判定公式不得作为常驻主 UI 正文。
...
| 3.2 作祟检定 | 抽预兆后按所有玩家已持有预兆总数掷骰，5+ 开始作祟，掷骰者为作祟揭秘者 | 已补作祟检定计数轨读模型：`resolveBetrayalNumberTracks` 会从全员当前持有预兆派生 `haunt-risk`，记录当前预兆、下次骰数、5+ 阈值、自动作祟和进度百分比；牌堆区预兆状态块只消费同源当前预兆数和短状态，完整检定规则进入 tooltip / aria / 规则层，抽预兆时作祟检定骰数仍与 `resolveBetrayalHauntRisk` 一致；物品 / 预兆抽牌已接入 `pendingCardResolutionQueue`，器械库代表链会逐步确认 1/3、2/3、3/3，普通预兆代表链保留“获得预兆”和“作祟检定”两条结果事实，但发现面板同屏承接并只暴露一次玩家确认；无需后续玩家选择的即时事件效果会进入单步确认。仍缺交易 / 死亡掉落后真实入口预兆状态条回归、更多预兆组合真实入口回归，以及选择型事件效果和更多房间文字进入同一通用发现结算队列 | `omenCount`, `hauntRoll`, `hauntRevealerPlayerId`, `numberTracks[]`, `pendingCardResolutionQueue` | 牌堆区预兆状态条 + 骰盘 + 发现面板确认按钮 | 领域单测覆盖全员预兆总数、下次骰数、`haunt-risk` 计数轨、器械库逐步确认队列、普通预兆单次确认队列和即时事件效果单步确认队列；Board 组件测试覆盖牌堆区分段大格槽进度条、器械库逐步确认按钮、圣符预兆作祟骰盘旁的单次确认和即时事件效果 1/1 确认 | `implemented-needs-remodel` |
...
| 3.4 作祟开始公开介绍和设置 | 英雄、叛徒先公开读介绍和设置，再分开读秘密目标 | 已补代表读模型和牌桌揭示层短提示：一名叛徒作祟提示公开读英雄介绍 / 设置，再公开读叛徒介绍 / 设置；灰尘按隐藏叛徒处理，只提示英雄公开读法、不要公开谁是叛徒，之后各自阅读自己的目标 / 秘密规则；牌桌保留手动剧本书入口，不能把 setup 队列明细、状态或秘密分发项堆成前景信息墙；作祟揭示期必须保持单主焦点，作祟后进度条 / 主动作 / 交换或攻击提示 / 武器选择在玩家点“返回牌桌”后才出现，不能通过可见层、sr-only、aria/status 或 title 和揭示提示同屏抢焦点；首剧本只写“作祟开始”也按作祟开场处理；新增 `resolveBetrayalHauntSetupCommandPreviews`，能把当前 setup 队列拆成“可由当前状态确认 / 仍需人工确认”的命令预览，并列出隐藏疾病、研究 token、奇异护符、巨魔手、幻影摄影师、魔法相机、Essence 和首玩家目标；`CONFIRM_HAUNT_SETUP_ENTRY` 已能正式确认灰尘怪物参考卡位置和研究 token 池并写回 setup 队列，牌桌“开局承接”卡片已有“确认怪物参考卡 / 确认研究 token”短按钮。仍缺其它作祟 setup 条目的正式确认命令、段落级秘密可见性、隐藏叛徒完整阵营和自由混战 | `resolveBetrayalHauntRevealProtocol`, `resolveBetrayalHauntSetupQueue`, `resolveBetrayalHauntSetupProgress`, `resolveBetrayalHauntSetupCommandPreviews`, `publicSteps`, `secretBoundary`, `hauntSetupQueue`, `CONFIRM_HAUNT_SETUP_ENTRY` | 轻量揭示提示 + 剧本书入口；灰尘 setup 确认按钮；setup 队列和命令预览仍主要是领域/内部合同 | 公开设置顺序领域测试 + 灰尘隐藏叛徒领域测试 + 魔法相机 setup 队列领域测试 + setup 命令预览 / 灰尘 setup 确认命令领域测试 + Board 组件测试 + 真实入口 E2E 截图 | `implemented-needs-remodel` |
...
| 5.4 骰子 | 8 枚骰子全员可用 | 已补统一骰池上限：属性检定、作祟检定、普通事件骰、死亡保护、怪物移动和攻击等通用投骰入口都经同一骰面函数，单颗骰面只会是 0/1/2，投骰数量最多 8；作祟检定读模型中的下一次骰数也按 8 骰上限表达，但不作为常驻主 UI 正文 | `BETRAYAL_DICE_POOL_SIZE=8`, `normalizeBetrayalDiceCount`, `recentRoll.dice[]` | 骰盘 / 结果条 / 预兆状态条 tooltip | 骰池上限领域测试 + 作祟检定骰数测试；仍需逐作祟怪物骰和全部 UI 回归 | `implemented-needs-remodel` |
| 5.5 三类牌堆 | 事件、物品、预兆分类洗混面朝下 | 已补领域代表链：事件牌结算后掩埋回事件牌堆底部，牌堆数量不减少且不进入弃牌；物品 / 预兆获得后才离开对应牌堆；器械库展示出的非武器物品牌掩埋回物品牌堆底部，且发现摘要会记录“房间获得武器 / 掩埋展示牌 / 符号抽牌入持有区”的结构化步骤；器械库代表链已把这些步骤写入 `pendingCardResolutionQueue`，发现面板必须按 1/3、2/3、3/3 逐步确认；普通预兆会保留“获得预兆”和“作祟检定”两条结果事实，但发现面板同屏显示、一次确认关闭；无需后续玩家选择的即时事件效果会生成单步确认，确认完关闭后牌堆区保留最近结算短状态，避免玩家丢失刚才展示 / 掩埋 / 获得 / 作祟检定 / 事件效果结果；兔脚回滚事件效果时会恢复持有牌牌堆顺序快照；事件牌页面承接 E2E 已覆盖当前接入事件池的翻牌、选择、投骰、结果和关闭链；当前正式运行事件池已按 23 张官方事件合同收口，不再把“未录入事件牌”列作当前缺口。仍缺完整牌堆区实体化可视化、物品 / 预兆更多真实入口、选择型事件效果进入通用发现结算队列和未来新增卡牌的独立合同 | `eventOrder`, `possessionOrderByKind`, `deckCounts`, `discardCounts`, `latestDiscovery.resolutionSteps`, `pendingCardResolutionQueue`, `ACKNOWLEDGE_CARD_RESOLUTION`, `recentRoll.eventEffectSnapshot` | 牌堆区 / 翻牌详情 / 逐步确认按钮 / 最近结算短状态 | 事件掩埋、器械库展示牌掩埋、兔脚回滚测试、器械库结构化发现步骤测试、器械库逐步确认队列测试、普通预兆单次确认测试、即时事件效果确认测试、牌堆区最近结算组件测试、`event-choice-coverage.e2e.ts` 当前接入事件池代表链 | `implemented-needs-remodel` |
...
| 6.1 房间板块区域 | 房间背面标明地下室 / 一楼 / 二楼；正面可能有事件 / 物品 / 预兆符号 | 房间 catalog 必须先从规则和房间主真相源重新录入规则定义字段：楼层 / 背面区域、门位、正面发现符号、房间文字效果、来源定位和未决项；2026-07-29 审计发现当前运行时是按 `drawOrder` 给新房间分配发现类型，不是消费房间板块固有符号，说明先前房间合同未锁定，必须先回 S0 重录再重构 | `roomTileCatalog.discoverySymbol`, `roomInstances.discoveryReward`, 当前差距字段 `drawOrder / exploreIndex` | 房间板块本体 + 教程符号说明 | S0 房间对象全集和字段合同 + 房间数据完整性测试 + 探索抽牌回归 + 教程文案 / E2E | `needs-s0-reintake` |
| 6.2 事件卡 | 朗读斜体文本，按指示，检定表只读匹配结果，完成后掩埋到底部 | 已补事件牌堆顺序语义：探索抽事件牌堆顶，结算后把该事件牌放回事件牌堆底部，`discardCounts.event` 保持 0；事件选择和兔脚重掷链路复用该牌堆快照；真实入口已覆盖当前 23 张官方事件池的自动属性检定、固定骰、可选事件骰、可选作祟检定、选择检定属性、全属性检定、后续属性奖励 / 通用伤害 / 目标房间选择、结果关闭回牌桌；无需后续玩家选择的即时事件效果已接入单步确认队列。仍缺未来新增事件牌的独立合同，以及选择型事件效果从领域状态到 UI 的通用发现结算队列 | `eventOrder`, `latestDiscovery`, `pendingEventChoice`, `recentRoll`, `recentAllTraitCheck`, `eventEffectSnapshot`, `pendingCardResolutionQueue` | 翻牌亮相 + 事件选择层 + 骰盘 + 当前结果 + 发现确认步骤 | 事件掩埋到底部测试 + 即时事件效果确认测试 + `event-choice-coverage.e2e.ts` 多模式事件完整链路 + 兔脚事件回滚测试 | `implemented-needs-remodel` |
...
| 6.4 预兆卡 | 朗读，获得，立即作祟检定 | 预兆和物品一样 owner 化，但额外触发作祟检定 | `possessions[omenId]`, `hauntRollPending` | 持有区 + 预兆状态条 + 骰盘 | 预兆抽取测试 | `design-ready` |
...
| 8 伤害 | 物理分配力量 / 速度；精神分配知识 / 神志；一般由玩家选择属性 | 已补选择属性事件一般伤害、固定属性即时事件一般伤害、掷骰事件物理 / 精神伤害和复合事件内嵌一般伤害死亡保护代表链、火炉房回合末物理伤害、倒塌房间结束回合速度检定失败坠落物理伤害、普通首剧本英雄攻击叛徒 / 叛徒或杰克之灵攻击英雄的非致死伤害切片、普通首剧本无头骨死亡保护的致死攻防分配后死亡 / 杰克之灵 / 英雄全灭收口切片、普通首剧本攻击头骨死亡保护分配后判断切片、普通攻击兔脚重掷后回算防守者非致死伤害的领域切片、12 号作祟「援手」力量攻击奖励伤害和巨魔手怪物攻击伤害的玩家确认分配切片、3 号作祟「灰尘」回合末未交换疾病 2 骰一般伤害分配切片、灰尘冲动一般伤害分配到骷髅后的头骨死亡保护成功 / 失败真实入口代表链，灰尘普通攻击致死真实入口下的伤害分配确认后狂热病患化 / 叛徒终局代表链，以及灰尘火炉房 / 倒塌房间等房间伤害致死后的伤害分配确认后狂热病患化代表链；这些入口都会停在伤害分配面板或死亡保护骰盘，受伤玩家确认后才扣属性 / 推进。头骨不会在攻击阶段提前预判，只有玩家实际把伤害分到骷髅后才掷死亡保护；灰尘冲动一般伤害同样在确认分配到骷髅后显示头骨死亡保护骰盘，成功不死亡且不生成狂热病患，失败才进入狂热病患化，并且确认骰盘前不会交接下一名玩家；固定属性即时事件一般伤害若直接扣到骷髅，也会先显示头骨死亡保护骰盘，头骨失败后才进入死亡和狂热病患化，兔脚成功可回滚；掷骰事件物理 / 精神伤害若扣到骷髅，也会先显示头骨死亡保护骰盘，头骨失败后才进入死亡和狂热病患化，兔脚成功可回滚；复合事件内嵌一般伤害若扣到骷髅，也会递归识别子效果并先显示头骨死亡保护骰盘，兔脚成功可回滚死亡和狂热病患化且保留其它已结算副作用；灰尘领域矩阵另覆盖物理攻击伤害、精神攻击伤害、一般伤害三类待分配伤害下头骨成功 / 失败后的狂热病患化结果；头骨失败后使用兔脚重掷死亡保护时，成功会回滚死亡和狂热病患化，仍失败会保持死亡和狂热病患化，且死亡玩家操控狂热病患的常规怪物回合仍不能主动使用兔脚。灰尘永久叛徒最终死亡转狂热病患时会掩埋物品和预兆，尸体不可搜刮。仍缺其它作祟 / 非 12 号怪物伤害来源统一接入；头骨 + 兔脚当前只覆盖灰尘代表组合，不代表全部死亡保护组合全排列 | `DamageAllocationInteraction`, `pendingDamageAllocation`, `recentRoll.deathPrevention.damageTraits`, `recentRoll.deathPrevention.damageKind=physical/mental/general` | 伤害分配面板 + 轨道预览 + 头骨死亡保护骰盘 | 火炉房领域 / Board 定向测试 + 倒塌房间坠落伤害领域 / Board 定向测试 + 普通攻击首剧本攻防领域 / Board 定向测试 + 普通攻击头骨死亡保护领域 / Board 定向测试 + 普通攻击兔脚回算领域测试 + 援手攻击奖励伤害领域 / Board 定向测试 + 巨魔手怪物攻击伤害领域 / Board 定向测试 + 灰尘未交换疾病伤害领域 / 真实入口 E2E + 灰尘普通攻击致死领域 / 真实入口 E2E + 灰尘房间伤害致死领域 / 真实入口 E2E + 灰尘头骨死亡保护领域 / 真实入口 E2E + 灰尘头骨伤害类型矩阵领域测试 + 灰尘头骨兔脚重掷领域测试 + 固定属性事件一般伤害头骨 / 兔脚领域测试 + 掷骰事件物理 / 精神伤害头骨 / 兔脚领域测试 + 复合事件内嵌一般伤害头骨 / 兔脚领域测试 + 灰尘死亡叛徒物品 / 预兆掩埋领域测试，证据见 `evidence/betrayal-the-dust-room-damage-death/e2e-test.md` 和 `evidence/betrayal-the-dust-skull-death-prevention/e2e-test.md`；后续补其它怪物、其它作祟伤害和死亡保护组合全排列 | `implemented-needs-remodel` |
...
| 9.2 作祟检定 | 按预兆总数掷骰，5+ 作祟，掷骰者为揭秘者 | 已补作祟检定独立读模型：按全员当前持有预兆 + 新抽预兆请求骰数，真实投骰最多 8 颗；结果记录为 `hauntRoll`，不走属性检定修正；预兆状态条和实际投骰同源，但常驻 UI 只显示当前预兆数和短状态。仍缺交易 / 死亡掉落后预兆状态条真实入口回归和更多预兆组合 | `BetrayalHauntRiskStatus`, `recentRoll.kind=hauntRoll`, `hauntRevealerPlayerId` | 预兆状态条 + 骰盘 | 作祟检定领域 / Board / E2E + 8 骰上限领域测试 | `implemented-needs-remodel` |
...
| 13 抽牌 | 带符号房间抽对应牌堆顶 | 已补领域抽牌顺序：事件从 `eventOrder[0]` 读取并进入事件牌页面承接链；物品 / 预兆从当前持有牌堆顶部读取，获得才移出牌堆；器械库代表链已把多步骤结果写入 `latestDiscovery.resolutionSteps` 和 `pendingCardResolutionQueue`，普通预兆会生成包含“获得预兆 / 作祟检定”的单次确认队列，无需后续玩家选择的即时事件效果会生成“事件效果”单步确认；发现面板会按步骤确认，未确认完会阻止直接结束回合，确认完关闭后牌堆区最近结算继续消费结构化步骤，不再只能拆中文 `detail`。这仍不是完整通用领域级发现结算队列。仍缺物品 / 预兆更多符号组合真实入口回归、选择型事件效果步骤入队和通用队列抽象 | `cardDrawRequest`, `eventOrder`, `possessionOrderByKind`, `latestDiscovery.resolutionSteps`, `pendingCardResolutionQueue`, `ACKNOWLEDGE_CARD_RESOLUTION` | 牌堆翻牌 + 发现面板步骤列表 + 逐步确认按钮 + 牌堆区最近结算 | 抽牌顺序 / 掩埋回底测试 + 事件牌真实入口 E2E + 器械库结构化步骤领域测试 + 器械库逐步确认领域 / 组件测试 + 普通预兆单次确认领域 / 组件测试 + 即时事件效果确认领域 / 组件测试 | `implemented-needs-remodel` |
...
| 13.3 预兆 | 预兆获得并进行作祟检定；抽到牌堆最后一张预兆时自动开始作祟 | 已补牌堆最后一张预兆自动作祟领域链和预兆状态计数轨；`haunt-risk` 会在牌堆只剩最后一张预兆时提供自动作祟读模型，普通预兆抽取仍按全员预兆总数掷骰；抽牌确认已接入同一 `pendingCardResolutionQueue` 机制，圣符预兆真实页面代表链会在作祟骰盘同屏时显示获得预兆和作祟检定，玩家只确认一次，确认前不能绕过发现面板。仍缺更多预兆组合、触发作祟时确认队列收口和交易 / 死亡掉落后预兆状态条真实入口回归 | `omenDeck.remaining`, `hauntAutoStartReason`, `numberTracks[]`, `pendingCardResolutionQueue` | 预兆状态条显示“再抽即作祟” + 发现确认按钮 + 作祟骰盘 | 最后一张预兆测试 + 预兆状态条组件测试 + 器械库逐步确认代表测试 + 圣符预兆作祟骰盘 / 单次确认组件测试 | `implemented-needs-remodel` |
...
| 18 开始作祟 | 英雄介绍和设置 -> 叛徒介绍和设置 -> 分开读秘密目标 / 规则 | 已补作祟揭示代表协议：公开步骤按官方顺序派生，牌桌揭示层只用短提示承接；灰尘已从旧“无叛徒”口径修正为隐藏叛徒，不公开隐藏身份；setup 队列保留为领域/内部状态，不在作祟揭示前景 UI 直接显示；作祟后常驻进度、主动作、交换 / 攻击提示和武器选择只能在揭示提示收口后恢复，且可见层、sr-only、aria/status 和 title 都必须遵守同一边界，避免把公开读法提示、进度条和内部 setup 状态堆成同屏信息墙；新增 setup 命令预览合同，已能把 1/3/12/33 代表作祟的 setup 条目拆成状态证据、目标玩家 / 房间 / 卡牌 / 怪物和缺口；其中灰尘“怪物参考卡放在作祟揭秘者左侧”和“准备 8 个研究 token”已能由正式确认命令写回队列，并由牌桌“开局承接”卡片提供短按钮。当前仍未逐作祟拆完其它公开设置操作和 UI 接线 | `resolveBetrayalHauntRevealProtocol`, `resolveBetrayalHauntSetupQueue`, `resolveBetrayalHauntSetupCommandPreviews`, `publicSteps`, `hauntSetupQueue`, `CONFIRM_HAUNT_SETUP_ENTRY` | 轻量公开揭示提示 + 手动剧本书入口；返回牌桌后恢复作祟进度 / 主动作；灰尘 setup 确认按钮；其它 setup 命令预览暂不进玩家前景 | 领域测试覆盖一名叛徒、灰尘隐藏叛徒、魔法相机 setup 队列、setup 命令预览和灰尘 setup 确认命令；组件测试覆盖不自动开书、手动剧本入口、首剧本开场、setup 队列不进入玩家前景、揭示收口后恢复进度和灰尘 setup 确认按钮；真实入口 E2E 覆盖一名叛徒 / 隐藏叛徒揭示与返回牌桌截图 | `implemented-needs-remodel` |
...
| 19 攻击时机 | 作祟开始后可攻击同房间敌方探索者 / 怪物；每回合只能攻击一次 | 已补首剧本和代表作祟攻击时机链：攻击动作只在作祟后开放，目标按英雄 / 叛徒 / 怪物阵营和同房间、枪的视线范围、十字弓的同板块/相邻板块范围过滤；攻击后会把 `haunt-attack` 记录进本回合已用来源，并拦截同一玩家本回合再次普通攻击；攻击结算后回到同一玩家牌桌继续可操作，不自动结束回合。旧“弩视线攻击”口径已按 TTS 图包牌面降级为历史证据，当前只证明首剧本普通攻防、武器声明、枪视线、十字弓相邻、魔法相机幻影摄影师和 12 号巨魔手代表链，不代表完整怪物系统或全部作祟攻击时机完成 | `HAUNT_ATTACK`, `usedCardIdsThisTurn`, `resolveBetrayalAttackTargetPlayerIds`, `teamModel`, `lineOfSightGraph` | 攻击动作入口 / 目标高亮 / 视线线条 / 武器选择面板 | 首剧本攻防 / 武器声明 / 枪视线 / 十字弓相邻 / 魔法相机 / 大宅饿了领域和 Board 代表链；旧弩视线 E2E 只能作为历史 UI 证据，不再作为当前十字弓规则依据 | `implemented-needs-remodel` |
...
| 19.1 武器 | 每次攻击最多一件；可选择不用；攻击后本回合不能交易；刚获得不能用；不能防御 | 已补攻击武器状态读模型：攻击面板保留所有已识别攻击武器，区分可用、刚获得不可用、已使用不可用，并显示禁用原因；徒手默认仍可选；可用武器仍能进入地图目标选择并完成攻击结算。当前只证明徒手 / 砍刀 / 匕首 / 指环 / 枪 / 十字弓 / 电锯等代表武器和禁用原因表达，不等于所有武器牌完整录入、完整攻击声明 interaction 或全部作祟攻击完成 | `AttackDeclarationInteraction`, `BetrayalAttackWeaponCardStatus`, `resolveAttackWeaponCardStatuses`, `turnStartInventoryCardIds`, `usedCardIdsThisTurn` | 武器选择面板保留不可用武器本体 + 禁用原因短提示 | 领域 / Board 定向测试 + 历史真实入口 E2E；当前新增枪 / 十字弓 / 电锯为最小领域和组件证据，未跑 E2E | `implemented-needs-remodel` |
| 19.2 视线 | 同方向且不换区域可视线，中间可隔房间；房间内所有角色都在视线内 | 已补基础读模型，按同楼层 / 同一直线 / 连续已发现板块判断房间视线；当前牌面合同下只有枪使用视线范围，十字弓使用同板块或相邻板块范围且不画视线线，近战和徒手仍限同房间；魔法相机幻影摄影师仍可攻击视线内英雄。旧“弩可高亮并连线视线内非同房间叛徒”的证据已降级为历史 UI 证据，不再作为当前十字弓规则依据；仍需完整怪物系统、所有怪物视线攻击和更多真实远程牌面组合 | `lineOfSightGraph` / `resolveBetrayalLineOfSightRoomIds`, `resolveBetrayalAttackTargetPlayerIds`, `attackWeaponRangeKind` | 目标高亮 / 视线线条（枪和怪物视线） | 基础视线单测已覆盖；当前领域测试覆盖枪可攻击视线目标、十字弓拒绝仅视线非相邻目标并允许相邻目标；Board 组件测试覆盖枪画视线线、十字弓相邻目标高亮且不画视线线；未在本轮跑 E2E | `implemented-needs-remodel` |
...
| 22 计数轨 / 参考资料 | 某些作祟用 0-9 计数轨记录时间 / 进度 / 力量等；玩家参考卡、作祟书和怪物参考卡必须按阶段与阵营控制权限 | 已补通用 `numberTracks[]` 读模型原语：作祟前 `haunt-risk` 表达预兆状态，作祟 1 / 3 / 33 代表运行态分别派生驱魔圈、研究 token、摄影师 / 相机、Essence 进度，并标记 `representativeOnly`；新增 `resolveBetrayalReferenceCardAccess` 参考资料权限读模型，作祟前只开放基础参考卡，公开叛徒作祟区分英雄书 / 叛徒书，隐藏叛徒作祟不开放叛徒书，有怪物运行态才开放怪物参考卡。仍缺实体指针、完整配件 catalog、玩家可调整计数轨命令、参考资料 UI 接线和 50 个作祟逐条合同 | `numberTracks[]`, `resolveBetrayalReferenceCardAccess` | 牌堆区预兆状态进度条；后续作祟目标条 / 计数轨 / 参考资料入口权限 | 领域单测覆盖预兆状态轨、作祟目标代表轨和参考资料权限；Board 组件测试覆盖牌堆区进度条；参考资料 UI 接线仍需后续补测 | `implemented-needs-remodel` |
...
| 27 死亡 | 作祟后才会死亡；死亡时立牌倒在房间变尸体，持有物保留在角色板旁，可被搜刮 | 已补死亡状态读模型：`resolveBetrayalDeathStateSummary` 会表达作祟死亡规则是否启用、存活 / 死亡探索者、尸体所在房间、尸体立牌应倒伏、尸体保留的物品 / 预兆、灰尘死亡叛徒转狂热病患时物品 / 预兆被掩埋且不可搜刮、同房间当前探索者是否可搜刮，以及同一尸体本回合已搜刮后的禁用状态。当前只证明基础尸体搜刮和灰尘死亡叛徒掩埋例外，仍缺完整死亡 UI 倒伏表现、所有作祟死亡变体、特殊尸体用途和变怪物 / 特殊胜负接入 | `deathState`, `resolveBetrayalDeathStateSummary`, `corpseInventory`, `corpseLootedByPlayerIdsThisTurn` | 倒伏角色 / 尸体 marker / 搜尸选择器 | 死亡状态 / 基础搜尸领域测试 + 灰尘死亡叛徒物品 / 预兆掩埋领域测试；基础搜尸 E2E 已覆盖尸体可搜刮；完整死亡链仍需补测 | `implemented-needs-remodel` |
| 27 终局 | 首个达成作祟目标的阵营获胜，朗读该方 If You Win | 已补代表终局结果和终局读模型：首剧本英雄驱魔成功 / 叛徒击倒全部英雄、灰尘英雄治愈 / 疾病全员转叛徒、魔法相机英雄砸毁相机 + 摄影师全灭 / 叛徒夺取全部本质均能进入 `phase=endgame` 并写入胜方、获胜者和剧本标识；`resolveBetrayalEndgameReadModel` 会暴露胜方、赢家、`ifYouWinTextId`、If You Win 文本状态，并区分灰尘场景专属同时政策与其它作祟同时达成 / 平局政策缺口，避免通用胜负页冒充逐作祟结局；灰尘英雄 / 叛徒 If You Win 已接入官方英文原文和中文正式翻译稿，首剧本等其它代表作祟仍保留原文缺口。当前只证明代表作祟终局读模型和灰尘 If You Win 接入，不代表全部 50 个作祟 If You Win 原文、同时完成 / 平局处理或 12 号完整终局完成 | `phase=endgame`, `endgameResult.hauntId`, `endgameResult.outcome`, `endgameResult.winners`, `endgameReadModel.ifYouWinTextStatus`, `endgameReadModel.simultaneousCompletionPolicyStatus`, `sys.gameover` | 终局页 + 剧本文本回看；终局页按文本状态显示官方来源或摘要来源，不能提前把缺口作祟标为原文 | 首剧本、灰尘、魔法相机终局领域测试 + 终局读模型定向测试 + 首剧本 / 灰尘终局 Board 测试；后续需逐作祟终局和 If You Win 文本证据 | `implemented-needs-remodel` |
...
> 怪物移动 / 攻击补充（作祟 3）：狂热病患已有真实入口代表链，证明死亡叛徒自然进入速度 5 移动骰、返回牌桌后点击真实相邻房间移动、移动后交接给下一名玩家，并可从怪物动作槽选择同房英雄进入攻击骰盘。证据见 `evidence/betrayal-the-dust-feverish-natural-monster-turn/e2e-test.md`。这仍只覆盖作祟 3「灰尘」狂热病患自然怪物回合代表链，不代表灰尘完整感染交换、完整多分支终局或其它作祟自然怪物回合完成。

> 伤害分配补充（作祟 3）：灰尘回合末未交换疾病代表链已证明当前探索者本回合没有交换疾病标记时，结束回合进入“灰尘冲动”2 点一般伤害分配；玩家选满属性并确认后才交给下一名玩家；隐藏叛徒因该伤害死亡时，确认分配后才变成狂热病患。证据见 `evidence/betrayal-the-dust-end-turn-damage-allocation/e2e-test.md`。这仍只覆盖该伤害分配切片，不代表感染交换全排列或完整多分支终局完成。

> 房间伤害致死补充（作祟 3）：火炉房回合末物理伤害和倒塌房间失败坠落伤害这类非攻击房间伤害，作祟后也会进入玩家伤害分配；永久叛徒确认分配并死亡后会生成狂热病患。火炉房真实入口截图证明伤害来源为“火炉房”、确认前不死亡不生成狂热病患，确认后狂热病患 token 出现在火炉房且不误触发终局。证据见 `evidence/betrayal-the-dust-room-damage-death/e2e-test.md`。这仍只覆盖房间伤害致死代表链，不代表其它死亡保护组合、其它伤害来源或死亡叛徒怪物化全排列完成。

> 头骨死亡保护补充（作祟 3）：灰尘冲动一般伤害分配到骷髅后，头骨成功 / 失败均有真实入口代表链；成功分支显示头骨死亡保护骰盘后阻止死亡，不生成狂热病患；失败分支在玩家确认骰盘后才生成狂热病患并交接下一名玩家。领域矩阵另证明永久叛徒面对物理攻击伤害、精神攻击伤害、一般伤害三类待分配伤害时，头骨成功均阻止死亡且不生成狂热病患，头骨失败均正常死亡并生成狂热病患。头骨失败后的兔脚重掷已有领域代表组合：成功回滚死亡和狂热病患化，仍失败保持死亡和狂热病患化；永久叛徒最终死亡转狂热病患时，物品 / 预兆会掩埋回牌堆底部，尸体不可搜刮。证据见 `evidence/betrayal-the-dust-skull-death-prevention/e2e-test.md`。这仍只覆盖灰尘冲动真实入口、三类伤害领域矩阵和头骨 + 兔脚代表组合，不代表灰尘完整作祟、全部死亡保护组合全排列、其它具体伤害来源或死亡叛徒怪物化全排列完成。

> 疾病交换补充（作祟 3）：灰尘回合末同房强制交换代表链已证明当前探索者结束回合时，会逐个与多名同房探索者随机交换 1 个疾病标记；多次交换按最新疾病标记状态顺序结算，避免第二次交换仍引用旧 token；发生同房强制交换后不会触发“灰尘冲动”伤害分配；编号 1 的中间持有人和最终持有人都会永久成为叛徒；新玩家回合开始后本回合交换记录清空；若该交换后所有存活探索者都永久感染，真实终局页会进入灰尘叛徒胜利。证据见 `evidence/betrayal-the-dust-forced-sickness-exchange/e2e-test.md`。这仍只覆盖结束回合同房强制交换和该全员感染终局代表链，不代表完整多分支终局完成。

> 失败行动交换补充（作祟 3）：灰尘寻找治愈线索失败代表链已证明行动玩家在预兆房间点击“寻找解药”后，知识检定失败会跳过死亡玩家，与左侧下一名存活玩家随机交换 1 个疾病标记；治愈灰尘失败代表链已证明会按研究 token 加值计算失败，并复用同一条左侧存活玩家随机交换逻辑；编号 1 流入行动玩家后永久感染，且本回合已交换记录会阻止结束回合再触发“灰尘冲动”伤害分配；若寻找治愈线索失败交换后所有存活探索者永久感染，真实终局页进入灰尘叛徒胜利；若治愈灰尘失败交换后所有存活探索者永久感染，真实终局页进入灰尘叛徒胜利。证据见 `evidence/betrayal-the-dust-failed-action-sickness-exchange/e2e-test.md`。这仍只覆盖寻找治愈线索失败真实入口、寻找失败和治愈失败全员感染真实终局，不代表完整多分支终局完成。

> 控制冲动补充（作祟 3）：灰尘主动“控制冲动”代表链已证明行动玩家必须从真实牌桌点击同房探索者 token 发起疾病交换请求，并等待目标玩家同意 / 拒绝；目标同意后，系统随机交换双方 1 个疾病标记，编号 1 流入发起方后使其永久成为叛徒，并记录双方本回合已交换；目标拒绝后等待态清空，双方疾病标记不变，永久叛徒名单不变，也不记录本回合已交换；若同意后所有存活探索者都已永久感染，真实终局页会进入灰尘叛徒胜利。证据见 `evidence/betrayal-the-dust-control-impulses-sickness-exchange/e2e-test.md`。这仍只覆盖控制冲动同意 / 拒绝和该全员感染终局代表链，不代表其它交换来源、其它伤害来源或死亡事件的全排列完成。

> 疾病隐私补充（作祟 3）：灰尘疾病编号 / 隐藏叛徒代表链已证明不同玩家视角只显示本人疾病编号和本人是否永久感染；牌桌原始状态仍保留完整疾病编号和永久叛徒名单，但 Board 展示先走玩家视图遮蔽；死亡保护回看里的旧灰尘状态也按同一玩家视图遮蔽，不能通过回看泄露他人编号或永久感染名单。证据见 `evidence/betrayal-the-dust-sickness-privacy/e2e-test.md`。这仍只覆盖疾病编号和永久感染私密可见代表链，不代表完整多分支终局完成。

> 研究 / 治愈属性选择与 If You Win 补充（作祟 3）：灰尘研究与治愈成功代表链已证明玩家从真实牌桌点击“寻找解药”，成功后研究进度从 0 处变为 1 处、对应房间板块出现“研”研究标记，并切出“治愈灰尘”主动作；寻找解药全排列已证明知识 / 神志两种合法属性均可成功放置研究标记；治愈灰尘代表链已证明系统只默认高亮最高合法属性，不自动替玩家决定，玩家可手动选择力量 / 速度 / 知识 / 神志；三处研究标记会在进度条和房间板块上同时显示，并在领域层按 +6 加值进入英雄胜利终局；灰尘英雄 / 叛徒 If You Win 文本合同已标记可用，英雄胜利真实入口会先进入“官方 If You Win 原文 / 正式翻译”的独立结局朗读幕，再进入结果报告；领域层另证明灰尘终局读模型标记为场景专属事件优先政策。证据见 `evidence/betrayal-the-dust-research-and-cure-success/e2e-test.md`。这仍只覆盖研究计数、房间板块研究标记、寻找解药知识 / 神志成功全排列、治愈四属性选择截图、速度代表链、三研究标记 +6 加值、英雄胜利代表链、灰尘 If You Win 文本接入和事件优先政策，不代表完整多分支终局回归。

### 2.1 本轮灰尘死亡来源补充

- 作祟 3「灰尘」已补选择属性事件一般伤害致死代表链：作祟阶段的事件选择结算可继续处理事件一般伤害，玩家可把伤害分配到骷髅；灰尘永久叛徒因此死亡时生成狂热病患，并在事件结算后进入灰尘叛徒胜利检查。
- 同步补齐选择属性事件一般伤害触发头骨死亡保护成功 / 失败代表链：头骨成功时把探索者拉回濒死，不死亡、不生成狂热病患，也不提前触发灰尘叛徒胜利；头骨失败时会先建立死亡保护骰盘和回看窗口，再进入死亡 / 狂热病患化，因此兔脚仍能重掷刚刚这次死亡保护。
- 固定属性即时事件一般伤害补充：若事件直接扣属性并扣到骷髅，系统也会建立头骨死亡保护骰盘；头骨失败后才生成死亡 / 狂热病患化，兔脚重掷成功会回滚死亡和狂热病患化。
- 掷骰事件物理 / 精神伤害补充：事件先掷出物理或精神伤害点数，若扣到骷髅，系统同样建立头骨死亡保护骰盘；头骨失败后才生成死亡 / 狂热病患化，兔脚重掷成功会回滚死亡和狂热病患化。
- 复合事件内嵌一般伤害补充：顶层事件为复合效果、子效果包含固定属性一般伤害时，系统会递归识别内嵌伤害并建立头骨死亡保护骰盘；头骨失败后才生成死亡 / 狂热病患化，兔脚重掷成功会回滚死亡和狂热病患化，同时保留复合事件其它已结算副作用。
- 直接失去属性 / 全属性检定失败补充：事件不是“伤害”字样、而是直接失去属性时，也会进入同一死亡保护链；灰尘永久叛徒因直接失去属性或「吊死鬼」全属性检定失败扣到骷髅时，头骨失败后才死亡并生成狂热病患，兔脚重掷成功可回滚死亡和狂热病患化。
- 脑状食品真实牌面分支补充：脑状食品是当前事件池里同时登记“伤害 + 直接失去属性”的复合风险牌；已补真实中档“获得 1 点速度并失去 1 点神志”分支，证明灰尘永久叛徒失去神志扣到骷髅时会进入头骨死亡保护，头骨失败后才死亡并生成狂热病患，兔脚重掷成功可回滚死亡和狂热病患化。
- 真实直接失去属性事件分支补充：外星几何、蜘蛛！、一种怪异的感觉、葬礼 4 张真实事件牌已补逐牌行为断言；各自失去速度 / 神志 / 力量 / 神志扣到骷髅时，都会进入头骨死亡保护，头骨失败后才死亡并生成狂热病患。
- 真实选择型直接失去属性事件分支补充：夜幕众星、一瓶微尘、一条秘密通道 3 张事件已补逐牌行为断言；选择或目标结算后失去速度 / 力量 / 神志扣到骷髅时，都会进入头骨死亡保护，头骨失败后才死亡并生成狂热病患。至此当前直接失去属性 / 全属性检定失败分类内 9 张事件均有真实或代表分支断言。
- 真实一般伤害事件分支补充：标本剥制、小丑房间、咬一口！、磁带播放器、在你背后！5 张事件已补逐牌行为断言；固定一般伤害扣到骷髅时都会进入头骨死亡保护，头骨失败后才死亡并生成狂热病患。
- 真实掷骰伤害和复合 / 选择伤害事件分支补充：电话铃声、小机器人、最深的壁橱、肉质苔癣、一抹鲜红、脑状食品、上古旧宅的真实伤害分支已补逐牌 / 逐分支断言；扣到骷髅时都会进入头骨死亡保护，头骨失败后才死亡并生成狂热病患。至此当前 23 张事件池内死亡相关事件均有分类守卫和真实/代表死亡保护断言。
- 验证：`npx vitest run src\games\betrayal\__tests__\firstScenarioRuntime.test.ts --testNamePattern "灰尘复合事件" --configLoader native --pool threads --no-file-parallelism --maxWorkers 1`，2 passed / 315 skipped；扩大代表回归 `npx vitest run src\games\betrayal\__tests__\firstScenarioRuntime.test.ts --testNamePattern "灰尘复合事件|灰尘掷骰事件物理伤害|灰尘掷骰事件精神伤害|灰尘固定属性事件一般伤害|事件一般伤害.*头骨失败|事件一般伤害头骨失败|兔脚重掷.*狂热病患|兔脚重掷电话铃声|兔脚重掷小机器人" --configLoader native --pool threads --no-file-parallelism --maxWorkers 1`，15 passed / 302 skipped。
- 追加验证：`node scripts/infra/vitest-cli-safe.mjs run src/games/betrayal/__tests__/firstScenarioRuntime.test.ts --config vitest.config.core.ts --configLoader native --pool forks --no-file-parallelism --maxWorkers 1 -t "灰尘直接降属性事件|灰尘吊死鬼"`，3 passed / 317 skipped；`node scripts/infra/vitest-cli-safe.mjs run src/games/betrayal/__tests__/firstScenarioRuntime.test.ts --config vitest.config.core.ts --configLoader native --pool forks --no-file-parallelism --maxWorkers 1 -t "灰尘脑状食品"`，2 passed / 323 skipped；`node scripts/infra/vitest-cli-safe.mjs run src/games/betrayal/__tests__/firstScenarioRuntime.test.ts --config vitest.config.core.ts --configLoader native --pool forks --no-file-parallelism --maxWorkers 1 -t "灰尘真实直接失去属性事件"`，4 passed / 325 skipped；`node scripts/infra/vitest-cli-safe.mjs run src/games/betrayal/__tests__/firstScenarioRuntime.test.ts --config vitest.config.core.ts --configLoader native --pool forks --no-file-parallelism --maxWorkers 1 -t "灰尘真实选择型直接失去属性事件"`，3 passed / 329 skipped；`node scripts/infra/vitest-cli-safe.mjs run src/games/betrayal/__tests__/firstScenarioRuntime.test.ts --config vitest.config.core.ts --configLoader native --pool forks --no-file-parallelism --maxWorkers 1 -t "灰尘真实一般伤害事件"`，5 passed / 332 skipped；`node scripts/infra/vitest-cli-safe.mjs run src/games/betrayal/__tests__/firstScenarioRuntime.test.ts --config vitest.config.core.ts --configLoader native --pool forks --no-file-parallelism --maxWorkers 1 -t "灰尘真实掷骰伤害事件"`，6 passed / 337 skipped；`node scripts/infra/vitest-cli-safe.mjs run src/games/betrayal/__tests__/firstScenarioRuntime.test.ts --config vitest.config.core.ts --configLoader native --pool forks --no-file-parallelism --maxWorkers 1 -t "灰尘真实复合/选择伤害事件"`，4 passed / 343 skipped；当前 23 张事件池组合回归 28 passed / 319 skipped；ESLint 0 errors / 5 existing warnings。
- 边界：该补充证明当前 23 张事件池内死亡相关事件均有分类守卫和真实/代表死亡保护断言，但不代表未来新增事件、非事件伤害来源、死亡保护组合全排列或死亡叛徒怪物化路径全排列。
...
| 作祟 3 | 子账本已补齐；已有狂热病患自然怪物回合代表链，覆盖死亡叛徒自然进入速度 5 移动骰、真实房间移动、攻击同房英雄和回合交接；已有未交换疾病回合末 2 骰一般伤害分配代表链，覆盖玩家确认分配后交接和隐藏叛徒确认分配后才变狂热病患；已有火炉房 / 倒塌房间等房间伤害致死代表链，覆盖非攻击房间伤害作祟后也进入玩家分配、永久叛徒确认死亡后生成狂热病患；已有选择属性事件一般伤害致死、头骨成功 / 失败和头骨失败后兔脚成功 / 仍失败代表链，覆盖事件选择结算后分配到骷髅、死亡保护窗口先于灰尘狂热病患化保留、兔脚成功回滚死亡和狂热病患化、仍失败保持死亡 / 狂热病患化；已有固定属性即时事件一般伤害头骨失败 / 兔脚成功代表链，覆盖直接扣到骷髅时先建立死亡保护骰盘、头骨失败后再进入死亡 / 狂热病患化、兔脚成功回滚死亡和狂热病患化；已有掷骰事件物理 / 精神伤害头骨失败 / 兔脚成功代表链，覆盖先掷出物理或精神伤害点数、扣到骷髅时建立死亡保护骰盘、头骨失败后再进入死亡 / 狂热病患化、兔脚成功回滚死亡和狂热病患化；已有复合事件内嵌一般伤害头骨失败 / 兔脚成功代表链，覆盖顶层复合效果递归识别子效果伤害、头骨失败后再进入死亡 / 狂热病患化、兔脚成功回滚死亡和狂热病患化并保留其它已结算副作用；已有灰尘冲动一般伤害触发头骨死亡保护成功 / 失败代表链，覆盖分配到骷髅后才掷死亡保护、成功不死亡不生成狂热病患、失败确认后才生成狂热病患并交接；并已有物理 / 精神 / 一般三类待分配伤害触发头骨成功 / 失败后的狂热病患化领域矩阵；已有头骨失败后兔脚重掷成功 / 仍失败代表组合，覆盖成功回滚死亡和狂热病患化、仍失败保持死亡和狂热病患化、常规怪物回合仍不能主动使用兔脚；已有死亡叛徒物品 / 预兆掩埋代表链，覆盖永久叛徒最终死亡转狂热病患时物品和预兆回到对应牌堆底部、尸体不可搜刮；已有回合末同房强制交换代表链，覆盖多名同房探索者逐个随机交换、避免旧 token 预览跳过后续交换、不进入冲动伤害、编号 1 中间 / 最终持有人永久感染，并已有同房强制交换后所有存活者永久感染时进入灰尘叛徒胜利终局的真实入口代表链；已有寻找治愈线索失败 / 治愈灰尘失败交换代表链，覆盖左侧存活玩家随机交换、编号 1 永久感染和交换后不触发冲动伤害，并已有寻找治愈线索失败后所有存活者永久感染时进入灰尘叛徒胜利终局的真实入口代表链，以及治愈灰尘失败后所有存活者永久感染时进入灰尘叛徒胜利的真实入口代表链；已有控制冲动同意 / 拒绝代表链，覆盖同房目标点击、目标同意随机交换、目标拒绝不交换，并已有控制冲动同意后所有存活者永久感染时进入灰尘叛徒胜利终局的真实入口代表链；已有疾病编号 / 永久感染私密可见代表链，覆盖不同玩家只看本人疾病编号、永久感染状态不公开名单、死亡保护回看不泄露旧编号；已有研究成功 / 治愈成功代表链，覆盖研究进度、房间板块研究标记、治愈主动作、寻找解药知识 / 神志成功全排列、治愈四属性选择截图、速度代表链、三研究标记 +6 加值、英雄胜利终局、灰尘英雄 / 叛徒 If You Win 文本合同和灰尘场景专属事件优先政策；仍不能等同于灰尘完整作祟完成 | 后续实现继续检查死亡叛徒怪物化路径回归、死亡保护组合全排列和其它具体伤害来源 |
...
1. 作祟映射表覆盖所有剧本卡 + 预兆。
2. 作祟揭示公开步骤和秘密可见性：已补代表读模型、轻量揭示提示、灰尘隐藏叛徒口径、setup 队列只留在领域状态不进入玩家前景、参考资料权限读模型、setup 命令预览合同、灰尘两个 setup 人工确认项的正式命令和牌桌按钮，以及“揭示期单主焦点 / 返回牌桌后才恢复作祟进度、主动作、攻击或交换提示、武器选择和对应无障碍状态”的 UI 边界；仍需其它作祟 setup 确认命令、段落级可见性、参考资料 UI 接线和逐作祟公开设置。
3. 阵营模型支持四类作祟。
4. 怪物行动、击晕、移动和特殊能力。
...
- 禁止只在日志里表达预兆状态、牌堆最后一张预兆、计数轨或作祟目标；也禁止把作祟检定公式常驻到主 UI。
