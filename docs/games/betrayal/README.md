...
10. 当前正式发现池已经扩到 43 间房、11 张物品、9 张预兆，并首批接入 7 张已锁定且当前引擎可完整表达的官方事件；后续重点不再是“是否只有代表池”，而是继续审计每间房的门位 / 朝向 / 楼层适用性 / 房间效果、每张物品 / 预兆 / 事件的卡牌效果，以及房间背面与楼层板资源合同。事件牌不能再用旧占位事件凑数；未接入的 locked 事件需要先补对应引擎能力。
