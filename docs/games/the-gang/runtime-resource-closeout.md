...
- 再审计结论：The Gang 基础版素材矩阵中的牌面源图旧 `blocked/不接入` 口径已纠正；主 UI 已按 BGG 电子版 DOM/CSS/结算参考完成布局合同抽取、少框 Board 对照实现、真实页面 E2E、PureRef 看图和 AI 复看截图。
...
| The Gang BGG 电子版布局合同进入运行时 | 通过 | `layout-source-contract.md`、`Board.tsx` 的 `data-layout-contract="bgg-electronic"`、`data-bgg-zone="top-zone"`、`token-pile`、`card-river`、`bottom-zone`、`Board.runtime.test.tsx` 布局断言、2026-07-05 17:05 最新 E2E 截图 |
