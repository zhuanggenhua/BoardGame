...
- 当前游戏专项答案：The Gang 首教闭环落在“读底牌牌力 -> 拿筹码表达排名 -> 公共牌改变判断 -> 红筹码锁定最终排序 -> 摊牌结算”。
...
| 摊牌比较真实牌力顺序和红筹码顺序 | `reveal-showdown`、`showdown` | `摊牌` 按钮、摊牌结果区 | 已覆盖，玩家真实执行 `REVEAL_SHOWDOWN` | `教程摊牌结果反馈.jpg` |
...
| 查看摊牌结果 | 摊牌后结果区 | `the-gang-showdown-area` | 教程气泡只解释反馈 | 真实可用 |
