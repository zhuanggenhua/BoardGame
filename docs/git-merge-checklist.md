...
- 对共享骨架、通用 UI、规则/校验、测试断言、i18n key、类型定义，以及 **规则/规范文档（AGENTS.md、docs/ai-rules/**、.agent/rules/**、docs/git-merge-checklist.md）** 等高风险区域，默认按“**双方内容都可能有效**”处理，优先做块级合并而不是整份覆盖。
...
当冲突文件或大幅 diff 命中以下路径/关键词时，**必须额外做专项审查**，禁止只靠编译/merge audit 收口：

- `src/games/*/Board.tsx`
- `src/games/*/ui/**/*.tsx`
- `src/components/game/framework/**`
- `src/components/**/Hand*`
- 文件名包含 `Prompt` / `Overlay` / `HUD`

专项审查要求：
1. 三方对比（base / ours / theirs）+ 最近 2~3 次相关提交对比
2. 明确区分“PR 目标改动”与“被顺带带回/覆盖的旧逻辑”
3. 至少做 1 条关键交互回归验证（手牌选中、Prompt 进出、基地/随从点击等）

#### Silent merge regression 审计（强制）

即便 **没有冲突**，对上述高风险文件也必须做“父提交偏斜”检查：

```bash
# 查看 merge commit 相对双父的偏斜程度
git show -m --stat <merge-commit> -- path/to/high-risk-file
```

若出现“对父 A 改动巨大、对父 B 几乎无改动”，**必须人工复核是否静默回退/误覆盖**，并在冲突汇报文档中登记。
...
```bash
rg -n "^(<<<<<<<|=======|>>>>>>>)" AGENTS.md docs .agent src e2e
```
...
**若涉及 Board/UI/交互文件（高风险 UI 列表命中）**，至少补以下最小回归清单：
- [ ] 手牌选中/取消选中正常（选中态与 hover 态不互相替代）
- [ ] Prompt/Overlay 能正常进入与退出
- [ ] 基地/随从点击链路可达且不被意外阻断
- [ ] 共享 UI 样式未被旧逻辑覆盖
- [ ] 若改到游戏 Board/UI，至少跑 1 条对应 E2E 或最小代表性验证
