...
- **交付边界**：用户要“设计稿 / 视觉稿 / 效果图”时，默认交付物仍是 PNG / JPG / WebP 图片证据；imagegen 可用时走位图生图，imagegen 不可用且 Open Design 可用时自动转 Open Design artifact 候选稿。本项目固定采用“Open Design artifact + 固定视口浏览器渲染截图 + AI 图面核验 + 用户验收展示”的交付模式；未审计的 Open Design artifact、HTML 预览、产品运行页截图或 reference sheet 只能是工具产物 / 内部校准物，不能直接冒充设计稿。用户展示通道统一按项目 `.spec/skills/show-image-to-user/SKILL.md` 执行。
...
- **人工验收顺序**：候选图必须先经过 AI 图面核验，确认规则、素材、少边框和可复刻门禁均为 PASS 后，才允许进入用户验收展示；具体展示按项目 `.spec/skills/show-image-to-user/SKILL.md` 执行。
...
3. 所有用户可见设计改动必须先走 AI 图面核验；核验通过后再按项目 `.spec/skills/show-image-to-user/SKILL.md` 展示给用户。
