...
**默认模式（`npm run test:e2e -- e2e/<相关文件>.e2e.ts`）100% 安全**：
...
1. **日常测试**：使用 `npm run test:e2e -- e2e/<相关文件>.e2e.ts` 或 `npm run test:e2e -- --grep "<相关用例名>"`（完全隔离，自动启动测试服务器）
2. **测试前检查**：运行 `npm run test:e2e:check` 确认隔离状态
3. **测试后清理**：如果测试异常退出，运行 `npm run test:e2e:cleanup` 清理测试环境端口
4. **明确全量**：只在确实需要完整套件时使用 `npm run test:e2e:all`
5. **开发调试**：如需调试测试代码，可设置 `PW_USE_DEV_SERVERS=true` 使用开发服务器
6. **大量测试**：使用 `npm run test:e2e:parallel -- --grep "<相关用例名>"`（并行执行，每个 worker 独立端口）
7. **同时开发和测试**：可以同时运行 `npm run dev` 和定向的 `npm run test:e2e -- ...`，完全不冲突
