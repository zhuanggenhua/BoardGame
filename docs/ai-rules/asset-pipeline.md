...
1. **公开资源域名和协作者入口保持不变**：正式资源仍上传到 R2，运行时仍使用 `https://assets.easyboardgame.top/official/...`。不得要求协作者改成服务器 IP、隐藏源域名或另一套上传命令。
2. **R2 是正式对象真相源**：对象的 key、内容、哈希、元数据和文件索引以 R2 为准。服务器 `/home/admin/storage/assets/current` 只是由 R2 同步得到的只读派生镜像，不接受协作者直接上传。
3. **服务器命中不替代 R2 上传**：即使远端请求返回 `X-Asset-Source: server`，仍必须确认对应 R2 对象和索引发布成功。服务器命中只能证明镜像可用。
4. **服务器不可用时必须能返回同一对象**：普通素材在源站失败、超时、限流或缺失时会回退 R2。禁止只把新资源放进服务器镜像，否则会形成故障时 `404` 的半发布状态。
5. **大型包永远不进入服务器镜像**：`official/app-updates/**`、`official/mobile-packages/**`、`official/native-app-updates/**` 永久绕过服务器源并直接走 R2。
6. **上传前执行零付费容量门禁**：现有上传入口必须先计算 R2 当前用量和本批净增量；预计超过 9GiB 时，在写入第一个对象前失败。不得退化为服务器单边发布。
