...
优先级：① 用户当前对话给出的**卡牌/技能/token 原文**（含清晰截图或原文描述）→ ② 卡牌/技能/token 自身描述（卡面/提示板/真相源图片）→ ③ `rule/*.md` 规则文本 → ④ 历史实现/旧裁定。若网页文本、录入字段、实现描述或旧测试与清晰卡图/截图冲突，或仍存在不确定性，以卡图/清晰截图为准。**禁止用 i18n JSON、AbilityDef.description、代码注释作为权威输入**——这些是实现产物，可能带着错误理解。
