...
- **规则 bug 第一门禁是录入合同实现核对（强制）**：当用户反馈“规则不对 / 低级和高级不一样 / 不是这个等级能做 / 你审计漏了 / 为什么回归”时，第一步不是直接重看图，而是先找当前仓库已确认的录入合同、规则原文矩阵、evidence 子句表、user story 或已锁定规则文档，并判断“录入数据是否被实现正确消费”。旧单测、旧 E2E、旧实现、旧注释、旧 i18n 文案只能作为“当前系统如何运行或如何错”的证据，不能单独证明规则正确；若实现与已录入合同冲突，先按录入合同修实现并把旧测试降级为失效证据。
- **录入数据与反馈冲突时才回图面/规则源复核（强制）**：只有当已录入合同本身缺失、状态不是 locked、录入内容与用户 bug 描述冲突，或同一对象存在多个互相矛盾的合同来源时，才回到卡图/规则图、规则书、OCR 或重新裁图复核。此时结论只能写“规则合同未锁定/需复核图面”，不得直接改实现；复核后必须先更新录入合同或 evidence 子句表，再按新合同修代码。不得用“测试原来这么写”“实现一直这样”“同家族高级版是这样”绕过录入合同核对。
- **规则修复必须同步回写录入合同（强制）**：只要本轮确认“录入合同/规则描述/i18n 文案/evidence 子句表”与真实规则不一致，修复范围必须同时包含这些真相入口；不得只改 handler、reducer、测试或注释后交付。收口前必须反查对象中文描述、英文描述、能力定义注释、升级版描述、相关 evidence 是否仍保留旧错误语义；只要旧错误描述还存在，就只能写“实现已临时修正，规则合同未收口”，不得写“规则 bug 已修复/回归已收口”。
...
- **新增批次对象默认按全面审计执行（强制）**：当任务语义是“新增派系 / 新英雄 / 新角色 / 新对象接入 / 从素材做到可玩”时，默认交付就包含**对象级全面审计 + evidence 留档**。除非用户明确把范围缩成“只录入 / 只修一个 bug / 只做静态接入”，否则不得再追问“要不要补审 / 要不要留档 / 要不要全面审计”。
...
- **不可防御必须反查“共享防御消费合同”而不只看 damage payload（强制，通用）**：当描述出现“不可防御伤害 / 不可阻挡 / skip defense / 不进入防御阶段”，审计不能只看到 `damage(unblockable=true)`、日志里 `unblockable: true`，或某条局部伤害事件就收口。必须同时反查：① 攻击建立时 `isDefendable` 从哪里决定（如 ability/variant tags、shared helper、choice effect patch、`ATTACK_MADE_UNDEFENDABLE`）；② 真实流程里是否真的不会打开防御窗口；③ 若是“选择后才转为不可防御”，必须验证选择解决前后时序都符合合同。缺任一项，不得写“不可防御已实现”。
...
12. **回归扩审**：完成修复后按“同类扩审”规则扫同类入口，记录命中点与未命中理由。若暂未扩审完，最终口径只能写“当前点位已修复，仍有残余扩审范围”，不得写“已收口”。
...
> 所有审查/审计/新增功能验证时按维度检查。D1-D10 为原有维度（已扩展），D11-D55 为补充维度。
...
| D55 | **共享合同多消费者一致性** | 同一业务合同若被 validator、候选生成、reaction session、UI 可见态、AI legal-actions、auto-continue/watchdog 等多层共同消费，修复或审计时是否只打穿了其中一层？**触发条件**：新增/修改 `validate(...)`、`canAdvancePhase`、`buildReactionOptions`、`optionsGenerator`、`canActivate*`、窗口可用态、`scoreBases`/`responseWindow`/`afterScoring` 共享 helper，或修“命令已放行但 UI 没入口 / prompt 里没有选项 / AI 提前结束阶段 / watchdog 误关窗 / auto-continue 提前推进”类 bug 时必查。**核心原则**：共享合同不能只在一层为真。凡某条规则需要多个消费者共同表达，必须先找出唯一权威 helper/快照，再逐层确认所有消费者都读这份真相；只修 validator 而 reaction/AI/auto-continue 仍读旧字段，等于没有真正收口。**审查方法**：① 画出共享合同消费者清单：validator、候选生成、interaction/reaction prompt、UI 高亮、AI legal-actions、phase gate、watchdog/auto-continue、真实命令执行 ② 标出每层实际读取的字段/helper，查是否仍有旁路读取旧 `eligibleBaseIndices/currentPlayer/baseIndex` 等近似值 ③ 至少补两类回归：一条锁命令/validator，一条锁上层消费者（如 live reaction/AI/auto-continue）④ 若用户反馈是“整轮都开不了/看不到入口/AI 直接跳过”，必须把上层消费者证明打到 L4，不能只用 L2 validator 代替。**典型缺陷模式**：❌ `ACTIVATE_SPECIAL` validate 已允许 afterScoring 当前结算基地，但 `buildReactionOptions()` 仍按 `eligibleBaseIndices` 过滤，prompt 里没有 special ❌ validator 已修，`canAdvancePhase()` / AI 仍按旧 eligible 语义判断“无动作可做”，直接给出 `advance-phase` ❌ UI 高亮走 `canActivateSpecial`，命令校验走另一套 helper，导致“看起来能点，点了失败”或“命令能过，但入口被藏掉”。**修复策略**：① 抽共享 helper 表达唯一真相 ② 把所有消费者改为复用该 helper/快照 ③ 增加跨层合同测试，至少覆盖 validator + reaction/AI/auto-continue 中的两层。**排查信号**：① 用户描述是“所有/整轮/一类入口都开不了”，但单对象 executor/handler 单测正常 ② 修复后命令测试已绿，真实 prompt/AI 仍无入口 ③ 同一窗口里 human 玩家与 AI / UI 与命令层的结论不一致。**参考案例**：SmashUp `scoreBases -> ACTIVATE_SPECIAL` 曾只修到 validator，遗漏 `reactionSession.buildReactionOptions()` 与 `ai.canAdvancePhase()/buildSpecialActions()`，导致 afterScoring 当前结算基地即使已合法，live reaction 仍不暴露 special，AI 仍可能误给 `advance-phase`。 |
