...
- **“审计”默认就是全面审计（强制）**：用户或文档只说“审计 / 重审 / 审过 / 收口审计”时，默认含义是对当前目标范围内**所有对象、所有相关维度、所有适用证据层级**做完整审计，而不是抽样、专项或静态检查。若实际只做抽样、专项、L1 静态或代表链验证，必须在任务名、evidence 标题和汇报里显式写成“抽样审计 / 专项审计 / L1 结构审计 / 代表性验证”，禁止简称为“已审计”。
- **全面审计完成定义（强制）**：全面审计至少必须同时满足：① 建立目标对象全集清单；② 对每个对象逐项覆盖适用 D 维度；③ 对每个对象标明 L1/L2/L3/L4 当前证据等级；④ 所有规则效果至少达到 L2 行为证据，或明确写入阻塞/不适用原因；⑤ 所有真实 UI 交互入口达到 L3，或明确归入等价代表链并说明为何可代表；⑥ 所有时序/窗口/队列/跨阶段/共享根因对象达到 L4；⑦ evidence 回写旧结论失效与残余范围。缺任一项，不得写“全面审计完成”。
...
| 矩阵项 | 必填问题 | 必查实现层 | 常见错误 |
| --- | --- | --- | --- |
| 真相源语义 | 图片/规则/用户故事到底要求什么？时机、目标、伤害/资源/状态数字是什么？ | 图片、规则书、录入核对、用户故事 | 只按旧实现猜；把图片上的主阶段牌录成投掷修正。 |
| 静态定义 | `type/timing/trigger/effects/cost/tags/previewRef` 是否能独立解释对象行为？ | 数据定义、注册表、i18n、图集索引 | 运行时靠旁路硬编码，定义层是空壳或错误时机。 |
| 候选生成 | 什么时候出现在可用列表？骰面/阶段/资源/状态条件是否完整？ | rules/helper/canActivate/validator | UI 高亮条件和 reducer 接受条件不一致。 |
...
| 消耗与限制 | CP、卡牌、Token、once-per-turn、堆叠上限何时变化？失败/取消是否回滚？ | reducer、resource/token systems、event log | 效果生效但未消耗；取消也扣费。 |
...
5. 审计发现旧 evidence 曾把触发证据写成流程收口时，必须回写旧文档降级，并在新文档列出替代证据或残余缺口。
