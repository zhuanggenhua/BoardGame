...
| 维度 | 审计（Audit） | 测试（Test） |
|------|--------------|-------------|
| **目标** | 全面检查，发现问题 | 防止回归，确保正确性 |
| **方法** | 人工逐个检查，对照描述和代码 | 自动化测试，同类型只需一个代表性用例 |
| **输出** | 审计报告（发现的问题列表） | 可执行的测试代码 |
| **频率** | 一次性，或定期审查 | 持续运行（CI/CD） |
| **覆盖** | 全面审计（所有技能/功能） | 代表性覆盖（同类型一个） |

**关键区别**：
- **审计是全面的**：审计 50 个技能时，需要逐个检查所有 50 个
- **测试是代表性的**：测试 50 个技能时，同类型（如"伤害技能"）只需 1 个测试

**示例**：
- ✅ 审计：检查所有 `afterAttack` 技能是否有 `usesPerTurn` 或资源代价
- ✅ 测试：写 1 个测试验证 `afterAttack` 触发器的生命周期
- ❌ 错误：为每个 `afterAttack` 技能写一个测试（重复）

### E2E 测试的特殊性

**E2E 测试必须覆盖所有交互流程**，不能只测代表性用例。

**原因**：E2E 测试验证的是用户交互路径，不同技能的交互路径可能完全不同。

**示例**：
- ✅ 正确：为每个多步交互技能写一个 E2E 测试（如 mind_capture、illusion、withdraw）
- ❌ 错误：只写一个"多步交互"的 E2E 测试，假设能覆盖所有技能

---

## 核心原则

- **审计为主，测试为辅（强制）**：审计是思维框架（告诉你"该检查什么"），测试工具是执行手段（帮你"自动化检查"）。审计必须先做（人工思考），测试工具是补充（自动化覆盖）。
...
- **同类测试去重（强制）**：多个实体共享同一 handler/executor/reducer 分支时，只需一个代表性行为测试，其余用契约测试覆盖数据定义。≥5 个同类用 `test.each`。仅当条件分支/交互路径/边界行为/交叉影响不同时才需独立测试。**E2E 测试例外：必须覆盖所有交互流程。**
...
| D21 | **触发频率门控** | 触发型技能是否有合理的使用次数限制？频率控制方式：① 显式限制（`usesPerTurn`）② 资源代价（弃牌/献祭等）。不同触发器类型（`beforeAttack` vs `afterAttack`）有不同判定标准。 |
...
**D21 触发频率门控（强制）**（新增/修改触发型技能、或修"技能可以无限重复使用"时触发）：触发型技能是否有合理的使用次数限制。**核心原则：触发型技能的频率控制有两种方式：① 显式限制（`usesPerTurn`/`usesPerPhase`）② 资源代价（消耗手牌/单位/魔力等，代价本身就是频率控制）。不同触发器类型有不同的判定标准，必须理解游戏设计意图，不能机械地要求所有触发器都有 `usesPerTurn`。** 审查方法：

1. **识别触发型技能**：grep 所有 `trigger` 非 `activated`/`passive` 的技能定义

2. **按触发器类型分类判定**：

   **A. `beforeAttack`（攻击前准备动作）**：
   - **语义**：每次攻击前可选的准备动作（如弃牌增强、献祭友军、选择攻击模式）
   - **判定标准**：
     - ✅ 有资源代价（弃手牌/献祭单位/消耗充能）且代价足够限制频率 → 无需 `usesPerTurn`
     - ❌ 无代价或代价很低 + 效果强大 → 需要 `usesPerTurn`
   - **典型案例**：
     - ✅ `life_drain`（需要献祭友方单位）→ 无需 `usesPerTurn`（友方单位数量有限）
     - ✅ `healing`（需要弃手牌）→ 无需 `usesPerTurn`（手牌数量有限）
     - ✅ `holy_arrow`（需要弃手牌）→ 无需 `usesPerTurn`（手牌数量有限）
     - ❌ 无代价的攻击模式切换 → 需要 `usesPerTurn`（否则可以反复切换）

   **B. `afterAttack`/`afterMove`（攻击/移动后效果）**：
   - **语义**：攻击/移动完成后触发的效果（如推拉、抽牌、额外攻击）
   - **判定标准**：
     - ❌ 通常需要 `usesPerTurn`（除非是纯被动光环或有其他限制）
     - ✅ 纯被动光环（不产生事件，如 `auraStructureLife`）→ 无需限制
     - ✅ 有其他限制（如"只在首次攻击后触发"）→ 可以不用 `usesPerTurn`
   - **典型案例**：
     - ❌ `telepathy`（afterAttack，无代价推拉）→ 需要 `usesPerTurn: 1`
     - ❌ `telekinesis`（afterAttack，无代价推拉）→ 需要 `usesPerTurn: 1`
     - ✅ `auraStructureLife`（被动光环）→ 无需限制

   **C. `onPhaseStart`/`onPhaseEnd`（阶段触发）**：
   - **语义**：进入/离开阶段时触发的效果
   - **判定标准**：
     - ❌ 通常需要 `usesPerPhase`（除非是强制触发的阶段效果）
     - ✅ 强制触发的阶段效果（无玩家选择）→ 无需限制
   - **典型案例**：
     - ❌ `prepare`（onPhaseStart，可选充能）→ 需要 `usesPerPhase: 1`
     - ✅ 强制回合开始抽牌 → 无需限制

   **D. `onDamageCalculation`/`onAdjacentEnemyAttack`/`onAdjacentEnemyLeave`（条件触发）**：
   - **语义**：特定条件满足时触发（如相邻敌人攻击、伤害计算时）
   - **判定标准**：根据触发频率和效果强度判断
     - ✅ 触发条件本身就是限制（如"相邻敌人攻击"每回合最多 3 次）→ 可以不用 `usesPerTurn`
     - ❌ 触发条件频繁 + 效果强大 → 需要 `usesPerTurn`

3. **检查互斥关系**：同一效果有"攻击后触发"和"代替攻击"两个版本时，是否有互斥机制？
...
4. **验证资源代价的有效性**：
   - 代价是否真的能限制频率？（如"弃 1 张牌"在手牌 5 张时可以用 5 次）
   - 代价是否与效果强度匹配？（如"弃 1 张牌造成 10 点伤害"可能需要额外限制）
   - 代价是否可以被绕过？（如"献祭友军"但可以召唤无限友军）

**典型缺陷清单**：
- ❌ `trigger: 'afterAttack'` 但无 `usesPerTurn` 且无资源代价 → 攻击后可以无限次推拉/抽牌/检索
- ❌ `trigger: 'afterMove'` 但无 `usesPerTurn`（除非是 `extraMove` 类被动）
- ❌ 同一效果有 `afterAttack` 和 `activated` 两个版本，但没有共享使用次数
- ❌ `trigger: 'beforeAttack'` 有资源代价但代价可以被绕过（如无限召唤友军献祭）

**典型误判清单**（这些不是 bug）：
- ✅ `trigger: 'beforeAttack'` 但无 `usesPerTurn`，有弃牌/献祭代价 → 正确设计
- ✅ `trigger: 'onAdjacentEnemyAttack'` 但无 `usesPerTurn`，触发条件本身就是限制 → 正确设计
- ✅ 纯被动光环（不产生事件）无 `usesPerTurn` → 正确设计
...
> **反例（SummonerWars 吸取生命）**：`trigger: 'beforeAttack'` 但无 `usesPerTurn`，需要献祭友方单位。这是正确设计，无需修复（友方单位数量有限）
