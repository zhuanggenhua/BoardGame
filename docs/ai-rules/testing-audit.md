...
- **审计文档必须可复查（强制）**：文档不能只写“已核对正常”。至少要包含：① 审计范围 ② 权威来源 ③ 逐项/逐卡判断 ④ 命中的审计维度（如 D1/D5/D8/D34/D48/D49）⑤ 对应实现入口 ⑥ 已验证测试/证据 ⑦ 未覆盖风险或待补项。
...
审计文档建议命名为 `evidence/<game-or-module>-<scope>-audit-YYYY-MM-DD.md`，至少包含以下内容：
...
3. **逐项结论**
   ...
   - 每条至少写清：描述语义、实现入口、命中维度、判定结论。
4. **验证证据**
   - 已运行的测试、日志、截图、命令、人工观察结论。
5. **未覆盖风险**
   - 本轮没覆盖到的组合场景、共享层风险、后续待补测试。
6. **修订记录**
   - 若本对象曾被标记为“已审计”，本轮又发现漏项，必须追加“失效结论/修订原因/新增回归”。
