...
- **审计文档必须可复查（强制）**：文档不能只写“已核对正常”。至少要包含：① 审计范围 ② 权威来源 ③ 逐效果原子判断（对象/逐卡只能作为汇总层）④ 命中的审计维度（如 D1/D5/D8/D34/D48/D49/D50）⑤ 对应实现入口 ⑥ 已验证测试/证据 ⑦ 未覆盖风险或待补项。
...
- **抽样不得冒充全面审计（强制）**：当用户要求“全面审计 / 全部重审 / 全链路审计 / 这批都看完”时，抽样只能作为风险发现手段，不能作为完成证据。若抽样发现任何 HIGH/CRITICAL、语义反转、ID 漂移、注册未触发、测试断言仍沿旧语义等问题，原“全面审计完成 / 已收口”结论必须立即降级为“旧结论失效”，并回写原 evidence；随后必须回到完整对象清单逐项核销，直到每个对象都有明确 L0-L4 状态、链路结论和残余范围。禁止用“抽样已修复 / 代表性通过”重新宣布整批完成。
- **全链路审计必须逐效果原子覆盖（强制）**：对一批卡牌、基地、技能或交互宣称“全链路审计”时，最小审计单位是 effect atom（效果原子），不是卡牌名、技能名或“长描述代表”。一个对象包含多个独立时机、目标、选择、替代、排序、持续修正或后续连锁时，必须拆成多行逐项核销。evidence 每行至少覆盖：权威描述原文或原子语义、静态定义、注册入口、执行/handler、状态 reducer 或触发队列、UI/交互出口、测试/证据层级、结论。缺任一列的效果只能标 `L0-L1` 或 `仍有残余范围`，不得用同卡其他效果或同类对象测试外推。
- **共享合同可复用但必须可追溯（强制）**：多个效果原子走同一个 helper / handler / reducer / UI 组件时，不要求重复粘贴完整审计；必须建立或引用同一份 shared-contract 证据，并在每个 effect atom 行写明本效果绑定到该合同的参数、限定词、上下文和独有风险。共享合同一旦变更，所有引用该合同的 effect atom 必须标记为 dirty 并按影响面重审；禁止把“共享 helper 以前测过”当作永久免审。
- **避免重复审计必须靠合同签名，不靠记忆（强制）**：同一 shared-contract 可以被多条 effect atom 复用，但必须记录合同 ID、当前签名（关键 helper/handler/reducer/UI 出口）、本地参数、语义限定词和最后验证证据。只有合同签名未变、dirty 条件未命中、且本 effect atom 的本地参数/限定词已核对时，才允许引用旧合同免重复深挖；否则必须标 `dirty` 并重审。禁止用“以前审过这类效果”跳过本卡的 `your/may/same name/instead/this turn` 等语义限定词。
- **共享合同变更必须维护反向索引（强制）**：当一个 shared-contract 被多个 effect atom 引用，尤其是达到 `>=3` 个对象/原子，或这次修改已经命中 shared dirty 条件时，evidence 必须同时维护 `contract -> affected effect atoms` 的反向索引（至少写对象名 / atom ID / 证据文档位置）。共享实现一旦变更，先靠反向索引批量标脏，再决定哪些对象需要重审；禁止依赖“我记得哪些卡用了这个 helper”来扩审。没有反向索引时，最多只能汇报“共享合同已改、影响面待展开”，不得宣称该 shared 变更已完成收口。
- **对象级已闭合后必须换轴扩审（强制）**：某个 effect atom 若已具备 `shared-contract 签名已锁定 + 本地限定词已核对 + 至少 1 条 scoped L3/L4 真实证据`，后续审计不得继续围着同一对象、同一入口类型无限扩样。下一轮必须显式切到以下至少一类新增轴线之一：`新的触发来源/时机`、`新的 owner/controller/多人页归属`、`新的 manual-resync / waiting / overlay shared seam`、`新的语义限定词差异`、或 `shared-contract family` 的横向影响面。若仍要回到同一对象，evidence 必须先写明新的 `dirty reason`；没有新的 dirty 条件时，只允许引用既有闭环证据，禁止把“继续补几条类似样例”冒充新的深入审计。
- **语义审计不得被结构审计替代（强制）**：effect atom 审计必须先拆自然语言语义，再对照实现；不能因为 `targetType`、`abilityTags`、注册表或测试名看起来正确就跳过“你的/任意/其他玩家/至多/可以/然后/代替/任意顺序”等限定词核对。测试断言沿用错误语义时，测试本身也是 finding，必须同步修正。
...
- **`skip-only` hand prompt 必须单独验“是否还留着伪候选手牌”（强制）**：当 owner prompt 的 `targetType='hand'` 最终只剩 `skip / 放弃 / 让过` 这类辅助动作时，审计不能只断言 `skip` 按钮存在，更不能把“普通手牌还在画面里”解释成“只是背景”。必须额外证明：① 当前 prompt 已无任何 `cardUid/titanUid` 级真实候选；② 手牌区 DOM/截图都已经退场；③ 点击 `skip` 后不会再把这批普通手牌误读成后续 prompt 的候选。最低证据应包含一条 UI/E2E 断言（例如 `su-hand-area` 为 0）和一张真实截图。
- **有真实候选的 direct-hand prompt 必须单独验“是否只剩候选卡本体”（强制）**：当 owner prompt 的 `targetType='hand'` 仍有 1..N 张真实 `cardUid` 候选时，审计不能只断言“候选卡可见”或“skip 按钮存在”。必须额外证明：① 非候选手牌不会继续与本次 prompt 同屏；② 画面里真正留给用户点击的卡面，只对应本次 `PromptOption` 里的真实候选 uid；③ 点击 skip 或确认后，这批候选卡不会被误当成下一层 prompt 的残留。最低证据应包含一条 UI/E2E 断言（例如某张非候选手牌 `count=0`）和一张真实截图。
- **setaside 泰坦 focused prompt 必须单独验“文案与本体没有脱节”（强制）**：当 owner prompt 要求玩家点击 `setaside` 泰坦执行反应或进场时，审计不能只断言标题、`skip` 按钮或 prompt 归属存在。必须额外证明：① 当前可触发的泰坦本体仍在 DOM 和截图里可见；② `su-deck-stack`、`su-discard-toggle`、`su-end-turn-action-button`、`turn notice` 等外围壳层没有为了“把泰坦放回来”而一起回潮；③ 非 owner 页没有同一张泰坦的反应 badge / prompt。最低证据应包含一条 DOM/E2E 断言证明泰坦本体存在、至少一条 DOM/E2E 断言证明外围噪音控件不存在，以及一张真实截图证明画面只剩 prompt 所需对象。
- **棋盘直选态必须单独验“高亮是否假阳性”（强制）**：当交互不是中央 prompt，而是手牌/弃牌/泰坦激活后直接把棋盘切成可落点模式时，审计不能只看“最终点对了能成功”。必须额外证明：非法基地/非法目标在进入直选态时就已经置灰或不高亮，不能出现“视觉上可点、实际提交才被 validator 拒绝”的假阳性。最少证据应包含：一张进入直选态的截图，外加一条 UI/DOM 或 E2E 断言，证明非法目标已被前端收窄。
- **非 owner waiting overlay 必须单独验“是否还像可操作页面”（强制）**：当 prompt 只属于其他玩家或赢家页面时，审计不能只断言“当前页没有提交成功”。必须额外证明当前页只剩单一等待语义，而不是仍显示禁用确认按钮、禁用编辑工具、重复等待文案、原 prompt 标题、候选卡带/列表骨架或其它会被误读成可操作的控件。最低证据应包含：① 一条组件层或 E2E 断言证明操作按钮不存在；② 一条断言证明原 prompt 标题或候选骨架不存在；③ 一张截图证明等待页只剩等待态而非编辑态。
- **response-window waiting 必须单独验“是否还保留 responder 面板或全局工具入口”（强制）**：当 `Me First! / 计分后响应` 这类 overlay 只是在当前页显示“等待别人响应”，审计不能因为它属于合法 waiting family 就放松标准。必须额外证明：① 非响应者页面不存在 `让过` 按钮；② 原始标题和响应进度点阵已退场；③ `GameHUD/FAB`、设置/聊天/反馈等全局工具入口也已退场；④ 截图里只剩紧凑 waiting shell，而不是一张带标题/点阵的大卡片面板或一个角落悬浮球。否则仍属于“等待态看起来像可操作面板”的 UX 脏态。
- **展示型非阻塞 overlay 必须单独验“是否抢整屏点击”（强制）**：当 reveal / spotlight / 说明浮层对外口径是“非阻塞 / 可继续操作”时，审计不能只看它会不会出现、会不会关闭。必须额外证明：① overlay 根容器不会以整屏点击命中区抢走底层主操作；② 当前页面仍只暴露最小必要控件（如关闭按钮、放大入口），不会把整屏 backdrop 伪装成新 prompt；③ 至少一条组件层断言或真实链路证据能看出 `pointer-events-none` / scoped `pointer-events-auto` 的语义成立。
- **owner prompt 必须单独验“外围壳层是否整体退场”（强制）**：当当前玩家自己持有 prompt，且主视觉焦点已经切到标题 + 候选 + 确认区时，审计不能只看 prompt 会不会出现，也不能只验右下 `结束回合` 大按钮没了。必须继续核对所有会被误读成并列操作入口的外围壳层是否一起退场，包括但不限于：`结束回合` 大按钮、额度徽章、显隐切换、牌库/弃牌/rail 工具入口、记分板、调试浮球、设置浮球以及其它全局操作角标。最低证据应包含：① 至少一条 DOM/E2E 断言证明这批噪音控件为 `hidden/detached`；② 一张真实截图证明画面只剩当前 prompt 需要的对象本体与确认动作；③ prompt 解决后至少一条对照断言证明正常外围壳层会恢复。否则即使规则链正确，也仍属于可用性未收口。
- **共享 UX 合同变更后，旧截图不得继续充当正向证据（强制）**：当 `owner prompt focus`、`blocked waiting shell`、`non-blocking overlay`、`deck-inspect-reorder editor` 这类 shared seam 被修正后，任何仍带着旧噪音控件的历史截图都必须视为失效证据。禁止继续复用“修前图”证明“修后 UX 已达标”。最低要求二选一：① 重新跑真实链路并刷新截图；② 在原 evidence 文档明确标注该截图已 stale、为什么 stale、替代截图路径是什么。只要旧图里还看得到当前规范已禁止的按钮墙、等待壳层、结束回合簇或双层 overlay，就不得再作为通过证据引用。
- **“截图里选项太多”必须拆成动作数与对象数两列（强制）**：当审计对象是 `deck-inspect-reorder`、`只读 inspect waiting`、`setaside titan rail`、`direct-hand candidates` 这类会同时展示对象本体与少量工具动作的 UI，evidence 里必须显式写出：① 当前画面有多少个**显式可执行动作控件**；② 有多少个只是被 inspect / 被编辑 / 被选择的**对象本体**；③ 哪些元素属于已禁止的外围壳层噪音。禁止只凭“看起来很多”就把 3-5 张对象卡面误判成 3-5 个并列按钮，也禁止反过来把真实按钮墙说成“只是对象本体”。最低证据要求：至少一张真实截图，加一段文字逐项点名 `动作控件`、`对象本体`、`外围壳层` 三类元素。
- **棋盘 blocked waiting 必须单独验“等待牌是否夹带其它入口”（强制）**：当当前页不是 owner，`Board` 右下角进入 `等待对方操作` 这类 blocked waiting 时，审计不能只看“结束回合按钮没了”。必须继续核对同一区域是否还残留额度徽章、显隐切换、禁用工具或其它会被误读成可操作入口的配套控件。最低证据应包含：① 一张 blocked waiting 截图证明只剩单一状态牌；② 一条对照截图或断言证明正常可操作时结束回合按钮仍存在；③ 若结束回合 UI 支持手动隐藏，还要验证 blocked waiting 不会把状态牌一起隐藏掉。
...
- **纯按钮 / 纯玩家分支不得继续挂 generic targetType（强制）**：如果交互本质上只是在若干按钮里选一个结果，`targetType` 应声明为 `button`；如果交互本质上只是在若干玩家里选一个或多个玩家，`targetType` 应声明为 `player`。只有同时携带棋盘实体、卡面/玩家上下文或复合后续步骤时，才允许保留 `generic`。禁止把纯按钮/纯玩家分支都压成 `generic` 再依赖 UI 推断，否则会把“选什么”与“怎么展示”拆成两套真相。
- **纯棋盘直选 + 控制项不得继续挂 generic targetType（强制）**：如果交互候选本体已经是单一维度的棋盘实体（例如一组随从或一组基地），只是额外混入 `skip / done / cancel / confirm` 这类控制项，那么 `targetType` 仍应声明为对应的 `minion` / `base`，不能因为“顺手加了个 skip 按钮”就退回 `generic`。只有同一批候选里同时混入不同实体维度、卡面快照、排序上下文或复合后续步骤时，才允许保留 `generic`。否则会把本来可以做直点高亮/收窄候选的交互重新打回通用选择面，直接伤害可用性。
...
强制输出格式（审计文档逐效果原子至少要有这些列；对象级/逐卡表只能引用这些原子结论，不能替代）：
...
- **L4 治理证据**：残余范围、共享根因归类、旧结论失效回写、汇总文档降级、替代旧 claim 的新回归证据。
...
- 若用户要求“全部 / 全面 / 全链路”，对象清单必须覆盖本轮 scope 中的每个对象；抽样清单必须显式标为“抽样”，不得命名为“全面”或“收口”。
- 若对象包含多个独立时机、目标、选择、替代、排序、持续修正或后续连锁，必须在对象清单下面继续展开 effect atom inventory。全链路退出条件按 effect atom 核销，不按对象名核销。
- effect atom inventory 至少写明：`effectId`、自然语言原子语义、语义限定词、静态绑定、共享合同、本地参数、validator/canActivate、handler/reducer、UI/交互、测试/证据层级、dirty reason、结论。
- 每个对象都要显式标记当前层级：
  - `L0`：只建立了对象名，还没完成结构核对。
  - `L1`：结构层已核对。
  - `L2`：行为层已核对。
...
- 对象清单中只要还有 `pending / L0 / L1 / 未审 / 残余未声明`，禁止输出“全面审计完成”。
- effect atom 清单中只要还有 `pending / L0 / L1 / dirty / 仅继承共享合同但未核本地参数`，禁止输出“全链路审计完成”。

#### Step 1：每个 effect atom 必须串完整链路

- 每个 effect atom 至少要按下面顺序过一遍，缺一段都不能叫“深入审计”：
  - 规则/描述语义
  - 静态定义与注册入口
  ...
  - UI 消费链或可见交互出口
  - 真实入口验证
- 交付文档里必须写清“本 effect atom 链路具体落在哪些文件/函数”，不能只写一个总入口文件名。
- 共享 helper / handler / reducer / UI 组件可以引用 shared-contract 证据，但本 effect atom 的语义限定词、本地参数、上下文携带和额外防线仍必须单独核对。
...
1. **先定义“最后正常证据”**：必须给出至少一个可复查证据来源（`git log` / `git show` / 历史截图 / 旧 E2E 产物 / evidence 文档）。如果找不到，必须明确写“尚未找到最后正常证据”，并把这条不确定性作为风险点记录。
2. **先 diff 再假设**：对比最后正常版本与当前版本的具体文件（`git show <commit>:<file>` 或 `git diff <commit> -- <file>`），先列出变更点，再提出根因假设。禁止“先假设→重写→再找证据”的顺序。
3. **必须恢复到 Git 正常状态（强制）**：回归修复默认目标是“恢复到最后正常版本的行为/表现”，禁止擅自引入新的行为改动或重构。只有在**明确有更好修复方案**（例如：能消除共享根因、降低长期维护成本、或显著提升稳定性）且给出证据与权衡说明时，才允许偏离“恢复到正常版本”的修复路径。
4. **最小改动优先**：在“恢复到正常版本”目标下，优先选择最小变更面；只有当 diff 无法定位或根因属于共享抽象缺陷时，才升级为重构或结构修复。
5. **根因归类**：必须标注根因类型（数据/录入、单点实现、共享抽象、架构/时序），并说明为何不是其它类型。
6. **验证与证据**：至少补 1 条与回归直接相关的验证路径。UI 回归必须有 E2E + 截图证据；引擎/规则回归必须有 GameTestRunner 或单测断言。
7. **回归扩审**：完成修复后按“同类扩审”规则扫同类入口，记录命中点与未命中理由。
...
3. **UI 模式验证**：`multi` 存在 → UI 必须提供多选态与显式确认入口，但**不等于默认必须显示 `全选`**。只有候选数量较多、批量操作能明显降低成本时，才允许出现 `全选/取消全选`；若候选仅 1-2 个，或 `maxSelections` 很小、逐项点击已更清晰，则应保持紧凑动作栏，只保留必要的确认/清空选择/跳过。`multi` 不存在 → UI 应显示单选按钮或卡牌点击即确认。
4. **线性编辑器豁免口径**：若交互是 `deck-inspect-reorder` 这类“编辑对象顺序”的线性工具面板，审计“选项数量”时只计算真实可执行动作控件，不把被 inspect 的卡面本体误算成同级按钮。反之，若实现把排列结果、目标位置或候选组合直接展开成一堵按钮墙，仍判为 D5 不通过。
5. **`skip / 让过` 后续链路口径**：当当前动作只是“放弃这一次机会”时，审计不能机械要求 `skip` 后立即 `interaction.current == null`。必须先区分：
   - ① 当前这一步是否真的允许跳过；
   - ② 跳过后是否还存在合法 follow-up reaction / response / afterScoring prompt。
   若第②层存在，则 `skip` 后继续出现下一层 prompt 本身不算失败；真正的失败是旧 prompt 的标题、按钮、计数、浮动操作栏或 overlay 与新 prompt 同屏残留，或候选被强制执行。凡修这类链路，至少补一条 `skip -> 下一层 prompt -> 最终收口` 的完整断言。
...
优先级：① 用户当前对话给出的**卡牌/技能/token 原文**（含清晰截图或原文描述）→ ② 卡牌/技能/token 自身描述（卡面/提示板/真相源图片）→ ③ `rule/*.md` 规则文本 → ④ 历史实现/旧裁定。**禁止用 i18n JSON、AbilityDef.description、代码注释作为权威输入**——这些是实现产物，可能带着错误理解。
...
### 流程截图证据链（强制）
1. **流程截图必须通链路**：凡涉及“改投/重掷/分步确认/阶段推进”的交互，必须提供**至少 2 张连续截图**（触发前/触发后或改投前/改投后），且每张都写清观察结论。  
2. **奖励骰/骰子特写要求**：若交互会出现奖励骰特写或骰子特写，证据必须包含：  
   - 特写出现的截图（可见骰面/提示/重掷入口）  
   - 重掷/改投后的特写更新截图  
   - 若规则要求“主攻击骰盘不改动”，截图中必须能看出主骰盘保持原值  
   - 若同一容器会在前后两步 prompt/特写之间热切换，截图必须证明旧内容已经从浏览器 DOM 消失，新内容已经真实出现，不能只凭服务端 `sourceId` 或日志判断“已经切到下一步”
   ...
   **Prompt/特写切换截图门禁（强制）**：若本轮要证明“第一层 prompt 已被第二层 prompt 替换”或“旧特写已收口”，必须在截图观察里显式写清：旧标题/旧按钮/旧计数是否已经消失，新标题/新卡面/新按钮是否已经在浏览器页面上真实可见。凡截图里还能看到上一层 prompt 的标题、确认计数、候选按钮、旧卡面残影或旧等待态，都不得作为“当前页面已切换到新 prompt/已完成收口”的证据。
...
3. **无证据链不得收口**：未提供完整截图链路时，只能标记“待补/未达标”，**禁止宣告完成**。  
4. **必须覆盖成功路径（强制）**：若交互存在“成功/失败”两类结果（如改投成功 vs 改投失败提示），证据链截图**必须包含成功路径**；仅有失败提示截图只能标记“未达标/待补”，不得作为收口证据。  
5. **失败提示不得替代成功证据（强制）**：失败提示（toast/modal/红字提示）截图可作为补充证据，但**不能**替代“成功后状态已改变”的截图。  
6. **必须覆盖收口清理（强制）**：凡涉及“弹层/特写/分步交互关闭后继续结算”的链路，证据必须包含“关闭后已收口”的证明：  
   - UI 层：弹层/特写已关闭，且关键展示（如攻击修正/骰盘值）处于期望最终态；  
   - 状态层：对应 pending/临时标记已清空（命中 D14/D15），避免“只关 UI 不清状态”的假修复。  
7. **对外口径门禁（强制）**：如果你在对用户的最终回复中提到“已跑 E2E / E2E 通过 / 已验证成功”，则必须给出至少 1 张你实际核对过的关键截图的**完整绝对路径**；否则不得使用该口径收口。  
