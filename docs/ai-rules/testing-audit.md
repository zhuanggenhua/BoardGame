...
- **`prompt -> onResolve -> helper` 不得只测 prompt 出现（强制）**：凡对象实现经过 runtime prompt、simple choice、reaction choose、branching choice 或其他分段 callback seam，测试与审计都不能停在“prompt 建出来了 / 候选列表对了”。至少要补 1 条真实 `respond/resolve` 路径，验证 helper 真的执行到了最终权威状态；若规则存在可选拒绝/取消/空选，也必须补 1 条否定路径。像“把内联事件改成 shared helper”这类重构，旧测试如果只覆盖 prompt 创建，视为证据失效，必须升级到结算层。
- **shared helper 替换必须审 seam 依赖泄漏（强制）**：当代码从手写事件构造切到 `buildStandardDrawEvents`、随机选择 helper、掷骰 helper、排序 helper 等共享 helper 时，除了验证行为不变，还必须审查 callback seam 本身是否新增了 `random`、`timestamp`、`matchState`、`interactionData` 等运行时依赖。若某张牌本身只声明“抽牌/摸牌/结算”，就不应再让卡牌层手工透传底层随机依赖；优先收敛到接受整份 runtime args/context 的 helper，避免解构漏参型回归。
