...
- **卡牌效果不得被卡图/牌池审计吞掉（强制，通用）**：凡物品、预兆、事件、行动牌、装备牌、状态牌或等价牌对象进入发现池、牌堆、持有区或弃牌区，审计必须分别覆盖“卡图/文本录入正确”和“规则效果可执行正确”。只证明牌名、正面图、atlas 格位、抽取顺序、持有区渲染或数量不重复，不能写成卡牌效果已审计；效果审计必须补候选生成、使用时机、目标、消耗/弃置、分支、失败路径、清理与最终权威状态。
