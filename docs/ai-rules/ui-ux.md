...
- **组合爆炸类交互禁止枚举成按钮墙（强制）**：当交互本质是“同一组对象的排序、分组、分配、顶/底拆分、先后次序编辑”时，禁止把所有排列、组合或分割结果直接展开成按钮列表、卡片墙或长菜单。只要候选数量会随对象数呈阶乘级、指数级或明显超线性膨胀，就必须改成线性复杂度的编辑方式，例如分步选择、双列表分配、局部移动、拖拽排序或显式确认流。验收时必须提供真实链路截图，证明用户面对的是有限工具条/编辑器，而不是爆炸式选项清单。
