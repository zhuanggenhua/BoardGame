...
2. **游戏页面横屏建议**：游戏页面（`/play/*`）在移动设备竖屏时显示顶部横幅建议旋转（不阻止访问）
...
5. 测试游戏页面（访问 `/play/tictactoe/local`）：
   - 竖屏：应显示顶部横幅"建议旋转至横屏以获得更佳体验"，游戏内容仍可见
   - 横屏：不显示建议，正常显示游戏界面（缩放适配）
...
2. **可访问性**：某些场景下用户可能需要竖屏快速查看游戏状态
3. **渐进增强**：建议而非强制，尊重用户选择
...
- `src/components/common/MobileOrientationGuard.tsx` - 横屏建议组件（路由感知，顶部横幅）
