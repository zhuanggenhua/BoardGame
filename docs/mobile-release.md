...
- Web 和 Android App 的业务后端入口统一使用 `VITE_BACKEND_URL`；`VITE_ANDROID_BACKEND_URL` / `ANDROID_VITE_BACKEND_URL` 只作为迁移期旧别名，若与 `VITE_BACKEND_URL` 不一致，构建必须失败；`VITE_GAME_SERVER_URL`、`VITE_AUTH_API_URL`、`VITE_FEEDBACK_API_URL` 等服务级覆盖也不得在生产构建里指向不同后端
...
- OTA / native 发布后必须同时回查域名控制入口和 IP 主源：域名 `latest.json` 不能重定向，正文哈希必须等于本次发布；IP 主源可作为下载与新客户端兜底，但不能替代旧客户端控制入口。
