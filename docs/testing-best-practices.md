...
- 当这轮修改的是“等待页语义”“私有 prompt 归属”“重排/排序/分堆编辑器”“按钮墙减法”这类交互友好性问题时，测试不能只断言功能还通。
- 至少补 1 条 UI 合同断言，明确限制以下任一现象不得回归：
  - 非 owner waiting 页仍泄露旧标题、候选壳层或操作按钮；
  - `skip / 让过` 后旧 prompt 的浮动条、计数或按钮仍和下一层 prompt 同屏；
  - 排序/分堆类 prompt 又退化成枚举排列结果的按钮墙；
  - 棋盘直选态把非法目标继续高亮成“像可点一样”。
- 如果这类回归已经在真实浏览器截图里肉眼可见，测试标题必须直接写出 UX 合同，而不是只写“某某能力通过”。
- UI 合同测试改口径时，要同步更新断言文案，避免功能已经按新 UX 收窄，测试却还锁死旧标签或旧说明块。
