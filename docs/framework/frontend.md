...
- **一旦某个对象已对用户加上 `实施中` 状态，默认不得擅自移除（强制）**：除非满足以下任一条件，否则不得因为“这轮顺手收口”“视觉更干净”“本地看起来差不多能用”就把 `statusTag: 'under_construction'` 或对应横幅删掉：
  - 用户当轮明确允许移除；
  - 当前对象已经有同轮真实完成证据，能证明它不再属于实施中，并且相关状态契约、目录入口和证据文档已同步更新。
