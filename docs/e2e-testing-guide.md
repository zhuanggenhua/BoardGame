...
真实 E2E 只允许通过用户可执行的 UI 交互推进：点击、输入、键盘、hover、正常确认/取消、正常页面导航。

在真实 E2E 中，禁止用测试后门直接改核心对局状态，包括但不限于：
- `inject...Scene`
...
`page.evaluate(...)` 在真实 E2E 中只允许做只读检查、辅助断言与调试采样。

### 场景注入测试（非真实 E2E）

如果测试为了速度、稳定性或边界局面复现，直接通过 harness / debug panel / 页面内脚本改状态，这类测试应归类为“场景注入测试 / fixture test / harness test”，不应继续宣称为真实 E2E。

- 参考文档：`docs/e2e-state-injection-guide.md`
- 适合目标：边界场景、复杂前置局面、非真实链路回归定位
- 不适合目标：验证真实用户是否真的能完成该操作链
...
在开始写或改测试前，先判断测试属于哪一类：
1. **要验证真实用户链路** → 写真实 E2E，禁止状态注入。
2. **要快速构造复杂局面或做边界复现** → 写场景注入测试，不要冒充真实 E2E。
