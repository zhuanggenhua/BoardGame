...
export async function runE2ESafetyCheck(env = process.env) {
  ...
  if (headedByEnv) {
    console.log('\n⚠️ 警告：检测到可见浏览器模式环境变量');
    console.log(`   PW_HEADED=${env.PW_HEADED ?? '<unset>'}`);
    console.log(`   PWDEBUG=${env.PWDEBUG ?? '<unset>'}`);
