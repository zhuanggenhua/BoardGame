...
> **数据录入未闭合必须停在 S0，但不能停工（强制）**：新增游戏的阶段 0 必须先把基础版范围内的规则对象、卡牌/组件/房间/剧本/Token/能力、规则原文、结构化子句、素材/atlas/槽位和合同状态录入成可复查矩阵。只要基础版必需对象仍是 `missing / blocked / disputed / unknown-slot / source-mapped-contract-pending / not-in-runtime`，就必须停在 S0 数据录入/合同层；不得进入目录骨架、Board/UI、机制实现、E2E、截图验收、设计稿实现或“基础版完成”汇报。这里的“停在 S0 / blocked”只拦阶段升级和完成宣称，不表示停止执行；只要仍能从主真相源、atlas、完整裁图、现有合同或只读代码继续补证，就必须继续在 S0 合同层拆缺口、补字段、修正状态。
...
> **教程完整性是整体完成门槛**：新游戏或现有游戏补完不得在教程缺口未关时标为“整体完成”。必须先按 `.codex/skill/tutorial-workflow/SKILL.md` 和 `docs/ai-rules/tutorial-design.md` 补齐玩家可用元素覆盖矩阵、真实教程 E2E、关键截图核图和未覆盖项说明；`确认 1/2`、聚焦到自己位置/房间/对象、观察视角、状态/进度读法、只读规则入口等非直觉 UI 必须有教程/帮助落点或明确豁免。
...
- 新游戏静态事实默认使用严格 JSON `GameConfigPackage`，并按 `docs/ai-rules/game-config-package.md` 说明 JSON 文件位置、schema、表格审查范围和玩家修正提案流程；若暂不使用，必须在 OpenSpec proposal 或 design 写明跳过原因、影响范围和后续补齐项。
