...
> **数据录入未闭合必须卡住（强制）**：新增游戏的阶段 0 必须先把基础版范围内的规则对象、卡牌/组件/房间/剧本/Token/能力、规则原文、结构化子句、素材/atlas/槽位和合同状态录入成可复查矩阵。只要基础版必需对象仍是 `missing / blocked / disputed / unknown-slot / source-mapped-contract-pending / not-in-runtime`，就必须停在 S0 数据录入/合同层；不得进入目录骨架、Board/UI、机制实现、E2E、截图验收、设计稿实现或“基础版完成”汇报。
...
3. **规则-素材数量对账**：按类别记录规则说明数量、图包/atlas 实测正面数量、背面/空格/非对象数量、复用 alias、当前运行数量和对账结论。只有 `matched` 或 `matched-after-alias-collapse` 的类别能进入后续阶段；`blocked/disputed` 类别必须停在 S0。
...
- **数据录入未完成一票阻塞**：基础版必需对象、规则原文、原子子句、素材/atlas/槽位、对象归属或对照差异任一未录入/未裁定时，整体状态必须保持 `in_progress / blocked`；不得说“先实现基础流程再补数据”。
