...
8. 只有矩阵闭合、账本一致、布局合同完成、用户点名参考已读取可用或已明确阻塞、替代批准明确后，才能进入 Board、E2E、完成提案或收口文档。
...
2. 若发现缺口，必须先更新 proposal/tasks/spec，把缺口写成待实施项，再继续素材或规则实现；不得直接跑 E2E 寻找“看起来可玩”的绿灯。
3. E2E 只能验证真实运行时链路，不得依赖临时 HTML 元素、CSS 伪造素材、文字牌面、临时 mock 图或非正式资源目录。
4. 如果当前只能程序化表达，必须有用户明确批准或规则矩阵说明该对象本来不需要图片；否则端到端验收必须停止在阻塞状态。
