...
这一步不是“写提案”，而是可审计的数据录入和素材 intake。提案只能引用矩阵中的事实；如果矩阵缺失或仍有基础版必需对象未闭合，提案必须写成 `in_progress / blocked`，不得写成完成方案。

**数据录入未闭合必须卡住**：新增游戏只要还没把基础版范围内的规则对象、规则原文、原子子句、素材/atlas/槽位、对象归属和合同状态录入清楚，就必须停在 S0。可以分批录入已锁定对象，但未锁对象必须留在同一矩阵里并标明阻塞范围；未完成前不得进入目录骨架、Board/UI、机制实现、E2E、截图验收、设计稿实现或“基础版完成”汇报。
