...
## 脚本
```
./.codex/skill/atlas-crop/scripts/extract-atlas-crops.py
```
...
2. **执行裁切脚本（在仓库根目录运行）**
   ```bash
   python ./.codex/skill/atlas-crop/scripts/extract-atlas-crops.py \
     --image "<图集路径>" \
     --config "<配置JSON路径>" \
     --out "<输出目录>" \
     --shift-x -0.5 \
...
更完整的项目内使用清单见：`./.codex/skill/atlas-crop/references/boardgame-atlas-usage.md`
