...
- 多派系批量新增默认按**对象级深审**处理，不能只做每派系 1 条“代表路径”就宣称完成。
...
- 用户或任务只说“审计 / 全面审计 / 收口审计”时，本 skill 默认解释为**彻底做到底**：当前批次每个新增对象都必须有独立审计行和 L0/L1/L2/L3/L4 结论；除非已登记为“共享链路完全同构，仅配置不同”的合法复用，否则不得用代表性玩法验证替代对象级结论。
- 统一审计文档里只要还存在“未做对象级 L3/L4、但用代表链兜住”的情况，结论就不能写“已审计完成 / 当前发布口径已收口”，只能写“代表性验证完成”或“仍有残余范围”。
- “审计”默认就是做到底：派系清单中的每张卡、每个基地、每个 Token/状态/升级对象都必须有独立矩阵行，禁止只审代表卡或代表玩法。
