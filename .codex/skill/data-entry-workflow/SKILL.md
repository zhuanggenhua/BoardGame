...
2. 先锁定主真相源和对照源，并把“谁负责什么字段”写清楚。
   - 如果用户反馈的是“卡图不对 / 贴图和效果不一致 / 怀疑录入错误 / 怀疑索引用错”，必须先直接打开当前对象的主真相源图片（完整单卡、玩家板、提示板或原始图集），再判断是不是录入问题。
   - 录入过程中可以看从正式原图/正式图集切出的 `temp/` 裁片；但若要裁定冲突、否定玩家反馈或决定 atlas/运行时语义，必须回到正式原图/正式图集。
   - 代码、现有 rule 文档、测试、旧 evidence 只能辅助定位；在没看图前，禁止下“不是录入错误”的最终结论。
