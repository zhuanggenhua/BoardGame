...
启动后访问 http://localhost:4273 即可。重复运行时，已在 4273/18000 监听的本项目服务会被复用，不会递增端口或再启动一套；如果端口被其他进程占用但服务仍不可访问，启动日志会明确提示端口冲突，请先处理占用进程。
...
**协作者首次配置环境时，可以展开复制下面的提示词交给自己的 AI；**
...
准备添加新游戏时，把游戏需求交给 AI 编辑器即可，它会按仓库内工作流引导你完成。
