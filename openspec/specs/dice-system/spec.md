...
## Purpose
定义跨游戏复用的通用骰子能力，采用“游戏显式提供骰子定义 + 引擎纯函数消费”的模式，统一支持骰子实例创建、掷骰、统计计算与触发判定。
...
### Symbol vs Value
- `value`：数值，用于顺子判定和点数统计
- `symbol`：主符号，用于技能触发和主要 UI 展示
- `symbols`：完整符号列表，用于多符号骰面结算

### Pure-Function API
骰子能力以显式 `DiceDefinition` 和纯函数 API 为准，不依赖全局 singleton。
