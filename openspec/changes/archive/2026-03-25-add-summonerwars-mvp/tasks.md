...
- [x] 3.7 手牌区改为点击选中模式（非拖拽），使用精灵图显示卡牌
  ...
  - 交互模式：点击选中 → 点击目标位置放置
- [x] 3.8 导入完整卡牌数据：精灵图配置 + 卡牌属性
  - 已完成：各阵营卡牌均已接入 atlasId / spriteIndex / spriteAtlas 映射
  - 已完成：完整卡牌属性（名称、费用、生命、攻击力、能力）已落地到 `src/games/summonerwars/config/factions/*.ts`
  - 已完成：`cardRegistry.ts` 已聚合各阵营卡池供运行时使用
