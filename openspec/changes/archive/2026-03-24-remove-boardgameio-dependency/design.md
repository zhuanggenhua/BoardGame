...
> **状态：已完成**。旧框架（boardgame.io）已完全移除，自研传输层（GameTransportServer + GameTransportClient）已全面接管。本文档保留为历史记录。

项目已自建完整引擎层（pipeline、systems、adapter、transport），旧框架已被完全替换。
...
```
IOServer (单实例)
├── /game — 游戏状态同步（替代旧框架的 /default namespace）
...
替换旧框架的 `Client()` HOC 为 React Hook + Context：
...
旧框架提供的 REST API（`/games/:name/create`、`/join`、`/leave`）改为自建 Koa 路由。
...
保留 MongoStorage/HybridStorage 的 MongoDB 操作逻辑，但移除旧框架 `StorageAPI.Async` 接口约束。
简化为项目自有接口：
...
移除 adapter.ts 中对旧框架 `random` 对象的包装。
...
5. **Phase 5: 清理** — 移除旧框架依赖、adapter.ts 旧代码、LobbyClient 调用

## Risks / Trade-offs

| 风险 | 缓解 |
|------|------|
| 状态同步可靠性（旧框架经过多年验证） | 核心逻辑简单（服务端执行 → 广播状态），E2E 测试覆盖 |
| 重连/断线恢复 | socket.io 内置重连 + 服务端保存最新状态，客户端重连后拉取 |
| 并发命令冲突 | 服务端串行执行（单 match 单锁），与旧框架行为一致 |
...
- **离线交互裁决**：保留。功能内置到 `GameTransportServer`——断线回调直接调用 pipeline 执行 `CANCEL_INTERACTION`，比旧架构更简洁。不需要单独重构，作为迁移的一部分自然替换。
