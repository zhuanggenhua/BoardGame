...
- 为大杀四方新增 `牌堆可查` 房规项，作为与 `泰坦`、`DIY` 同组的多选项之一，默认开启。
- 当 `牌堆可查` 关闭时，牌库区仍显示当前剩余数量，但不能展开剩余牌详情。
- 当 `牌堆可查` 开启时，保留现有的大杀四方牌库查看能力，并允许切到对手视角后查看当前视角玩家的余牌。
- 为大厅房间列表新增“已开启扩展”公开摘要，展示这个多选组里已开启的公开 tag，不直接暴露整份 `setupData`。
