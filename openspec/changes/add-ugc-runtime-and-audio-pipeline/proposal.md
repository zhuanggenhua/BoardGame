...
- UGC 资产上传/压缩/存储只有类型与占位实现，缺少与 服务器资源管线的实际接入，且需要明确“不保留原始文件”的空间策略。
- 主页与分类缺少 UGC 游戏展示入口，无法形成“上传 → 浏览 → 游玩”的闭环。

## What Changes
- UGC 联机/教程入口复用现有大厅/房间与教程路由，服务端按已发布 packageId 动态注册游戏引擎。
- 规则/教程入口来自 UGC 包内文件（如 domain.js 与教程脚本），并可由 UGC 配置自动生成 manifest。
- 新增 UGC 包服务：发布、拉取、列表（仅已发布），本地预览仍允许使用本地数据。
- 落地 UGC 资源上传/压缩/存储流程，接入 服务器资源发布规范，**不保留原始文件**。
