...
#### Scenario: 表格覆盖运行时消费字段
- **GIVEN** 某个游戏 schema 中的字段会被运行时、牌组构建、起始部署、素材加载、能力绑定或特殊规则消费
- **WHEN** 系统生成配置审查表
- **THEN** 表格 MUST 为这些字段提供可见单元格
- **AND** 字段值 MUST 来自同一个物化配置对象
- **AND** 系统 MUST NOT 只把被消费字段藏在调试 payload、sourceContext 或不可见元数据中
...
- **AND** 表格 UI MUST 使用该字段清单生成可见列、读取当前值、判断字段适用性和解析玩家建议值
...
- **AND** 提案 MUST 保存 `currentValue` 与 `suggestedValue`
