...
#### Scenario: 表格覆盖玩家可修正的静态事实字段
- **GIVEN** 某个游戏 schema 中的字段会被运行时、牌组构建、起始部署、素材加载、能力绑定或特殊规则消费
- **WHEN** 系统生成配置审查表
- **THEN** 表格 MUST 为其中玩家可理解、可录入或可修正的静态事实字段提供可见单元格
- **AND** 字段值 MUST 来自同一个物化配置对象
- **AND** 系统 MUST NOT 只把玩家需要核对的静态事实藏在调试 payload、sourceContext 或不可见元数据中

#### Scenario: 主表隐藏内部定位字段
- **GIVEN** 某个配置对象包含内部对象 ID、字段路径、来源上下文、图集 ID、帧索引、源码路径或运行时 debug 字段
- **WHEN** 系统生成玩家配置审查主表
- **THEN** 系统 MUST NOT 将这些内部字段作为主表可见列
- **AND** 系统 MAY 在结构化提案 payload、隐藏上下文、开发工具或后台审查视图中保留这些字段用于定位
- **AND** 卡图或素材结果 MUST 通过玩家可理解的预览、状态或试听控件呈现，而不是要求玩家阅读图集或帧编号

#### Scenario: 旧游戏 adapter 使用字段覆盖清单
- **GIVEN** 某个旧游戏通过只读 adapter 生成配置审查表
- **WHEN** adapter 建立字段覆盖清单
- **THEN** 清单 MUST 从规则真相源和运行时消费代码反推必审字段
- **AND** 清单中每个必审字段 MUST 记录现实含义、字段 key、值类型、适用对象、字段路径、值读取方式和证据来源
- **AND** 表格 UI MUST 使用该字段清单生成可见列、读取当前值、判断字段适用性和解析玩家修改后值
- **AND** 系统 MUST NOT 在 UI 页面里维护一套与 adapter 字段清单独立的列定义或字段类型定义
...
- **WHEN** 玩家在该字段对应的表格单元格输入修改后值并提交
- **THEN** 系统 MUST 保存游戏 ID、配置版本、对象 ID、字段路径、当前值、修改后值、玩家可见对象名、玩家可见字段名、当前显示值、修改后显示值、说明和可选证据
- **AND** 该提案 MUST 进入待审状态

#### Scenario: 暂存多个字段后统一提交
- **GIVEN** 玩家需要修正同一配置表中的多个字段
- **WHEN** 玩家明确进入单元格编辑态并暂存多个修改后值
- **THEN** 表格 MUST 保持默认只读展示，不常驻输入框
- **AND** 系统 MUST 显示待提交修正数量和无效输入数量
- **AND** 玩家 MUST 能通过单独提交入口一次性发送这些字段级修正

#### Scenario: 禁止用整行替代字段提案
- **GIVEN** 玩家只修改了某个已有对象的单个字段
- **WHEN** 系统创建配置修正提案
- **THEN** 提案字段路径 MUST 指向该单元格对应的 schema 字段
- **AND** 提案 MUST 保存机器可审查的 `currentValue` 与 `suggestedValue`
- **AND** 玩家可见 UI MUST 将该变化展示为“当前值 / 修改后值”
- **AND** 系统 MUST NOT 用模糊的整行修正替代字段级提案

#### Scenario: 宽表翻页后回到左侧
- **GIVEN** 玩家在配置审查表中横向滚动到右侧字段
- **WHEN** 玩家切换页码、搜索、筛选或修改每页行数
- **THEN** 表格横向滚动位置 MUST 回到最左侧
- **AND** 普通鼠标滚轮 MUST 保持纵向滚动语义

#### Scenario: 音效字段可试听
- **GIVEN** 某个玩家可修正字段绑定音效
- **WHEN** 系统在配置审查表中展示该字段
- **THEN** 主表 MUST 展示中文友好的音效名称
- **AND** 主表 MUST 提供单条试听、停止和加载失败反馈
- **AND** 系统 MUST NOT 只向玩家显示音效 key
...
#### Scenario: AI 建议接受
- **GIVEN** 玩家提交的修改后值与配置 schema、规则证据和现有实现一致
- **WHEN** AI 审查完成
