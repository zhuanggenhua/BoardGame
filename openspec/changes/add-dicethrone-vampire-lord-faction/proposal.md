...
仓库收到吸血鬼领主（`vampire_lord` / Vampire Lord）的玩家板、提示卡、骰子、能力卡和状态图标素材，当前 DiceThrone 运行时尚未接入该角色。用户要求参考上一个派系正确命名并上传素材，同时标记为实施中。

## What Changes

- 新增 `vampire_lord` 角色目录、角色目录项、骰面、状态 / Token、卡牌、角色板槽位和资源预加载合同。
...
- 角色目录保留 `setupOptionStatus: 'in_progress'`，复用现有 `implementation_in_progress` 标记；复杂血力、催眠和复合卡机制留作后续完整规则实现。
