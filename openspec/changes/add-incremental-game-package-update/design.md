## Context
- 当前发布侧已经具备基础设施：`scripts/mobile/publish-android-game-packages.mjs` 会从 `public/assets` 筛选 package-managed 游戏资源，生成完整 ZIP、`file-index/*.json`，并把 `fileIndexUrl/fileIndexChecksum` 写入游戏包 manifest。
- 当前资源发布入口已经切到服务器主源：`scripts/assets/upload-to-server.js` 负责发布到服务器活动版本，公开读取仍走 `https://assets.easyboardgame.top/official`。
...
- 当前缺口不是“从零新增 file-index”，而是：日常服务器单文件资源更新后，必须刷新远端 `file-index` 与 manifest，并证明 App 只下载变更文件；不能把旧完整 ZIP 可读或全量兜底当成差异更新成功。

## Goals / Non-Goals
- Goals:
  - 服务器单文件资源更新后，能刷新对应游戏或共享音频包的远端 `file-index` 与最新 manifest，不把完整 ZIP 重发作为默认动作。
  ...
  - 发布、日志和验收口径必须区分“差异更新已完成”和“全量 ZIP 兜底已刷新”。
- Non-Goals:
  - 不做字节级 patch。
...
- 服务器单文件对象负责承载实际素材内容，`mobile-packages/android/<channel>/file-index/...json` 负责描述目标版本完整文件集合，`games/<gameId>.json` 负责把 App 指到当前目标版本。
- `assetPack.url` 指向完整 ZIP，但在已有远端索引和本地安装状态可用时不再代表默认下载路径。
- 发布成功判据必须来自服务器公开域名上的本次对象、`file-index` 正文大小和 SHA-256。

### Decision: `assets:upload` 默认触发索引/manifest 差异刷新
- 当 `upload-to-server.js` 发布 package-managed 游戏资源后，后续动作应按影响范围刷新：
  - 单个游戏资源变化：刷新该游戏的远端 `file-index` 与 `games/<gameId>.json`。
  - 共享音频变化：刷新共享音频索引，并刷新依赖共享音频的游戏 manifest 指针。
  - 强制重建、首次发布、兼容兜底：允许继续生成并发布完整 ZIP。
- 需要为发布脚本提供 index/manifest-only 路径，避免“小素材替换默认重发完整 ZIP”。
...
```text
public/assets
  ├─ upload-to-server.js -> 服务器活动版本 official/<relativePath> 单文件对象
...
远端 file-index + 本地 installed-files-index
  ...
  -> changedFiles 从服务器单文件 URL 下载到 staging/assets
...
- 如果服务器单文件对象和 `file-index` 不一致，App 可能下载到错误版本；发布脚本必须把变更对象、file-index 和 manifest 原子写入服务器 release，并在报告成功前读取本次 file-index 正文校验大小和 SHA-256，不能用旧 fallback ZIP 代替。

## Migration Plan
1. 调整 OpenSpec 与文档口径：明确当前已有 `file-index` 基础设施，目标是接通服务器主源差异更新闭环。
2. 扩展发布脚本：增加 index/manifest-only 或等价模式，允许服务器单文件更新后刷新目标 manifest 而不上传完整 ZIP。
3. 调整 `upload-to-server.js`：默认对 package-managed 资源变更触发差异索引刷新；只有显式参数或必要场景才重发完整 ZIP。
...
6. 验证 DiceThrone 单图替换：替换火法玩家面板后，服务器单文件、远端 manifest/file-index、App 安装日志和本地文件哈希都证明只下载差异文件。
