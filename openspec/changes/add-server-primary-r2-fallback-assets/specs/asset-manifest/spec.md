...
系统 SHALL 使用官方 manifest/file-index 和显式排除规则生成服务器运行时镜像集合，不得默认把 R2 `official/` 全量同步到生产机。

#### Scenario: 生成运行时镜像集合
- **WHEN** 后台同步准备新的服务器素材 release
- **THEN** 系统 MUST 从权威清单解析仍被运行时引用的普通素材
- **AND** MUST 排除应用更新、移动素材包、原生更新包和其他明确的大型发布产物
...
### Requirement: R2 发布必须执行零付费容量门禁
系统 SHALL 在素材上传前计算 R2 当前用量和本批净增量，并将预计用量限制在项目配置的硬上限以下。

#### Scenario: 本批上传在容量预算内
- **WHEN** 当前用量加本批净增量不超过 9GiB
- **THEN** 系统 MAY 继续现有上传和校验流程
