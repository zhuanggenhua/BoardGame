...
系统 SHALL 保持现有素材、manifest、file-index 和移动发布命令不变；命令内部通过受限 SSH 直接原子更新服务器活动版本，并仅将显式标记的当前恢复关键对象加入异步 R2 灾备队列。
...
- **AND** MUST 将当前恢复必需的包、file-index 和 latest manifest 加入 R2 后台灾备队列
- **AND** MUST NOT 将重复别名、历史版本清单或可重新生成的游戏单素材散件加入 R2 灾备队列

#### Scenario: 发布命令验证本次对象
- **WHEN** 服务器已经存在旧 fallback 包或同路径旧对象
- **THEN** 大型新产物 MUST 使用服务器来源头和本次预期大小完成验证
- **AND** 新 file-index 或 latest manifest MUST 使用服务器返回正文的大小和 SHA-256 完成验证
- **AND** 旧 fallback 包可读取 MUST NOT 被视为本次发布成功

#### Scenario: R2 无法容纳本批对象
- **WHEN** 本批关键对象灾备预计会使 R2 超过项目配置的零付费硬上限
- **AND** 没有足够的已隔离安全候选可清理
- **THEN** 系统 MUST 保留关键对象灾备队列并输出容量告警
