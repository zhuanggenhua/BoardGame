...
- 三类更新路径的历史对象合计约 8.25GiB；普通素材的 12 份分层清单约 2.55GiB，叠加当前发布对象后预计活动集合约 3GiB，适合在服务器镜像普通素材并只保留当前发布集合，而不是复制历史发布全集。
...
- 活动集合从所有 `official/**/assets-manifest.json`、各平台 `latest.json`、游戏包指针和共享包指针递归解析；普通素材按 `basePrefix/files/variants` 展开，移动 `file-index.files[].path` 映射为 `official/<path>`，发布路径只保留当前 bundle、manifest、file-index 和安装包。
