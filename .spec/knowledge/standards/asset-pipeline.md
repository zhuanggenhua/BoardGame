...
- 对 `408`、`425`、`429`、`5xx`、`524` 以及连接重置/临时网络错误等传输级失败，`assets:upload` 必须先自动重试；若仍失败，自动降级为 `128 KiB` 分块、单并发再重试一次。只有恢复尝试全部失败，才能中断并报告实际未上传的对象；鉴权、路径、权限和对象 key 错误不得被吞掉或伪装成可恢复超时。
