...
- 本节只规定文档表达：先说现实对象和动作，再写内部名词；一条规则只做一个判断。
- 规范是否重复、是否迁移、是否删除，统一按 [`spec-steward`](../../skills/spec-steward/SKILL.md) 的维护流程裁决；本文件不再复制治理清单。

## 迁移与整理

- 移动文件后必须同步更新索引、入口文档和直接引用，并检查仓内是否仍有旧路径。
...
- **入口写成正文**：索引、适配和模板只指向主源，不复写判定；主从裁决回到 `spec-steward`。
- **证据反向造规范**：evidence 只记证据和差距，新要求先补主源。
- **一句话多处手写**：需要多入口可见时，用链接、索引或生成副本，并标明主从。
