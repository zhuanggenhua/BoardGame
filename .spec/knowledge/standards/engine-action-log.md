...
- 日志里的 `playerId`、`targetPlayerId` 等玩家参数在渲染和反馈导出前必须解析成玩家可读名称；游戏 i18n 模板把它们当“玩家名”使用，禁止再硬写 `玩家 {{playerId}}`、`Player {{playerId}}` 或裸编号前缀。确实没有玩家名时，只能由公共展示层统一回退。
- 纯确认、已读、关闭或继续按钮不算玩家可见规则结果；如果命令没有产生新的公开动作或规则结果，`formatEntry` 必须返回 `null`，不得为了“有记录”把确认文案写进日志。
