...
- **棋盘特效尺寸匹配（强制）**：棋盘上的召唤、命中、推拉、传送、buff、装备、结界等局部反馈，默认必须以本次反馈对象的可见尺寸为参照，而不是以整格、整行、整屏或参考游戏截图为参照。普通单位召唤光柱、命中爆发和落点光团应包住单位或单位周边，不能大到遮挡半个格子、整格或相邻单位；全屏暗角、全场聚光和跨格大光幕只允许用于明确的大招、结算页或全局事件，并必须在游戏侧调参中显式声明。E2E / 图面验收不能只证明“有 canvas / 有亮点 / 目标格平均变亮”，还必须限制特效盒或可见主体相对目标格、单位卡、附件槽的最大比例，防止越大越容易通过。
...
- **对象离开区域必须有离场承接（强制）**：如果对象从某个可见区域被抽走、拿走、移除或收入另一区域，原区域不得让对象瞬间消失后只在目标区域播放进入动画。必须保留离场影子对象或等价承载层，直到离场动画结束；同时检查原区域剩余对象的补位/重排是否平滑。若做不到稳定离场，宁可不加这段动画，不得交付“先真实落位再补动画”的瞬移效果。
- **真实列表与动画对象不得同帧重影（强制）**：状态列表已经逻辑增删时，前端真实列表不得先渲染目标对象再补动画，也不得让同一对象同时出现在真实列表和动画层。必须用视觉缓冲/held object 控制显隐，并由动画完成回调释放；对象、提示文案、占位影子必须共用同一生命周期，禁止用固定 timer 分别关闭。
