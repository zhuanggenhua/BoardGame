...
本文档承载 Reducer 消耗、写入-消耗对称、多来源竞争、清理、UI 同步、条件优先级、组合场景、伤害管线、架构假设和 handler 状态一致性细则；正文可引用其他 D 编号作为交叉门禁。
...
**D15 子项：规则状态、视图摘要与估算隔离（强制）**（新增/修改玩家可见正式数值、AI 估算、动画骰面、日志摘要或调试读模型时触发）：同一个现实数值若同时存在规则状态、玩家显示、AI 预估或动画预览，必须先声明哪一个是规则写入口。**核心原则：命令 / 事件 / reducer / domain helper 承担规则写入；UI selector、playerView、动画、日志和 AI hint 只能读取或估算，不能补写、回推或冒充正式规则结果。** 审查方法：
1. **列出数值身份**：把本轮数值拆成正式规则状态、玩家可见摘要、AI/策略估算、动画/hover 预览、测试/debug 辅助五类；每一类写清只读还是可写。
2. **锁唯一写入口**：会影响合法性、响应窗口、最终事件、血量/资源/骰子/分数结果的值，只能从命令、事件、reducer 或明确的 domain helper 进入 core 状态。
3. **selector 缺值 fail-close**：玩家可见正式摘要缺少规则状态时，应返回缺数据、隐藏该摘要或报清晰错误；不得从 AI 估算、技能定义、动画骰面、测试标签或旧快照静默补算。
4. **估算 helper 命名隔离**：如果规则门槛或 AI 需要动态估算，helper 名称和注释必须说明适用范围；同一 helper 不得同时服务玩家正式摘要和 AI/门槛估算。
5. **回归断言形状**：至少补一条负向测试证明非权威值不能进入玩家正式摘要或最终结算；若规则门槛确实需要估算，再补一条正向测试证明该估算只在门槛/AI 场景生效。
6. **跨游戏代表检查**：发现 DiceThrone 伤害类问题后，至少抽一个非伤害游戏数值代表（如 Betrayal 骰子结果、属性检定、怪物移动骰）验证正式结果来自规则提交状态，而不是动画、预览文案或 AI 计划。
...
8. **新增 / 实施中批次补审范围**：若该问题出现在新增派系 / 新角色 / 新英雄，或当前配置 / UI 已标记为“实施中”的批次里，不能仅凭批次标签把补审矩阵扩大为对象全集。先列出共享消费者、同一合同源或其它直接影响证据；只有这些证据实际命中多个对象，或用户/任务明确要求整批审计时，才按最小受影响对象集列出全集。不命中召唤阶段或 AI 消费的对象仍需在已锁定扩审范围内写明“不适用原因”，范围外对象登记为候选，不自动复核。
