...
- 允许连续的、在最新乐观状态上仍然合法的玩家操作继续预测；不得为了避免一次重复输入而把整个连续预测链路关闭或退化成每步都等待确认。
- 对无法在乐观状态上安全预测、或与正在执行命令竞争的重复输入，最多保留一个待重试命令并进入唯一恢复 / 重同步入口；内部排队提示不能被误当成业务成功，也不能无限堆积命令。
- 阶段推进、确认、结束回合等单步按钮应在确认前提供明确的处理中反馈，AI 或其他玩家同时改变状态时旧版本命令应快速拒绝并立即同步；恢复后必须继续允许后续合法操作。
- `stateID` 是全局权威版本；按座位裁剪后的投影视图可以合法跳过不可见版本。客户端只有在补丁无法应用或收到倒退版本时才请求全量同步，不能把“版本有间隙”本身当成断线。
