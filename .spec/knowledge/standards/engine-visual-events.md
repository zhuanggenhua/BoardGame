...
接入数值动画时，正式数值必须已经先写入 core；冻结值只能由“正式 core 值 +/− 本次正式变化量”反推为动画前显示值。UI 组件读取时必须以 core 正式值作为 `get(key, coreValue)` 的 fallback，动画层不得写 HP、资源、骰子点数等正式数值，也不得用预估或展示态生成正式变化。
...
- 数值条、血条、骰子面等 UI 只能消费正式状态同步出来的当前值和最大值；需要动画延迟时，用视觉缓冲临时覆盖显示值，impact 后释放回正式状态。禁止让动画、浮字、投影结果或估算 helper 成为正式数值的写入来源。
...
通用框架组件，用于需要玩家阅读、复盘或确认的卡牌特写。基于 EventStream 驱动，支持队列堆叠（有上限），通过玩家主动关闭动作关闭当前特写。

**适用边界（强制）**：只有展示本身承担“玩家必须读懂这张牌 / 这个结果后才能合理继续”的语义时，才接入 `CardSpotlightQueue`。普通仪式性反馈、飞牌、闪卡、飘字、分数飞行等不承担读牌权的瞬时反馈，继续走 FX / animation 自动退场；不得因为本节存在，就把纯动效升级成“看清后可关闭”。大杀四方行动卡打出展示会影响玩家理解刚刚打出的牌，属于卡牌可读特写，必须接入 `CardSpotlightQueue`，不得走短计时自动退场 FX。
...
- **明确关闭**：只能通过玩家主动关闭动作关闭当前项；当前项目允许的主动关闭动作是点击空白背景或清楚的关闭按钮。不得用卡牌本体点击、鼠标移出、自动计时、prompt 出现、联机确认或重同步来关闭
- **透明收口层**：卡牌特写可以使用无暗罩的透明全屏点击层承接“点击空白关闭”，但不得铺整屏暗色 backdrop、毛玻璃遮罩或解释性 prompt 壳层遮住棋盘上下文；卡牌本体点击不得误关闭
- **迁移不得改变既有可见合同**：把展示从一种实现迁到另一种实现时，默认保持原有玩家可见合同，包括主展示对象的位置、尺寸、层级、遮罩强度、入退场方式、关闭动作和可点击区域。辅助控件、队列提示、标签、命中层或测试便利元素不得改变主展示对象的既有锚点和视觉重心。只有存在明确 UI 合同、历史证据或用户当轮授权时，才允许改变这些可见维度；改变后必须补与该维度对应的真实入口断言或截图证据。
...
- **阻塞阅读禁止自动退场**：需要阅读/复盘/确认且玩家没读完就无法合理继续的卡牌特写、对手进攻技能特写、能力槽裁切展示、升级卡展示、揭示牌、连续发现展示队列等 UI，不允许用 `autoCloseDelay`、短 timer、卡面/技能本体点击、hover/blur 或 prompt 切换来关闭。必须提供点击空白背景、关闭按钮或继续按钮这类明确玩家动作；测试必须断言超过旧自动关闭时间后仍可见，并通过明确玩家动作收口。反过来，纯动效反馈不得套用本条升级成交互式特写；剧本书 / 规则书 reader 属于长文阅读界面，不能用来替代卡牌/发现特写队列验收。
...
|------|---------|-------------------|
| 交互性 | `pointer-events-none`，纯展示 | 点击空白背景或关闭按钮关闭；卡牌本体点击不关闭 |
| 生命周期 | 定时自动消失（`timeoutMs`） | 用户主动关闭 |
| 适用场景 | 力量浮字、VP 飞行、飞牌、飘字等瞬态特效 | 需要玩家阅读/复盘/确认的卡牌展示 |

### 已接入游戏

- **SmashUp（大杀四方）**：行动卡打出展示接入本队列；超过旧 FX 超时时间后仍应可见，直到玩家点击空白背景或关闭按钮。
- **DiceThrone**：当前使用游戏层自建的 `CardSpotlightOverlay` + `useCardSpotlight`；卡牌特写默认不自动关闭，点击空白背景关闭。只有具体规则或教程合同显式传入自动关闭时间时，才允许短 timer。
