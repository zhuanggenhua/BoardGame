...
动代码前先按 [`conversation-handoff-target-lock`](../../knowledge/standards/conversation-handoff-target-lock.md) 锁定通用四项前提。本 skill 额外补齐策略专属内容：决策阶段 / 动作族、命中原始症状的反馈与规则资料、AI 运行入口，以及用于判断收益和风险的代表性回归场景。缺任一项时，先补证据或问最小问题，不要直接改。
