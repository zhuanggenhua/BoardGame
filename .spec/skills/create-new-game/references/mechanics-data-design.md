...
本文档承载机制分解、引擎原语映射、数据结构设计、数据录入、类型定义与领域建模门禁。
...
| 授权玩家 / 禁止动作 | 谁能提交该动作；当前窗口里哪些按钮或命令必须不可见 / 不可用 |
...
- **权限从焦点 / 视角 / 当前回合玩家推断**：规则要求响应、打断、结束阶段、结算攻击、防御、私有选择或 AI 自动动作时，必须先按 `.spec/knowledge/standards/rule-driven-interaction-design.md` 建立权限矩阵；焦点玩家、视角玩家或当前回合玩家不能直接授予规则权限。
...
2. **数据量大、用户只提供了部分时**：
   ...
   - 只要基础版对象全集仍有缺口，阶段状态必须保持 `in_progress / S0-blocked` 或 `in_progress / downstream-blocked`，不得用“已有数据先落地”替代完整性门禁；这里的 blocked 是阶段门禁，不是数据层停工理由
...
#### 静态配置默认走 GameConfigPackage（新游戏强制）

新游戏的卡牌、单位、事件、建筑、token、派系、牌组构成、起始部署、素材引用和能力绑定等静态事实，默认先建模为 `GameConfigPackage`，再由运行时加载、校验和物化。详细边界以 `.spec/knowledge/standards/game-config-package.md` 为准。
...
2. 官方仓库真相源统一用严格 JSON；JSONC、YAML、XLSX 只作为未来导入、导出或玩家编辑辅助，进入系统前必须先转换成 JSON，不能作为运行时唯一真相源。
3. 特殊效果只在配置里声明 `abilityId + params`；代码能力模块负责实际执行。能力 ID 不存在、需要代码支持但未实现、参数不符合合同，都必须在加载校验时暴露。
4. 表格审查视图必须来自配置包物化结果；不得维护一份和运行时分叉的展示表。
5. 玩家修正走结构化反馈提案，保存对象 ID、字段路径、当前值、建议值、说明、证据和上下文；AI 只做首轮审查，不直接写官方配置。
6. 若新游戏暂时跳过配置包，必须在 OpenSpec 写明跳过原因、影响范围和后续补齐任务；不得只因为 TypeScript 写起来快就把静态事实直接写死进代码对象。

旧游戏不按本条自动重构。已有清晰 TypeScript 配置、注册表和测试的游戏，若只需要玩家审查，优先做只读 adapter 生成配置表格；只有明确要迁移官方真相源时，才单独建立重构任务。
...
2. **缺口必须显式保留在数据层，不得藏进实现层**
   ...
   - 基础版必需项存在 `missing / blocked / disputed / unknown-slot / not-in-runtime` 时，阶段状态必须保持 `in_progress / S0-blocked` 或 `in_progress / downstream-blocked`；不得进入 Board/UI、机制实现、E2E 或“基础版完成”。这只表示依赖该缺口的后续实现被拦住，不表示数据层停工；仍能从主真相源、atlas、完整裁图、现有合同或只读代码继续核对时，必须继续补录合同字段和缺口说明。
...
**强制要求（新游戏）**：
- 禁止自行实现 statusEffects / tempAbilities / DamageModifier / PowerModifierFn / abilityRegistry；必须复用上述 primitives（详见 `AGENTS.md` 与 `.spec/knowledge/standards/engine-systems.md`）。

**若缺口存在**：优先补充 `src/engine/primitives/`（通用工具函数）；领域语义放在游戏层（`src/games/<gameId>/domain/`）。若工作量大，记入延后清单，在后续阶段补充。

### 2.5 领域建模前置审查（强制门禁）

> 完整规范见 `.spec/knowledge/standards/engine-systems.md`「领域建模前置审查」节。
> 核心原则：**规则文本 → 领域模型 → 实现**，禁止跳过建模直接写实现。

1. **领域概念建模**：为每个规则术语定义精确语义边界和事件映射（如"影响"= 哪些具体事件）。
2. **决策点与权限矩阵识别**：标记所有需要玩家选择的点（强制/可选/无），并按 `.spec/knowledge/standards/rule-driven-interaction-design.md` 推导授权玩家、禁止动作、关闭条件和消费者一致性；评估引擎是否支持该交互模式。
...
产出：术语→事件映射表 + 决策点清单 + 权限矩阵 + 基础规则语义覆盖矩阵 + 引擎缺口清单。
...
- **基础规则语义覆盖矩阵已闭合**：基础版必需项没有 `missing`、`blocked`、自动代选未批准、权限从焦点/视角/当前回合玩家推断、裸数值替代轨道、日志替代进度承接、空间朝向/连接数据缺失
