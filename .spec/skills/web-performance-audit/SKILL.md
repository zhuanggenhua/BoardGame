...
改代码前先按 [`conversation-handoff-target-lock`](../../knowledge/standards/conversation-handoff-target-lock.md) 锁定通用四项前提。本 skill 额外记录性能专属口径：目标场景、浏览器与视口、是否经过图片门禁、基线指标和可感知结果。若复测跳过关键图片门禁，必须明说本次只验证页面渲染性能，不代表图片预加载链路健康。
...
4. 动画/特效性能优化必须遵守 [`animation-effects`](../../knowledge/standards/animation-effects.md) 的“设计时先锁表现与成本”和“性能优化不得偷改表现”；本 skill 只负责排查、复测和收口，不另建第二套动画表现规则。
5. 重计算结果用 `useMemo` 或提前归一化，但不要为了理论问题新增第二套真相源或兜底状态。
6. React 优化优先稳定 props、拆小组件、避免无意义 context 扇出；只有实测命中时才加 `memo`。
7. 图片优化优先修加载层和资源体积；跳过图片门禁只能叫“绕过验证入口”，不能叫图片链路修复。
8. 止血、降噪、跳过、吞异常只能标为缓解；原始用户症状必须回到同一入口复测。
...
- 对动画/特效类修复，除性能指标外至少保留关键视觉证据：前后截图、录屏或 start/mid/end 关键帧说明；只能证明效果仍可见时，不能声称逐帧等价。
