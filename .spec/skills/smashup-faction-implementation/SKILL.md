...
1. 先建立批次清单
   - 至少到单卡 / 单基地 / 单对象粒度
   - 每项标明当前状态：`未核图 / 已核图 / 已修数据 / 已过最终状态 / 已过真实入口 / 已过生命周期`
2. 默认连续清空当前批次
   - 用户连续发“继续”时，语义是“继续做下一个未完成对象”，不是“继续汇报上一个对象”
3. 默认先修真相，再补证据
   - 先解决卡图/数据/语义偏差，再补对象级真实入口和生命周期证据
4. 对象级补证不得偷换成整包收口
   - 只要批次清单里仍有未完成对象，就只能维持“仍有残余范围”
5. 每次停下都必须有剩余项清单
   - 若遇到硬阻塞或用户切换任务，必须明确写出“剩余哪些对象未做、分别卡在哪层”
6. 只有同链路纯配置差异才允许复用深层证据
   - 若两张卡/两个基地只是 `id/名称/文案/数值/目标枚举/筛选参数` 不同，而真实入口、validator、handler、resolver、reaction/queue 与清理链完全一致，才允许复用同一条代表链
...
6. 抽样发现问题后必须回到全量矩阵
   ...
   - 全量矩阵每行至少覆盖：牌面或规则原文、原子语义、语义限定词、静态字段、注册入口、validator/canActivate、handler/trigger/reducer、UI/交互出口、测试证据、当前证据轴和残余范围。
...
- 在本工作流里，用户或任务只说“审计 / 重审 / 收口审计 / 深入审计”时，默认含义是对**已锁定对象范围**做对象级彻底审计，不自动扩大到整批派系。
- 对象级彻底审计的最低要求：当前锁定范围内每张新增卡、每个新增基地都必须有独立矩阵行，逐子句写出真相源、静态接入、最终状态、真实入口、生命周期证据或阻塞原因。
...
- **真实入口**
  ...
  - 任何对象 / 目标 / 支付物 / 顺序 / 数量 / 模式选择都必须证明玩家手动选择；即使当前合法候选只有 1 个，也必须截图并断言该选择态真实停住，不能被 `autoResolveIfSingle` 或等价默认隐式代选吞掉。截图里的候选本体必须清楚无遮挡；如果被卡牌预览、hover 特写、displayCard、提示浮层或 HUD 遮住，不能算该步真实入口证据。只有所有玩家决策都完成后的固定结果收口，才允许自动推进。
- **生命周期**
...
- 只要本轮新增对象引入了**新的交互类型**或**新的 UI 表现 / 新的交互组件 / 新的操作方式**，就必须至少补 `1` 条该新类型 / 新 UI 的 direct E2E，并覆盖真实入口与生命周期。
...
- 只有真相源 / 静态接入通过，最多只能写“结构已对齐”；
- 只有最终状态通过，最多只能写“行为级已验证”；
- 只有真实入口和生命周期补齐，才允许把单对象升级为“对象级正路径证据”；
- 缺生命周期时，不得因为 E2E 通过就默认流程完整。
- 真实入口 / 生命周期证据的复用只允许发生在“共享链路完全同构，仅配置不同”的对象之间；否则即使入口类型相似，也必须单独做对象级证据。
