...
> 角色：index / adapter。
>
> 本文件只负责渐进式披露和阅读顺序，不承载新的审计规则正文。审计规则的唯一真相源仍是 .spec/knowledge/standards/ 下对应的规范；测试工具文档只说明工具用法，不证明规则正确。

## 先按问题类型路由

按下面顺序判断，不要按“反馈”“截图”或“测试”单个关键词直接选文档：
...
3. .spec/knowledge/standards/testing-audit.md：审计入口、证据层级和结论边界。
4. .spec/knowledge/standards/testing-audit-core-principles.md：fail-close、范围、深度审计步骤和核心矩阵。
5. .spec/knowledge/standards/testing-audit-dimensions.md：只用于选择命中的 D 维度。
6. .spec/knowledge/standards/audit-evidence-template.md：需要写 evidence 或对外使用“已审计/已收口”时读取。
...
不要默认把所有 D 维度分卷全文加载。先在维度索引里按用户原始症状选择，再只打开命中的主题分卷：

- 语义、对象、基础版/升级版差异：先看语义/交互分卷。
- 资源、CP、Token、扣除、触发时机：先看资源/时序分卷。
- reducer、pending 状态、跨阶段读取、清理、UI 同步：先看状态管线分卷。
- 延迟交互、恢复、补发、等待和最终收口：先看 deferred interaction 分卷。

如果用户一次反馈包含多个独立症状，必须按症状逐行选择维度；不能用一条代表链或一张截图替代整批。
...
- 只读 testing-audit.md 入口，不读核心原则、命中 D 维度和 evidence 模板，却声称“全面审计”。
