...
#### 5.0 用户反馈“AI 瞬间 / 太快 / 1 秒无效”时先跑真实时间线

只要用户反馈 AI 行动太快、瞬间轮回真人、1 秒延迟无效或语义等价问题，先保留原始体感描述并跑真实页面诊断；不得先用静态可见 / 隐藏分类口径否定为非 bug。

最低证据必须同时包含：

1. 真实入口的行动权时间线：从真人交给 AI、AI 当前玩家 / 阶段、到真人重新可操作的毫秒数。
2. 当前 AI 执行权归属：浏览器本地 AI 还是服务端在线 AI；若 `AI_RUNTIME_TRUTH` 显示 `authority=server-online-ai-executor`，不能只等 `[LOCAL_AI_PERF]`，还要看服务端状态切换或 `[ONLINE_AI_BATCH_TRACE]`。
3. 动作延迟计划：可见动作记录 `minimumDelayMs / remainingDelayMs / waitedMs`；静默动作记录为什么不等待。
4. 复现前后的同一用例对照：修前诊断可以失败，修后必须回到同一真实入口通过。

这类首轮 E2E 是诊断 / 红线基线，不是最终验收；只有修复后同一链路通过，才允许说“这次修复覆盖了用户反馈的瞬间问题”。
