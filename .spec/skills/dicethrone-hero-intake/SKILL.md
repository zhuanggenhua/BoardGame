...
至少写清主真相源、对照源、获取日期、当前 worktree、本轮 scope、冲突项、已直接查看的完整单卡 / 玩家板槽位 / 提示板块，以及当前差异属于录入错、索引错还是运行时消费错。每个能力和 Token 还必须按项目通用录入规范登记使用门槛、实际成本、效果结果、次数与重置、累计档位、选择目标和负向路径；提示卡中的原文不能被一句摘要替代。每张实际读取过的图片都必须在合同中有登记行、最终处置和处置依据；没有完整图片登记、处置依据和字段消费映射，只能标记为 `partial / blocked`，不得进入机制实现、审计通过或玩家可见生命周期。

录入责任门禁：只要提示卡或能力板写出“拥有 / 花费 N 个 Token”“每回合每个效果一次”或多个递增档位，未逐项保留图面原文、结构化字段和实现去向，就只能标记为 `partial / blocked`，不得进入机制实现、审计通过或玩家可见生命周期。审计发现合同与完整提示卡冲突时，先退回录入合同修正，不能让旧合同、旧测试或旧实现继续作为规则依据。
