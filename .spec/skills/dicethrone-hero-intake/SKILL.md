...
完整录入的唯一通用定义来自 [`data-entry.md` 的完整语义录入门禁](../../knowledge/standards/data-entry.md#完整语义录入门禁)。Dice Throne 录入时必须特别枚举：
...
- Dice Throne 新派系默认使用共享玩家板 `v2` 布局；只有已有证据证明仍是旧物理排版时，才允许显式登记 `v1`。
- 新英雄接入共享 UI 前，必须在唯一布局注册表中显式登记正式玩家板原图的实际宽高和布局版本；不能依赖未知角色回退到其它英雄尺寸或旧版布局。
- 实现前必须只读对照至少一个已收口英雄，逐项核对玩家板宽高比、左右技能槽、终极槽、提示板组合和放大预览；缺少对照证据时只能标 `blocked`，不得进入真实入口验收。
- 共享层缺少新英雄登记、仍命中默认回退、实际素材尺寸与注册尺寸不一致，或 `v2` 物理槽与底图标题不一致时，先修几何合同和测试，再谈资源 / 规则 / 截图通过。
...
- **真实入口**：按 [新增派系玩家可见生命周期](../add-new-faction/SKILL.md#玩家可见生命周期) 验收；隐藏阶段先证明玩家入口不可见，同时用内部状态注入或测试入口验证玩家板、提示板、手牌 / 卡图和 HUD；进入实施中或完成态后才要求真实在线双玩家可选。最终说 E2E 通过时，必须给出本轮实际核对过的截图 `VIEWER_URL`；不在最终回复裸露完整图片路径。
- **交互截图**：若新英雄新增 Token 主动消费、状态移除、强迫重掷、临时骰面结果、升级后新 variant、防御投骰、可选确认、响应窗口或其它玩家新交互，必须按 [E2E 流程截图](../../knowledge/standards/e2e-verification.md#流程截图) 分交互族提供真实牌桌截图组，或在 evidence 写清共享流程判等。选角图、角色卡、玩家板静态图只算生命周期入口或资源显示证据，不能证明新交互通过。

### 玩家可见状态闸门

生命周期的唯一规则来源是 [新增派系玩家可见生命周期](../add-new-faction/SKILL.md#玩家可见生命周期)，本专项只规定 DiceThrone 的落地方式：

- `hidden`：实现或当前范围审计尚未完成时使用。角色保留在完整内部目录，但必须从真人选角、AI 自动选角和直接玩家命令中过滤。
- `in_progress`：实现已经完成且当前范围审计 evidence 已完成后使用。角色才允许出现在真人选角入口，继续保留 `implementation_in_progress` 标记；AI 自动选角仍过滤。
- 完成态：只有用户在当前轮明确批准进入完成态后，才能移除 `implementation_in_progress` 标记并允许 AI 自动选角。审计通过、测试通过、截图通过、任务勾选以及用户只说“继续”都不能替代这次真人确认。

不得由测试、审计脚本、E2E、任务清单或 agent 自动把 `in_progress` 改成完成态；没有真人明确批准时，即使当前范围已经审计通过，也必须继续保留实施中状态。
...
至少写清主真相源、对照源、获取日期、当前 worktree、本轮 scope、冲突项、已直接查看的完整单卡 / 玩家板槽位 / 提示板块，以及当前差异属于录入错、索引错还是运行时消费错。每个能力、Token、状态和卡牌还必须按项目通用录入规范建立完整特殊含义元素清单，并登记使用门槛、实际成本、效果结果、次数与重置、累计档位、选择目标和负向路径；提示卡或牌面中的原文、图案语义和区域关系不能被一句摘要替代。每张实际读取过的图片都必须在合同中有登记行、最终处置和处置依据；没有完整图片登记、特殊含义元素清单、处置依据和字段消费映射，只能标记为 `partial / blocked`，不得进入机制实现、审计通过或玩家可见生命周期。

录入责任门禁：只要提示卡、能力板或卡牌存在费用角标、类型条、伤害框色、骰面 / Token / 状态图标、重复数量、分区对应关系，或写出“拥有 / 花费 N 个 Token”“此外”“每有 / 每层”“否则”“至多”“每回合每个效果一次”和多个递增档位，未逐项保留图面原文或描述、结构化字段和处置 / 实现去向，就只能标记为 `partial / blocked`，不得进入机制实现、审计通过或玩家可见生命周期。审计发现合同与完整提示卡或完整牌面冲突时，先退回录入合同修正，不能让旧合同、旧测试或旧实现继续作为规则依据。
...
- 不得因为运行时暂无字段或当前机制尚未实现，就从录入合同删除牌面上已经确认的特殊含义；必须保留并标记暂未消费的去向。
...
- 玩家板实际尺寸、宽高比、布局版本、技能槽坐标、终极槽和放大预览几何。
...
- 资源本地路径与远端回查表。
- 测试命令与结果。
- 每组关键截图的 `VIEWER_URL` 与肉眼观察；若包含新交互，按交互族列出截图组，不能只列选角或入口图。源路径只保留在 evidence。
