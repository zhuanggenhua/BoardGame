...
- 本轮只要实际执行了 `git commit`，阶段汇报或最终汇报必须回报完整提交消息：单提交至少列出提交 SHA、标题和正文要点；多提交逐条列出，后续为修 hook / push / deploy 追加的修复提交也必须一起回报。
...
- 默认继续做当前 Git 目标所需的最小修补，直到改动可提交 / 可推，不能因为“需要编辑”本身停下。
- 只有出现必须人工裁决的问题才停止，例如：要接受或新增质量债、更新 warning baseline、改生产行为含义、改测试期望、回补旧行为、处理未归属脏改、决定是否放弃某边有效内容，或选择绕过门禁。
- 纯机械 i18n key、格式、小类型字段、空白、明显遗漏导入 / 导出等可直接修；i18n 缺 key、可由现有代码和相邻语言包确定的标题 / 标签也直接补齐并重跑检查。
- 需要人工判断时必须说清现实后果、当前证据、为什么不能继续自动修，以及最小可选动作；不得只把工具失败、字段名或测试名丢给用户。
...
- 不把“要不要 fork”作为用户选择题。用户通常不知道 fork、upstream、origin 的含义，AI 负责根据当前认证身份和远端写权限自动选路。
- 有主仓写权限时，直接在主仓创建特性分支、推送分支并创建 PR。
- 没有主仓写权限，或直推明确返回 `403` / `Write access to repository not granted` 时，自动切换到当前账号的可写 fork，再向主仓目标分支创建 PR。
- 对用户只说明现实结果：当前账号不能直接写主仓，AI 已使用可写副本提交 PR。不要要求用户理解或手动决定 fork。
- 只有目标分支、提交范围、是否提交或是否合并等会改变结果的事项，才向用户询问。

固定路由确定后，不能直推主仓时走 fork + PR：
