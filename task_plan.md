...
- [x] 2026-06-07 01:20 +08：继续沿《七大恨》正式规则实施推进，这轮先把 `src/games/qidahen/domain/index.ts` 里混入的 Git 合并冲突标记清掉，恢复《七大恨》定向测试可跑基线；随后继续补 `payment-selection.test.ts` 里几条仍偏弱的真实焦点守卫，没有新建 OpenSpec spec/change，也没有新增领域实现语义。当前补强并确认的入口/等待态包括：`征召军队` 进入 `recruit-choice` 时焦点稳定留在 `song-jin`；`驱虎吞狼` 目标同意后进入 `dispatch-targeting` 时焦点与来源区都保持 `jinzhou`；`新年防线维护` 进入 `season-resolution` 等待态时焦点保持 `song-jin`；`孙元化与袁崇焕同时在场时会先进入弃 2 牌打科技选择` 这条链当前真实焦点不是点击的 `song-jin`，而是继续保持既有当前区 `city-region-25`；`超限弃牌` 收尾回到 `action-window` 时焦点保持 `city-region-14`；`raid` 进入 `resolve-pending` 的第一拍焦点会先收回真实来源区 `jinzhou`；`调骑 4` 的限制态与“无骑兵不进目标选择”两条链，当前都已锁到真实来源区 `city-region-16`；两条 `dispatch-targeting` 入口也补上了 `selectedRegionId / sourceRegionId` 的显式守卫，避免只断 `turnPhase`。验证结果：`node scripts/infra/vitest-cli-safe.mjs run src/games/qidahen/__tests__/payment-selection.test.ts src/games/qidahen/__tests__/movementRules.test.ts --configLoader native --pool threads --no-file-parallelism --maxWorkers 1` 为 `261 passed`；`npx eslint src/games/qidahen/domain/index.ts src/games/qidahen/__tests__/payment-selection.test.ts --max-warnings 0` 通过；`npx tsc --noEmit --pretty false` 通过；对整个 `src/games/qidahen/__tests__` 跑“`turnPhase` 邻域缺少 `selectedRegionId / sourceRegionId`”窄筛，当前结果已清零。边界：这轮主要价值是“先恢复 runner，再继续补真实焦点守卫，并把当前测试层同型残口窄筛清到 0”，不代表《七大恨》所有剩余相位都已穷尽；下一步继续从其它未被这类测试扫描覆盖的实现边界或更高层验证入口推进。
...
## Current Addendum（2026-06-07 06:58 +08）

- [x] 已修复两类会直接阻断《七大恨》UI 焦点验证推进的真实 blocker：
  ...
  - `e2e/qidahen-basic-flow.e2e.ts`
- [x] 本轮修复内容：
  - 清掉 `src/games/qidahen/manifest.ts` 残留的 Git 冲突标记，恢复 isolated-single E2E 游戏服务打包基线
  - 重生成 `src/games/manifest.generated.ts` / `manifest.client.generated.tsx`，去掉过期的 `./archview/manifest` 导入，恢复 `/play/qidahen/tutorial` 前端加载
  - 修正“蒙古跨到后金后进入新年防线维护”链路：进入 `season-resolution` 时显式把 `selectedRegionId` 重锚到 `song-jin`，不再沿当前玩家默认选区漂到 `city-region-13`
  - 新增领域回归：`蒙古跨到后金的新年防线维护等待态会重新锚定逻辑区辽西，而不是沿当前玩家默认选区漂到建州`
  - 已把 UI 层 3 组 `data-map-selected` 焦点断言全部跑通：
    - 手动超限弃牌：`city-region-14`
    - 驱虎吞狼同意/调度目标：`jinzhou`
    - 新年防线维护等待态：`song-jin`
- [x] 已完成验证：
...
- [ ] 当前仍未完成：
  - 《七大恨》正式实施主线仍未整体收口
  - 这轮已把测试层 + UI 层的一批焦点守卫和两类启动基线 blocker 收掉，但仍要继续找其它未被当前批次覆盖的高层边界
- [ ] 下一步：
