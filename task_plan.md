...
遵循 `.codex/skill/add-new-faction/SKILL.md` 与 `docs/games/dicethrone/workflows/dicethrone-hero-intake.md`，把 DiceThrone 新英雄战术家（`zhanshujia`）与咒缚海盗（`cursed_pirate`，素材目录 `cursed`）从本地素材接入到可验证的角色数据、资源链、规则文档、机制实现、审计 evidence 与真实入口 E2E。若遇到必须人工裁定的图面/规则不确定项，立即停止 goal 并汇报阻塞点。
...
- 2026-06-06：completion audit 的剩余项已从“看起来还有几块没收口”继续压成可执行子 family。当前 `诅咒金币` 应按 `direct writer / continuation writer / self-remove / later consumer` 这 4 段看，`火药桶` 应按 `direct grant / threshold writer / multi-target choice / upkeep transfer / no-attack passive` 这 5 段看；咒缚海盗奖励骰也不再笼统记成一桶，而是已拆成 `起锚/虚张声势` 单骰即时分派、`瞭望台` 信息/弃牌交互、`干票大的` 双骰聚合资源、`死亡印记/抽筋剥皮` 攻击期多骰、`啜呼` 目标方 choice + 状态双写入。当前 hold 只剩这些子 family 的 final verdict 与双面 face-by-face completion audit，不再是“技能待重录”。
- 2026-06-06：咒缚海盗状态 family 又向前推进了一步：`凋零 / 休战` 现在都不再只依赖单一攻击来源的 live consumer。最新定向 E2E 已补到 `死亡吐息` 这条第二来源，分别锁定 `凋零 1` 会把 `7` 点攻击伤害压到 `6`，以及 `休战 1` 会在同一 `breath-of-death-small` 攻击链里阻断攻击伤害、但仍保留 `凋零 / 火药桶` 状态写入并在阶段结束清理。当前这收掉的是 family next-proof gate 里“还缺第二条来源”的旧口径；整派系仍未完成，剩余继续回到 `诅咒金币 / 火药桶` 的 family 级合法复用登记、奖励骰 family 分组封版、双面 face-by-face completion audit，以及最终是否允许移除 `implementation_in_progress`。
- 2026-06-06：completion audit 的对外口径已进一步收紧。当前应明确区分“实现已落地但审计未封版”和“规则仍未实现”两类状态：咒缚海盗 human/normal 面、人类面板资源链、human 面 9 个对象与 cursed 面 9 个对象，当前都不再属于“要整套重录”的范围；更准确的剩余项是状态 family、奖励骰 family、双面 face-by-face completion audit，以及最终是否允许移除 `implementation_in_progress`。后续若继续回写 evidence，应优先维持这条口径，不再把双面 remaining 表述成“human 面尚未实现”。
- 2026-06-06：最新一轮 completion audit 已把 intake 尾段重新按权威分段补跑，而不再把“30 分钟 full-file 命令未跑完”误记成业务红灯。当前新增静态验证 `zhanshujia-cursed-pirate-intake.test.ts -> 8 passed`；尾段真实入口补跑里，`human-cursed 无币翻回咒缚面` 为 `1 passed`，`走跳板 / 做好标记 / 弯刀突刺` 为 `3 passed`，`无情诅咒 4 人 targetingRoll / 地毯式轰炸双敌链` 为 `2 passed`。这一轮唯一打出的真实红灯是两条 `火药桶` 转交 E2E：helper 仍旧等待“回到空白 discard 态”，而当前真实运行时合同已经是 `upkeep + simple-choice + currentChoiceSourceAbilityId=upkeep-powder-keg`；现已在 `e2e/dicethrone/zhanshujia-cursed-pirate-intake.e2e.ts` 修正 `playPowderKegUpkeepTransfer(...)` 的等待口径，并复跑 `火药桶维持阶段投 6 后的转交链 / 火药桶转交给已持有者时的原桶爆炸链` 为 `2 passed`。当前口径应更新为：intake 尾段最新真实入口链已补齐到当前代码状态，但整派系仍未完成，剩余继续回到双面 completion audit、family 级 `L4` 合法复用登记，以及最终是否允许移除 `implementation_in_progress`。
...
- 2026-06-06：`海盗的一生` 已从“只有机制层分支锁定”推进到双面对象级真实入口。当前新增并打绿的权威单跑包括：`真实入口应展示并结算海盗的一生在咒缚面的治疗分支`（截图 `210-211`）与 `真实入口应展示并结算海盗的一生在普通面的诅咒金币选择链`（截图 `212-214`）；两条都使用显式后台日志落盘方式绕过本机当前 heavy-budget/guard 噪声，结果均为 `1 passed`。这一步收掉的是 `海盗的一生` 仍只到 `L2` 的旧口径；当前剩余项继续回到双面 completion audit、状态/手牌 family 的 `L4` 合法复用登记，以及最终是否允许移除 `implementation_in_progress`。
...
- 2026-06-05：已补齐 `public/locales/zh-CN/game-dicethrone.json` 中咒缚海盗 human 面 9 个对象的正式中文描述，移除“待正式录入：当前仅接入名称与槽位映射”占位。当前前台展示层已与运行时能力定义、真相源表和录入核对文档对齐；这收掉的是文案层录入缺口，不等于 `cursed_pirate` 已整体完成，`implementation_in_progress` 继续保留。
- 2026-06-04：已补咒缚海盗 human 面剩余 3 个对象的运行时缺口。`点燃炸药` 显式把伤害归到 `preDefense`；`判决指令` 与 `无情劫掠` 改为本地 choice continuation，在诅咒金币选择完成后继续结算后续 `休战 / 火药桶 / 伤害`。验证：`npx vitest run src/games/dicethrone/__tests__/zhanshujia-cursed-pirate-mechanics.test.ts -t "human 面点燃炸药|human 面判决指令|human 面无情劫掠" --configLoader native` 通过；`npx vitest run src/games/dicethrone/__tests__/zhanshujia-cursed-pirate-mechanics.test.ts src/games/dicethrone/__tests__/zhanshujia-cursed-pirate-intake.test.ts src/games/dicethrone/__tests__/ninja-slash-stand-tall-regression.test.ts --configLoader native` 为 `3 files / 67 tests passed`；`npx tsc -p tsconfig.json --noEmit` 通过。当前不能说“规则都实施了”，真实剩余项已从这 3 个对象收窄回对象级彻底审计、复杂交互逐项 L3/L4 与整体 soak 稳定性。
- 2026-06-04：`无情劫掠` 的 direct E2E 已恢复到真实业务位点并转绿。`node scripts/infra/run-e2e-single.mjs default e2e/dicethrone/zhanshujia-cursed-pirate-intake.e2e.ts "真实入口应通过人类面 ultimate 槽位触发并结算无情劫掠的诅咒金币续结链"` 为 `1 passed`，说明此前挡住该链的 `MatchRoomWithAudio.tsx` 动态导入失败不是当前源码静态编译必现红灯；独立 Vite 直取 `/src/pages/MatchRoomWithAudio.tsx` 与 `/src/pages/MatchRoom.tsx` 也均返回 `200`。当前 human 面这 3 个对象的真实剩余项已从“独立 direct E2E 缺口”回到对象级审计边界、shared/representative 合法复用登记与复杂交互逐项 L3/L4。
- 2026-06-04：`惊魂动魄 / human-cursed / 走跳板 / 弯刀突刺` 的独立真实入口 direct E2E 已补齐。`惊魂动魄` 已证明 `calm` 槽位进入移除诅咒金币选择并在选择后收口；`human-cursed` 已分别证明“有币移 1 层并保持 human 面”与“无币翻回咒缚面”；`走跳板` 已证明 human 面 `lotus` 槽位会先进入结算方式选择，并在弃牌分支中自然打开对手手牌选择弹窗；`弯刀突刺` 已证明 human 面 `fist` 槽位会解析到 `cutlass-stab-4`，并在真实 `countermeasures` 防御链后收口到 `Host HP 50 -> 48 / Host powderKeg 1`。这是当时的阶段性收口，后续剩余两条也已继续补齐。
- 2026-06-04：`做好标记 / 嘿，老兄` 的独立真实入口 direct E2E 也已补齐。`做好标记` 已证明 human 面 `chi` 槽位会进入 3 颗奖励骰结算，并按实际弯刀/战利品/骷髅结果收口；`嘿，老兄` 已证明这条防御链会由真实 `sabre-thrust-3` 攻击流自然打开，并在 `2 弯刀 + 1 骷髅 + 1 战利品` 防御盘面下收口到 `Host HP 48 / Guest HP 48 / Guest CP 6 / Guest cursedCoin 1`。当前 human 面独立 direct E2E 缺口已清空，真实剩余项回到对象级彻底审计、双面对象级重审计、逐对象 L3/L4 与整体 soak 稳定性。
- 2026-06-04：咒缚海盗双面合同已进一步收紧到当前真实现状：开局为 human 面并自带 3 个诅咒金币；诅咒金币维持阶段掉血只作用于非海盗持有者；human 面与咒缚面两套逐槽合同都已正式回写到 `咒缚海盗真相源表.md / 咒缚海盗录入核对.md`；human 面 9 个对象已接入运行时、补齐 L2，并已全部拿到独立 direct E2E。当前不能说“规则都实施了”，真实剩余项已收窄为双面对象级重审计、咒缚面 remaining representative 条目的逐对象 L3/L4，以及 `implementation_in_progress` 下的整体 soak 稳定性。
- 2026-06-04：咒缚面 `死亡吐息` 已补独立真实入口 direct E2E。旧 blocker 不是业务未实现，而是测试夹具把“小顺子”误配成 `[1,2,3,5,6]` 非顺子盘面；修正为 `[1,2,3,4,6]` 后，`node scripts/infra/run-e2e-single.mjs default e2e/dicethrone/zhanshujia-cursed-pirate-intake.e2e.ts "真实入口应通过咒缚面 combo 槽位触发并结算死亡吐息的小顺子多状态链"` 已 `1 passed`，并收口到 `Host HP 50 -> 43 / Host powderKeg 1 / Host wither 1`。当前剩余项不再包含这条对象的首条 direct E2E 缺口，而是双面对象级重审计、remaining representative 条目的逐对象 L3/L4 与整体 soak 稳定性。
- 2026-06-04：咒缚面 `灵魂突刺 / 灵魂指令` 也已升级到独立真实入口 direct E2E。`灵魂突刺` 已证明咒缚面 `fist` 槽位会解析到 `soul-stab-3`，并在真实防御链后收口到 `Host HP 50 -> 45 / Host powderKeg 1`；`灵魂指令` 已证明咒缚面 `lightning` 槽位会触发 `soul-command` 并收口到 `Host HP 50 -> 42 / Host parley 1 / Host powderKeg 1 / Host wither 1`。当前剩余项继续收窄回双面对象级重审计、逐对象 L3/L4 与整体 soak 稳定性，而不是这两条仍缺首条 direct E2E。
- 2026-06-05：已继续收紧咒缚海盗文档口径，修掉一批“实现已到位但规则表/真相源仍停在旧状态”的滞后项。`瞭望台` 不再保留“真实 UI/E2E 展示待补”的旧说法；其弯刀查看手牌、战利品自选弃牌与骷髅随机弃牌三分支已统一回写为截图 `13-17` 的既有证据。与此同时，`死亡印记 / 咒缚 / 深海潜行 / 亡灵之爪 / 无情诅咒 / 起锚 / 诅咒卡牌 / 虚张声势` 都已从旧的 remaining representative 示例里移出。
- 2026-06-05：`起锚` 已继续从 representative 推进到对象级双分支真实证据。新增并打绿用例 `真实入口应命中并结算起锚的默认抽牌分支`；通过 `setHarnessRandomQueue(...0.0...)` 稳定命中非骷髅默认分支，收口到 Guest 从真实牌库抽到 `送你们去喂鱼`、源卡进入弃牌堆、Host 不获得 `休战`。结合既有 `74-75` 骷髅分支证据，`起锚` 当前两条规则分支都已拿到对象级真实入口链。
- 2026-06-05：`诅咒卡牌` 已继续从单代表分支推进到三分支对象级真实证据。新增并打绿用例 `真实入口应展示并结算诅咒卡牌的抽 1 张牌分支` 与 `真实入口应展示并结算诅咒卡牌的受 2 伤害抽 2 分支`；结合既有 `46-47` 的“受 4 伤害抽 3”链，当前三条规则分支都已拿到真实手牌入口、选择窗与收口状态证据。
- 2026-06-05：`虚张声势` 的真实入口奖励骰控制层已从猜测 `setHarnessRandomQueue(...0.99...)` 收紧为显式 `harness.dice.setValues([1/4/6])`，以便稳定命中弯刀 / 战利品 / 骷髅三分支。先前新增的骷髅施加火药桶分支已转绿并落截图 `151-152`；随后弯刀分支与战利品抽 2 分支也已顺跑转绿，说明 helper 改法没有带坏旧链。结合既有 `95-96` 与本轮新增 `149-150 / 151-152`，`虚张声势` 当前三条规则分支都已拿到真实入口奖励骰覆盖层与最终收口证据。
- 2026-06-05：按当前对象审计复核，`你还嫩了点 / 封舱 / 坏血病 / 劫掠 / 休战` 也都已经具备明确的合法 representative L3 依据，不应再继续挂在“剩余 representative 条目”清单里。当前真实剩余项已进一步收窄为：双面对象级重审计、对象级更高层级 L3/L4 深度，以及整份 intake 的 soak 稳定性，而不是还缺这几张手牌/防御对象的代表链。
- 2026-06-05：已确认上一轮把 soak blocker 指向 `MatchRoom` 白屏的判断过时。当前 `npx tsx scripts/infra/diagnose-dicethrone-room-entry.ts --attempts 1 --character-selection-timeout 60000 --scope diag-cursed-room-entry` 能把 host/guest 都送进角色选择页，首条 intake 用例“真实在线双玩家应能选择战术家和咒缚海盗并看到面板、提示板、手牌与 HUD”也已重新 `1 passed`。随后真正打出来的业务红灯是 `深海潜行` E2E 场景仍停在 human 面，却去断言咒缚面 `lotus=deep-sea-dive`；现已把 `setupDeepSeaDiveAttackScenario(...)` 改成显式 `playerBoardFace='cursed' + getCharacterAbilitiesForFace('cursed_pirate', 'cursed')`，并复跑 `深海潜行 / 走跳板 / human-cursed 翻回咒缚面` 三条双面链均为 `1 passed`。当前剩余项继续保持为：双面对象级重审计、对象级更高层级 L3/L4 深度，以及整份 intake 的长跑 soak 稳定性；`implementation_in_progress` 继续保留。
...
- 2026-05-30 18:28：补最小 `selectHandCard` 交互合同，深海潜行现在由对手自己选择 1 张手牌弃置；AI 兜底选择首张手牌，UI 只对交互所有者显示手牌选择。火药桶、防御 resolver、专属手牌、审计、E2E、上传仍未完成。
- 2026-05-30 18:35：补咒缚被动确定子句：咒缚海盗自己维持阶段受到 4 点 direct/unblockable 伤害；“对手在其进攻投掷阶段未造成一次攻击则施加火药桶”仍需新增回合级追踪合同，未猜测实现。
- 2026-05-30 18:47：补两个防御 resolver：战术家反制措施按军刀/旗帜/勋章结算反击、防伤与战术优势；咒缚海盗你还嫩了点按弯刀/战利品/骷髅结算反击、CP、防伤与诅咒金币。组合验证 `zhanshujia-cursed-pirate-intake.test.ts` + `zhanshujia-cursed-pirate-mechanics.test.ts` 为 2 files / 19 tests passed；`npx tsc --noEmit --pretty false` 与本轮相关 ESLint 均通过。
- 2026-05-30 18:55：补统一状态施加 helper，普通 `grantStatus`、条件/default `grantStatus`、奖励骰状态施加、咒缚海盗 custom resolver 与 `selectPlayer` 交互授予状态均会在目标为咒缚海盗本人且状态为诅咒金币时生成“获得 / 不获得”选择。机制测试覆盖接受后到上限、拒绝后层数不变；组合验证仍为 2 files / 19 tests passed，`npx tsc --noEmit --pretty false`、`npm run i18n:check` 和定向 ESLint 均通过。
- 2026-05-30 19:13：补咒缚“对手在其进攻投掷阶段未发起攻击则施加火药桶”的 L2 机制：`ATTACK_INITIATED` 记录本回合已发起攻击，`offensiveRoll` 阶段末若活跃玩家是咒缚海盗对手且未发起攻击，则施加火药桶。新增正/负路径测试；组合验证为 2 files / 21 tests passed，`npx tsc --noEmit --pretty false` 通过，定向 ESLint 0 errors（保留既有 warnings）。
- 2026-05-30 19:36：补无情诅咒“对至多两名对手施加火药桶”的 L2 机制：结算时生成可跳过的 `simple-choice`，4 人 2v2 只列敌队两名对手，不列队友；选择两名对手后分别施加火药桶，选择跳过则不施加。新增正/负路径测试；组合验证为 2 files / 23 tests passed，`npx tsc --noEmit --pretty false`、`npm run i18n:check` 和定向 ESLint 均通过。
- 2026-05-30 19:45：补火药桶（图面“炸药桶”）完整 L2 机制：维持阶段投 1 骰；1-2 爆炸移除并造成 3 点 direct/unblockable 独立伤害；3-5 无事并保留；6 创建任意玩家转交选择，选择自己保持原状态，选择其他玩家则移动给目标；重复获得时原桶立即爆炸并保留新桶。新增 4 条机制测试；组合验证为 2 files / 27 tests passed，`npx tsc --noEmit --pretty false`、`npm run i18n:check` 和定向 ESLint 均通过。
- 2026-05-30 22:46：按完整单卡裁图完成英雄专属手牌 L1 录入：战术家 slot 17-31 共 15 张，咒缚海盗 slot 17-32 共 16 张；同步 `cards.ts`、中英文 i18n、卡牌录入核对文档，并修正两名新英雄的 `card-unexpected` 通用牌图集索引（战术家 32、咒缚海盗 33）。复杂响应窗、复合升级替换、目标选择、对手支付、手牌查看/弃牌等专属手牌 L2 仍未收口。验证：JSON parse 通过，`npx tsc --noEmit --pretty false` 通过，intake 单测 1 file / 6 tests passed，intake+mechanics 组合 2 files / 28 tests passed，`npm run i18n:check` 通过且仅保留既有 3 条 warning，定向 ESLint 0 errors。
...
- 2026-06-03 09:24：补战术家“军刀突刺 II”对象级真实入口链：新增 `setupSabreThrust2Scenario(...)`，在线场景中显式把 Host 的 `sabre-thrust` 替换为 `SABRE_THRUST_2`，并写入 `abilityLevels['sabre-thrust']=2`；盘面固定为 `3 军刀 + 2 旗帜` 且 3 个军刀骰值同为 `1`，确保 `fist` 槽位真实解析到 `sabre-thrust-2-3`。新增用例“真实入口应通过玩家板槽位触发并结算军刀突刺 II 的三同值紧缚链”，截图 `100-host-sabre-thrust-2-entry.png`、`101-guest-sabre-thrust-2-defense-entry.png`、`102-host-sabre-thrust-2-applied.png` 已落地；服务器状态断言 `Guest HP 50 -> 45`、`Guest bind 0 -> 1`、`Host HP 保持 50`，证明升级后的 `5` 点伤害与“三同值施加紧缚”已经在真实 UI 中闭环。验证：`npx eslint e2e/dicethrone/zhanshujia-cursed-pirate-intake.e2e.ts` 通过，`npx tsc --noEmit --pretty false` 通过，`node scripts/infra/run-e2e-single.mjs default e2e/dicethrone/zhanshujia-cursed-pirate-intake.e2e.ts "真实入口应通过玩家板槽位触发并结算军刀突刺 II 的三同值紧缚链"` 为 `1 passed`。当前整批仍不能宣称完成，`implementation_in_progress` 继续保留；下一步优先回到 `地毯式轰炸 II` 的旗帜 4 分支或 shared / representative 条目的逐对象收口。
- 2026-06-03 09:41：补战术家“地毯式轰炸 II”旗帜 4 分支对象级真实入口链：新增 `setupCarpetBombing2StrategyScenario(...)`，在线场景中显式把 Host 的 `carpet-bombing` 替换为 `CARPET_BOMBING_2`，写入 `abilityLevels['carpet-bombing']=2`，并把牌库固定为 `战略防御！` 与 `占得上风！` 两张可验证抽牌；盘面固定为 `4 旗帜 + 1 军刀`，确保 `chi` 槽位真实解析到 `carpet-bombing-2-strategy`。新增用例“真实入口应通过玩家板槽位触发并结算地毯式轰炸 II 的旗帜分支”，截图 `103-host-carpet-bombing-2-strategy-entry.png`、`104-host-carpet-bombing-2-strategy-applied.png` 已落地；服务器状态断言 `Host 战术优势 0 -> 3`、手牌数 `0 -> 2` 且手牌为 `战略防御！ / 占得上风！`、`pendingAttack` 保持为空，证明这条升级分支已在真实 UI 中完成 `grantToken + draw 2` 闭环。验证：`npx eslint e2e/dicethrone/zhanshujia-cursed-pirate-intake.e2e.ts` 通过，`npx tsc --noEmit --pretty false` 通过，`node scripts/infra/run-e2e-single.mjs default e2e/dicethrone/zhanshujia-cursed-pirate-intake.e2e.ts "真实入口应通过玩家板槽位触发并结算地毯式轰炸 II 的旗帜分支"` 为 `1 passed`。当前整批仍不能宣称完成，`implementation_in_progress` 继续保留；下一步应回到 shared / representative 条目的逐对象合法复用登记、复杂交互 L3/L4 覆盖与 runtime 稳定性。
...
| 项 | 状态 | 证据 |
|---|---|---|
| 项目通用 skill `.codex/skill/add-new-faction/SKILL.md` | passed | `PYTHONUTF8=1 python D:\codex-home\skills\.system\skill-creator\scripts\quick_validate.py .codex\skill\add-new-faction` -> `Skill is valid!` |
| `data-entry-workflow` 路由到通用新增派系 skill | passed | `.codex/skill/data-entry-workflow/SKILL.md` 已包含“通用新增派系 / 新增角色 / 新增英雄”路由 |
...
- [x] 2026-04-24 强化通用工作流：更新 `.codex/skill/data-entry-workflow/SKILL.md` 与 `docs/games/smashup/workflows/smashup-faction-implementation.md`，新增“长期任务连续执行”强制规则
...
- [x] 2026-04-29 回写长期任务 / 派系重审 workflow 门禁：把“批量派系重审批次清单”“E2E 场景 defId 预检”“L0-L4 分层验收”“reaction session 抽样门禁”补进 `.codex/skill/data-entry-workflow/SKILL.md`、`docs/games/smashup/workflows/smashup-faction-implementation.md`、`docs/ai-rules/testing-audit.md`
...
### Phase
- [x] 把 `人类反馈 > 系统自动反馈` 回写到 `.codex/skill/feedback-closeout/SKILL.md` 与本计划
...
## Addendum（2026-06-03 08:45 +08）：`诱饵` 已补到真实攻击修正对象链

- [x] `e2e/dicethrone/zhanshujia-cursed-pirate-intake.e2e.ts`
  - 已将 `诱饵` 用例从错误的 `bonusDamage` 口径改为 `真实入口应展示并结算诱饵的 2 点攻击伤害`。
  - 当前真实链路已锁定为：Guest 先通过真实玩家板 `soul-stab-3` 槽位建立攻击链，再从真实手牌打出 `诱饵`；该卡不会写 `pendingAttack.bonusDamage / attackModifierBonusDamage`，而是在仍处于 `offensiveRoll` 时立即对 Host 写入 `2` 点攻击伤害。
- [x] 最新定点验证
  - `node scripts/infra/run-e2e-single.mjs default e2e/dicethrone/zhanshujia-cursed-pirate-intake.e2e.ts "真实入口应展示并结算诱饵的 2 点攻击伤害"` -> `1 passed (2026-06-03)`
  - `npx eslint e2e/dicethrone/zhanshujia-cursed-pirate-intake.e2e.ts` -> 通过
  - `npx tsc --noEmit --pretty false` -> 通过
- [x] 当前已锁定的对象级事实
  - `97-guest-shark-bait-attack-entry.png` 证明 Guest 不是 direct injection，而是先通过真实 `soul-stab-3` 攻击入口建立 `pendingAttack`。
  - `98-guest-shark-bait-modifier-active.png` 证明 Guest 从真实手牌打出 `诱饵` 后，攻击修正徽标会在真实 UI 中出现。
  - `99-host-shark-bait-attack-damage-applied.png` 对应服务器断言 `phase=offensiveRoll / Host HP=48 / Guest CP=4 / discard 包含 card-cursed-pirate-shark-bait`，证明 `诱饵` 的真实落点是即时 `2` 点攻击伤害，而不是 `pendingAttack.bonusDamage` 写入。
- [x] 当前剩余方向更新
  - `诱饵` 已可从独立缺口里移除。
  - 对象级高价值独立缺口已清空；后续剩余阻塞回到 shared / representative 条目的逐对象合法复用登记、复杂交互 L3/L4 覆盖与 runtime 稳定性，而不是新的单对象首条 direct E2E 缺失。

## Addendum（2026-06-03 10:08 +08）：completion audit 已把两条共享合同矩阵从过时 `partial` 收窄为当前证据口径

- [x] `evidence/dicethrone/zhanshujia-cursed-pirate-object-audit-2026-05-31.md`
  - 已把 `grantStatus` 特例合同从 `L2 partial` 改成 `L2 passed；... representative L3`。
  - 已把 `奖励骰/随机` 合同从 `L2 partial` 改成 `L2 passed；... representative L3`。
  - 已改写 `对象级 L3/L4 | partial` 的剩余原因：
    - 不再把原因写成“shared / representative 尚未逐对象登记”；
    - 改为当前真实剩余门禁：大量对象仍停在 `representative L3` 而非逐对象独立 `L3/L4`，以及整份 intake runtime 仍缺少稳定重复通过/soak 证据。
- [x] 当前已锁定的 completion-audit 事实
  - `zhanshujia-cursed-pirate-mechanics.test.ts` 已覆盖 `grantStatus` 这批共享消费者需要的关键分支：诅咒金币拒绝/上限/不可移除、火药桶维持与重叠、凋零减攻击伤害、休战阻伤与清理、紧缚 `1CP` 门禁与清理、锁定写入。
  - 同一份机制测试 + 现有定点 E2E 已足以证明奖励骰共享合同本身不是当前主要缺口；当前缺的是“有多少对象可以只保留 representative L3，哪些还必须补独立 L3/L4”。
- [x] 当前剩余方向更新
  - 后续优先级继续停留在 completion audit，而不是再随机补新对象 E2E。
  - `implementation_in_progress` 继续保留。

## Addendum（2026-06-03 10:20 +08）：runtime risk-watch 已用双人 / 四人简单开局基线收窄

- [x] `e2e/dicethrone/dicethrone-simple-start.e2e.ts`
  - `Online match: Can start a game successfully` 在当前 isolated single-worker runtime 下通过。
  - `Online 4-player room: create claim-seat join and start successfully` 也在当前 isolated single-worker runtime 下通过。
  - 这两条基线说明：当前 runtime 风险已不再是“简单进房/开局稳定失败”，而是更长链路、更多对象与整份 intake 的连续 soak 问题。
- [x] 当前已锁定的 runtime 事实
  - 双人和四人简单开局都能在本轮托管 runtime 中落地，说明 4 人 readiness 不是稳定 blocker。
  - `isolated single-worker DiceThrone runtime 启动稳定性` 的描述应随之收窄为长跑/soak 风险，而不是最小进房必败。
- [x] 当前剩余方向更新
  - 后续优先级继续停留在 completion audit 与长跑 soak，而不是再把简单开局当作主阻塞。
  - `implementation_in_progress` 继续保留。

## Addendum（2026-06-03 10:28 +08）：representative L3 已拆成“可继续复用”与“优先补 L4”两层

- [x] 当前已确认的分层
  - 纯共享参数差异 / 组合链对象可以继续合法保留 representative L3，不需要把每个都改成独立 direct E2E。
  - 真正应优先补独立 L4 的，是带明显交互窗、多个选择步骤、跨阶段收口的对象。
- [x] 后续优先补 L4 的对象
  - 战术优势
  - 反制措施 / 你还嫩了点
  - 无情诅咒
  ...
  - 啜呼
- [x] 当前剩余方向更新
  - 后续不再把所有 representative L3 混成同一桶。
  - `implementation_in_progress` 继续保留。

## Addendum（2026-06-03 10:34 +08）：战术优势 C5 守护改为真实选人交互

- [x] `src/games/dicethrone/domain/customActions/zhanshujia.ts`
  - `zhanshujia-tactical-advantage-grant-protect` 由“直接给自己加守护”改为 `selectPlayer + tokenGrantConfig`。
  - 该动作现在与 `card-zhanshujia-strategic-defense` 保持同型：先出玩家选择交互，再由引擎结算 `TOKEN_IDS.PROTECT`。
- [x] `src/games/dicethrone/__tests__/zhanshujia-cursed-pirate-mechanics.test.ts`
  - “战术优势可按动作消耗 token 取得 CP、重掷、抽牌、锁定、守护和转移入口” 已同步改成先验交互、再选目标、再验收 token 的链路。
  - 定点机制测试已重跑通过：`45 passed`。
- [x] `src/games/dicethrone/rule/战术家录入核对.md`
  - `tactical_advantage / C5` 已从 `L2 partial` 改为 `L2 passed`。

## Addendum（2026-06-03 11:29 +08）：`紧缚` 旧红链已复核恢复，整份 intake E2E 当前 39/39 通过

- [x] `e2e/dicethrone/zhanshujia-cursed-pirate-intake.e2e.ts`
  - `真实入口应展示紧缚在额外投掷中的 CP 门禁与阶段清理` 已在当前托管 isolated runtime 下单跑通过，不再属于环境阻断中的旧红链。
  - 本轮没有继续改业务逻辑；现状更接近“此前失败为旧结果/运行时噪音，当前用例已可稳定打绿”。
- [x] 最新验证
  - `node scripts/infra/run-e2e-single.mjs default e2e/dicethrone/zhanshujia-cursed-pirate-intake.e2e.ts "真实入口应展示紧缚在额外投掷中的 CP 门禁与阶段清理"` -> `1 passed (2026-06-03)`
  - `node scripts/infra/run-e2e-single.mjs default e2e/dicethrone/zhanshujia-cursed-pirate-intake.e2e.ts` -> `39 passed (2026-06-03)`
  - `npx eslint e2e/dicethrone/zhanshujia-cursed-pirate-intake.e2e.ts` -> 通过
- [x] 当前结论更新
  - 旧的“`紧缚` 仍被 2 人 online 加载阻断、暂不能回写代表链”的判断已过时。
  - 当前剩余门禁继续收敛为：对象级彻底审计、representative L3 到逐对象 L3/L4 的边界，以及整份 intake 的长跑/soak 稳定性。
  - `implementation_in_progress` 继续保留，不能把这次 `39/39` E2E 收口误报成两名新英雄已完整完成。

## Addendum（2026-06-03 21:05 +08）：咒缚海盗人类面玩家板已接入正式资源链与运行时选图

- [x] 资源链收口
  - 已把 `public/assets/i18n/zh-CN/dicethrone/images/cursed/人类面板.png` 同步为运行时别名 `human-player-board.png`。
  - 已生成 `public/assets/i18n/zh-CN/dicethrone/images/cursed/compressed/human-player-board.webp`。
  - `npm run assets:manifest` 后，`public/assets/i18n/zh-CN/dicethrone/assets-manifest.json` 与 `public/assets/i18n/assets-manifest.json` 已包含：
    - `images/cursed/human-player-board`
    - `images/cursed/compressed/human-player-board`
- [x] 运行时合同收口
  - `ASSETS.PLAYER_BOARD('cursed_pirate', 'normal')` 现指向 `dicethrone/images/cursed/human-player-board`。
  - 主棋盘与攻击特写都已消费 `playerBoardFace` 选图；咒缚面继续走 `player-board`，normal 面走 `human-player-board`。
  - 已核图确认人类面与咒缚面槽位版式同构，继续复用现有 `v2` 技能槽布局，不新增第二套 `abilitySlotLayout` / `abilitySlotMapping`。
- [x] 当前边界
  - 这次收口的是“人类面底图正式进资源链并被 UI 消费”，不是“官方双面机制已完整完成”。
  - 仍未证明 normal 面会在真实流程中自动切换，也未证明其是否存在与咒缚面不同的完整技能语义。
  - `implementation_in_progress` 继续保留。

## Addendum（2026-06-03，本轮图面对照）：normal 面已确认是独立技能语义，后续必须重录与重审计

- [x] 已确认的新事实
  - `player-board.png`（咒缚面）与 `人类面板.png`（normal 面）虽然槽位版式同构，但技能标题、数值句子与被动语义并不相同。
  - 人类面可直接读到 `弯刀突刺 / 做好标记 / 点燃炸药 / 判决指令 / 走跳板 / 惊魂动魄` 等对象；这与当前 runtime 仍在使用的 `灵魂突刺 / 死亡印记 / 死亡吐息 / 灵魂指令 / 深海潜行 / 亡灵之爪` 不是同一套。
  - 人类面 `咒缚` 被动也改成“回合结束移除 1 个诅咒金币；若没有可移除金币则翻面”，不能再沿用咒缚面的 upkeep 自伤/施桶合同。
- [ ] 因此必须追加的正式工作
  - 逐槽重录 normal 面 9 个玩家板技能与被动文本，形成新的真相源/录入核对表。
  - 为 normal 面建立对应领域实现或双面能力分流，而不是只靠 `playerBoardFace` 选图。
  - 设计并实现 normal 面 <-> 咒缚面的正式翻面合同。
  - 按新语义补机制测试、真实入口测试与对象级审计回写。
- [x] 当前边界更新
  - “规则都实施完”当前不能成立。
