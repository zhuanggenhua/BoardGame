...
- 本项目默认手机横屏验收分辨率固定为 `16:9`；用户未另行指定时，优先使用 `800x450`。
...
- 移动端适配验收必须先跑出 `test-results/evidence-screenshots/` 的显式截图，再逐张人工阅读截图内容；**只看断言、日志、DOM、locator、控制台输出，一律不算验收完成**。
...
- **看图验收是强制项**：跑完移动端 E2E 后，必须实际打开 `test-results/evidence-screenshots/` 中的显式截图，逐张阅读；**没有逐张看图，就视为没有完成移动端验收**。
...
2. 再看 `test-results/evidence-screenshots/` 里的手机/平板主状态图，逐张和 `PC` 对照“整体比例、锚点、遮挡、主次关系”。
