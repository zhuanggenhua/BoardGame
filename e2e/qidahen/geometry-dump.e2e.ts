...
test('只截图第一步选择参与部队高亮', async ({ page }) => {
    ...
    const geometryDump = await page.evaluate(() => {
        const summarize = (selector: string) => Array.from(document.querySelectorAll(selector)).map((el: Element) => {
            const html = el as HTMLElement;
            const rect = html.getBoundingClientRect();
            return {
                testId: html.getAttribute('data-testid'),
                type: html.getAttribute('data-qidahen-map-token-type'),
                faction: html.getAttribute('data-qidahen-map-token-faction'),
                region: html.getAttribute('data-qidahen-map-token-region'),
                face: html.getAttribute('data-qidahen-army-face'),
                selected: html.getAttribute('data-pending-committed-selected'),
                index: html.getAttribute('data-pending-committed-index'),
                guideX: html.getAttribute('data-guide-target-x'),
                guideY: html.getAttribute('data-guide-target-y'),
                action: html.getAttribute('data-action'),
                d: html.getAttribute('d'),
        ...
        const map = document.querySelector('[data-testid="qidahen-map-layer"]') as HTMLElement | null;
        const mapRect = map?.getBoundingClientRect();
        return {
            map: mapRect ? { left: Math.round(mapRect.left), top: Math.round(mapRect.top), width: Math.round(mapRect.width), height: Math.round(mapRect.height) } : null,
            city22Tokens: summarize('[data-qidahen-map-token-region="city-region-22"]'),
            songjinTokens: summarize('[data-qidahen-map-token-region="song-jin"]'),
            guide: summarize('[data-testid="qidahen-map-guide-hit-target-city-region-22"],[data-testid^="qidahen-map-guide-route-city-region-22"],[data-testid^="qidahen-map-guide-target-focus-city-region-22"],[data-testid^="qidahen-map-guide-line-city-region-22"],[data-testid^="qidahen-map-guide-arrow-head-city-region-22"]'),
        };
    });
    console.log('QIDAHEN_GEOMETRY_DUMP ' + JSON.stringify(geometryDump));
