/**
 * 召唤师战争 - 选择建筑 + 推拉方向 E2E 测试
 * 
 * 覆盖范围：
 * - 建筑转移（structure_shift）：移动后推拉 3 格内友方建筑 1 格
 * - 建筑选择 UI 交互
 * - 方向选择 UI 交互
 * - 建筑移动动画和视觉反馈
 * - 在线对局状态同步
...
const prepareStructureShiftState = (coreState: any) => {
  ...
  let summonerPlaced = false;
  let structurePlaced = false;

  // 在玩家区域放置召唤师（丝瓦拉）
  for (let row = 5; row < 8 && !summonerPlaced; row++) {
    for (let col = 0; col < 6 && !summonerPlaced; col++) {
      const cell = board[row][col];
      if (!cell.unit && !cell.structure) {
        board[row][col].unit = {
          instanceId: `summoner-${row}-${col}`,
          cardId: 'frost-summoner-1',
          card: {
            id: 'frost-summoner',
            name: '丝瓦拉',
            cardType: 'unit',
        ...
        for (const nearby of nearbyPositions) {
          if (nearby.row >= 0 && nearby.row < 8 && nearby.col >= 0 && nearby.col < 6) {
            if (!board[nearby.row][nearby.col].unit && !board[nearby.row][nearby.col].structure) {
              board[nearby.row][nearby.col].structure = {
                id: `structure-${nearby.row}-${nearby.col}`,
                type: 'wall',
                owner: '0',
                life: 5,
                damage: 0,
  ...
  if (!summonerPlaced || !structurePlaced) {
    throw new Error('无法准备建筑转移测试状态：未能放置所需单位或建筑');
  }
...
test.describe('召唤师战争 - 选择建筑 + 推拉方向', () => {
  test('建筑转移：移动后推拉友方建筑', async ({ browser }, testInfo) => {
    ...
    const structureShiftCore = prepareStructureShiftState(coreState);
    await applyCoreState(hostPage, structureShiftCore);
    ...
    // 查找召唤师
    const summoner = hostPage.locator('[data-testid^="sw-unit-"][data-owner="0"]').filter({
      has: hostPage.locator('[data-unit-class="summoner"]')
    }).first();
    ...
    // 点击相邻空位（触发移动）
    const emptyCell = hostPage.locator('[data-testid^="sw-cell-"]:not(:has([data-testid^="sw-unit-"])):not(:has([data-testid^="sw-structure-"]))').first();
    ...
    const structureSelectionPrompt = hostPage.locator('[data-testid="sw-ability-prompt"]').or(
      hostPage.locator('[class*="prompt"]').filter({ hasText: /选择建筑|Select structure|建筑转移|Structure shift/i })
    );
    ...
    const directionSelector = hostPage.locator('[data-testid="sw-direction-selector"]').or(
      hostPage.locator('[class*="direction"]').filter({ hasText: /选择方向|Choose direction/i })
    );
    await expect(directionSelector).toBeVisible({ timeout: 8000 });

    // 选择一个方向（例如：向上）
    const upButton = directionSelector.locator('button').filter({ hasText: /上|Up|↑/i }).first();
    ...
    // 验证建筑位置改变
    await expect.poll(async () => {
      const currentTestId = await hostPage.locator('[data-testid^="sw-structure-"][data-owner="0"]').first().getAttribute('data-testid');
