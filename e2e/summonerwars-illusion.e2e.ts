...
const prepareIllusionState = (coreState: any) => {
  ...
  let illusionistPlaced = false;
  let soldierPlaced = false;

  // 在玩家区域放置心灵巫女
  for (let row = 5; row < 8 && !illusionistPlaced; row++) {
    for (let col = 0; col < 6 && !illusionistPlaced; col++) {
      const cell = board[row][col];
      if (!cell.unit && !cell.structure) {
        board[row][col].unit = {
          instanceId: `illusionist-${row}-${col}`,
          cardId: 'trickster-illusionist-1',
          card: {
            id: 'trickster-illusionist',
            name: '心灵巫女',
            cardType: 'unit',
            faction: 'trickster',
            cost: 2,
          ...
          if (nearby.row >= 0 && nearby.row < 8 && nearby.col >= 0 && nearby.col < 6) {
            if (!board[nearby.row][nearby.col].unit && !board[nearby.row][nearby.col].structure) {
              board[nearby.row][nearby.col].unit = {
                instanceId: `soldier-with-ability-${nearby.row}-${nearby.col}`,
                cardId: 'trickster-soldier-swift-1',
                card: {
                  id: 'trickster-soldier-swift',
                  name: '清风弓箭手',
                  cardType: 'unit',
                  faction: 'trickster',
                  cost: 1,
  ...
  if (!illusionistPlaced || !soldierPlaced) {
    throw new Error('无法准备幻象测试状态：未能放置所需单位');
  }
...
test.describe('召唤师战争 - 选择友方单位复制', () => {
  test('幻象：移动阶段开始时复制友方士兵技能', async ({ browser }, testInfo) => {
    ...
    const illusionPrompt = hostPage.locator('[data-testid="sw-ability-prompt"]').or(
      hostPage.locator('[class*="prompt"]').filter({ hasText: /选择友方|Select ally|幻象|Illusion|复制|Copy/i })
    );
    ...
    // 点击友方士兵
    await clickBoardElement(hostPage, '[data-testid^="sw-unit-"][data-owner="0"][data-unit-name*="清风弓箭手"]');
