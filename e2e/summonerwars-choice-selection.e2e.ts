/**
 * 召唤师战争 - 选择控制/伤害（二选一）E2E 测试
 * 
 * 覆盖范围：
 * - 冰霜战斧（frost_axe）：选择充能自身 或 附加到友方士兵
 * - 血符文（blood_rune）：选择控制 或 伤害
 * - 二选一选择界面
 * - 选择后的不同效果执行
 * - 在线对局状态同步
 * 
 ...
 * - 附近有友方士兵可附加
 */
const prepareFrostAxeState = (coreState: any) => {
...
test.describe('召唤师战争 - 选择控制/伤害（二选一）', () => {
  test('冰霜战斧：选择充能自身', async ({ browser }, testInfo) => {
  ...
  test('冰霜战斧：选择附加到友方士兵', async ({ browser }, testInfo) => {
    test.setTimeout(120000);
    const baseURL = testInfo.project.use.baseURL as string | undefined;
