 ...
 * 注意：此测试需要两回合（第一回合制造弃牌，第二回合回收）
 */
test.describe('Cardia 一号牌组 - 沼泽守卫', () => {
    test('影响力13 - 沼泽守卫：能力激活成功', async ({ browser }) => {
        ...
        try {
            ...
            // 1. 注入测试场景：P1 手牌包含影响力13（沼泽守卫），P2 手牌包含影响力16
            console.log('注入 P1 手牌：影响力13（沼泽守卫）');
            await injectHandCards(p1Page, '0', [
                { defId: 'deck_i_card_13' } // 沼泽守卫
            ]);
            ...
            // 7. 验证：能力已执行（从弃牌堆回收卡牌）
            // 注意：沼泽守卫能力需要弃牌堆有牌，简化测试只验证激活成功
            const stateAfter = await readCoreState(p1Page);
            
            console.log('能力执行后:', {
                phase: stateAfter.phase,
                p1DiscardSize: stateAfter.players['0'].discard.length,
