...
test.describe('山屋惊魂驱魔失败伤害完整链路', () => {
    test('最终驱魔失败从真实入口到伤害结算再关闭回牌桌', async ({ page, context }) => {
        test.setTimeout(120000);
        await initBetrayalContext(context);
        const diagnostics = attachPageDiagnostics(page, 'betrayal-exorcism-failure-full-chain');
        ...
        expect(physicalTraitTotal(afterFailureCore, actorId)).toBe(actorPhysicalBefore - 1);
        expect(physicalTraitTotal(afterFailureCore, teammateId)).toBe(teammatePhysicalBefore - 1);
        expect(afterFailureCore.phase).toBe('haunt');
        expect(afterFailureCore.endgameResult).toBeNull();
        expect(afterFailureCore.scenarioRuntime.deadExplorerPlayerIds).not.toContain(actorId);
        expect(afterFailureCore.scenarioRuntime.deadExplorerPlayerIds).not.toContain(teammateId);
        ...
        assertNoFatalFrontendErrors([{ label: 'betrayal-exorcism-failure-full-chain', diagnostics }]);
    });
});
