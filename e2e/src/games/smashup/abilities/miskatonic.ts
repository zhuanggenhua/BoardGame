...
function miskatonicThingOnTheDoorstep(ctx: AbilityContext): AbilityResult {
    ...
    if (topMinions.length === 1) {
        ...
        // 行动卡来源：若目标受“烟雾弹”等 action 保护，则不能消灭
        const protectedOptions = buildActionMinionTargetOptions(
            [{ uid: target.uid, defId: target.defId, baseIndex, label: target.defId }],
            { state: ctx.state, sourcePlayerId: ctx.playerId, effectType: 'destroy' },
        );
        if (protectedOptions.length === 0) {
            return { events: [buildAbilityFeedback(ctx.playerId, 'feedback.all_protected', ctx.now)] };
        }
        return { events: [destroyMinion(target.uid, target.defId, baseIndex, target.owner, undefined, 'miskatonic_thing_on_the_doorstep', ctx.now, 'action')] };
    }
    ...
    const promptOptions = buildActionMinionTargetOptions(options, { state: ctx.state, sourcePlayerId: ctx.playerId, effectType: 'destroy' });
    if (promptOptions.length === 0) {
        return { events: [buildAbilityFeedback(ctx.playerId, 'feedback.all_protected', ctx.now)] };
    }

    return resolveOrPrompt(ctx, promptOptions, {
