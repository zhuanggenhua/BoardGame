...
describe('P7: PromptSystem 集成', () => {
    ...
    describe('AI 交互兜底', () => {
        it('无可选项时返回取消交互，避免卡死', () => {
            const state = {
                core: { gameResult: null },
                sys: {
                    interaction: {
                        current: {
                            id: 'smashup-ai-empty-options',
                            playerId: '0',
                            kind: 'simple-choice',
                            data: {
                                options: [],
            ...
            const actions = buildSmashUpAiLegalActions({ playerId: '0', state });
            expect(actions).toHaveLength(1);
            const commandType = actions[0]?.commands?.[0]?.type;
            if (commandType === INTERACTION_COMMANDS.RESPOND) {
                expect(actions[0]?.kind).toBe('interaction-choice');
                expect(actions[0]?.commands?.[0]?.payload).toMatchObject({ optionId: '__emergency_skip__' });
            } else {
