...
async function dismissSmashUpSpotlightQueueIfVisible(page: Page): Promise<void> {
  const dismissOverlayQueue = async (overlay: Locator): Promise<void> => {
    const content = page.getByTestId('card-spotlight-content');
  ...
  await dismissOverlayQueue(spotlightQueue);
  ...
  for (let step = 0; step < 8; step += 1) {
    const visible = await revealOverlay.isVisible().catch(() => false);
    if (!visible) {
      await page.waitForTimeout(150);
      break;
    }
...
test.describe('SmashUp yuanhou 四派系 intake 真实入口验证', () => {
  ...
  test('变形者-Cellular Bonding-本体被 Shielding 清掉后即使宿主残留 copied Missing Uplink metadata 也不应在回合结束继续抽牌', async ({ page, game }, testInfo) => {
    test.setTimeout(120000);
    await setChineseLocale(page.context());
    await game.openTestGame('smashup', { skipInitialization: true }, 45000);
    await game.setupScene({
        ...
        core: {
          ...
          players: {
            ...
            '1': {
              ...
              vp: 0,
              hand: [
                { uid: 'cell-stale-enemy-shield', defId: 'cyborg_apes_shielding', type: 'action', owner: '1' },
              ],
              deck: [
                { uid: 'cell-stale-p1-draw-a', defId: 'cyborg_apes_going_bananas', type: 'action', owner: '1' },
    ...
    await expect(host).toBeVisible({ timeout: 15000 });
    await page.locator('[data-card-uid="cell-stale-enemy-shield"]').click();
    await host.click();
    ...
    await expect.poll(async () => {
      ...
      return state?.sys?.interaction?.current == null
        && hasUid(liveHost?.attachedActions, 'cell-stale-enemy-shield')
        ...
        && !hasUid(state?.core?.players?.['1']?.hand, 'cell-stale-enemy-shield');
    }, {
      message: '敌方 Shielding 真实结算后应清掉宿主上的 Cellular Bonding 与原始 Missing Uplink，但宿主 metadata 仍残留以检验 stale copied trigger',
      timeout: 15000,
    }).toBe(true);
  ...
  test('时间旅行者-Into the Time Slip-真实入口让被他人控制的随从回到其拥有者而不是控制者手牌', async ({ page, game }, testInfo) => {
    ...
    await expect.poll(async () => {
      ...
      return prompt?.data?.sourceId === 'time_travelers_into_the_time_slip_choose'
        && prompt?.playerId === '0'
        && optionIds.includes('borrowed-slip-minion');
    }, {
      message: 'Into the Time Slip 打出后应进入真实场上牌选择 prompt，并能选择被他人控制的 borrowed-slip-minion',
      timeout: 15000,
    }).toBe(true);

    await game.screenshot('yuanhou-into-the-time-slip-borrowed-minion-choice-prompt', testInfo);

    await page.locator('[data-option-id="borrowed-slip-minion"]').click();
    ...
    await expect.poll(async () => {
      ...
      message: 'Into the Time Slip 选中被借走的随从后，应回到 owner=P0 手牌，而不是 controller=P1 手牌',
      timeout: 15000,
    }).toBe(true);
  ...
  test('超级间谍-秘密火山总部-真实计分前会让每位玩家展示牌库顶一张，并只把展示出的随从打到这里', async ({ page, game }, testInfo) => {
    ...
    await revealOverlay.click({ position: { x: 12, y: 12 }, force: true });
    await expect(revealOverlay).toBeVisible({ timeout: 5000 });
    await revealOverlay.click({ position: { x: 12, y: 12 }, force: true });
    await expect(revealOverlay).toBeHidden({ timeout: 5000 });
  ...
  test('超级间谍-Mole-真实计分窗口可通过特技把同基地行动作为特殊行动打出', async ({ page, game }, testInfo) => {
    ...
    await expect.poll(async () => {
      ...
      message: 'Mole 的唯一合法基地应在点击 Going Bananas 后自动收口，不再额外弹第二层基地选择 prompt',
      timeout: 15000,
    }).toBe(true);
  ...
  test('时间旅行者-Wormhole-真实计分后可选择任意数量己方随从洗入牌库', async ({ page, game }, testInfo) => {
    ...
    await expect(page.getByRole('button', { name: /让过/ })).toBeVisible({ timeout: 15000 });
