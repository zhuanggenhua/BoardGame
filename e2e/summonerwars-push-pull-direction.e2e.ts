/**
 * 召唤师战争 - 攻击后推拉方向选择 E2E 测试
 * 
 * 覆盖范围：
 * - 念力（telekinesis）：攻击后推拉 2 格内目标
 * - 高阶念力（high_telekinesis）：攻击后推拉 3 格内目标
 * - 方向选择 UI 交互
 * - 推拉动画和视觉反馈
 * - 在线对局状态同步
 * 
...
const prepareTelekinesisState = (coreState: any) => {
  ...
  for (let row = 5; row < 8 && !magePlace; row++) {
    for (let col = 0; col < 6 && !magePlace; col++) {
  ...
  if (!magePlace || !enemyPlaced) {
    throw new Error('无法准备念力测试状态：未找到清风法师或无法放置敌方单位');
  }
...
test.describe('召唤师战争 - 攻击后推拉方向选择', () => {
  test('念力：攻击后推拉目标单位', async ({ browser }, testInfo) => {
    ...
    // 查找清风法师
    const mage = hostPage.locator('[data-testid^="sw-unit-"][data-owner="0"]').filter({
      has: hostPage.locator('[data-unit-name*="法师"]')
    }).first();
    ...
    // 验证敌方单位位置改变
    await expect.poll(async () => {
      const currentTestId = await hostPage.locator('[data-testid^="sw-unit-"][data-owner="1"]').first().getAttribute('data-testid');
