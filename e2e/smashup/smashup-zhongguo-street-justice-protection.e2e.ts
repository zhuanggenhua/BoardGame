...
test.describe('SmashUp - zhongguo 街头正义保护链路', () => {
    test('街头正义在基地生效后，对手的打到穿越目标列表里不应出现被保护的己方随从', async ({ game }, testInfo) => {
            ...
            bases: [
                {
                    ...
                    ongoingActions: [{ uid: 'justice-live', defId: 'vigilantes_street_justice', ownerId: '1' }],
                },
                {
        ...
        await game.screenshot('zhongguo-street-justice-protection-ready', testInfo);
        ...
        await game.screenshot('zhongguo-street-justice-protection-target-options', testInfo);
        ...
        await expect.poll(async () => {
            const state = await game.getState() as StreetJusticeState;
            return {
        ...
        await game.screenshot('zhongguo-street-justice-protection-resolved', testInfo);
    });
});
