...
test.describe('大杀四方四人局布局', () => {
    test('四人局布局应显示四位玩家与基地信息', async ({ page, game }, testInfo) => {
        ...
        await expect.poll(async () => {
            ...
            return {
                ...
                vpByPlayer: state.core.turnOrder.map((playerId: string) => state.core.players[playerId]?.vp ?? null),
                firstBaseMinions: state.core.bases[0]?.minions?.length ?? 0,
            };
        ...
        await game.screenshot('four-player-layout-harness', testInfo);
    });
});
