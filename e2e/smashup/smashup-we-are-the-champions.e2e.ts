...
test.describe('SmashUp - 我们乃最强 afterScoring 回归', () => {
    ...
    test('计分后打出我们乃最强但没有合法接收目标时，应直接给反馈并自动收口', async ({ page, game }, testInfo) => {
        test.setTimeout(90000);
        ...
        await game.waitForResponseWindow('afterScoring', 10000);
        await game.screenshot('champions-no-target-after-scoring-window', testInfo);

        await game.playCard('giant_ant_we_are_the_champions', { targetBaseIndex: 0 });

        const feedbackToast = page.getByText('场上没有符合条件的目标').last();
        await expect(feedbackToast).toBeVisible({ timeout: 5000 });
        await game.screenshot('champions-no-target-feedback-toast', testInfo);
        ...
        expect(finalState.core.players['0'].discard.some((card: any) => card.defId === 'giant_ant_we_are_the_champions')).toBe(true);
        expect(finalState.core.bases[0]?.defId).not.toBe('base_the_homeworld');

        await game.screenshot('champions-no-target-final-state', testInfo);
    });
});
