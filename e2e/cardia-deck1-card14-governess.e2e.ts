 ...
 * 注意：此测试需要两回合（第一回合使用其他能力，第二回合复制）
 */
test.describe('Cardia 一号牌组 - 女导师', () => {
    test('影响力14 - 女导师：能力激活成功', async ({ browser }) => {
        ...
        try {
            ...
            // 1. 注入测试场景：P1 手牌包含影响力14（女导师），P2 手牌包含影响力16
            console.log('注入 P1 手牌：影响力14（女导师）');
            await injectHandCards(p1Page, '0', [
                { defId: 'deck_i_card_14' } // 女导师
            ]);
            ...
            // 7. 验证：能力已执行（复制其他牌的能力）
            // 注意：女导师能力需要场上有其他已使用过能力的牌，简化测试只验证激活成功
