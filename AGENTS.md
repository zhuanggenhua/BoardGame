...
- 详细规则统一迁至 `docs/ai-rules/e2e-verification.md`，本节只保留入口。
- UI 交互、多人协作、动画特效默认使用专用测试模式 + 状态注入 E2E；只有要证明房间创建、联机同步、`setupData -> runtime`、权限/座位/多端同步等跨入口合同时，才升级真实开房链路。
- **“端到端”术语定义（强制）**：当用户说“端到端”“E2E 验证”“端到端看一下”且语义是在要最终验证产物时，默认不只表示“测试跑过”，还表示**必须给出本轮实际查看过的端到端截图绝对路径**。若本轮覆盖多个关键入口/状态，需给出足以支撑结论的对应截图路径；不得只报 `passed`、日志、断言名或测试名来代替截图交付。
- 任何最终汇报提到“已跑 E2E / E2E 通过”时，仍必须实际打开截图验收，并附至少 1 张本轮核对过的关键截图绝对路径。
- **UI / 截图收口必须真的开图（强制）**：凡是用截图证明本轮 UI 改动、视觉修复、布局调整、弹窗/浮层/HUD 改动已达标时，必须先实际调用图片查看工具打开关键截图，再给结论。若当前会话可用 `view_image`，必须先用它核图；确认通过后，还必须调用仓库脚本 `npm run verify:open-image -- <截图绝对路径>` 立即打开同一张关键图。没有真实开图，不得宣称“UI 已验收”或“看图没问题”。
- **UI 改动完成门禁默认是本轮真实截图（强制）**：只要本轮改动触及用户可见 UI（样式、布局、对齐、层级、动效、浮层、卡牌/角标/标记、按钮、文案展示、响应式表现），默认完成门禁不是“代码已改”也不是“测试已过”，而是**已有本轮真实截图并已实际核图**。没有新的关键截图证据时，只能汇报“代码已修改/测试已通过，UI 仍未验收”，不得直接说“已完成”“结束了”或“没问题了”。
- **截图证据必须与本轮任务目标一致（强制）**：截图不是“有一张 UI 图就行”，而是必须直接证明**当前这轮正在验收的那个目标**。如果本轮任务是缩放跟随、居中、点击热区、层级遮挡、动画节奏、描边范围中的某一个，所给截图必须肉眼能直接判断该目标是否达标；不得拿上一轮的描边图去收当前缩放问题，也不得用布局图替代点击热区、用主态图替代 hover/选中态、用局部旁证替代当前验收对象本体。
- **开图动作必须可追责（强制）**：执行 `npm run verify:open-image -- <截图绝对路径>` 后，若脚本未明确成功、或外部图片查看器未能可靠拉起，不得只报“已打开”。此时最终回复必须至少给出同一张图的**完整绝对路径**，并明确说明“已尝试打开，但请直接按此完整路径定位”。禁止只回 `OPENED_IMAGE`、相对路径或“已经开了你看下”。

#### 测试文件管理（强制）
1. **先搜索现有测试，但禁止继续制造巨型测试垃圾桶**：补用例前必须先找现有覆盖；如果现有文件已经跨多个不直接相关的规则、基地、英雄、页面、反馈编号或交互链路，就不得继续往同一个文件里堆新场景。把所有回归都塞进 `newBaseAbilities.test.ts` 这类泛化大文件是明确反模式。
2. **新增测试文件不是默认禁止，而是必须按主题目录化**：当新增覆盖属于新的规则簇、能力簇、页面行为、反馈批次或跨文件共享场景时，应在对应 `__tests__/` 下建立清晰子目录或主题测试脚本，例如按游戏模块、能力组、基地/英雄、交互链路拆分；文件名必须表达测试对象和行为，不得用 `new*`、`misc`、`regression`、`feedback` 这类继续吸纳一切的泛名。
3. **拆分优先级高于“就近塞一个用例”**：如果一次改动会让单个测试文件继续扩大主题边界，必须优先新建或迁移到更聚焦的测试脚本；只有同一主题、同一夹具、同一行为簇内的少量补充，才允许追加到现有文件。
4. **典型测试优先**：同类型风险通常只保留正常流程 1 个用例 + 边界条件 1-2 个关键场景，禁止同一种交互模式反复堆叠 E2E。
5. **已有测试只有真实覆盖才可复用**：现有同类测试若没覆盖本次改动的核心路径、关键断言和风险点，不能因为“仓库里已有类似测试”就跳过。

#### 代码文件组织反模式（强制）
1. **禁止单文件承载多个无关职责**：业务实现、UI 组件、规则注册、测试、脚本、证据生成都不能长期堆在一个“万能文件”里。只要一个文件开始同时承载多个能力簇、多个页面/游戏、多个反馈编号或多个不共享上下文的行为链路，就必须视为需要拆分的设计异味。
2. **目录先承载边界，文件再承载职责**：新增或扩展代码时，优先建立能表达领域边界的文件夹，再在文件夹内放聚焦脚本/组件/规则/测试。禁止为了省一个目录，把一批相邻但不同主题的逻辑横向堆进同一个文件。
3. **反模式识别信号**：文件名含 `new`、`misc`、`common`、`helpers`、`regression`、`feedback` 但实际持续吸纳新业务；单文件出现多个不共享夹具的大段 `describe`；同一文件既处理 A 游戏又处理 B 游戏；同一组件同时做布局、业务判定、网络副作用和弹窗编排。这些都必须在方案或 review 中标记为反模式。
4. **当前任务不一定要立即重构历史债务**：如果用户只要求修 bug 或提交现有改动，可以先不迁移旧文件；但新增代码不得继续扩大反模式。发现必须追加到历史大文件时，至少在汇报中说明这是遗留结构限制，并优先选择后续可拆分的位置。
...
- **E2E 测试环境与工具（强制）**：
  ...
  - **启动口径**：普通开发默认先走 `npm run dev:lite`；但只要进入 E2E，就必须切到完整环境
  ...
  - **状态注入方案（默认）**：默认使用状态注入方案跳过前置步骤直接构造目标场景；只有要证明房间创建、联机同步、`setupData -> runtime`、权限/座位/多端同步等跨入口合同时，才使用真实开房链路。详见 `docs/e2e-state-injection-guide.md` 与 `docs/ai-rules/e2e-verification.md`
