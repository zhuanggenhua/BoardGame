...
7.2.1. **关键控件明显不可读时不得收口（强制）**：只要截图或真机画面里出现按钮、tab、输入框、搜索图标、切换项、分页/排序入口等关键控件低对比、字色贴底色、图标倾斜错位、选中态不清、边框/阴影叠到难以辨认，即使 E2E 断言全部通过，也必须明确判定“未达到验收标准”，继续重做视觉或实现。禁止用“元素存在 / 可点击 / 热区足够”替代可读性验收。
