...
4.5. **流程关键截图必须可继续操作（强制）**：凡是用于证明“流程可继续/可完成”的截图（如创建房间、确认弹窗、支付/提交、加入房间、密码输入），画面里必须同时可见下一步主操作控件（如“确认/创建/提交”按钮）且控件不被裁切、不在可视区外。若截图里看不到下一步主按钮或无法判断点哪里继续，该截图一律判定为无效证据，不得收口。
