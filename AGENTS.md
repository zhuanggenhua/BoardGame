...
- **修完 Bug 主动扫同类问题**：根因确认后，必须基于根因关键词、触发模式、文案模式或调用模式，用 `rg` 横向搜索同模块/同游戏/同类能力或组件，主动检查是否存在相同缺陷；禁止只修单点就结束，除非已明确说明搜索范围与“未发现同类问题”的结论。用户无需额外催促。
- **连续两次未解决**：切换为"假设列表 → 验证方法 → 多方案对比"排查模式
- **输出总结**：每次回复末尾必须包含 `## 总结` 区块
- **排查完整性自检**：修复 bug 后，总结中必须包含调用链检查报告、根因反思
