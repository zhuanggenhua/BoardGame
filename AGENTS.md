...
- **禁止把上位不变量拆成低级重复特例（强制）**：如果已有规则已经能直接推出某个结论，新增或改写规范时必须优先强化上位不变量和触发证据，不得把该结论再写成并列特例、端侧特例或对象特例。例如已有“端侧适配不得改变既有交互语义”这类上位规则时，就不要再单列“某端某按钮不能新增”这类派生结论；除非该特例有独立例外、独立证据或独立执行动作。写完后必须自审：删掉这个特例后，上位规则是否仍能阻止本次错误；若能阻止，应删除特例而不是继续堆规则。
...
- **长期默认授权（补充）**：允许主 agent 按需派生子 agent 进行代码分析/设计/实现/修复/测试/审查；子 agent 默认继承主 agent 当前模型与推理强度；不要写死具体模型版本，避免规范过时。只有工具无法继承且不显式指定会导致降级时，才指定与当前主 agent 等效的配置；若用户当轮明确禁止，则以当轮为准。
- **允许并行的典型场景**：
...
- **用户报 bug 默认就是正常路径**：用户说某个体验坏了、失败了、闪退了、自动执行了或“我什么都没做”时，必须先按正常玩家路径下的真实问题排查；不得先改写成误点、乱点、手太快、没更新、缓存残留、队友/玩家连续执行或边界误触。只有同一真实入口的端到端证据、录屏、日志或埋点能证明正常路径健康且症状必须由异常行为触发时，才允许把用户行为作为结论。
