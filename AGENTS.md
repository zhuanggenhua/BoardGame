...
- `docs/deploy.md` — 部署、构建产物、环境变量注入、线上/本地差异、CDN/R2 资源问题时必读。
  ...
  - **生产最新部署 tag 口径（强制）**：用户说“更新部署 / 部署最新 / 发线上”且未明确指定版本时，默认动作是“服务器更新 + Android stable OTA 发布”。服务器步骤必须使用 `bash scripts/deploy/deploy-image.sh update`，也就是部署 CI 推送到 GHCR 的 `latest`；OTA 步骤必须走项目内正式发布链路。只有用户明确说“只更新服务器 / 不发 OTA”时，才允许缩小为只做服务器更新。禁止根据 commit SHA、短 SHA、run number 或个人推测临时拼出 `update <tag>`；只有用户明确指定 tag，或已经用 CI 输出 / GHCR / `docker manifest inspect` 证明 `web` 与 `game-server` 两个镜像都存在同一个精确 tag 时，才允许执行 `update <tag>`，并必须在汇报中写明验证证据。
