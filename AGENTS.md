...
- **存在多个 worktree 时，必须先判断“应该实施在哪棵树”，不能把当前 cwd / main 当默认答案（强制）**：当仓库里同时存在两个或以上 `git worktree`，且本轮任务要改 UI、跑页面、给截图、修回归、补测试、更新 evidence、继续某个游戏专项、或处理某个 feature 分支对应的工作线时，动手前必须先判断**哪一棵 worktree 才是本轮应实施的位置**，而不是只确认“我现在站在哪”。最低核对动作必须同时覆盖：① 候选 worktree 清单；② 这些 worktree 里哪些与当前游戏/模块同名、同分支或同任务线相关；③ 各候选 worktree 是否已有未提交修改、任务计划、专项文档或运行中页面；④ 为什么本轮实施应落在其中哪一棵，而不是另一棵。  
- **根目录 / main 不是默认正确落点（强制）**：只因为当前 `cwd` 在根目录、当前分支是 `main`、或根目录也属于一个 worktree，**都不能**推出“应该在这里实施”。只要存在更具体的专项 worktree（例如 `feat/game-fantasyrealms`）且它与本轮目标直接相关，默认应先把该专项 worktree 作为优先候选；若它已经有相关修改，更不得跳过它直接在 `main` 开改。
- **专项 worktree 一旦承载当前任务线，就默认是该任务真相源（强制）**：如果某个专项 worktree 已直接承载当前游戏/模块/PR 的最新分支、计划文档、专项 evidence、未提交实现或正在运行的页面，那么这棵 worktree 默认就是本轮实现真相源。根目录误改、旁支试验或其他 worktree 的同类改动，只能作为待吸收输入，不得再被当成与专项 worktree 并列的“选择性真相源”。
...
- **禁止行为**：禁止把“当前 cwd 正好是这里”“main 更通用”“根目录也能跑起来”“那棵 worktree 只是兄弟目录”“我先在这边改完再说”当成实施依据；禁止一边在 A worktree 改代码、一边引用 B worktree 的运行页/截图/旧实现当依据；也禁止在多个 worktree 同时存在相关修改时，跳过专项 worktree 直接在根目录 / `main` 实施。
...
- **改前先锁定当前执行现场（强制）**：开始改代码前，必须同时确认 ① 当前 `cwd` 对应的是哪一棵 worktree、哪个分支；② 该分支的主任务/主游戏/主 PR 是什么；③ 本轮准备改的文件是否属于这条分支职责。三项里任一未锁定，只能继续核对，不能直接开改。
