...
- **提交范围以口令时刻冻结（强制）**：用户说“提交 / 都提交 / 一起提交 / 检查后提交”时，提交范围只包含收到该口令后第一次 `git status` 快照中已经存在、且本轮核对过的改动。口令之后新出现的文件或修改（包括后续审计文档、计划回写、工具生成物、hook 恢复出的未暂存改动、并发进程写入）一律视为“后续改动”，不得自动并入本次提交，也不得为了补齐它们追加第二个提交。确需纳入时，必须先向用户说明新增对象与原因并获得明确确认。唯一例外是 commit hook 对已暂存文件做纯机械格式化且不新增业务内容/文件，可随原提交继续。
...
4.2. **证据截图必须看得到“对应物”本体（强制）**：凡是宣称“某对象/某特写/某控件/某奖励骰/某弹窗/某表情已经出现、位置正确或视觉达标”的截图，肉眼必须能直接看到该对象本体、完整主体轮廓或足以识别的关键内容；如果截图里只有外围布局、遮罩、容器边界、背景、文案或其它无关元素，看不到被宣称对象本体，则该截图一律视为无效证据，不得用于收口。像“骰子特写已出现”这类结论，截图里必须能看到骰子/骰面本体，不能只看到棋盘、手牌或泛化浮层。
4.2.1. **对象被裁切时不得判定视觉通过（强制）**：凡截图用于证明浮层、表情、popover、HUD overlay、特写或控件的视觉结果时，如果目标对象被视口边界、截图边界、容器裁剪、遮挡层或固定定位截断到只露边角/只露局部，最多只能证明“事件触发 / DOM 存在 / 局部进入视口”，不得证明“视觉达标 / 用户看得清 / 可收口”。最低证据是：①截图能看到目标对象完整主体或足以识别的主要轮廓与关键内容；②若要证明锚点位置，截图必须同时看见锚点参照物和目标对象完整边界；③全页截图收不全或边界看不清时，必须补局部放大截图、调整截图时机或调整视口后重验。只有用户明确要求证明“部分进入视口”，或对象设计本身就是出血/半露效果时，才允许把裁切图作为有效证据，并且结论必须显式写明该验收口径。
...
4.4. **用户要求“恢复原来的效果”时先锁 git 基线（强制）**：必须先通过 `git log` / `git show` / `git blame` 给出 `last known good` 与 `first known bad` 候选，再决定是恢复历史行为还是在历史行为上做增量修复；如果历史上根本不存在一个更好的已提交版本，也必须明确说清，禁止假装找到了可直接恢复的旧实现。**仅仅报出提交号不算完成**：必须实际打开并阅读对应旧实现、旧截图或旧 evidence，明确旧版本“当时怎么实现、怎么显示、怎么验收”。未完成这一步，禁止擅自发明新布局、新交互策略或新抽象解释。
