...
- **口语化提交口令单独解释**：像“看下没问题就提交”“检查没问题就推”这类表达，默认指**先做静态代码审查**（阅读 `git diff`、关键实现与明显风险点），再提交/推送；**不默认主动跑测试，也不授权修复问题**。若审查发现 bug、明显风险、范围外改动、钩子/生成命令带出的新代码差异或需要补门禁的事项，必须按 review 口径停止并汇报 finding / blocker，等待用户明确说“修复 / 补齐 / 继续改”后才能动代码。**但如果用户当轮或当前对话已经明确给出持续修复授权（如“简单的决策不用停下来问我，直接修就行然后继续”），则对当前 diff 直接暴露、修法单一、影响范围清晰的 blocker，应直接做最小修复并继续；只有涉及范围升级、语义取舍、删除/回滚他人改动或多种修法分歧时，才停下确认。**若仓库已配置对应 `pre-commit` / `pre-push` 钩子，默认直接依赖钩子执行同类门禁，**不再额外手工预跑一遍同类测试/检查**。只有明确说出“无校验 / 直接提交 / 直接 push / 不跑测试”时，才进入快速路径。
