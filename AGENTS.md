...
- **口语化提交口令单独解释**：像“看下没问题就提交”“检查没问题就推”这类表达，仍按“先确认范围与当前状态，再提交/推送”理解；若仓库已配置对应 `pre-commit` / `pre-push` 钩子，默认直接依赖钩子执行同类门禁，**不再额外手工预跑一遍同类测试/检查**。只有明确说出“无校验 / 直接提交 / 直接 push / 不跑测试”时，才进入快速路径。
...
- **用户说“都提交”时**：才按整个当前工作区执行 `git add/commit`。
- **提交信息语言默认按强制口径执行**：`git commit -m` 必须使用中文；只有用户当轮明确要求英文、指定上游约定格式、或自动化发布流程有硬性模板时，才允许偏离。
- **当分支已 `ahead N` 且用户目标是 `push` 时**：默认直接执行 `git push`；未提交改动不会随 push 上远端，不得先因工作区脏而岔开去排查。
...
- **修反馈不等于提交/发布授权（强制）**：当用户说“修反馈 / 看反馈 / 处理线上反馈”时，默认只授权**定位、修复、验证、留证、回写状态建议**；未经用户当轮明确要求，不得擅自 `git commit`、`git push`、更新生产镜像/服务器、执行部署脚本或把“本地已修好”自动延伸为“直接上线”。即使确认根因和本地修复都成立，也只能按“待代码审查 / 待提交 / 待部署”口径汇报，不能把修反馈动作偷换成发布动作。
...
### 验证测试（Playwright 优先）
- **测试触发、E2E 强制要求、无校验豁免统一以前文“测试编写规范”与“执行优先级与归并口径”为准；本节只补充工具、环境和命令。**
