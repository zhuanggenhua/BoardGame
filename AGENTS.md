...
- **回归事故更新规范时，先检查是不是“没按回归流程做事”（强制）**：如果事故发生在“恢复原效果 / 以前能用现在不能用 / UI 回归 / 布局回归 / 行为回归”场景，更新规范时必须先检查根因是不是**没有先锁 git 基线、没有读旧实现、没有读旧截图、没有读旧 evidence，就擅自发明新方案**。若是，则优先补强“回归问题必须先读历史正确实现”的流程门禁，而不是把后果改写成新的布局规则、共享抽象规则或系统规则。只有在完成旧实现与旧证据阅读后，仍证明历史正确实现本身共享同一抽象缺陷，才允许把规范上升到共享机制层。
