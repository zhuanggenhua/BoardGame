...
- `docs/ai-rules/data-entry.md` — **根据图片/规则书/Wiki/截图录入业务数据时必读**。含权威来源优先级、Markdown 核对契约、零猜测 OCR、图片索引录入、先文档后实现的通用流程。
...
4. **必须实际打开截图，不得只看测试通过**：所谓“自审截图”必须真的打开图片逐张检查，不能只根据测试通过、文件存在或代码逻辑推断 UI 正确
5. **证据文档必须写出肉眼观察结论**：每张关键截图至少写出 1-3 条人工观察到的具体现象，例如“左侧第一个基地不再被切边”“泰坦仍在基地上方中间”“持续行动之间没有额外缝隙”，禁止只写“截图正常”“UI 正确”
6. **必须先看图再写结论**：结论、证据文档、对用户的汇报都必须建立在“已经实际打开并看过图”的前提上；禁止先写“已修复/正常”，再回头补看图
7. **外部 R2/CDN 资源缺失时仍必须看图**：如果测试环境没有稳定拉到牌面美术，必须在结论里明确说明“资源未渲染”，并把验收重点放在布局、相对大小、层级、遮挡、溢出、对齐；禁止把“白卡面”直接当成布局失败，也禁止借此跳过布局检查
8. **必须创建证据文档**：`evidence/<功能名>-e2e-test.md`，嵌入截图并分析内容
9. **绝对禁止用单元测试糊弄**：UI 交互/多玩家协作/动画特效 → 必须用 E2E 测试
10. **测试失败处理**：加日志定位根因 → 审查调用链 → 修复代码或调整测试

#### 测试文件管理（强制）
1. **禁止创建新测试文件**：先搜索现有测试，在相关文件中补充用例
2. **典型测试优先**：同类型测试只需 1-2 个典型用例，不需要穷举所有场景；禁止为同一种 UI 交互模式、同一种入口语义、同一条风险链路反复堆叠多个端到端
3. **同类型测试可复用才可忽略**：仅当现有同类型测试已真实覆盖本次改动的核心路径、关键断言与风险点时，才可以不新增或不重复运行同类型测试；禁止仅因“仓库里已经有类似测试”就跳过
4. **测试覆盖范围**：正常流程 1 个用例 + 边界条件 1-2 个关键场景

#### 测试实现验证（强制）
1. **先验证实现是否工作**：检查验证逻辑、执行逻辑、触发时机
2. **运行现有测试**：修改实现后运行相关测试确认没有破坏功能
3. **测试失败时先修实现**：优先检查实现是否正确，而不是修改测试

#### 测试用例设计（强制）
1. **使用真实场景**：测试用例必须模拟真实游戏场景，不能使用不可能的状态
2. **清晰的断言**：断言必须明确验证功能的核心行为
...
- **开工前分支职责检查（强制）**：开始任何实质工作前，必须先确认当前所在分支/工作树，并判断它是否与当前任务职责一致。若当前分支明显服务于另一条任务线、另一个游戏、另一个 PR 或另一个 worktree 目的，**禁止**直接在该处继续修改、提交、推送；必须先向用户说明冲突，并切到职责正确的位置后再开工。
- **需求歧义先澄清（强制）**：当用户对目标效果、比较基准、验收口径的表述存在多种合理解释时，必须先用最短问题澄清，再继续执行；禁止自作主张脑补用户真正想法。尤其是 UI/UX 任务里，像“太宽/太挤”“像桌面端”“为什么一排三个”“差不多”“参考这个效果”这类表述，一律先确认含义，不能自动翻译成自己的实现目标。
- **禁止把表现问题偷换成布局枚举题（强制）**：当用户指出的是“间距太大 / 太散 / 不像桌面端 / 密度不对 / 视觉关系不对”这类**表现问题**时，禁止擅自改写成“2 列还是 3 列”“允许不允许一排三个”“是否以两列为主”这类**布局枚举题**。除非用户明确问列数，否则默认聚焦：卡间距、容器内边距、元素占位、比例关系、视觉密度，而不是列数本身。
- **禁止默认套用移动端常规模板（强制）**：当用户明确要求“更紧凑”“更像桌面端”“保留桌面端密度/气质”时，禁止默认把实现收敛到“手机端通常两列 / 更大触控间距 / 更宽松留白”这类通用移动模板。应优先围绕目标视觉密度选择布局，必要时允许移动端采用更接近桌面端的列密度与占位策略。
- **先解根因，再动表象（强制）**：收到 bug / 体验反馈时，必须先判断“用户真正不满的结果是什么、当前看到的现象只是表面还是根因”。禁止把用户举的现象词（如三列、按钮位置、字号、某一处间距）直接当成最终需求；必须先提炼真正目标，再选择实现手段。交付标准是解决根本体验问题，不是只把表面现象改掉。
...
- **提交信息中文化（强制）**：所有 `git commit -m` 提交信息默认使用中文，禁止再用英文提交信息概括本项目改动；需要包含多个范围时，也用中文概括主线内容。
- **路径表达规范（强制）**：只有截图、测试截图、测试产物路径在对用户汇报时**必须**给出工作区绝对路径；普通源码文件、文档、目录路径**不做强制要求**，可按相对路径、文件名或在需要精确定位时再给绝对路径。
...
- **Git worktree 使用规范（强制）**：未经用户明确许可，禁止自行创建 `git worktree` 或其他独立工作区。若确需用 worktree 隔离未提交改动、规避工作区噪音或执行推送/验证，必须先说明目的、影响范围，并获得用户确认。**补充强约束**：worktree 与本地分支默认视为一一对应的固定关系，不能把 worktree 当作“先进去再随手切到别的分支”的通用工作壳。开始工作前必须先确认“当前 worktree 对应的分支”是否就是这次任务该工作的地方；如果用户要求“回到 main 工作树”，应直接去 `main` 所在 worktree 路径开展后续工作，而不是留在当前 worktree 里继续做，或尝试在已绑定其他分支职责的 worktree 中强行切换到不相干分支。
...
- **PR 回归登记（强制）**：当任务涉及 GitHub PR 的审查、修复、推送、合并时，最终收尾前必须登记“回归与行为变化”，至少区分三类：1）原 PR 目标问题；2）本次审查/补修过程中额外发现的真实回归；3）仅属于业务口径/规则变化的项。若属于第 3 类，必须明确建议同步更新的规范或规则文档落点。详细执行要求见 `docs/git-merge-checklist.md` 的“回归与行为变化登记（强制）”。
...
- **--no-verify 使用规范（强制）**：
  ...
  - **用户显式要求优先**：如果用户明确要求“直接上传”“先 push 再说”“无校验上传”或等价口径，可以直接使用 `git push --no-verify`，不要再先跑一轮本地校验把 push 卡住。
  - **正确做法**：修复所有 errors 后再提交；如果工作区存在其他 AI 的并发改动，不得默认用 `git stash` 清空工作区，除非用户明确要求。
- **Push 校验阻塞处理规范（强制）**：推送时若 `pre-push` / 强制校验失败是由**工作区未提交改动**引起，而非待推送提交本身引起，除非用户明确要求“忽略校验”或允许跳过，否则**不能因为这类工作区修改直接卡住整个 push**。应先定位失败源头，优先修复当前工作区问题；若需要隔离工作区与待推送提交，必须先征得用户同意，再使用 `worktree`、`stash` 等隔离手段。
- **Push 超时策略（强制）**：用户明确要求执行 `push` 时，默认直接给长超时（至少 30 分钟），不要先用短超时试一次再重跑；若网络层出现 `408`、`SSL_ERROR_SYSCALL`、sideband 断流等上传抖动，优先延长超时或切换 `HTTP/1.1` 后重试，而不是先把任务判成失败。
- **文件移动/复制规范（强制）**：
...
- **主动跑测试（强制）**：完成实现后必须主动运行与本次改动直接相关的测试，不能等用户额外提醒才执行验证。纯样式/文案/文档修改按既有规则可不跑；其余改动至少运行对应层级的相关测试（逻辑改动跑 Vitest / GameTestRunner，UI 交互改动跑 E2E）。
- **主动跑测试的唯一例外（强制）**：只有在现有同类型测试已经覆盖本次改动，且本次变更不会新增测试价值、不会扩大风险面时，才可以不新增或不重复运行同类型测试；做出该判断时，必须先核对现有测试的覆盖路径与断言是否足够。
- **E2E 测试三板斧（强制）**：所有 E2E 测试必须使用"新框架 + 专用测试模式 + 状态注入"，详见 `docs/automated-testing.md`。
  ...
  **标准模板**：
  ...
  test('测试名称', async ({ page, game }, testInfo) => {
      ...
      await game.setupScene({ gameId: 'smashup', player0: {...}, currentPlayer: '0', phase: 'playCards' });
  ...
  **禁止**：使用 Fixture 方式、`harness.state.patch()`、`/play/<gameId>/test` 测试模式
...
- **E2E 测试必须通过并自审截图（强制）**：
  ...
  - 打开截图所在文件夹时，每轮只允许打开 1 个目标文件夹；禁止连续枚举或打开多个兄弟目录刷屏
  - 必须实际打开截图逐张确认，不得只根据“测试通过”或“代码看起来没问题”就宣称已看图
  - 必须先完成看图，再写证据文档和对用户的结论；禁止“先下结论后补看图”
  - 证据文档中的截图分析必须写出肉眼可见的具体现象，至少包含布局位置、遮挡/溢出、间距/对齐三类中的一类；禁止只写“正常”“符合预期”
  - 向用户汇报测试产物路径时，必须给出工作区绝对路径，例如 `F:\gongzuo\webgame\BoardGame\test-results\evidence-screenshots\...`，不要只写相对目录
  - 向用户汇报截图位置时，只给实际核对过的截图完整绝对路径；除非用户另要目录，否则不要再额外罗列多个文件夹路径
...
- **素材数据录入规范（强制）**：根据图片素材提取业务数据时，必须全口径核对、逻辑序列化、关键限定词显式核对，输出 Markdown 表格作为核对契约。具体流程、来源优先级、图片索引处理和“先文档后实现”要求，统一以 `docs/ai-rules/data-entry.md` 为准。**图片文字辨识零猜测原则（强制）**：任何文字（名称、描述、数值、关键词）只要有一点看不清或不确定，必须立即停止当前数据录入工作，向用户说明哪些位置无法辨认，并索要更清晰的图片。禁止根据上下文、常识或英文原版"猜测"看不清的中文文字。已猜测录入的数据视为缺陷，必须重新核对。
...
> **核心原则：用户反馈 bug 时，默认就是 bug，立即全链路排查，禁止猜测是正常情况**

#### 截图/用户反馈分析（强制第一步）
> **禁止看截图就猜测，必须先用 grepSearch 搜索关键词定位代码**

1. **搜索关键词定位代码**：用户说"牌库" → 搜索 `t('ui.deck')`
2. **读取代码确认渲染逻辑**：该组件应该渲染什么？用的是什么数据？
3. **对比截图与代码**：截图实际显示了什么？与代码预期的差异在哪里？
4. **禁止**：看截图就猜测、不搜索就修改、凭视觉相似性假设

#### 调用链全面检查（强制）
> **所有 bug 修复都必须完整检查整条调用链**

对于调用链的每一层（A 调用 B），必须检查：
1. **存在性**：B 是否存在？是否已注册？文件是否被 import？
2. **契约**：A 传递了什么参数？B 期望什么参数？类型是否匹配？
3. **返回值**：B 返回了什么？A 如何使用？是否符合预期？

**禁止**：跳过检查项、只检查"我改的地方"、假设"肯定存在"、假设"这层没问题"、看到一个定义就假设唯一
...
- 在总结中输出完整的检查报告
- 禁止直接猜测修改代码，必须先完成调用链检查

#### 用户反馈 Bug 排查规范（强制）
> **核心原则：用户反馈 bug 时，默认就是 bug，立即全链路审查代码，找不到问题再加日志**

1. **直接相信用户反馈**：默认就是 bug，不做"可能是正常情况"的假设
2. **先全链路审查代码**（禁止直接加日志）：
   - 定位调用链：搜索关键词 → 追踪调用链
   - 逐层检查：每一层都检查存在性、契约、返回值
   - 检查 early return：每个 `if (xxx) return null` 的条件是否会触发
   - 检查数据流：数据从哪里来？经过哪些转换？最终传递给谁？
3. **代码审查没发现问题才加日志**：在关键决策点加日志，记录所有影响决策的变量
4. **绝对禁止**：看到 bug 就加日志、创建诊断脚本、在父组件加日志后就停止、只验证"写入链"不验证"消费链"

#### 其他排查规范
- **同名函数全量排查**：搜索是否存在同名定义，确认调用点 import 的是哪个
- **回归 bug 先 diff**：`git log` + `git show`/`git diff` 对比最后正常版本
- **资源归属先查消费链路**：追踪实际消费链路（谁加载、从哪加载、是产物还是源码）
- **事实/未知/假设**：提出方案前列出已知事实、未知信息、假设及验证方法
- **修 Bug 证据优先**：证据不足时不得直接改代码"试试"
- **禁止用"强制/绕过"掩盖问题**：不得放开安全限制/扩大白名单/关闭校验
- **写入-消费全链路排查**：画出时间线，标注写入时机、消费窗口、清理时机
- **API→Context→UI 三层排查**：从数据源头开始，逐层核查（API 返回 → Context 映射 → UI 读取）
- **单点修复必须全链路验证**：沿数据流向下游走完，每一跳都确认行为正确
- **修完 Bug 主动扫同类问题**：根因确认后，必须基于根因关键词、触发模式、文案模式、事件类型、状态字段或调用模式，用 `rg` 横向搜索同模块/同游戏/同类能力、共享 reducer/handler/hook/system 或组件，主动检查是否存在相同缺陷；禁止只修单点就结束，除非已明确说明搜索范围、命中结果、处置结论与“未发现同类问题”的依据。用户无需额外催促。
- **Bug 根因分级与处置（强制）**：修 bug 不能停留在表面现象，必须先判断根因属于“数据/录入缺陷”“单点实现缺陷”“共享抽象缺陷”“架构/时序/系统边界缺陷”中的哪一类。前两类可局部修复；后两类默认必须优先重构共享抽象、时序设计或系统边界，禁止以补丁叠补丁方式继续累积技术债。若因范围、风险或用户明确要求只能先做临时止血，必须同步登记技术债，写清临时方案、残留风险、回填条件与触发时机。
- **连续两次未解决**：切换为"假设列表 → 验证方法 → 多方案对比"排查模式
- **输出总结**：每次回复末尾必须包含 `## 总结` 区块
- **排查完整性自检**：修复 bug 后，总结中必须包含调用链检查报告、根因分类、同类问题扩审结果，以及本次为何选择局部修复或重构的理由
- **百游戏自检**：每次修改后回答"这样能不能支持未来 100 个游戏？"
...
> **核心原则：修改前先验证，选对工具，一次成功，失败即停**

#### 1. 修改前强制验证（强制）
- ❌ **绝对禁止**：基于假设或记忆修改代码，不先验证问题是否存在
- ✅ **强制流程**：
  1. 用 `grepSearch` 搜索相关代码，确认问题确实存在
  2. 用 `readFile` 读取相关代码，验证假设是否正确
  3. 确认问题后再修改，不得跳过验证步骤
...
**判断流程**：
```
文件包含大量中文（>50 行中文注释/字符串）？
├─ 是 → 使用 Node.js 脚本（.mjs）+ 显式 UTF-8 编码
└─ 否 → 修改范围 >50 行？
    ├─ 是 → 使用 Node.js 脚本（.mjs）
    └─ 否 → 使用 IDE 工具（strReplace/editCode）
```

**工具特性**：
- **Node.js 脚本（.mjs）**：
  - ✅ 适用：大段代码（>50 行）、精确行号定位、复杂逻辑、多文件操作、**包含大量中文的文件**
  - ✅ 优势：完全控制编码（显式 `'utf-8'`），不会破坏中文
  - ⚠️ 注意：必须显式指定 `'utf-8'` 编码（读写都要）
  
- **IDE 工具（strReplace/editCode）**：
  - ✅ 适用：单文件小范围修改（<50 行）、简单替换、**且文件中文内容少**
  - ❌ **绝对禁止**：对包含大量中文的文件（>50 行中文）使用 `strReplace`
  - ⚠️ 原因：`strReplace` 可能破坏整个文件的 UTF-8 编码，导致所有中文变成乱码（如 `检测到` → `妫€娴嬪埌`）
  
- **PowerShell**：
  - ✅ 仅允许只读（`Get-Content`/`Select-String`）
  - ✅ 读取任何含中文的源码/规则/Markdown/plan 文件时，必须先切到 UTF-8 会话（优先 `. .\scripts\infra\enable-utf8.ps1`，至少执行 `chcp 65001 > $null`），并显式使用 `Get-Content -Encoding UTF8`
  - ✅ 如果终端输出仍出现乱码，必须立即改用 `python -c "from pathlib import Path; print(Path(...).read_text(encoding='utf-8'))"` 或 Node 显式 UTF-8 读取确认真实内容；禁止拿乱码输出直接做补丁上下文、规则判断或文档结论
  - ❌ 禁止写入（`Set-Content`/`Out-File`/`>`）
...
// 2. 定位目标行（精确行号或搜索）
const targetIndex = lines.findIndex(line => line.includes('目标特征'));

// 3. 修改逻辑（替换/插入/删除）
lines[targetIndex] = '新内容';
// 或：const newLines = [...lines.slice(0, start), newCode, ...lines.slice(end + 1)];

// 4. 写回文件（显式指定 utf-8）
writeFileSync('file.ts', lines.join('\n'), 'utf-8');
...
**验证流程**：
1. 用 `readFile` 读取文件，确认修改正确
2. 运行 `npx eslint <文件路径>` 检查语法错误
3. 用 `Get-Content <文件路径> | Measure-Object -Line` 检查行数是否正常

**验证失败处理**：
- ✅ **正确做法**：立即停止 → 向用户说明（失败原因、文件当前状态、建议方案）→ 等待用户指示
- ❌ **绝对禁止**：验证失败后继续用脚本修复（会导致错误累积和文件重复）
- ⚠️ **一次修复原则**：同一个文件的同一个问题，只允许尝试修复一次
- ⚠️ **手动修复优先**：优先使用 IDE 工具（`strReplace`/`editCode`）手动修复，`git restore` 是最后手段
...
- 常见截断示例：把残缺词补成完整词，例如 `随从`、`能力`、`消灭`、`牌库底`

**批量替换**：
- 必须先备份：优先使用 `Copy-Item` 等显式备份方式；未经用户明确许可，禁止把 `git stash` 当作默认备份手段
- 优先使用 Node.js 脚本批量处理，而非多次调用 IDE 工具
...
#### 动态选项生成（强制）
- **问题**：同时触发多个交互时，后续交互可能包含已失效的选项
- **解决方案**：opt-in 模式，默认不刷新，需要时显式声明
- **使用方法**：
  1. **简单场景（autoRefresh）**：选项是引用类型，使用 `autoRefresh` 字段
     ```typescript
     createSimpleChoice(id, playerId, title, options, {
       sourceId: 'wizard_scry',
       autoRefresh: 'deck', // 支持: hand/discard/deck/field/base/ongoing
     });
     ```
  2. **复杂场景（optionsGenerator）**：选项基于数量、多来源、复杂过滤
     ```typescript
...
- **E2E 测试必须使用联机模式**：所有游戏 `allowLocalMode=false`，通过调试面板注入状态
---
