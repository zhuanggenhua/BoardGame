...
- **多 afterScoring 交互链式传递（已修复 bug）**：当基地能力和随从 trigger 都创建 afterScoring 交互时（如母舰 + 侦察兵），`_deferredPostScoringEvents` 必须存到第一个交互中，确保链式传递到最后一个交互。修复位置：`src/games/smashup/domain/index.ts` 第 370 行。**这是通用修复，影响所有多 afterScoring 交互场景（基地能力+随从trigger、多个随从trigger等）**。测试覆盖：2个交互（母舰+侦察兵）和4个交互（母舰+2侦察兵+大副）。**教训**：多交互场景必须考虑延迟事件的传递链路，不能假设只有一个交互。
